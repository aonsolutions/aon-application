...
//			pict.resize(0.8,3);
	...
	public void ITR() {
		...
		row = sheet.createRow(rowCount++);
		String text = "La entidad est� sujeta a la obligaci�n de identificar al titular real proque no cotiza en mercados regulados";		
		...
		row = sheet.createRow(rowCount++);
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), 0, 7));
		CellUtil.createCell(row, 0, "Titular real persona f�sica con porcentaje de participaci�n superior al 25%", headerCellStyle);
		ssHeader(new String[]{"Nombre y Apellidos", "DNI / C�digo de Identificaci�n Extranjero", "Fecha de Nacimiento", "Nacionalidad",
				"Pais de Residencia", "% Participaci�n Directa", "% Participaci�n Indirecta", ""}, 8, 3);
		...
		row = sheet.createRow(rowCount++);
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), 0, 7));
		CellUtil.createCell(row, 0, "Titular real persona f�sica asimilada", headerCellStyle);
		ssHeader(new String[]{"Nombre y Apellidos", "DNI / C�digo de Identificaci�n Extranjero", "Fecha de Nacimiento", "Nacionalidad",
		...
		ssHeader(new String[]{"DNI / C�digo de Identificaci�n Extranjero", "Nivel en la cadena de control", "Denominacion Social",
				"NIF / C�digo de Identificaci�n Extranjero", "Nacionalidad", "Domicilio Social", "Datos Registrales / LEI", ""}, 8, 3);
	...
	public void CVA() {
		addSheet("Declaraci�n Covid 19");
		header(5);

		row = sheet.createRow(rowCount++);
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), 0, 7));
		CellUtil.createCell(row, 0, "Declaraci�n Covid 19", headerCellStyle);
		row = sheet.createRow(rowCount++);		
		cellCount = 0;
		...
		else if("2".equals(CVA8220010)) CVA8220010Str = "Por causas t�cnicas-econ�micas-organizativas";
		...
		if("1".equals(CVA8220020)) CVA8220020Str = "Suspensi�n de contratos";
		else if("2".equals(CVA8220020)) CVA8220020Str = "Reducci�n de jornada";
		...
		idacell("N�mero de trabajadores en plantilla antes del ERTE: " + CVA8220030, 0, 3);
		
		String CVA8220035 = getD2Deposit().getMap().get(D2DepositHeaderKey.CVA8220035.getCode());
		idacell("N�mero de trabajadores afectados por el ERTE: " + CVA8220035, 4, 7);
		...
		idacell("Duraci�n (N�mero de d�as): " + CVA8220065, 4, 7);
		row.setHeight(height0.shortValue());
		...
		row = sheet.createRow(rowCount++);
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), 0, 7));
		CellUtil.createCell(row, 0, "Alquileres (art�culos 1 al 15 Real Decreto-Ley11/2020)", headerCellStyle);
		...
		else if("2".equals(CVA8220080)) CVA8220080Str = "Reestructuraci�n de deudas";
		...
		idacell("Peque�os arrendadores. Ha concedido moratorias voluntarias a los arrendatarios: " + getBoolText(CVA8220090), 0, 7);
		
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		String CVA8220100 = getD2Deposit().getMap().get(D2DepositHeaderKey.CVA8220100.getCode());
		idacell("Ha recibido ayudas financieras p�blicas(incluidos avales) al alquiler del local de negocios: " + getBoolText(CVA8220100), 0, 7);
		...
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		idacell("Importe del aval concedido por el ICO en aplicaci�n de los establecido en los articulos 29 y 30 del Real Decreto-Ley 8/2020, de 17 de marzo", 0, 7);
		...
		row = sheet.createRow(rowCount++);
		sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), 0, 7));
		CellUtil.createCell(row, 0, "Ayudas p�blicas", headerCellStyle);
		
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		idacell("Describir el plan o programa al que se acoge, el concedente y el sistema(avales, moratoria, aplazamiento, inter�s bonificado etc.)", 0, 7);
		...
		idacell("Moratoria hipotecaria (art�culos 16 a 19 Real Decreto-Ley 11/2020): " + getBoolText(CVA8220140), 0, 7);
		
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		String CVA8220150 = getD2Deposit().getMap().get(D2DepositHeaderKey.CVA8220150.getCode());
		idacell("Moratoria no hipotecaria (art�culo 18, 21 a 26 Real Decreto-Ley 11/2020): " + getBoolText(CVA8220150), 0, 7);
		
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		String CVA8220160 = getD2Deposit().getMap().get(D2DepositHeaderKey.CVA8220160.getCode());
		idacell("Se ha solicitado flexifibilizaci�n y suspensi�n de suministros(art�culos 42 a 44 Real Decreto-Ley 11/2020): " + getBoolText(CVA8220160), 0, 7);
		row.setHeight(height0.shortValue());

		row = sheet.createRow(rowCount++);
		cellCount = 0;
		String CVA8220170 = getD2Deposit().getMap().get(D2DepositHeaderKey.CVA8220170.getCode());
		idacell("Se ha acogido a las medidas de apoyo del sector del Turismo de los art�culos 12 y 13 del Real Decreto-Ley 7/2020, de 12 de marzo: " + getBoolText(CVA8220170), 0, 7);
		row.setHeight(height0.shortValue());
	}
	...
	public void SRA() {
		...
		String text = "Esta hoja S�LO debe rellenarse si la sociedad ha realizado, durante el presente ejercicio, alguna operaci�n de prestaci�n de servicios a terceros de los contemplados en el articulo 2.1 o) de la Ley 10/2010, de 28 de abril, de prevenci�n del blanqueo de capitales y de la financiaci�n del terrorismo. �Rellenar hoja?";	
		...
		row = sheet.createRow(rowCount++);
		cellCount = 0;
 		idacell("�mbito territorial de operaciones:", 0, 1);
		...
		three("", "Ejercicio " + getD2Deposit().getYear(), "Ejercicio " + (getD2Deposit().getYear()-1)
			, "N�mero de Operaciones", D2PDepositConstants.SRP_KEYS, new D2DepositHeaderKey[][]{}, null);
	}
	...
	public void AP6C(){
		...
		sheet.createRow(rowCount++);
		// 8
		general(pageMaxNumber, 4, new String[]{"Valoraci�n y variaciones de valor de inversiones financieras valoradas a valor razonable"
	...
	public void IP(){
		...
		idacell(getBoolText(b2) + " Informe sobre informaci�n no financiera", 2, 3);
		idacell(getBoolText(c2) + " Declaraci�n de identificaci�n del titular real", 4, 5);
		...
		idacell(getBoolText(b3) + " Certificaci�n acuerdo", 2, 3);
