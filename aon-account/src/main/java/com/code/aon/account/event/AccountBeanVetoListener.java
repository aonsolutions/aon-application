...
public class AccountBeanVetoListener extends ManagerBeanVetoListenerAdapter {
    ...
    public void vetoableBeanInserted(ManagerBeanEvent evt) throws ManagerBeanVetoListenerException {
		...
		try {
			...
			criteria.addEqualExpression(accountBean.getFieldName(IAccountAlias.ACCOUNT_CODE),to.getCode());
	...
	private void checkParent(Account account) throws ManagerBeanVetoListenerException {
		if (account.getLevel()>1) {
			try {
				...
				IManagerBean accountBean = BeanManager.getManagerBean(Account.class);
				Criteria criteria = new Criteria();
				criteria.addEqualExpression(accountBean.getFieldName(IAccountAlias.ACCOUNT_CODE),parentCode);
