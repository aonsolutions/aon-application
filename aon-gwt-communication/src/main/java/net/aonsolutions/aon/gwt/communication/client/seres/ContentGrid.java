package net.aonsolutions.aon.gwt.communication.client.seres;
...
import com.esferalia.aon.gwt.api.client.communication.JsCommunicationInfo;
...
import net.aonsolutions.aon.gwt.communication.client.CommunicationPrincipal;
import net.aonsolutions.aon.gwt.communication.shared.CommunicationTarget;
import net.aonsolutions.aon.gwt.communication.shared.SeresFileStatus;
...
public class ContentGrid extends ResizeComposite implements RequiresResize {
	...
	@UiField(provided = true) public CustomDataGrid<JsCommunicationInfo> dataGrid; 
	
	private CommunicationPrincipal parent;
	...
	public ContentGrid(CommunicationPrincipal parent) {
		this(parent, new LinkedList<>());	
	}

	public ContentGrid(CommunicationPrincipal parent, List<JsCommunicationInfo> list) {
		this.parent = parent;		
		dataGrid = new CustomDataGrid<JsCommunicationInfo>(Integer.MAX_VALUE, resources,
				JsCommunicationInfo.PROVIDES_KEY);
...
//				}
	...
	LinkedList<JsCommunicationInfo> selFiles = new LinkedList<>();
	
	private void load(List<JsCommunicationInfo> list) {
		DefaultKeyboardSelectionHandler<JsCommunicationInfo> selHandler = new DefaultKeyboardSelectionHandler<JsCommunicationInfo>(dataGrid){
			@Override
			public void onCellPreview(CellPreviewEvent<JsCommunicationInfo> event) {
				 if(BrowserEvents.CLICK.equals(event.getNativeEvent().getType())){
					 JsCommunicationInfo object = dataProvider.getList().get(dataGrid.getKeyboardSelectedRow());
					 if(event.getColumn() == 0){
						 ...
						 for(JsCommunicationInfo f :dataProvider.getList()){
					...
					if(parent.command.equals(CommunicationTarget.OUTCOME_DELIVERY)
						...
						parent.getReopenAll().setVisible(enable);
					}
...
//							&& !"cp_cobros".equals(sii) && !"cp_pagos".equals(sii));
		...
		ListHandler<JsCommunicationInfo> sortHandler = getSortHandler();
		dataGrid.addColumnSortHandler(sortHandler);
		final MultiSelectionModel<JsCommunicationInfo> selectionModel = new MultiSelectionModel<JsCommunicationInfo>(JsCommunicationInfo.PROVIDES_KEY);
		
		dataGrid.setSelectionModel(selectionModel,
				DefaultSelectionEventManager.<JsCommunicationInfo> createCheckboxManager());
	...
	private ListDataProvider<JsCommunicationInfo> dataProvider = new ListDataProvider<JsCommunicationInfo>();
	
	public void addAll(List<JsCommunicationInfo> list){
		dataProvider.getList().addAll(list);
	}

	public void addDataDisplay(HasData<JsCommunicationInfo> display, List<JsCommunicationInfo> list) {
		dataProvider = new ListDataProvider<JsCommunicationInfo>(list);
		dataProvider.addDataDisplay(display);
	}
		
	private ListHandler<JsCommunicationInfo> getSortHandler() {
		return new ListHandler<JsCommunicationInfo>(dataProvider.getList()){
			@Override
			public void onColumnSort(ColumnSortEvent event) {
				super.setList(dataProvider.getList());
				super.onColumnSort(event);
				List<JsCommunicationInfo> aux  = super.getList();
				List<JsCommunicationInfo> aux2 = new LinkedList<JsCommunicationInfo>();
	...
	private void initTableColumns(final MultiSelectionModel<JsCommunicationInfo> selectionModel, ListHandler<JsCommunicationInfo> sortHandler) {
		
		/** Check Column **/
		Column<JsCommunicationInfo, Boolean> checkColumn = new Column<JsCommunicationInfo, Boolean>(new CheckboxCell(true, true) {
			@Override
			...
			@Override
			public Boolean getValue(JsCommunicationInfo object) {
				return selectionModel.isSelected(object);
			}
		};
		
		/** Code Column **/
		Column<JsCommunicationInfo, String> referenceCodeColumn = new Column<JsCommunicationInfo, String>(new TextCell()) {

			@Override
			public String getValue(JsCommunicationInfo object) {
		...
		sortHandler.setComparator(referenceCodeColumn,new Comparator<JsCommunicationInfo>() {
			
			@Override
			public int compare(JsCommunicationInfo o1, JsCommunicationInfo o2) {
				return o1.getReferenceCode().compareTo(o2.getReferenceCode());
			}
		});

		/** Date Column **/
		Column<JsCommunicationInfo, String> taxDateColumn = new Column<JsCommunicationInfo, String>(new TextCell()) {
		...
		sortHandler.setComparator(taxDateColumn,new Comparator<JsCommunicationInfo>() {
		...
		/** Name Column **/
		Column<JsCommunicationInfo, String> customerColumn = new Column<JsCommunicationInfo, String>(new TextCell()) {
		...
		sortHandler.setComparator(customerColumn,new Comparator<JsCommunicationInfo>() {
		...
		/** Status Column **/
		Column<JsCommunicationInfo, String> statusColumn = new Column<JsCommunicationInfo, String>(new TextCell()) {
			@Override	
			public void render(Context context, JsCommunicationInfo object, SafeHtmlBuilder sb) {
...
//				else if(object.getStatus().equals("AceptadoConErrores")) icon = AON.AON_CSS.aonIconPointOrange();
		...
		sortHandler.setComparator(statusColumn,new Comparator<JsCommunicationInfo>() {
...
//		dataGrid.addColumn(checkColumn, "");
	...
	public final class CheckboxHeader extends Header<Boolean> {
	    ...
	    private final MultiSelectionModel<JsCommunicationInfo> selectionModel;
	    private final ListDataProvider<JsCommunicationInfo> provider;

	    public CheckboxHeader(MultiSelectionModel<JsCommunicationInfo> selectionModel,
	    		ListDataProvider<JsCommunicationInfo> provider) {
	    ...
	    public void onBrowserEvent(Context context, Element elem, NativeEvent event) {
	        ...
	        for (JsCommunicationInfo element : provider.getList()) {
	...
	public void reopen(String action) {
	...
	private void reopenIngenetSales() {
		HashMap<String, LinkedList<String>> map =  new HashMap<>();
		LinkedList<String> list = selFiles.stream().map(s -> s.getId() + "").collect(Collectors.toCollection(LinkedList::new));
		map.put("id_list", list);
		selFiles.clear();
		...
		getAPI().getCommunication().reopenIngenetSales(map, new AsyncCallback<JSON<JsObject>>() {
			@Override
			public void onSuccess(JSON<JsObject> result) {
				VerticalPanel vp = new VerticalPanel();
