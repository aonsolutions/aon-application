...
import com.esferalia.aon.gwt.api.client.communication.JsAttachFile;
import com.esferalia.aon.gwt.api.client.communication.JsCommunicationInfo;
import com.esferalia.aon.gwt.api.client.communication.JsSummary;
...
public class CommunicationPrincipal extends Composite{
	
	interface Binder extends UiBinder<Widget, CommunicationPrincipal> {}
	...
	@UiField public MinimizePanel footPanel;
	@UiField public TabLayoutPanel tabLayout; 
	@UiField public ScrollPanel errorPanel;
	@UiField public ScrollPanel historyPanel;

	public CommunicationMain parent;
	public CommunicationPrincipal me;
	public CommunicationTarget command;
	...
	public CommunicationPrincipal(CommunicationMain parent) {
...
//					});
	...
	public void gridContent(CommunicationTarget command){
		...
		if(command!=null) { 
			getFilterMap().put("communication",new LinkedList<>(Arrays.asList(new String[] {command.getValue()})));
		} else {
	...
	private Widget createSummaryPanel() {
		...
		getAPI().getCommunication().getSummary(getFilterMap(), new AsyncCallback<JSON<JsSummary>>() {
...
//					}
	...
	public void reloadContentGrid(ContentGrid contentGrid) {
		if(CommunicationTarget.OUTCOME_DELIVERY==command){
			getAPI().getCommunication().getOutcomeDelivery(getFilterMap(), new AsyncCallback<JSON<JsCommunicationInfo>>() {
			...
			getAPI().getCommunication().getOutcomeInvoice(getFilterMap(), new AsyncCallback<JSON<JsCommunicationInfo>>() {
			...
			getAPI().getCommunication().getIncomeSales(getFilterMap(), new AsyncCallback<JSON<JsCommunicationInfo>>() {
			...
			getAPI().getCommunication().getIncomeInvoice(getFilterMap(), new AsyncCallback<JSON<JsCommunicationInfo>>() {
	...
	public void consoleLog(String string) {
		CommunicationMain.consoleLog(string);
	}
