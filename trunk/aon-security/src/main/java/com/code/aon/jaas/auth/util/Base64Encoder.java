...
/**
 * BASE64 encoder implementation. Provides encoding
 * methods, using the BASE64 encoding rules, as defined in the MIME
...
public final class Base64Encoder {
     ...
     */
    private static final byte[] ENCODING = { (byte) 'A', (byte) 'B',
    ...
    /**
     * Encode the supplied byte array and write the
     * encoded data to the OutputStream <i>out </i>.
     * 
     * @param input
     *            byte[]
     * @param out
    ...
    /**
     * Encode the given string,and return the
     * encoded version as a string.
     * 
     * @param input
     *            String
    ...
    public static String encode(String input) throws IOException {
        byte[] bytes;
        bytes = input.getBytes(ISO_8859_1);
        return encode(bytes);
    }

    /**
     * Encode the given byte array and return the
     * result as a string.
     * 
     * @param bytes
     *            byte[]
     ...
     * 1) { System.out.println("[" + Base64Encoder.encode(args[0]) + "]"); //
     * joe:eoj -> am9lOmVvag== // 12345678:87654321 -> MTIzNDU2Nzg6ODc2NTQzMjE= }
     * else if (args.length == 2) { byte[] hash =
     * java.security.MessageDigest.getInstance(args[1])
     * .digest(args[0].getBytes()); System.out.println("[" +
     * Base64Encoder.encode(hash) + "]"); } else { System.out .println("Usage:
     * Base64Encoder <string> <optional hash algorithm>"); } }
     */

    // Private ----------------------------------------------------------------
    private static int get1(byte buf[], int off) {
        return (buf[off] & 0xfc) >> 2; // $codepro.audit.disable
    }

    private static int get2(byte buf[], int off) {
        return ((buf[off] & 0x3) << 4) | ((buf[off + 1] & 0xf0) >>> 4); // $codepro.audit.disable
    }

    private static int get3(byte buf[], int off) {
        return ((buf[off + 1] & 0x0f) << 2) | ((buf[off + 2] & 0xc0) >>> 6); // $codepro.audit.disable
    }

    private static int get4(byte buf[], int off) {
        return buf[off + 2] & 0x3f; // $codepro.audit.disable
    }

    /**
     * Process the data: encode the input stream to
     * the output stream. This method runs through the input stream, encoding it
    ...
    private static void process(InputStream in, OutputStream out)
        ...
        while ((got = in.read(buffer, off, BUFFER_SIZE - off)) > 0) {
            if (got >= 3) { // $codepro.audit.disable numericLiterals
                got += off;
                off = 0;
                while (off + 3 <= got) { // $codepro.audit.disable
                    // numericLiterals
                    ...
                    switch (count) {
                    case 73: // $codepro.audit.disable numericLiterals
                        ...
                        count = 1;
                        break;
                    case 74: // $codepro.audit.disable numericLiterals
                        out.write(ENCODING[c1]);
                        out.write(ENCODING[c2]);
                        out.write('\n');
                        out.write(ENCODING[c3]);
                        out.write(ENCODING[c4]);
                        count = 2; // $codepro.audit.disable numericLiterals
                        break;
                    case 75:// $codepro.audit.disable numericLiterals
                        out.write(ENCODING[c1]);
                        out.write('\n');
                        out.write(ENCODING[c2]);
                        out.write(ENCODING[c3]);
                        out.write(ENCODING[c4]);
                        count = 3; // $codepro.audit.disable numericLiterals
                        break;
                    case 76: // $codepro.audit.disable numericLiterals
                        out.write('\n');
                        out.write(ENCODING[c1]);
                        out.write(ENCODING[c2]);
                        out.write(ENCODING[c3]);
                        out.write(ENCODING[c4]);
                        count = 4; // $codepro.audit.disable numericLiterals
                        break;
                    default:
                        out.write(ENCODING[c1]);
                        out.write(ENCODING[c2]);
                    ...
                    off += 3; // $codepro.audit.disable numericLiterals
                }
                // Copy remaining bytes to beginning of buffer:
                for (int i = 0; i < 3; i++) {// $codepro.audit.disable
                    // numericLiterals
