...
public class CampaignProjectExtended implements ITransferObject {
	...
	private CampaignProject campaignProject;
	...
	public CampaignProject getCampaignProject() {
		return campaignProject;
	}
	public void setCampaignProject(CampaignProject campaignProject) {
	...
	public String getAssigned() {
		String s = null;
		if (getTask() != null) {
			if (!getTask().isUnassigned()) {
				s = getTask().getTaskHolder().getRegistry().getName();
	...
	public String getProject() {
		String fn = getCampaignProject().getProject().getRegistry().getFullName();
		fn = StringUtils.abbreviate(fn, 25);
		fn = fn + " [" + getCampaignProject().getProject().getName() + "]";
		return fn;
	}
}