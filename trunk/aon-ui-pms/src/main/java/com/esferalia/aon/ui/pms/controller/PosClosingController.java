...
public class PosClosingController {
    ...
    public Double getTotalCashAmount() throws ManagerBeanException {
        if(getPosShift()!=null && getPosShift().getId()!=null){
            ...
            criteria.addEqualExpression(bean.getFieldName(IEntityAlias.POS_SHIFT_COUNT_POS_SHIFT_ID), getPosShift().getId());
            criteria.addInExpression(bean.getFieldName(IEntityAlias.POS_SHIFT_COUNT_PAY_METHOD_ID), getCashPayMethodIds());
            Projection amountProjection = Projection.sum(bean.getFieldName(IEntityAlias.POS_SHIFT_COUNT_AMOUNT));
            Double amount = (Double)bean.getUniqueResult(amountProjection, criteria);
            return CommonUtil.round(amount==null?0:amount);
        }
        return null;
    }
   
    private List<Integer> getCashPayMethodIds() throws ManagerBeanException {
    ...
    public void onHotelChange( ActionEvent event ){
        try {
            ...
            String msg = "Error al buscar las cajas del hotel.";
