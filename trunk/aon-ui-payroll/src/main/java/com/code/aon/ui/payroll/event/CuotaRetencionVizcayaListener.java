...
public class CuotaRetencionVizcayaListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		CuotaRetencionVizcaya n = (CuotaRetencionVizcaya) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(CuotaRetencionVizcaya.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.CUOTA_RETENCION_VIZCAYA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				CuotaRetencionVizcaya cra = (CuotaRetencionVizcaya) iter.next();
