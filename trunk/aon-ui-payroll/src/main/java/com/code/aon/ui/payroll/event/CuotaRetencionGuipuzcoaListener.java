...
public class CuotaRetencionGuipuzcoaListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		CuotaRetencionGuipuzcoa n = (CuotaRetencionGuipuzcoa) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(CuotaRetencionGuipuzcoa.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.CUOTA_RETENCION_GUIPUZCOA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				CuotaRetencionGuipuzcoa cra = (CuotaRetencionGuipuzcoa) iter.next();
