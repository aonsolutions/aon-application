...
public class CuotaRetencionGuipuzcoaController extends PayrollBasicController {
	...
	public void onSearch(ActionEvent event) {
		...
		try {
			if(searchFecini != null){
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.CUOTA_RETENCION_GUIPUZCOA_ID_FECINI), searchFecini);
			}
			if(searchFecfin != null){
			
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.CUOTA_RETENCION_GUIPUZCOA_FECFIN), searchFecfin);
			}
