...
public class RemesaCertificadoGenerationWizard implements Serializable {
	...
	private static final String[] STEPS = { "remesaCertificadoGenerationWizard_step0", "remesaCertificadoGenerationWizard_step1", "remesaCertificadoGenerationWizard_step2" };
...
//	}
	...
	private DataModel initializeModel() throws PayrollException {
		List<RemesableEmpleadoCertificate> list = transformList(getEmpresaDAO().getEmpleados(params));
	...
	private boolean existEmpresa(IEmpresa empresa) {
		...
		while(iterator.hasNext()){
			IRemesaCertificadoEmpresa remesa= (IRemesaCertificadoEmpresa)iterator.next();
...
//					r.setCodigoCcc();
	...
	@SuppressWarnings("unchecked")
	private List<IRemesaCertificadoEmpresaDetalle> getRemesaDetalleList(IRemesaCertificadoEmpresa remesa) {
		List<IRemesaCertificadoEmpresaDetalle> list = new ArrayList<IRemesaCertificadoEmpresaDetalle>();
		
		for (RemesableEmpleadoCertificate d : (List<RemesableEmpleadoCertificate>) getSelectedModel().getWrappedData()) {
			if (d.getEmpleado().getEmpresa().equals(remesa.getEmpresa())) {
				IRemesaCertificadoEmpresaDetalle detalle = getEmpresaDAO().getNewRemesaDetalle(d.getEmpleado());
