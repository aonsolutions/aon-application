...
public class CustomerAccountCheckerSearchListener extends RegistryPayMethodSearchListener {
	...
	protected void completeCriteria(Criteria criteria) throws ManagerBeanException, ExpressionException {
		if (isShowEmptyAccountsOnly()) {
			criteria.addNullExpression(getFieldName(IEntityAlias.CUSTOMER_ACCOUNT));
		} else if (getAccount() != null && getAccount().getId() != null) {
			criteria.addEqualExpression(getFieldName(IEntityAlias.CUSTOMER_ACCOUNT), getAccount().getId());
		}
