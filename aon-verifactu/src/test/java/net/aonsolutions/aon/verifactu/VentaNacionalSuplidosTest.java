...
class VentaNacionalSuplidosTest {
	...
	private void assertInvoice( VerifactuContext vc ) throws VerifactuException {
	    ...
	    // ------------ Linea con IVA
	    DetalleType dt = AonCollectionUtils.stream(listaDesglose)
	    	.filter( d -> d.getCalificacionOperacion() ==  CalificacionOperacionType.S_1)
	    	.findFirst()
	    	.orElseThrow( () -> new IllegalStateException("Desglose no encontrado")) 
    	;
	    assertNotNull( dt );
	    ...
	    // ------------ Linea con IVA
	    DetalleType sup = AonCollectionUtils.stream(listaDesglose)
	    	.filter( d -> d.getCalificacionOperacion() ==  CalificacionOperacionType.N_1)
	    	.findFirst()
	    	.orElseThrow( () -> new IllegalStateException("Suplido no encontrado")) 
    	;
	    assertNotNull( sup );
	    assertEquals( TipoImpuesto.IVA.getValue() , sup.getImpuesto() );
	    assertEquals( ClaveRegimen.C01_PREPAYMENT.getValue() , sup.getClaveRegimen() );
	    assertEquals( CalificacionOperacionType.N_1 , sup.getCalificacionOperacion() );
	    assertNull( sup.getOperacionExenta() );		
	    assertNull( sup.getTipoImpositivo());
	    assertEquals( "500" , sup.getBaseImponibleOimporteNoSujeto());
	    assertNull( sup.getBaseImponibleACoste() );
	    assertNull( sup.getCuotaRepercutida());
	    assertNull( sup.getTipoRecargoEquivalencia() );		
	    assertNull( sup.getCuotaRecargoEquivalencia() );
	    // ------------ FIN linea con IVA
