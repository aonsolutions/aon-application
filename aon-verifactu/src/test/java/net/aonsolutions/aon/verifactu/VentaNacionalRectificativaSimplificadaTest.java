...
class VentaNacionalRectificativaSimplificadaTest {
	
	@Test
	void ventaNacionalSimpleNoAct() throws VerifactuException {
		Company c = company();
		List<Invoice> invoices = new LinkedList<>();
		invoices.add( InvoiceTypes.VENTA_NACIONAL_RECTIFICATIVA_SIMPLIFICADA() );
	...
	void ventaNacionalSimpleActGeneral() throws VerifactuException {
		Company c = company();
		List<Invoice> invoices = new LinkedList<>();
		invoices.add( InvoiceTypes.VENTA_NACIONAL_RECTIFICATIVA_SIMPLIFICADA().setActivity(InvoiceTypes.ACTIVITY_GENERAL));
		VerifactuContext vc = new VerifactuContext()
			.setConfig( config() )
			.setCompany( c )
			.setInvoices(invoices);
