...
class VentaNacionalSimpleCriterioCajaTest extends AbstractVerifactuTest {
	...
	void ventaNoAct() throws VerifactuException {
		...
		invoices.add( InvoiceTypes.Invoices.VENTA_NACIONAL_SIMPLE_CRITERIO_CAJA.get( ctx ) );
	...
	void ventaActGeneral() throws VerifactuException {
		Company c = company();
		List<Invoice> invoices = new LinkedList<>();
		invoices.add( InvoiceTypes.Invoices.VENTA_NACIONAL_SIMPLE_CRITERIO_CAJA.get( ctx ).setActivity( InvoiceTypes.ACTIVITY_GENERAL) );
		VerifactuContext vc = new VerifactuContext()
			.setConfig( config() )
			.setCompany( c )
			.setInvoices(invoices);
