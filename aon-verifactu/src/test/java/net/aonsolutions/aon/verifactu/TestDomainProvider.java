...
class TestDomainProvider {
	...
	private static void initializeDomain(AONContext context, Domain domain) {
		...
		insertAccounts(context, domain);
		insertTaxes( context, domain );		
	...
	private static void insertTaxes(AONContext ctx, Domain domain) {
		ctx.getDslContext().insertInto(TAX)
			.set(TAX.DOMAIN, domain.getId())
			.set(TAX.NAME , "Tipo General")
			.set(TAX.TAX_TYPE , TaxType.VAT.value())
			.set(TAX.PERCENTAGE , 21.0 )
			.set(TAX.SURCHARGE , 5.2 )
			...
			.execute();
		ctx.log().info("TAX Tipo General inserted");
		
		ctx.getDslContext().insertInto(TAX)
			.set(TAX.DOMAIN, domain.getId())
			.set(TAX.NAME , "Tipo Reducido")
			.set(TAX.TAX_TYPE , TaxType.VAT.value())
			.set(TAX.PERCENTAGE , 10.0 )
			.set(TAX.SURCHARGE , 1.4 )
			.set(TAX.START_DATE , AonDateUtils.toSql(AonDateUtils.getDate( 2010, 0, 1) ))
			.set(TAX.VAT_DEDUCTION_TYPE,  VatDeductionType.WITH_RIGHT.value() )
		.execute();
		ctx.log().info("TAX Tipo Reducido inserted");

		ctx.getDslContext().insertInto(TAX)
			.set(TAX.DOMAIN, domain.getId())
			.set(TAX.NAME , "Tipo Superreducido")
			.set(TAX.TAX_TYPE , TaxType.VAT.value())
			.set(TAX.PERCENTAGE , 4.0 )
			.set(TAX.SURCHARGE , 0.5 )
			.set(TAX.START_DATE , AonDateUtils.toSql(AonDateUtils.getDate( 2010, 0, 1) ))
			.set(TAX.VAT_DEDUCTION_TYPE,  VatDeductionType.WITH_RIGHT.value() )
		.execute();
	...
	private static void insertAccounts(AONContext ctx, Domain domain) {
		for (Account acc : ACCOUNTS) {
			Account account = AccountDAO.get(ctx, acc.getCode());
			if (account == null) {
				account = acc.setDomain(domain.getId()).setActive( true );
				AccountDAO.save(ctx, account);
			}
		}
		ctx.log().info("Cuentas contables insertadas correctamente");
	}
