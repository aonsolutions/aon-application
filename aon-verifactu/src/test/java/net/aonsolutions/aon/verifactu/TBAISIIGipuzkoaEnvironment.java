...
final class TBAISIIGipuzkoaEnvironment extends VerifactuEnvironmentAbs {
	protected String domainName = "tbaisiigipuzkoatest.aonsolutions.test";	
	...
	public void initializeDomain(AONContext ctx) {
		...
		ctx.log().info("TBAI ENABLED! ( GIPUZKOA / TEST)");
		
		cd = new CommunicationData()
			.setDomain(ctx.getDomainId())
			.setEnterprise(enterprise.getId())
			...
			.setAdministration(Administration.GIPUZKOA)
		;
		ICCDAO.enableSii(ctx, ctx.getDomainId(), cd);
		ctx.log().info("SII ENABLED! ( GIPUZKOA / TEST)");
	}
