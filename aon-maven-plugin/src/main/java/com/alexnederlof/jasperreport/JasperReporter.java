...
public class JasperReporter extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		List<CompileTask> tasks = generateTasks(sourceDirectory,
				outputDirectory);
	 ...
	 */
	private void checkOutDirWritable(File outputDirectory)
			throws MojoExecutionException {
	...
	private void configureJasper() {
		DefaultJasperReportsContext jasperReportsContext = DefaultJasperReportsContext
				.getInstance();
		jasperReportsContext.setProperty(
				JRReportSaxParserFactory.COMPILER_XML_VALIDATION,
				String.valueOf(xmlValidation));
		jasperReportsContext.setProperty(JRCompiler.COMPILER_PREFIX,
	...
	private void checkIfOutpuCanBeCreated() throws MojoExecutionException {
		if (!outputDirectory.mkdirs()) {
			throw new MojoExecutionException(this,
					"Output folder could not be created", "Outputfolder "
	...
	private void checkIfOutputDirIsWritable() throws MojoExecutionException {
		if (!outputDirectory.canWrite()) {
			throw new MojoExecutionException(this,
					"Could not write to output folder",
					"Could not write to output folder: "
							+ outputDirectory.getAbsolutePath());
		}
	}

	private List<CompileTask> generateTasks(File sourceDirectory,
			File destinationDirectory) {
		Validate.isTrue(sourceDirectory.isDirectory(),
				sourceDirectory.getName() + " is not a directory");
	...
	private void generateTasks(File destinationDirectory,
			List<CompileTask> tasks, File f) {
		if (f.isDirectory()) {
			tasks.addAll(generateTasks(f,
					createNewDest(f, destinationDirectory)));
		} else { // It is a file
			if (f.getName().endsWith(sourceFileExt)) {
				tasks.add(new CompileTask(f, destinationDirectory,
						sourceFileExt, outputFileExt, log, verbose));
			} else if (verbose) {
				log.info("Skipped " + f.getName()
						+ " because it doesnt have the extension "
						+ sourceFileExt);
			}
		}
	}
	...
	private void executeTasks(List<CompileTask> tasks)
			throws MojoExecutionException {
		try {
			long t1 = System.currentTimeMillis();
			List<Future<Void>> output = Executors.newFixedThreadPool(
					numberOfThreads).invokeAll(tasks);
			long time = (System.currentTimeMillis() - t1);
			getLog().info(
					"Generated " + output.size() + " jasper reports in "
							+ (time / 1000.0) + " seconds");
			checkForExceptions(output);
		} catch (InterruptedException e) {
			log.error("Failed to compile Japser reports: Interrupted!", e);
			throw new MojoExecutionException(
					"Error while compiling Jasper reports", e);
	...
	private void checkForExceptions(List<Future<Void>> output)
