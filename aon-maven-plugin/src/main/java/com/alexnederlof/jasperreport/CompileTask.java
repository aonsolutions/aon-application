...
public class CompileTask implements Callable<Void> {
	 ...
	 */
	public CompileTask(File source, File destFolder, String sourceExtension,
			String outExtension, Log log, boolean verbose) {
		super();
		this.source = source;
	...
	public Void call() throws Exception {
		...
		try {
			if (source.getAbsolutePath().endsWith(sourceExtension)) {
				...
				JasperCompileManager.compileReportToStream(new FileInputStream(
						source), new FileOutputStream(out));
				if (verbose) {
	...
	private String getNewFilename() {
		return source.getName().substring(0,
				source.getName().lastIndexOf(sourceExtension))
				+ outExtension;
	}
