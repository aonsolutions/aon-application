...
public class BudgetTemplate extends PdfFile{
	...
	private static void draw_products(BudgetTemplate template) {
		...
		products.forEach( p ->{
			...
			.x						(template.x() + 370)
			.y						(template.y())
			.content				(toLatinNumber(p.getPrice().orElse(0.00)) + currency)
			.horizontalAlignment	(RIGHT);
	...
	private static void draw_conditions(BudgetTemplate template) {
		...
		template.budget.getTerms().orElse(new ArrayList<Term>()).stream().forEach(term ->{
			...
			int jumping_line = text.drawMultiple(template.limitY);
			
			if(jumping_line != -1) {
				new_page(template);
				text.restart(template.contents,jumping_line, template.y()).drawMultiple(template.limitY);
			}
