package com.esferalia.aon.in.payroll.pdf.maker.invoice.templates;
...
public class InvoiceTemplate {
	...
	public void draw_detailed_entries(PDDocument doc) throws IOException {
		if (bill.getEntries() != null) {
			for (InvoiceEntry entry : bill.getEntries()) {
				...
				ArrayList<String> divided = (ArrayList<String>) getLines(entry.getDescription(), 240, PdfFonts.HELVETICA, 8);
				...
				PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 69, 15), PdfFormats.toLatinNumber(entry.getQuantity()) + "", PdfColors.BLACK, PdfFonts.HELVETICA, 8, 4.5f, 0);
				x += 70;

				PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 69, 15), PdfFormats.toLatinNumber(entry.getPrice()), PdfColors.BLACK, PdfFonts.HELVETICA, 8, 4.5f, 0);
				x += 70;

				String percent = "";
				if (entry.getPercent() > 99)			percent = "100%";
				else if (entry.getPercent() != 0) 		percent = PdfFormats.toLatinNumber(entry.getPercent()) + "%";


				PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 39, 15), percent, PdfColors.BLACK, PdfFonts.HELVETICA, 8, 4.5f, 0);
				x += 40;

				PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 69, 15), PdfFormats.toLatinNumber(entry.getAmount()), PdfColors.BLACK, PdfFonts.HELVETICA, 8, 4.5f, 0);
				y = dy - 10;
	...
	public void draw_simplified_entries(PDDocument doc) throws IOException {
		if (bill.getEntries() != null) {
			for (InvoiceEntry entry : bill.getEntries()) {
				...
				PDFToolkit.drawText(contents, PDFToolkit.croppedString(entry.getDescription(), 420, PdfFonts.HELVETICA, 8), x + 5, y, PdfColors.BLACK, PdfFonts.HELVETICA, 8);
				x += 430;
				PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 69, 15), PdfFormats.toLatinNumber(entry.getAmount()), PdfColors.BLACK, PdfFonts.HELVETICA, 8, 4.5f, 0);
				y -= 10;
			}
	...
	private void draw_taxes() throws IOException {
		...
		for (InvoiceTax tax : bill.getTaxes()) {
			x = 240;
			PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 79, 15), PdfFormats.toLatinNumber(tax.getBase()), PdfColors.BLACK, PdfFonts.HELVETICA, 7, 5, -12);
			x += 80;

			String percent = "";
			if (tax.getPercentage() > 99) percent = "100%";
			else if (tax.getPercentage() != 0) percent = PdfFormats.toLatinNumber(tax.getPercentage()) + "%";
			...
			x += 60;
			PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 49, 15), PdfFormats.toLatinNumber(tax.getQuota()), PdfColors.BLACK, PdfFonts.HELVETICA, 7, 5, -12);
			x += 50;
		...
		PDFToolkit.drawTextRight(contents, new PDRectangle(x, bottom + 107, 69, 15), PdfFormats.toLatinNumber(sum) + " \u20AC", PdfColors.BLACK, PdfFonts.HELVETICA_BOLD, 8, 5, -14);
	}
	...
	private void draw_finances() throws IOException {
		...
		for (InvoiceFinance finance : bill.getFinances()) {
			...
			x += 160;
			PDFToolkit.drawTextRight(contents, new PDRectangle(x, y, 69, 15), PdfFormats.toLatinNumber(finance.getAmount()), PdfColors.BLACK, PdfFonts.HELVETICA, 7, 5, -12);
