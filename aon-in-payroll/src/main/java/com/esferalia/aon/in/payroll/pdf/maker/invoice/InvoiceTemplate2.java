...
public class InvoiceTemplate2 {
	...
	public void drawDetailedEntries(PDDocument doc, Invoice invoice, PrintInvoiceConfiguration config) throws IOException {
		if (invoice.getDetails() != null){
			for (InvoiceDetail detail : invoice.getDetails()) {
				...
				drawTextRight(contents, new PDRectangle(x, y, 69, 15), toLatinNumber(detail.getQuantity()) + "", BLACK, HELVETICA, 8, 4.5f, 0);
				x += 70;

				drawTextRight(contents, new PDRectangle(x, y, 69, 15),toLatinNumber(detail.getPrice()), BLACK, HELVETICA, 8, 4.5f, 0);
				x += 70;

				drawTextRight(contents, new PDRectangle(x, y, 39, 15), detail.getDiscountExpression(), BLACK, HELVETICA, 8, 4.5f, 0);
				x += 40;

				drawTextRight(contents, new PDRectangle(x, y, 69, 15), toLatinNumber(detail.getTaxableBase()), BLACK, HELVETICA, 8, 4.5f, 0);
				y = dy - 10;
	...
	private void drawTopInfo(Invoice invoice) throws IOException {
		...
		drawText(contents, "Numero: " + invoice.getReferenceCode(), x, y, BLACK, HELVETICA, 11);
		y -= 4;
		...
		if (invoice.getIssueDate() != null)
			drawText(contents, "Fecha: " + formatDate(invoice.getIssueDate(), "dd,MM,yyyy").get(), x, y,BLACK, HELVETICA, 11);
		else
		...
		drawText(contents, "N.I.F: " + invoice.getRegistryDocument(), x, y, BLACK, HELVETICA, 11);
		y -= 4;
		...
		x += 10;
		y  = height - top - 45;
		String str = new String(invoice.getRegistryName().replaceAll("\t", " ").getBytes(Charset.forName("ASCII")), Charset.forName("UTF-8") );
		...
		String zipCityProvince = invoice.getAddressZIP() + " "+ invoice.getAddressTown() + " " + invoice.getAddressProvince(); 
		drawText(contents, zipCityProvince, x, y, BLACK, HELVETICA, 9);
	}
	...
	private void drawTaxes(Invoice invoice) throws IOException {
		...
		for (InvoiceBreakdown tax : invoice.getBreakdown()) {
			...
			drawTextCenter(contents, new PDRectangle(x, y, 59, 15), tax.getTaxType().getName(), BLACK, HELVETICA, 7, -12);
			x += 60;
	...
	private void drawFinances(Invoice invoice) throws IOException {
		...
		for (Finance finance : invoice.getFinances()) {
			...
			drawText(contents, finance.getPayMethodType().getDescription(), x + 5f, y - 12, BLACK, HELVETICA,
					7);
			x += 80;
			if(finance.getBankAccount() != null)
				drawText(contents, finance.getBankAccount().getIban(), x + 5f, y - 12, BLACK, HELVETICA, 7);
			x += 160;
