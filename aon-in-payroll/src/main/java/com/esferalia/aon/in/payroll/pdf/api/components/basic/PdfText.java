...
import static com.esferalia.aon.in.payroll.pdf.api.settings.PdfSettings.VERTICAL_ALIGNMENT.DOWN;
...
public class PdfText extends PdfComponent {
	...
	@Deprecated
	public PdfText(float x, float y, float width, float height, float marginX, PDPageContentStream stream,
	...
	@Deprecated
	public PdfText(float x, float y, float width, float height, float marginX, float marginY,
			PDPageContentStream stream, String content, Color color, PDFont font, Float fontSize, ALIGNMENT alignment) {
		this.startPointer();
		this.x(x);
		this.y(y);
		this.width(width);
