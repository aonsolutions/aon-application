...
public class SalaryHoursUpdateMonthDays implements Update {
	...
	private SalaryHoursUpdateMonthDays() {
		super();
	}
	...
	public void upgrade(Connection connection) {
		...
		.and(SYSTEM_DATA.EXPRESSION.contains("DIAS_NOMINA/DIAS_MES"))) == 1;

		if ( upgraded ) 
			return;

		// DOMAIN = 0 , GENERAL
		UpdateConditionStep<SystemDataRecord> updateMonthDays = 
		dslContext
		.update(SYSTEM_DATA)
		.set(SYSTEM_DATA.EXPRESSION, 
		DSL.replace(SYSTEM_DATA.EXPRESSION, "DIAS_NOMINA/30", "DIAS_NOMINA/DIAS_MES"))
		.where(SYSTEM_DATA.DOMAIN.eq(0))
		.and(SYSTEM_DATA.NAME.eq(SALARY_HOURS))
		;
