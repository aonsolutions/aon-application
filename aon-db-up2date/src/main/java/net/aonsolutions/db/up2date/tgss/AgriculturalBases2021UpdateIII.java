...
public class AgriculturalBases2021UpdateIII implements Update {

	public static AgriculturalBases2021UpdateIII AGRICULTURALBASES2021UPDATEIII = new AgriculturalBases2021UpdateIII();
	...
	public void upgrade(Connection connection) {
		...
		UpdateConditionStep<SystemDataRecord> close2019BasesCgcMin =
		dslContext
		.update(SYSTEM_DATA)
		...
		Date start2016Date = new Date(calendar.getTimeInMillis());

		UpdateConditionStep<SystemDataRecord> close2016BasesCgpMin =
		dslContext
		.update(SYSTEM_DATA)
		.set(SYSTEM_DATA.END_DATE, endAugust2021Date)
		.where(SYSTEM_DATA.DOMAIN.eq(DOMAIN))
		.and(SYSTEM_DATA.NAME.in(BASE_CGP_MIN_MES, BASE_CGP_MIN_DIA))
		.and(SYSTEM_DATA.START_DATE.eq(start2016Date))
		;

		InsertOnDuplicateStep<SystemDataRecord> insertSeptember2021BaseCgcMinMes = 
		dslContext
		...
		InsertOnDuplicateStep<SystemDataRecord> insertSeptember2021BaseCgcMinDia = 
		dslContext
		.insertInto(SYSTEM_DATA, 
		SYSTEM_DATA.DOMAIN,
		SYSTEM_DATA.NAME,
		...
		InsertOnDuplicateStep<SystemDataRecord> insertSeptember2021BasesCgpMin = 
		dslContext
		.insertInto(SYSTEM_DATA, 
		SYSTEM_DATA.DOMAIN,
		SYSTEM_DATA.NAME,
		...
		dslContext.transaction( (config) -> {
			...
			close2019BasesCgcMin.execute();
			close2016BasesCgpMin.execute();
			
			insertSeptember2021BaseCgcMinMes.execute();
			insertSeptember2021BaseCgcMinDia.execute();
			insertSeptember2021BasesCgpMin.execute();
