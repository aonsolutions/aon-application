...
public class GarantizadoUpdate implements Update {

	public static final GarantizadoUpdate GARANTIZADOUPDATE = new GarantizadoUpdate();
	
	private GarantizadoUpdate() {
	}
	...
	public void upgrade(Connection connection) {
		...
		dslContext.transaction( (config) -> {
			...
			dslContext
			.update(PAYMENT_CONCEPT)
			.set(PAYMENT_CONCEPT.EXPRESSION, "isdef DIAS_ENFERMEDAD_COMUN ?/*user*/GTZDO(SALARIO_BASE)/**/: REMOVE() ")
			.where(PAYMENT_CONCEPT.DOMAIN.eq(0))
			.and(PAYMENT_CONCEPT.CODE.eq("GARANTIZADO"))
