...
public class HomeBases2023UpdateIII implements Update {
	...
	private static final String TABLE = "($ in [ "
			...
			+"[1098.00,1018.00], "
			+"[1260.00,1260.00], "
			+"[Double.MAX_VALUE,BASE_CGC_BRUTA] ] if $[0] >= ( BASE_CGC_BRUTA / DIAS_COTIZADOS * DIAS_MES ) )[0][1] / DIAS_MES * DIAS_COTIZADOS";
	
	public static final HomeBases2023UpdateIII HOMEBASES2023UPDATEIII = new HomeBases2023UpdateIII();
	...
	public void upgrade(Connection connection) {
		...
		boolean upgraded =
			...
			.and(SYSTEM_DATA.NAME.eq(MONTH_DAYS))
			.and(SYSTEM_DATA.START_DATE.eq(start2023Date))) >= 1;
		...
		dslContext.transaction( config -> {
			...
			.set(SYSTEM_DATA.NAME, MONTH_DAYS)
			.set(SYSTEM_DATA.EXPRESSION, "30.00")
