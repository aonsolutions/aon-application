...
public class HomeBasesUpdate implements Update {
	...
	public void upgrade(Connection connection) {
		...
		dslContext.transaction(config -> {
			...
			for (String aux : new String[] {BASE_CGC_MAX, BASE_CGP_MIN, BASE_CGP_MAX} )
				dslContext
				...
				.select(DSL
					.select(SYSTEM_DATA.DOMAIN, DSL.val(aux), SYSTEM_DATA.EXPRESSION,  SYSTEM_DATA.START_DATE, SYSTEM_DATA.END_DATE, SYSTEM_DATA.READ_ONLY, SYSTEM_DATA.COMMENTS)
					.from(SYSTEM_DATA)
					.where(SYSTEM_DATA.DOMAIN.eq(-106))
					.and(SYSTEM_DATA.NAME.eq(BASE_CGC_MIN))
				).execute()
