...
public class AlterFsMod2002019 implements Update {
	...
	private static final Logger LOGGER  = Logger.getLogger(AlterFsMod2002019.class.getName());
	...
	public void upgrade(Connection connection) {
		...
		if (!ultimateDocumentExists) {
			try {
				LOGGER.info("\tAlterFsMod2002019. ultimate_document must be created.");
				dslContext.alterTable(FS_MODEL200).addColumn("ultimate_document", SQLDataType.VARCHAR.length(15)).execute();
				LOGGER.info("\tAlterFsMod2002019. ultimate_document CREATED!");
			} catch (Exception e) {
				LOGGER.info("\tAlterFsMod2002019. ultimate_document NOT CREATED!");
				e.printStackTrace();
			}
		} else {
			LOGGER.info("\tAlterFsMod2002019. ultimate_document already exists.");
		}
		
		//	ultimate_document_country, varchar(2), Grupo - Clave 00081 - Datos de la sociedad matriz última: Código país
		if (!ultimateDocumentCountryExists) {
			try {
				LOGGER.info("\tAlterFsMod2002019. ultimate_document_country must be created.");
				dslContext.alterTable(FS_MODEL200).addColumn("ultimate_document_country", SQLDataType.VARCHAR.length(2)).execute();
				LOGGER.info("\tAlterFsMod2002019. ultimate_document_country CREATED!");
			} catch (Exception e) {
				LOGGER.info("\tAlterFsMod2002019. ultimate_document_country NOT CREATED!");
				e.printStackTrace();
			}
		} else {
			LOGGER.info("\tAlterFsMod2002019. ultimate_document_country already exists.");
		}
		
		//	ultimate_name, varchar(40), Grupo - Clave 00081 - Datos de la sociedad matriz última: Nombre o razón social
		if (!ultimateNameExists) {
			try {
				LOGGER.info("\tAlterFsMod2002019. ultimate_name must be created.");
				dslContext.alterTable(FS_MODEL200).addColumn("ultimate_name", SQLDataType.VARCHAR.length(40)).execute();
				LOGGER.info("\tAlterFsMod2002019. ultimate_name CREATED!");
			} catch (Exception e) {
				LOGGER.info("\tAlterFsMod2002019. ultimate_name NOT CREATED!");
				e.printStackTrace();
			}
		} else {
			LOGGER.info("\tAlterFsMod2002019. ultimate_name already exists.");
		}

		//	ultimate_country, varchar(2), Grupo - Clave 00081 - Datos de la sociedad matriz última: País o jurisdicción
		if (!ultimateCountryExists) {
			try {
				LOGGER.info("\tAlterFsMod2002019. ultimate_country must be created.");
				dslContext.alterTable(FS_MODEL200).addColumn("ultimate_country", SQLDataType.VARCHAR.length(2)).execute();
				LOGGER.info("\tAlterFsMod2002019. ultimate_country CREATED!");
			} catch (Exception e) {
				LOGGER.info("\tAlterFsMod2002019. ultimate_country NOT CREATED!");
				e.printStackTrace();
			}
		} else {
			LOGGER.info("\tAlterFsMod2002019. ultimate_country already exists.");
		}
