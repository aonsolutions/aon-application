...
public class BaseCgpMin2022FixIII implements Update {

	public static BaseCgpMin2022FixIII BASECGPMIN2022FIX = new BaseCgpMin2022FixIII();
	...
	public void upgrade(Connection connection) {
		...
		dslContext.select()
		.from(SYSTEM_DATA)
		.where(SYSTEM_DATA.DOMAIN.in(0,-105))
		.and(SYSTEM_DATA.NAME.eq(BASE_CGP_MIN))
		.and(SYSTEM_DATA.START_DATE.eq(start2022Date))
		.and(SYSTEM_DATA.EXPRESSION.contains("DIAS_NOMINA/30"))
		) > 0;
		...
		"MAX(7.03, ((COEFICIENTE_PARCIALIDAD < 1.00) ? BASE_CGC_MIN : 1166.70 * (DIAS_NOMINA == DIAS_MES ? 1 : DIAS_NOMINA/30)))")
		...
		.set(SYSTEM_DATA.EXPRESSION,"MAX(7.03, 1166.70 * (DIAS_NOMINA == DIAS_MES ? 1 : DIAS_NOMINA/30) * COEFICIENTE_PARCIALIDAD)")
		;
