...
public class RefreshMod202Result implements Update {
	
	public static final RefreshMod202Result REFRESH_MOD202_RESULT = new RefreshMod202Result();

	private RefreshMod202Result() {
	}
				...
				dslContext.select(FS_MODEL.ID,FS_MODEL.ADMINISTRATION,FS_MODEL.DOMAIN )
					...
					.forEach(reco -> {
						...
						if (resultKey1 != null) {
							...
							Double result1 = dslContext.select(FS_MODEL_DETAIL.AMOUNT)
								.from(FS_MODEL_DETAIL)
								.where(FS_MODEL_DETAIL.FS_MODEL.eq(id))
								.and(FS_MODEL_DETAIL.TYPE.eq(resultKey1))
								.fetch()
								.map(recRes -> recRes.getValue(FS_MODEL_DETAIL.AMOUNT))
								.stream()
								.findFirst()
								.orElse(0.0);
							if ( Math.round(result1) == 0 ) {
								resultKey = "202-C03";
							} else {
								resultKey = "202-C34";
							}
