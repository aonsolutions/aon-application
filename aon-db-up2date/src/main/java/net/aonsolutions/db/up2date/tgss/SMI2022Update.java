...
public class SMI2022Update implements Update {

	public static SMI2022Update SMI2022UPDATE = new SMI2022Update();

	private static final String SMI = "SMI";
	
	private SMI2022Update() {
		super();
	}
	...
	public void upgrade(Connection connection) {
		...
		calendar.set(Calendar.YEAR, 2021);
		Date september2021Date = new Date(calendar.getTimeInMillis());
		...
		Date end2012Date = new Date(calendar.getTimeInMillis());
		...
		;
		// SMI
		UpdateConditionStep<SystemDataRecord> update2010SMI = dslContext
		.update(SYSTEM_DATA)
		...
		UpdateConditionStep<SystemDataRecord> update2011SMI = dslContext
		.update(SYSTEM_DATA)
		.set(SYSTEM_DATA.END_DATE, end2012Date)
		.where(SYSTEM_DATA.DOMAIN.eq(0))
		.and(SYSTEM_DATA.NAME.eq(SMI))
		.and(SYSTEM_DATA.START_DATE.eq(start2011Date));

		UpdateConditionStep<SystemDataRecord> update2013SMI = dslContext
		.update(SYSTEM_DATA)
		...
		UpdateConditionStep<SystemDataRecord> update2021SMI = dslContext
		.update(SYSTEM_DATA)
		.set(SYSTEM_DATA.END_DATE, end2021Date)
		.where(SYSTEM_DATA.DOMAIN.eq(0))
		.and(SYSTEM_DATA.NAME.eq(SMI))
		.and(SYSTEM_DATA.START_DATE.eq(september2021Date));
		
		InsertSetMoreStep<SystemDataRecord> insert2022SMI = dslContext
		...
		.set(SYSTEM_DATA.EXPRESSION, "1000.00" )
		...
		dslContext.transaction( (config) -> {
			...
			update2010SMI.execute();
			update2011SMI.execute();
			update2013SMI.execute();
			update2021SMI.execute();
			insert2022SMI.execute();
