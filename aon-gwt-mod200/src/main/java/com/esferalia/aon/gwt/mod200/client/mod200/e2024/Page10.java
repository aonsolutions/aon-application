...
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0006;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0013;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0022;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0063;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0071;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0083;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0085;
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Key.C0088;
...
import com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024LQ579Key;
...
public class Page10 extends PageAbs {
	...
	private static final String FOOTER_547 = "(*) S\u00F3lo debe cumplimentarse esta fila si la entidad tiene bases imponibles negativas por otro per\u00EDodo impositivo iniciado tambi\u00E9n en 2024, pero inferior a 12 meses y previo al ejercicio declarado.";
	private static final String FOOTER_561 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene cuotas negativas por otro per\u00EDodo impositivo iniciado tambi\u00E9n en 2024, pero inferior a 12 meses y previo al ejercicio declarado.";
	private static final String FOOTER_1032 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene reservas pendientes de integrar en un per\u00EDodo impositivo anterior iniciado en 2024.";
	private static final String FOOTER_1033 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene reducciones pendientes de integrar correspondientes a un per\u00EDodo impositivo anterior iniciado en 2024.";
	
	public Page10( Model2002024PageCallback callback ) {
		super(callback);
	}
	...
	protected void initializeTable() {
		...
		for (Mod2002024Key key : Mod2002024Constants.LIQUIDATION_II_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002024Key.LQ1029) {
					paintDescription(table, "Entidades que forman parte de grupos de consolidaci\u00F3n fiscal", row,0, true);
					table.getCellFormatter().addStyleName(row, 0, AON.AON_CSS.aonFiscalPaddingLeft());
					row++;
					margin = true;
				}
				if (key == Mod2002024Key.LQ550TG || key == Mod2002024Key.LQ550T0 ) {
					margin = true;
				}
				
				if (key == Mod2002024Key.LQ541 || key == Mod2002024Key.LQ1887 || key == Mod2002024Key.LQ1576) {
				...
				if (key == Mod2002024Key.LQ547 || key == Mod2002024Key.LQ550 || key == Mod2002024Key.LQ552 || key == Mod2002024Key.LQ558 || 
					key == Mod2002024Key.LQ562 || key == Mod2002024Key.LQ1032 || key == Mod2002024Key.LQ1330) {
					margin = false;
				}
				...
				if (key == Mod2002024Key.LQ1890
						&& callback.getMod200Object().getMod200().isNotChecked(Mod2002024Key.C0009) 
						&& callback.getMod200Object().getMod200().isNotChecked(Mod2002024Key.C0010)) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.LQ1887, Mod2002024LQ243Key.values(), HEADERS_4, FOOTER_547);
				}
				
     			if (key == Mod2002024Key.LQ554) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.LQ554, Mod2002024LQ554Key.values(), HEADERS_1);
				}
				if (key == Mod2002024Key.LQ561) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.LQ561, Mod2002024LQ561Key.values(), HEADERS_2, FOOTER_561);
				}
    			if (key == Mod2002024Key.LQ579) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.LQ579, Mod2002024LQ579Key.values(), null);
				}
    			if (key == Mod2002024Key.LQ1032 
					&& callback.getMod200Object().getMod200().isNotChecked(Mod2002024Key.C0009) 
					&& callback.getMod200Object().getMod200().isNotChecked(Mod2002024Key.C0010)) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.LQ1032, Mod2002024LQ1032Key.values(), HEADERS_3, FOOTER_1032);
				}
	...
	protected int paintKeyBreakdownLinkLQ1033(FlexTable tab, int row) {
		...
		int r = 2;
		int col = 0;
		for (IMod200KeysProvider key : Mod2002024LQ1033_1Key.values()) {
			...
			for (final IMod200Key k : key.getKeys() ) {
				if (k != null && k!= Mod2002024Key.LQ1033){
		...
		r = 1;
		for (IMod200KeysProvider key : Mod2002024LQ1033_2Key.values()) {
			Label desc = new Label(key.getDescription() );
			if ("Total".equals(key.getDescription()))
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if ((key == Mod2002024Key.LQ1032 || key == Mod2002024Key.LQ1887 || key == Mod2002024Key.LQ1890) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002024Key.C0009) || callback.getMod200Object().getMod200().isChecked(Mod2002024Key.C0010))) {
			return false;
		}
		
		if (key == Mod2002024Key.LQ562 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002024Key.C0030) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002024Key.C0047))) {
			return false;
		}
		...
		if ((key == Mod2002024Key.LQ520 || key == Mod2002024Key.LQ521) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002024Key.C0064))) {
			return true;
		}
