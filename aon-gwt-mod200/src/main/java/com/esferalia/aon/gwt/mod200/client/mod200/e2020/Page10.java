...
public class Page10 extends PageAbs {
	...
	protected void initializeTable() {
		...
		for (final Mod2002020Key key : Mod2002020Constants.LIQUIDATION_III_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002020Key.BN570) {					
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN570,Mod2002020BN570Key.values(),HEADERS_1);
				} 
				if (key == Mod2002020Key.BN1344) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN1344,Mod2002020BN1344Key.values(),HEADERS_1);
				}
				if (key == Mod2002020Key.BN1280) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN1280,Mod2002020BN1280Key.values(),HEADERS_2);
				}
				if (key == Mod2002020Key.BN572) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN572,Mod2002020BN572Key.values(),HEADERS_1);
				} 
				if (key == Mod2002020Key.BN571) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN571,Mod2002020BN571Key.values(),HEADERS_1);
				} 
				if (key == Mod2002020Key.BN573) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.BN573,Mod2002020BN573Key.values(),HEADERS_2);
				}
	...
	protected boolean isDisabled(IMod200Key key) {
		if (key == Mod2002020Key.BN575)  {
			return callback.getMod200Object().getMod200().isNotChecked(Mod2002020Key.C0007);
		}
