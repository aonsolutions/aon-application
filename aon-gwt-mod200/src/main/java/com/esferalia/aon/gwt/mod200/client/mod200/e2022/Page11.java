...
public class Page11 extends PageAbs {
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (final Mod2002022Key key : Mod2002022Constants.LIQUIDATION_IV_KEYS) {
			row = paintKey(table,key,row);
			if (callback.getMod200Object().isVisible(key)) {
				if (key == Mod2002022Key.BN585) {
					row = paintKeyBreakdownLink(table, row, Mod2002022Key.BN585, Mod2002022BN585Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002022Key.BN584) {
					row = paintKeyBreakdownLink(table, row, Mod2002022Key.BN584, Mod2002022BN584Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002022Key.BN588) {
					row = paintKeyBreakdownLink(table,row,Mod2002022Key.BN588,Mod2002022BN588Key.values(), HEADERS_588, FOOTER_1, FOOTER_588_1, FOOTER_588_2);
				} 
				if (key == Mod2002022Key.BN082) {
					row = paintKeyBreakdownLink(table, row, Mod2002022Key.BN082, Mod2002022BN082Key.values(), HEADERS_082, FOOTER_1, FOOTER_082);
				}
				if (key == Mod2002022Key.BN565) {
					row = paintKeyBreakdownLinkBN565(table, row);
				}
				if (key == Mod2002022Key.BN590) {
					row = paintKeyBreakdownLink(table,row,Mod2002022Key.BN590,Mod2002022BN590Key.values(),HEADERS_1, FOOTER_1);
				}
				if (key == Mod2002022Key.BN1040) {
					row = paintKeyBreakdownLink(table,row,Mod2002022Key.BN1040,Mod2002022BN1040Key.values(),HEADERS_3, FOOTER_1);
				}
	...
	private int paintKeyBreakdownLinkBN565(final FlexTable tab, int row) {
		...
		paintKeyBreakdownLinkContainer(container, Mod2002022Key.BN565, Mod2002022BN565_1Key.values(), headers, FOOTER_1);
		headers[0] = "DONACIONES PARA ACTIVIDADES PRIORITARIAS DE MECENAZGO Y OTRAS CON DERECHO A DEDUCCION INCREMENTADA";
		paintKeyBreakdownLinkContainer(container, Mod2002022Key.BN565, Mod2002022BN565_2Key.values(), headers, FOOTER_1);
		headers[0] = "TOTAL DEDUCCIONES A ENTIDADES SIN FINES DE LUCRO (LEY 49/2002)";
		headers[2] = "";
		paintKeyBreakdownLinkContainer(container, Mod2002022Key.BN565, Mod2002022BN565Key.values(), headers, null);
		...
		paintKey(table, Mod2002022Key.BN974, 1);		
		container.add(table);
	...
	private void paintKeyBreakdownLinkContainer(FlowPanel container, Mod2002022Key breakdownKey, IMod200KeysProvider[] keysProvider, String[] headers, String footernote) {
