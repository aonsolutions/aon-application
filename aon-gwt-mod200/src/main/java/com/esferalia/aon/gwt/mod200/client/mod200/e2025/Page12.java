...
public class Page12 extends PageAbs {
	...
	public Page12( Model2002025PageCallback callback ) {
		super(callback);
	}
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (final Mod2002025Key key : Mod2002025Constants.LIQUIDATION_IV_KEYS) {
			row = paintKey(table,key,row);
			if (callback.getMod200Object().isVisible(key)) {
				
				// Casillas con desglose
				
				if (key == Mod2002025Key.BN585) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.BN585, Mod2002025BN585Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002025Key.BN584) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.BN584, Mod2002025BN584Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002025Key.BN588) {
					row = paintKeyBreakdownLink(table,row, Mod2002025Key.BN588, Mod2002025BN588Key.values(), HEADERS_588, FOOTER_1, FOOTER_588_1, FOOTER_588_2);
				} 
				if (key == Mod2002025Key.BN082) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.BN082, Mod2002025BN082Key.values(), HEADERS_082, FOOTER_1, FOOTER_082);
				}
				if (key == Mod2002025Key.BN565) {
					row = paintKeyBreakdownLinkBN565(table, row);
				}
				if (key == Mod2002025Key.BN590) {
					row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN590,Mod2002025BN590Key.values(),HEADERS_1, FOOTER_1);
				}
				if (key == Mod2002025Key.BN1040) {
					row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN1040,Mod2002025BN1040Key.values(),HEADERS_3, FOOTER_1);
				}
				if (key == Mod2002025Key.BN1041) {
					row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN1041,Mod2002025BN1041Key.values(),HEADERS_3, FOOTER_1);
				}				
				if (key == Mod2002025Key.BN2315
					...
					row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN2315,Mod2002025BN2315Key.values(),HEADERS_1, FOOTER_1);
				}
				
				if (key == Mod2002025Key.BN1039) {  
					if (callback.getMod200Object().getMod200().isNotChecked(Mod2002025Key.C0009) && 
						callback.getMod200Object().getMod200().isNotChecked(Mod2002025Key.C0010))
						row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN1039,Mod2002025BN1039Key.values(),HEADERS_1039_2314, FOOTER_1);
				}	
				
				if (key == Mod2002025Key.BN2314) {  
					if (callback.getMod200Object().getMod200().isNotChecked(Mod2002025Key.C0009) && 
						callback.getMod200Object().getMod200().isNotChecked(Mod2002025Key.C0010))
						row = paintKeyBreakdownLink(table,row,Mod2002025Key.BN2314,Mod2002025BN2314Key.values(),HEADERS_1039_2314, FOOTER_1);
				}
		...
		addTable("", Mod2002025Constants.FILM_PRODUCTIONS);
	...
	private int paintKeyBreakdownLinkBN565(final FlexTable tab, int row) {
		...
		paintKeyBreakdownLinkContainer(container, Mod2002025Key.BN565, Mod2002025BN565_1Key.values(), headers, FOOTER_1);
		headers[0] = "DONACIONES PARA ACTIVIDADES PRIORITARIAS DE MECENAZGO Y OTRAS CON DERECHO A DEDUCCION INCREMENTADA";
		paintKeyBreakdownLinkContainer(container, Mod2002025Key.BN565, Mod2002025BN565_2Key.values(), headers, FOOTER_1);
		headers[0] = "TOTAL DEDUCCIONES A ENTIDADES SIN FINES DE LUCRO (LEY 49/2002)";
		headers[2] = "";
		paintKeyBreakdownLinkContainer(container, Mod2002025Key.BN565, Mod2002025BN565Key.values(), headers, null);
		...
		paintKey(table, Mod2002025Key.BN974, 1);		
		container.add(table);
	...
	private void paintKeyBreakdownLinkContainer(FlowPanel container, Mod2002025Key breakdownKey, IMod200KeysProvider[] keysProvider, String[] headers, String footernote) {
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if ((key == Mod2002025Key.BN2315 || key == Mod2002025Key.BN1039 || key == Mod2002025Key.BN2314 ) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002025Key.C0009) || callback.getMod200Object().getMod200().isChecked(Mod2002025Key.C0010))) {
			return false;
		}
