...
import static com.esferalia.aon.occam.mod200.api.model.mod200_2024.Mod2002024Behaviour.BEHAVIOUR_KEYS_MAP;
...
public abstract class PageAbs extends ResizeComposite {
	...
	protected Model2002024PageCallback callback = null;
	
	protected PageAbs() {
		super();
	}
	
	protected PageAbs(Model2002024PageCallback callback) {
	...
	protected int paintKey(FlexTable tab, final Mod2002024Key key, int row) {
		paintKeyDescription(tab, key, row, 0);
		paintKeyField(tab, key, row, 1);	
		return ++row;
	}
	
	protected int paintKey(FlexTable tab, final Mod2002024Key key, int row, boolean bold) {
		paintDescription(tab, key.getDescription(), row, 0, bold);
		paintKeyField(tab, key, row, 1);	
		return ++row;
	}
	
	protected int paintKey(FlexTable tab, final Mod2002024Key key, int row, boolean bold, boolean padding) {
		paintDescription(tab, key.getDescription(), row, 0, bold);
		paintKeyField(tab, key, row, 1, padding);	
		return ++row;
	}
	
	protected void paintKeyDescription(FlexTable tab, Mod2002024Key key, int row, int col) {
	...
	protected void paintKeyField(FlexTable tab, final Mod2002024Key key, int row, int col) {
	...
	protected void paintKeyField(FlexTable tab, final IMod200Key k, int row, int col, int visibleLength, boolean padding, String code, boolean enabled) {
		...
		text.addValueChangeHandler( event -> {
			try {
				...
				if (k==Mod2002024Key.BN103A) {						
					AonDoubleBox db = inputs.get(Mod2002024Key.BN103B);
					db.setValue(d,true);
				}
				else if (k==Mod2002024Key.BN103B) {						
					AonDoubleBox db = inputs.get(Mod2002024Key.BN103C);
					db.setValue(d,true);						
				}
				else if (k==Mod2002024Key.BN103C) {						
					AonDoubleBox db = inputs.get(Mod2002024Key.BN103D);
					db.setValue(d,true);
				}
				else if (k==Mod2002024Key.BN103D) {						
					AonDoubleBox db = inputs.get(Mod2002024Key.BN103A);
					db.setValue(d,true);
				}
	...
	protected int paintKeyBreakdownLink(final FlexTable tab, int row, Mod2002024Key breakdownKey, IMod200KeysProvider[] keysProvider, String[] headers, String... footernotes) {
		...
		for (IMod200KeysProvider key : keysProvider) {
			...
			for (final IMod200Key k : key.getKeys() ) {
				...
				if (k != null && k != breakdownKey && k != Mod2002024Key.LQ1890) {  // Caso especial desglose CASILLA 243, lleva dos subtotales
					if (paintDesc) {
						// Casillas [02287] y [00814] comienzan bloque de información adicional, dentro del desglose de la [590] y [082] respectivamente
						if (k == Mod2002024Key.BN2287 || k == Mod2002024Key.BN814) {							
	...
	protected void paintKeysProvider(IMod200KeysProvider[] keysProvider, final FlexTable tab, int row, boolean padding, String... headers) {
		...
		for (IMod200KeysProvider kp : keysProvider) {
			...
			for (IMod200Key key : kp.getKeys()) {				 
				if (key != null && callback.getMod200Object().isVisible((Mod2002024Key) key)) {
	...
	protected void addTable(String title, Mod2002024Key[] keys) {

		FlexTable table = addTable(title);

		int row = 0;
		for (Mod2002024Key key : keys) {
	...
	protected boolean isChecked(Mod2002024Key key) {
		return callback.getMod200Object().getMod200().isChecked(key);
	}
	
	protected boolean isCheckedOr(Mod2002024Key... keys) {
		boolean result = false;
		for (Mod2002024Key key : keys) {
			result = result || callback.getMod200Object().getMod200().isChecked(key);
		}
		return result;
	}
