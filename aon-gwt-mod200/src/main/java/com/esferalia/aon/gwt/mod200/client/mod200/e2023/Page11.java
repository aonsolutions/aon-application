...
public class Page11 extends PageAbs {
	...
	private static final String FOOTER_1 = "(*) S\u00F3lo debe cumplimentarse si tiene deducciones pendientes de aplicar correspondientes a un per\u00EDodo impositivo anterior iniciado en 2023.";
	...
	private static final String FOOTER_588_2 = "(****) Programas cuya vigencia se inicia a partir de 2023: S\u00F3lo debe cumplimentarse esta fila si la entidad tiene un per\u00EDodo impositivo que no coincida con el a\u00F1o natural y ha realizado gastos con derecho a deducci\u00F3n a partir de 2023.";
	...
	protected void initializeTable() {
		...
		for (final Mod2002023Key key : Mod2002023Constants.LIQUIDATION_IV_KEYS) {
			...
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002023Key.BN584) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN584, Mod2002023BN584Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002023Key.BN588) {
					row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN588,Mod2002023BN588Key.values(), HEADERS_588, FOOTER_1, FOOTER_588_1, FOOTER_588_2);
				} 
				if (key == Mod2002023Key.BN082) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN082, Mod2002023BN082Key.values(), HEADERS_082, FOOTER_1, FOOTER_082);
				}
				if (key == Mod2002023Key.BN565) {
					row = paintKeyBreakdownLinkBN565(table, row);
				}
				if (key == Mod2002023Key.BN590) {
					row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN590,Mod2002023BN590Key.values(),HEADERS_1, FOOTER_1);
				}
				if (key == Mod2002023Key.BN1040) {
					row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN1040,Mod2002023BN1040Key.values(),HEADERS_3, FOOTER_1);
				}
				...
				if (key == Mod2002023Key.BN2315
					...
					row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN2315,Mod2002023BN2315Key.values(),HEADERS_1, FOOTER_1);
				}
				
				if (key == Mod2002023Key.BN1039) {  
					if (callback.getMod200Object().getMod200().isNotChecked(Mod2002023Key.C0009) && 
						callback.getMod200Object().getMod200().isNotChecked(Mod2002023Key.C0010))
						row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN1039,Mod2002023BN1039Key.values(),HEADERS_1039_2314, FOOTER_1);
...
//					paintKey(table2,Mod2002023Key.BN1039M,0);
//					table.setWidget(row, 0, table2);
//					row++;
				}	
				
				if (key == Mod2002023Key.BN2314) {  
					if (callback.getMod200Object().getMod200().isNotChecked(Mod2002023Key.C0009) && 
						callback.getMod200Object().getMod200().isNotChecked(Mod2002023Key.C0010))
						row = paintKeyBreakdownLink(table,row,Mod2002023Key.BN2314,Mod2002023BN2314Key.values(),HEADERS_1039_2314, FOOTER_1);
...
//				   	table2.getColumnFormatter().setWidth(1, "150px");
//					table2.addStyleName(AON.AON_CSS.aonFiscalPaddingLeft());					
//					paintKey(table2,Mod2002023Key.BN2314M,0);
//					table.setWidget(row, 0, table2);
//					row++;
				}
	...
	private int paintKeyBreakdownLinkBN565(final FlexTable tab, int row) {
		...
		paintKeyBreakdownLinkContainer(container, Mod2002023Key.BN565, Mod2002023BN565_1Key.values(), headers, FOOTER_1);
		headers[0] = "DONACIONES PARA ACTIVIDADES PRIORITARIAS DE MECENAZGO Y OTRAS CON DERECHO A DEDUCCION INCREMENTADA";
		paintKeyBreakdownLinkContainer(container, Mod2002023Key.BN565, Mod2002023BN565_2Key.values(), headers, FOOTER_1);
		headers[0] = "TOTAL DEDUCCIONES A ENTIDADES SIN FINES DE LUCRO (LEY 49/2002)";
		headers[2] = "";
		paintKeyBreakdownLinkContainer(container, Mod2002023Key.BN565, Mod2002023BN565Key.values(), headers, null);
		...
		paintKey(table, Mod2002023Key.BN974, 1);		
		container.add(table);
	...
	private void paintKeyBreakdownLinkContainer(FlowPanel container, Mod2002023Key breakdownKey, IMod200KeysProvider[] keysProvider, String[] headers, String footernote) {
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if ((key == Mod2002023Key.BN2315 || key == Mod2002023Key.BN1039 || key == Mod2002023Key.BN2314 ) && 
