...
public class Page15 extends PageAbs {
	...
	protected void initializeTable() {
		...
		for (int i = 0; i < Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_1.length; i++) {
			Mod2002018Key[] keys = Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_1[i];
			if (keys == null) {
				if (i == 0) {
				...
				for (int x = 0; x < keys.length; x++) {
					...
					if (key != null && callback.getMod200Object().isVisible(key)) {
						...
						if (key == Mod2002018Key.LM1250 || key == Mod2002018Key.LM1251
						 || key == Mod2002018Key.LM1252 || key == Mod2002018Key.LM1253
						 || key == Mod2002018Key.LM1254) {
		...
		++row;
		for (int i = 0; i < Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_2.length; i++) {
			Mod2002018Key[] keys = Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_2[i];
			paintDescription(table1, Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_DESCRIPTIONS_2[i], row, 0, false);
			for (int x = 0; x < keys.length; x++) {
				Mod2002018Key key = keys[x]; 
				if (key != null && callback.getMod200Object().isVisible(key)) {
		...
		++row;
		for (int i = 0; i < Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_3.length; i++) {
			Mod2002018Key[] keys = Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_3[i];
			paintDescription(table2, Mod2002018Constants.DEDUCIBLE_LIMITATION_KEYS_DESCRIPTIONS_3[i], row, 0, false);
			for (int x = 0; x < keys.length; x++) {
				Mod2002018Key key = keys[x]; 
				if (key != null && callback.getMod200Object().isVisible(key)) {
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && callback.getMod200Object().getMod200().isNotChecked(Mod2002018Key.C0009) 
  		  &&  callback.getMod200Object().getMod200().isNotChecked(Mod2002018Key.C0010);
		return av;
	}
