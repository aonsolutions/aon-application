	...
	protected void initializeTable() {
		...
		for (final Mod2002021Key key : Mod2002021Constants.LIQUIDATION_III_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002021Key.BN570) {					
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN570,Mod2002021BN570Key.values(),HEADERS_1);
				} 
				if (key == Mod2002021Key.BN1344) {
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN1344,Mod2002021BN1344Key.values(),HEADERS_1);
				}
				if (key == Mod2002021Key.BN1280) {
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN1280,Mod2002021BN1280Key.values(),HEADERS_2);
				}
				if (key == Mod2002021Key.BN572) {
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN572,Mod2002021BN572Key.values(),HEADERS_1);
				} 
				if (key == Mod2002021Key.BN571) {
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN571,Mod2002021BN571Key.values(),HEADERS_1);
				} 
				if (key == Mod2002021Key.BN573) {
					row = paintKeyBreakdownLink(table,row,Mod2002021Key.BN573,Mod2002021BN573Key.values(),HEADERS_2);
				}
