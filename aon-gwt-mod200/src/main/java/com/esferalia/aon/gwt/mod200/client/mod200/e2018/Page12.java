...
public class Page12 extends PageAbs {
	...
	protected void initializeTable() {
		...
		paintTable(table , Mod2002018Constants.LIQUIDATION_V_KEYS_1, 2, "retention");
		paintTable(table1, Mod2002018Constants.LIQUIDATION_V_KEYS_2, 2, "diputation");
		paintTable(table2, Mod2002018Constants.LIQUIDATION_V_KEYS_3, 2, null);
		paintTable(table3, Mod2002018Constants.LIQUIDATION_V_KEYS_4, 2, null);
		paintTable(table4, Mod2002018Constants.LIQUIDATION_V_KEYS_5, 3, null);
	}
	
	private void paintTable(FlexTable table, Mod2002018Key[][] liquidationKeys, int numCols, String header) {
		table.setWidth("100%");
		table.setCellSpacing(0);
		...
		if (header=="retention") {
			...
			++row;
		} else if (header=="diputation") {
//			table.getFlexCellFormatter().addStyleName(row, 0, AON.AON_CSS.aonPaddingLeft());
//			table.getFlexCellFormatter().setColSpan(row, 0, 0);
//			table.setWidget(row, 1, new Label("Estado"));
//			table.getFlexCellFormatter().addStyleName(row, 1, AON.AON_CSS.aonTextCenter());
//			table.getFlexCellFormatter().addStyleName(row, 1, AON.AON_CSS.aonBold());
//			table.setWidget(row, 2, new Label("D. Forales / Navarra (totales)"));
//			table.getFlexCellFormatter().addStyleName(row, 2, AON.AON_CSS.aonTextCenter());
//			table.getFlexCellFormatter().addStyleName(row, 2, AON.AON_CSS.aonBold());
//			++row;
		}
		
		for (Mod2002018Key[] keys : liquidationKeys) {
