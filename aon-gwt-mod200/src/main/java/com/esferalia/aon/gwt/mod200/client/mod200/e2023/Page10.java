...
public class Page10 extends PageAbs {
	...
	public Page10( Model2002023PageCallback callback ) {
		super(callback);
	}
		...
		for (final Mod2002023Key key : Mod2002023Constants.LIQUIDATION_III_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002023Key.BN1344) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN1344, Mod2002023BN1344Key.values(), HEADERS_1, FOOTER_1, FOOTER_2);
				}
				if (key == Mod2002023Key.BN1280) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN1280, Mod2002023BN1280Key.values(), HEADERS_2, FOOTER_2);
				}
				if (key == Mod2002023Key.BN572) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN572, Mod2002023BN572Key.values(), HEADERS_1, FOOTER_2);
				} 
				if (key == Mod2002023Key.BN571) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN571, Mod2002023BN571Key.values(), HEADERS_1, FOOTER_1, FOOTER_2);
				} 
				if (key == Mod2002023Key.BN573) {
					row = paintKeyBreakdownLink(table, row, Mod2002023Key.BN573, Mod2002023BN573Key.values(), HEADERS_2, FOOTER_2);
				}
