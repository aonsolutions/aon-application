...
import com.esferalia.aon.occam.mod200.api.model.mod200_2025.Mod2002025LQ579Key;
...
public class Page10 extends PageAbs {
	...
	protected void initializeTable() {
		...
		for (Mod2002025Key key : Mod2002025Constants.LIQUIDATION_II_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {
				...
				if (key == Mod2002025Key.LQ1029) {
					paintDescription(table, "Entidades que forman parte de grupos de consolidaci\u00F3n fiscal", row,0, true);
					table.getCellFormatter().addStyleName(row, 0, AON.AON_CSS.aonFiscalPaddingLeft());
					row++;
					margin = true;
				}
				if (key == Mod2002025Key.LQ550TG || key == Mod2002025Key.LQ550T0 ) {
					margin = true;
				}
				
				if (key == Mod2002025Key.LQ541 || key == Mod2002025Key.LQ1887 || key == Mod2002025Key.LQ1576) {
				...
				if (key == Mod2002025Key.LQ547 || key == Mod2002025Key.LQ550 || key == Mod2002025Key.LQ552 || key == Mod2002025Key.LQ558 || 
					key == Mod2002025Key.LQ562 || key == Mod2002025Key.LQ1032 || key == Mod2002025Key.LQ1330) {
					margin = false;
				}
				...
				if (key == Mod2002025Key.LQ1890
					...
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.LQ1887, Mod2002025LQ243Key.values(), HEADERS_4, FOOTER_547);
				}
				
     			if (key == Mod2002025Key.LQ554) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.LQ554, Mod2002025LQ554Key.values(), HEADERS_1);
				}
				if (key == Mod2002025Key.LQ561) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.LQ561, Mod2002025LQ561Key.values(), HEADERS_2, FOOTER_561);
				}
    			if (key == Mod2002025Key.LQ579) {
					row = paintKeyBreakdownLink(table, row, Mod2002025Key.LQ579, Mod2002025LQ579Key.values(), null);
				}
	...
	protected int paintKeyBreakdownLinkLQ1033(FlexTable tab, int row) {
		...
		int r = 2;
		int col = 0;
		for (IMod200KeysProvider key : Mod2002025LQ1033_1Key.values()) {
			...
			for (final IMod200Key k : key.getKeys() ) {
				if (k != null && k!= Mod2002025Key.LQ1033){
		...
		r = 1;
		for (IMod200KeysProvider key : Mod2002025LQ1033_2Key.values()) {
			Label desc = new Label(key.getDescription() );
			if ("Total".equals(key.getDescription()))
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if ((key == Mod2002025Key.LQ1032 || key == Mod2002025Key.LQ1887 || key == Mod2002025Key.LQ1890) && 
		...
		if (key == Mod2002025Key.LQ562 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002025Key.C0030) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002025Key.C0047))) {
			return false;
		}
		...
		if ((key == Mod2002025Key.LQ520 || key == Mod2002025Key.LQ521) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002025Key.C0064))) {
			return true;
		}
