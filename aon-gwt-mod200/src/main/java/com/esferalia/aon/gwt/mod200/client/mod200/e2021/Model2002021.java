...
public class Model2002021 extends DockLayoutPanel {
	...
	public Model2002021(Model200Callback mod200Callback, Mod2002021 mod200) {
		super(Unit.PX);
		...
		mod200Object = new Mod2002021Object(options, mod200);
...
//		addNorth(getDeclarationToolbarPanel(), AonToolbar.HEIGTH); // FALTA - BOTONES DEL ESTADO
	...
	private class PopupAsyncCallback implements AsyncCallback<Mod2002021> {
		PopupPanel popup;
	...
	private Widget getToolbar() {
		...
		initializeButton.addClickHandler(event -> {
			...
			mod200Object.initializeMod200(new AsyncCallback<Mod2002021>() {
				@Override
				public void onSuccess(Mod2002021 result) {
					popup.hide();
					dump(false);
					...
					try {
						...
						mod200Object.delete(new AsyncCallback<Void>() {
							...
							public void onSuccess(Void result) {
								...
								Model200.getMod2002021Service().createMod2002021(options.getOccam(), 2021
		...
		importAccountingButton.addClickHandler(event -> {
			...
			Upload upload = new Upload() {
				...
				protected void onUpload(String data) {
					mod200Object.fillMod2002021AccountingData(options.getDomainName(), options.getDomain(), options.getUser(), data, new AsyncCallback<Mod2002021>() {
						@Override public void onSuccess(Mod2002021 result) {	
							markAsDirty();
						}
		...
		aeatAccountingFileButton.addClickHandler(event -> {
			...
			cd.confirm("Se va a proceder a la generaci\u00F3n de un fichero\n"
				...
				new AonConfirmDialogCallback() {
					...
					@Override
					public void onAccept() {
						submitForm("/aon_gwt_mod200/ms/Model2002021AccountingFile");
					}
				}
			);
		});
		...
		aeatFileButton.addClickHandler(event -> {
			...
			cd.confirm("Se va a proceder a la generaci\u00F3n de un fichero\n"
				...
				new AonConfirmDialogCallback() {
					...
					@Override
					public void onAccept() {
						submitForm("/aon_gwt_mod200/ms/Model2002021File");
					}
				}
			);
		});
		...
		aeatPrintButton.addClickHandler(event -> {
			...
			cd.confirm("La impresi\u00F3n se genera en los servidores de la Agencia Tributaria "
				...
				,new AonConfirmDialogCallback() {
					...
					@Override
					public void onAccept() {
						submitForm("/aon_gwt_mod200/ms/Model2002021Print");
					}
				}
			);
		});
...
////			MOD2002021_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod2002021>() {
////				@Override
////				public void onSuccess(Mod2002021 result) {
...
////			MOD2002021_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.SENT, new AsyncCallback<Mod2002021>() {
////				@Override
////				public void onSuccess(Mod2002021 result) {
////					getCallback().reload(result.getId());
////				}
...
////			MOD2002021_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod2002021>() {
////				@Override
////				public void onSuccess(Mod2002021 result) {
////					getCallback().reload(result.getId());
////				}
