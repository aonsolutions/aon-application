...
public class Page05 extends PageAbs {
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (Mod2002018Key key : Mod2002018Constants.PYG_KEYS) {
	...
	protected boolean isDisabled(Mod2002018Key key) {
		if (callback.getMod200Object().getMod200().getPygType() == BalanceType.NORMAL) {
			if (key == Mod2002018Key.PG255
			  ||key == Mod2002018Key.PG279
			  ||key == Mod2002018Key.PG309
			 ) {
				return true;
			}
		}
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && (callback.getMod200Object().getMod200().isNotChecked(Mod2002018Key.C0026));
		return av;
	}
}
