...
public class Page16 extends PageAbs {
	...
	private static final String FOOTER_1212_1 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene gastos financieros pendientes por otro per\u00EDodo impositivo iniciado en 2025, pero inferior a 12 meses y previo al declarado.";
	...
	private static final String FOOTER_538_1 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene pendiente de adici\u00F3n por l\u00EDmite beneficio operativo no aplicado por otro per\u00EDodo impositivo iniciado en 2025, pero inferior a 12 meses y previo al declarado.";
	...
	public Page16( Model2002025PageCallback callback ) {
		super(callback);
	}
	...
	protected void initializeTable() {
		...
		for (int i = 0; i < Mod2002025Constants.DEDUCIBLE_LIMITATION_KEYS_1.length; i++) {
			Mod2002025Key[] keys = Mod2002025Constants.DEDUCIBLE_LIMITATION_KEYS_1[i];
			if (keys == null) {
				if (i == 0) {
				...
				for (int x = 0; x < keys.length; x++) {
					Mod2002025Key key = keys[x]; 
					if (key != null && callback.getMod200Object().isVisible(key)) {						
						paintDescription(table, key.getDescription(), row, 0, key == Mod2002025Key.LM1260);
						if (key == Mod2002025Key.LM1250 || key == Mod2002025Key.LM1251
						 || key == Mod2002025Key.LM1252 || key == Mod2002025Key.LM1253
						 || key == Mod2002025Key.LM1254 || key == Mod2002025Key.LM2368) {
		...
		++row;
		paintKeysProvider(Mod2002025LM1212Key.values(), table1, row, false, AON.MSG.liquiMsg1(), AON.MSG.liquiMsg21(), AON.MSG.remainder(), AON.MSG.liquiMsg3(), AON.MSG.liquiMsg21(), AON.MSG.remainder());
		paintFooterNote(basePanel, FOOTER_1212_1, FOOTER_1212_2);
		
		// Pendiente de adición por límite beneficio operativo no aplicado
		
		paintKeysProvider(Mod2002025LM538Key.values(), addTable(AON.MSG.pendingAddinngs(), 3), AON.MSG.liquiMsg1(), "Importe generado. Pendiente de aplicaci\u00F3n a principio del periodo", AON.MSG.liquiMsg3(), AON.MSG.liquiMsg4());
