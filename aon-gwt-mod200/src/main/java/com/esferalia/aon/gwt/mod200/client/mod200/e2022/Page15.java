...
public class Page15 extends PageAbs {
	...
	protected void initializeTable() {
		...
		for (int i = 0; i < Mod2002022Constants.DEDUCIBLE_LIMITATION_KEYS_1.length; i++) {
			Mod2002022Key[] keys = Mod2002022Constants.DEDUCIBLE_LIMITATION_KEYS_1[i];
			if (keys == null) {
				if (i == 0) {
				...
				for (int x = 0; x < keys.length; x++) {
					Mod2002022Key key = keys[x]; 
					if (key != null && callback.getMod200Object().isVisible(key)) {						
						paintDescription(table, key.getDescription(), row, 0, key == Mod2002022Key.LM1260);
						if (key == Mod2002022Key.LM1250 || key == Mod2002022Key.LM1251
						 || key == Mod2002022Key.LM1252 || key == Mod2002022Key.LM1253
						 || key == Mod2002022Key.LM1254) {
