...
public class Page18 extends PageAbs {
	...
	private static final String FOOTER_1494_1 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene dotaciones pendientes de integrar en un per\u00EDodo impositivo anterior iniciado en 2024.";
	...
	private static final String FOOTER_1579 = "(*) S\u00F3lo debe cumplimentarse si la entidad tiene dotaciones pendientes de integrar correspondientes a un per\u00EDodo impositivo anterior iniciado en 2024.";

	public Page18( Model2002024PageCallback callback ) {
		super(callback);
	}
	...
	protected void initializeTable() {
		...
		row++;
		paintKeysProvider(Mod2002024LM1494Key.values(), table, row, false, 
				AON.MSG.liquiMsg1(),
				AON.MSG.dot11(),
				AON.MSG.dot12(),
				AON.MSG.dot22(),
		...
		paintKeyField(table1,Mod2002024Key.RV000 , 0, 1,  5, false); // Número de período impositivo (*)
		paintDescription(table1, "Dotaciones pendientes de integraci\u00F3n a principio del per\u00EDodo", 1, 0, false);
		paintKeyField(table1,Mod2002024Key.RV941 , 1, 1, 10, false); // Dotaciones pendientes de integración a principio del período
		paintDescription(table1, "Dotaciones integradas en esta liquidaci\u00F3n: DT 16\u00AA.1 y 2 LIS", 2, 0, false);
		paintKeyField(table1,Mod2002024Key.RV2810, 2, 1, 10, false); // Dotaciones integradas en esta liquidación DT 16ª.1 y 2 LIS
		paintDescription(table1, "Dotaciones integradas en esta liquidaci\u00F3n: DT 16\u00AA.3 LIS", 3, 0, false);
		paintKeyField(table1,Mod2002024Key.RV990 , 3, 1, 10, false); // Dotaciones integradas en esta liquidación DT 16ª.3 LIS
		paintDescription(table1, "Dotaciones pendientes de integraci\u00F3n en per\u00EDodos futuros", 4, 0, false);
		paintKeyField(table1,Mod2002024Key.RV991 , 4, 1, 10, false); // Dotaciones pendientes de integración en períodos futuros
		...
		row++;
		paintKeysProvider(Mod2002024LM1535Key.values(), table2, row, false, 
				AON.MSG.liquiMsg1(),  
				AON.MSG.dot30() + " (*)",
				AON.MSG.dot41(),
				AON.MSG.dot31(),
		...
		row++;
		paintKeysProvider(Mod2002024LM1561Key.values(), table3, row, false, 
				AON.MSG.liquiMsg1(),
				AON.MSG.dot40(),
				AON.MSG.dot41(),
				AON.MSG.dot42(),
		...
		for (int i = 0; i < Mod2002024Constants.DOTACION_KEYS_5.length; i++) {
			Mod2002024Key key = Mod2002024Constants.DOTACION_KEYS_5[i]; 
		...
		paintKeysProvider(Mod2002024LM1579Key.values(), table5, 1, false, 
				AON.MSG.liquiMsg1(),
				AON.MSG.dot50(),
				AON.MSG.dot51(),
				AON.MSG.dot52(),
