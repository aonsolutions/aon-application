...
import static com.esferalia.aon.occam.mod200.api.model.mod200_2022.Mod2002022Behaviour.BEHAVIOUR_KEYS_MAP;
...
public abstract class PageAbs extends ResizeComposite {
	...
	protected int paintKey(FlexTable tab, final Mod2002022Key key, int row) {
		paintKeyDescription(tab, key, row, 0);
		paintKeyField(tab, key, row, 1);	
		return ++row;
	}
	
	protected int paintKey(FlexTable tab, final Mod2002022Key key, int row, boolean bold) {
		paintDescription(tab, key.getDescription(), row, 0, bold);
		paintKeyField(tab, key, row, 1);	
		return ++row;
	}
	
	protected int paintKey(FlexTable tab, final Mod2002022Key key, int row, boolean bold, boolean padding) {
		paintDescription(tab, key.getDescription(), row, 0, bold);
		paintKeyField(tab, key, row, 1, padding);	
		return ++row;
	}
	
	protected void paintKeyDescription(FlexTable tab, Mod2002022Key key, int row, int col) {
	...
	protected void paintKeyField(FlexTable tab, final Mod2002022Key key, int row, int col) {
	...
	protected void paintKeysProvider(IMod200KeysProvider[] keysProvider, final FlexTable tab, int row, boolean padding, String... headers) {
		...
		for (IMod200KeysProvider kp : keysProvider) {
			...
			for (IMod200Key key : kp.getKeys()) {				 
				if (key != null && callback.getMod200Object().isVisible((Mod2002022Key) key)) {
	...
	protected void addTable(String title, Mod2002022Key[] keys) {

		FlexTable table = addTable(title);

		int row = 0;
		for (Mod2002022Key key : keys) {
