...
public class Model2002016 extends ResizeComposite  {
	...
	public Model2002016(Model200ModuleOptions options, Model200Callback mod200Callback) {
...
//	}
	...
	void onInitializeClick(ClickEvent event) {
		...
		mod200Object.initializeMod200(null, new AsyncCallback<Mod2002016>() {
			@Override
			public void onSuccess(Mod2002016 result) {
				popup.hide();
				dump(false);
			...
			new ConfirmDialogCallback() {
				...
				public void onAccept() {
					diskForm.setAction(GWT.getHostPageBaseURL()
							+ "/aon_gwt_mod200/Model2002016AccountingFile");
	...
	void onAeatFileButtonClick(ClickEvent event) {
		...
		cd.confirm("Se va a proceder a la generaci\u00F3n de un fichero\n"
			...
			new ConfirmDialogCallback() {
				...
				public void onAccept() {
					diskForm.setAction(GWT.getHostPageBaseURL()
							+ "/aon_gwt_mod200/Model2002016File");
					modIdHidden.setValue(String.valueOf(mod200Object.getMod200().getId()));
					domainIdHidden.setValue(String.valueOf(options.getDomain()));
					domainNameHidden.setValue(options.getDomainName());
					diskForm.submit();
	...
	void onAeatPrintButtonClick(ClickEvent event) {
		...
		cd.confirm("La impresi\u00F3n se genera en los servidores de la Agencia Tributaria "
			...
			,new ConfirmDialogCallback() {
				...
				public void onAccept() {
					diskForm.setAction(GWT.getHostPageBaseURL()
							+ "/aon_gwt_mod200/Model2002016Print");
					modIdHidden.setValue(String.valueOf(mod200Object.getMod200().getId()));
					domainIdHidden.setValue(String.valueOf(options.getDomain()));
					domainNameHidden.setValue(options.getDomainName());
					diskForm.submit();
	...
	void onImportAccountingButtonClick(ClickEvent event) {
		Upload upload = new Upload() {
			...
			protected void onUpload(String data, String type) {
				mod200Object.fillMod2002016AccountingData(data, new AsyncCallback<Mod2002016>() {
					@Override
					public void onSuccess(Mod2002016 result) {
