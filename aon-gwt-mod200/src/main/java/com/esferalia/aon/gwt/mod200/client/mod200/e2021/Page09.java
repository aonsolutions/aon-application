...
import com.esferalia.aon.occam.mod200.api.model.mod200_2021.Mod2002021LQ579Key;
...
public class Page09 extends PageAbs {
	...
	protected void initializeTable() {
		...
		for (Mod2002021Key key : Mod2002021Constants.LIQUIDATION_II_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {			
				if (key == Mod2002021Key.LQ578) {
				...
				if (key == Mod2002021Key.LQ560) {
					paintDescription(table, "S\u00F3lo sociedades cooperativas", row,0, true);
					table.getCellFormatter().addStyleName(row, 0, AON.AON_CSS.aonFiscalPaddingLeft());
					row++;
					margin = true;
				}				
				if (key == Mod2002021Key.LQ550 || key == Mod2002021Key.LQ558 || key == Mod2002021Key.LQ562 || key == Mod2002021Key.LQ1032 || key == Mod2002021Key.LQ1330) {
					margin = false;
				}
	...
	protected boolean isDisabled(IMod200Key key) {
		if (key == Mod2002021Key.LQ1032 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002021Key.C0009) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002021Key.C0010))) {
			return false;
		}
		if (key == Mod2002021Key.LQ562 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002021Key.C0030) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002021Key.C0047))) {
			return false;
		}
		if ((key == Mod2002021Key.LQ520 || key == Mod2002021Key.LQ521) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002021Key.C0064))) {
			return true;
		}
