// LIQUIDACION (IV): OTRAS DEDUCCIONES, CUOTA LIQUIDA, PRODUCCIONES CINEMATOGRAFICAS
...
public class Page12 extends PageAbs {
	...
	private static final String FOOTER_1 = "(*) S\u00F3lo debe cumplimentarse si tiene deducciones pendientes de aplicar correspondientes a un per\u00EDodo impositivo anterior iniciado en 2024.";
	...
	private static final String FOOTER_588_2 = "(****) Programas cuya vigencia se inicia a partir de 2024: S\u00F3lo debe cumplimentarse esta fila si la entidad tiene un per\u00EDodo impositivo que no coincida con el a\u00F1o natural y ha realizado gastos con derecho a deducci\u00F3n a partir de 2024.";
	...
	public Page12( Model2002024PageCallback callback ) {
		super(callback);
	}
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (final Mod2002024Key key : Mod2002024Constants.LIQUIDATION_IV_KEYS) {
			row = paintKey(table,key,row);
			if (callback.getMod200Object().isVisible(key)) {
				
				// Casillas con desglose
				
				if (key == Mod2002024Key.BN585) {
					row = paintKeyBreakdownLink(table, row, Mod2002024Key.BN585, Mod2002024BN585Key.values(), HEADERS_1, FOOTER_1);
				} 
				if (key == Mod2002024Key.BN584) {
				...
				if (key == Mod2002024Key.BN565) {
					row = paintKeyBreakdownLinkBN565(table, row);
				}
				if (key == Mod2002024Key.BN590) {
					row = paintKeyBreakdownLink(table,row,Mod2002024Key.BN590,Mod2002024BN590Key.values(),HEADERS_1, FOOTER_1);
				}
				if (key == Mod2002024Key.BN1040) {
					row = paintKeyBreakdownLink(table,row,Mod2002024Key.BN1040,Mod2002024BN1040Key.values(),HEADERS_3, FOOTER_1);
				}
				if (key == Mod2002024Key.BN1041) {
					row = paintKeyBreakdownLink(table,row,Mod2002024Key.BN1041,Mod2002024BN1041Key.values(),HEADERS_3, FOOTER_1);
				}				
				if (key == Mod2002024Key.BN2315
					...
					row = paintKeyBreakdownLink(table,row,Mod2002024Key.BN2315,Mod2002024BN2315Key.values(),HEADERS_1, FOOTER_1);
				}
				
				if (key == Mod2002024Key.BN1039) {  
		...
		// INFORMACION ADICIONAL PRODUCCIONES CINEMATOGRAFICAS ESPAÑOLAS Y ESPECTACULOS EN VIVO
		...
		paintLabel(basePanel, "Consigne, a continuaci\u00F3n, los datos relativos a las producciones cinematogr\u00E1ficas con derecho a deducci\u00F3n por parte del productor.", false);
		
		addTable("", Mod2002024Constants.FILM_PRODUCTIONS);
	...
	private int paintKeyBreakdownLinkBN565(final FlexTable tab, int row) {
		...
		paintKeyBreakdownLinkContainer(container, Mod2002024Key.BN565, Mod2002024BN565_1Key.values(), headers, FOOTER_1);
		headers[0] = "DONACIONES PARA ACTIVIDADES PRIORITARIAS DE MECENAZGO Y OTRAS CON DERECHO A DEDUCCION INCREMENTADA";
		paintKeyBreakdownLinkContainer(container, Mod2002024Key.BN565, Mod2002024BN565_2Key.values(), headers, FOOTER_1);
		headers[0] = "TOTAL DEDUCCIONES A ENTIDADES SIN FINES DE LUCRO (LEY 49/2002)";
		headers[2] = "";
		paintKeyBreakdownLinkContainer(container, Mod2002024Key.BN565, Mod2002024BN565Key.values(), headers, null);
		...
		paintKey(table, Mod2002024Key.BN974, 1);		
		container.add(table);
	...
	private void paintKeyBreakdownLinkContainer(FlowPanel container, Mod2002024Key breakdownKey, IMod200KeysProvider[] keysProvider, String[] headers, String footernote) {
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if ((key == Mod2002024Key.BN2315 || key == Mod2002024Key.BN1039 || key == Mod2002024Key.BN2314 ) && 
