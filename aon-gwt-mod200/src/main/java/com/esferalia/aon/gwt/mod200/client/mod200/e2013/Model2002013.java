...
public class Model2002013 extends ResizeComposite  {
	
	interface Model2002013Binder extends
			UiBinder<Widget, Model2002013> {
	}

	private static final Model2002013Binder binder = GWT
			.create(Model2002013Binder.class);
	...
	protected void paintHeaderTable(final Mod2002013 mod200) {
	...
	private class PopupAsyncCallback implements AsyncCallback<Mod2002013> {
		PopupPanel popup;
	...
	void onInitializeClick(ClickEvent event) {
		...
		mod200Object.initializeMod200(new AsyncCallback<Mod2002013>() {
			@Override
			public void onSuccess(Mod2002013 result) {
				popup.hide();
				dump();
...
//		aeatPrintButton.setVisible(mod200Object.isInitialized() && mod200Object.getMod200().getId() != null);
	...
	public void startModel(final Mod2002013Object modObject) {
	...
	private void populatePages(Mod2002013Object mod200Object) {
		page00.populate(mod200Object);
...
*/
	public void validationMessageSelected(ValidationMessage2013 msg) {
		if ( msg.getPage() >= 0 ) {
	...
	void onAeatAccountingFileButtonClick(ClickEvent event) {
		...
		diskForm.setAction(GWT.getHostPageBaseURL()
				+ "/aon_gwt_mod200/Model2002013AccountingFile");
	...
	void onAeatFileButtonClick(ClickEvent event) {
		Window.alert(
				...
				+ "El fichero se genera a partir de los datos guardados.");
		diskForm.setAction(GWT.getHostPageBaseURL()
				+ "/aon_gwt_mod200/Model2002013File");
		modIdHidden.setValue(String.valueOf(mod200Object.getMod200().getId()));
		domainIdHidden.setValue(String.valueOf(options.getDomain()));
		domainNameHidden.setValue(options.getDomainName());
		diskForm.submit();
	...
	void onAeatPrintButtonClick(ClickEvent event) {
		...
		diskForm.setAction(GWT.getHostPageBaseURL()
				+ "/aon_gwt_mod200/Model2002013Print");
		modIdHidden.setValue(String.valueOf(mod200Object.getMod200().getId()));
		domainIdHidden.setValue(String.valueOf(options.getDomain()));
		domainNameHidden.setValue(options.getDomainName());
		diskForm.submit();
	...
	private class WestFocusPanel extends FocusPanel {
		...
		private PageAbs content;
		
		public WestFocusPanel(String label, final PageAbs content) {
			super();
	...
	private void fillLinkContainer() {
		linkContainer.add(new WestFocusPanel("1.- " + AON.MSG.identification(), page00));
		linkContainer.add(new WestFocusPanel("2.- " + AON.MSG.administratorPage(), page01));
		linkContainer.add(new WestFocusPanel("3.- " + AON.MSG.participations(), page02));
		linkContainer.add(new WestFocusPanel("4.- " + AON.MSG.balanceActivo(), page03));
		linkContainer.add(new WestFocusPanel("5.- " + AON.MSG.balancePasivo(), page04));
		linkContainer.add(new WestFocusPanel("6.- " + AON.MSG.pyg(), page05));
		linkContainer.add(new WestFocusPanel("7.- " + AON.MSG.patrimonioIngresos(), page06));
		linkContainer.add(new WestFocusPanel("8.- " + AON.MSG.patrimonioCambios(), page07));
		linkContainer.add(new WestFocusPanel("9.- " + AON.MSG.liquidacionI(), page08));
		linkContainer.add(new WestFocusPanel("10.- " + AON.MSG.liquidacionII(), page09));
		linkContainer.add(new WestFocusPanel("11.- " + AON.MSG.liquidacionIII(), page10));
		linkContainer.add(new WestFocusPanel("12.- " + AON.MSG.liquidacionIV(), page11));
		linkContainer.add(new WestFocusPanel("13.- " + AON.MSG.incomeDistribution(), page12));
		linkContainer.add(new WestFocusPanel("14.- " + AON.MSG.deducibleLimitation(), page13));
		linkContainer.add(new WestFocusPanel("15.- " + AON.MSG.idDocument(), page14));
	}
}
