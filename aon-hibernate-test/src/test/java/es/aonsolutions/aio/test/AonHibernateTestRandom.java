...
public class AonHibernateTestRandom {
	...
	public static Integer integer(int nullThreshold, int maxLength) {
		return (gt(nullThreshold)) ? Integer.valueOf(getInt(0, 20)) : null;
	}
	...
	public static Date getFutureDate(int threshold) {
		return (gt(threshold)) ? truncate(faker.date().future(100, TimeUnit.DAYS, new Date())) : null;
	}
