package net.aonsolutions.aon.tbai.emision;

import static net.aonsolutions.aon.tbai.toolkit.EmisionToolkit.booleanToSiNoType;
import static net.aonsolutions.aon.tbai.toolkit.EmisionToolkit.countryToCountryType2;
import static net.aonsolutions.aon.tbai.toolkit.DataToolkit.formatDate;
...
import net.aonsolutions.aon.tbai.Invoice;
import net.aonsolutions.aon.tbai._beans.Entity;
import net.aonsolutions.aon.tbai._beans.InvoiceDetailData;
import net.aonsolutions.aon.tbai._beans.RectificationData;
import net.aonsolutions.aon.tbai._enums.IDtype;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.Breakdown;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.Delivery;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.Exempted;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.National;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.NoExempted;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.NoNational;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.NoSubject;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.Service;
import net.aonsolutions.aon.tbai.emision._beans.breakdowns.Subject;
...
public class EmisionInvoice extends Invoice {
	...
	public Sujetos getSujetos() throws XMLCreationException {
		try {
			...
			if(!this.getRecievers().isEmpty()) {
				...
				for (final Entity e : Receivers) {
					...
					if(country.isPresent()) other_id.setCodigoPais(countryToCountryType2(country.get())); 
			...
			return entities;
		}catch(Exception e) {throw new XMLCreationException("ERROR: SUBJECT DATA CORRUPTED");}
	}

	public Factura	getFactura() throws XMLCreationException {
		try {
			...
			if(expedition_date.isPresent()) 	{
				invoice_header.setFechaExpedicionFactura(formatDate(expedition_date.get(), "dd-MM-yyyy") .orElseThrow()); 	
				invoice_header.setHoraExpedicionFactura(formatDate(expedition_date.get(),  "hh:mm:ss")   .orElseThrow());		
			}
			...
			if(op_date.isPresent())					invoice_data.setFechaOperacion(formatDate(op_date.get(), "dd-MM-yyyy").orElseThrow()); 
			...
			return invoice; 	
		}catch(Exception e) {throw new XMLCreationException("ERROR: INVOICE DATA CORRUPTED",e);}
	}
	...
	private NoExentaType getNoExenta(final Optional<NoExempted> no_exenta) {
		...
		if(equiv_rech_or_simpl.isPresent())		iva_det.setOperacionEnRecargoDeEquivalenciaORegimenSimplificado(booleanToSiNoType(equiv_rech_or_simpl.get()));
	...
	public HuellaTBAI getHuellaTbai() { 
		...
		if(sf_dev.isPresent()) 	{	
			...
			if(country.isPresent()) 	id_other.setCodigoPais(countryToCountryType2(country.get()));
	...
	public static class TbaiEmisionInvoiceBuilder{
		...
		public EmisionInvoice build() {
			EmisionInvoice  i = new EmisionInvoice();
