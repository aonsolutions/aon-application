...
class ServiceEnterprise implements ICollection {
  ...
  setEnterprise(model: string, pKey: any): Promise<Response> {
    return new Promise((resolve, reject) => {
      this.getElement('enterprise', pKey)
        .then((response) => {
          if (response != null) {
...
let users = [{ username: 'test@aonsolutions.test', password: 'testing' }];

let enterprises = [new Enterprise('Pet Estudio', 'B16880148'), new Enterprise('MENG SA', 'U14241855'), new Enterprise('PORTABAGE SL', 'U53716270')];
