...
export class FolderFactory implements ISingleObjectCrudFactory<IFolder>, IMultipleObjectCrudFactory<IFolder> {
  ...
  createMultipleObjectCrud(): IMultipleObjectCrud<IFolder> {
    return new GenericMultipleObjectCrud<Folder>(
      APIEnvironment ? new APIFolderMultipleObjectCrudRepository() : new GenericMultipleObjectCrudRepository<Folder>(new StorableFolder(), Folder),
      Folder,
    );
  }
}
...
export class MessageFactory implements ISingleObjectCrudFactory<IMessage>, IMultipleObjectCrudFactory<IMessage> {
  ...
  createMessageSpecificMethods(): IMessageSpecificMethods {
    return new MessageSpecificMethods(APIEnvironment ? new APIMessageSpecificMethodsRepository() : new LocalMessageSpecificMethodsRepository());
  }
}
...
class PaymentMethodFactory implements IMultipleObjectCrudFactory<IPaymentMethod> {
  createMultipleObjectCrud(): IMultipleObjectReader<IPaymentMethod> {
    return new GenericMultipleObjectCrud<PaymentMethod>(
      new GenericMultipleObjectCrudRepository<PaymentMethod>(new StorablePaymentMethod(), PaymentMethod),
      PaymentMethod,
    );
  }
}
...
class InvoiceCategoryFactory implements IMultipleObjectCrudFactory<IInvoiceCategory> {
  createMultipleObjectCrud(): IMultipleObjectReader<IInvoiceCategory> {
    return new GenericMultipleObjectCrud<InvoiceCategory>(
      new GenericMultipleObjectCrudRepository<InvoiceCategory>(new StorableInvoiceCategory(), InvoiceCategory),
      InvoiceCategory,
    );
  }
}
...
class InvoiceSerieFactory implements IMultipleObjectCrudFactory<IInvoiceSerie> {
  createMultipleObjectCrud(): IMultipleObjectReader<IInvoiceSerie> {
    return new GenericMultipleObjectCrud<InvoiceSerie>(
      new GenericMultipleObjectCrudRepository<InvoiceSerie>(new StorableInvoiceSerie(), InvoiceSerie),
      InvoiceSerie,
    );
  }
}
...
class InvoiceActivityFactory implements IMultipleObjectCrudFactory<IInvoiceActivity> {
  createMultipleObjectCrud(): IMultipleObjectReader<IInvoiceActivity> {
    return new GenericMultipleObjectCrud<InvoiceActivity>(
      new GenericMultipleObjectCrudRepository<InvoiceActivity>(new StorableInvoiceActivity(), InvoiceActivity),
      InvoiceActivity,
    );
  }
}
...
class InvoiceTransactionTypeFactory implements IMultipleObjectCrudFactory<IInvoiceTransactionType> {
  createMultipleObjectCrud(): IMultipleObjectReader<IInvoiceTransactionType> {
    return new GenericMultipleObjectCrud<InvoiceTransactionType>(
      new GenericMultipleObjectCrudRepository<InvoiceTransactionType>(new StorableInvoiceTransactionType(), InvoiceTransactionType),
...
class ProductCodeFactory implements IMultipleObjectCrudFactory<IProductCode> {
  createMultipleObjectCrud(): IMultipleObjectReader<IProductCode> {
    return new GenericMultipleObjectCrud<ProductCode>(
      new GenericMultipleObjectCrudRepository<ProductCode>(new StorableProductCode(), ProductCode),
      ProductCode,
    );
  }
}
...
class ProductClassFactory implements IMultipleObjectCrudFactory<IProductClass> {
  createMultipleObjectCrud(): IMultipleObjectReader<IProductClass> {
    return new GenericMultipleObjectCrud<ProductClass>(
      new GenericMultipleObjectCrudRepository<ProductClass>(new StorableProductClass(), ProductClass),
      ProductClass,
    );
  }
}
...
class ProductCategoryFactory implements IMultipleObjectCrudFactory<IProductCategory> {
  createMultipleObjectCrud(): IMultipleObjectReader<IProductCategory> {
    return new GenericMultipleObjectCrud<ProductCategory>(
      new GenericMultipleObjectCrudRepository<ProductCategory>(new StorableProductCategory(), ProductCategory),
      ProductCategory,
    );
  }
}
...
class ProductTypeFactory implements IMultipleObjectCrudFactory<IProductType> {
  createMultipleObjectCrud(): IMultipleObjectReader<IProductType> {
    return new GenericMultipleObjectCrud<ProductType>(
      new GenericMultipleObjectCrudRepository<ProductType>(new StorableProductType(), ProductType),
      ProductType,
    );
  }
}
...
class ProductStatusFactory implements IMultipleObjectCrudFactory<IProductStatus> {
  createMultipleObjectCrud(): IMultipleObjectReader<IProductStatus> {
    return new GenericMultipleObjectCrud<ProductStatus>(
      new GenericMultipleObjectCrudRepository<ProductStatus>(new StorableProductStatus(), ProductStatus),
      ProductStatus,
    );
  }
}
...
interface ISingleObjectCrud<T extends ICollectable>
  extends ISingleObjectReader<T>,
    ISingleObjectCreator<T>,
    ISingleObjectUpdater<T>,
    ISingleObjectRemover<T> {}
...
interface IMultipleObjectCrud<T extends ICollectable>
  extends IMultipleObjectReader<T>,
    IMultipleObjectCreator<T>,
    IMultipleObjectUpdater<T>,
    IMultipleObjectRemover<T> {}
...
interface ISingleObjectCrudRepository<T extends ICollectable>
  extends IRepositorySingleObjectReader<T>,
    IRepositorySingleObjectCreator<T>,
    IRepositorySingleObjectUpdater<T>,
    IRepositorySingleObjectRemover<T> {}

interface IMultipleObjectCrudRepository<T extends ICollectable>
  extends IRepositoryMultipleObjectReader<T>,
    IRepositoryMultipleObjectCreator<T>,
    IRepositoryMultipleObjectUpdater<T>,
    IRepositoryMultipleObjectRemover<T> {}
...
class GenericMultipleObjectCrudRepository<T extends IModel> implements IMultipleObjectCrudRepository<T> {
  ...
  async update(collection: ICollection<T>): Promise<ICollection<T>> {
    collection.toArray().forEach((element) => {
      if (
        !(
          (element.getKey() != element.Key &&
            !this.instance.getCollection().exists(element.getKey()) &&
            this.instance.getCollection().exists(element.Key)) ||
  ...
  async delete(keys: string[]): Promise<void> {
    keys.forEach((key) => {
      if (!this.instance.getCollection().exists(key)) throw new ErrorResponse('0204');
    });
    keys.forEach((key) => {
...
class GenericSingleObjectCrudRepository<T extends IModel> implements ISingleObjectCrudRepository<T> {
  ...
  async get(key: string): Promise<T> {
    if (this.instance.getCollection().exists(key)) return this.instance.getCollection().get(key);
    else throw new ErrorResponse('0206');
  }
...
class APIGenericSingleObjectCrudRepository<T extends IModel> implements ISingleObjectCrudRepository<T> {
  ...
  async create(element: T): Promise<T> {
    ...
    let response = await this.httpRequest.httpRequest(
      BASE_URL + this.apiModel.getUrl(CREATE_SINGLE),
      this.apiModel.getMethod(CREATE_SINGLE),
      {},
      data,
    );
    return this.apiModel.parseDataToReceive(response, GET_SINGLE);
...
class APIFolderMultipleObjectCrudRepository extends APIGenericMultipleObjectCrudRepository<Folder> {
  ...
  async get(filter?: IFilter | undefined): Promise<ICollection<Folder>> {
    ...
    for (let folder of apiFolders) collection.add(folder);
...
class APIMessageSingleObjectCrudRepository extends APIGenericSingleObjectCrudRepository<Message> {
  ...
  async create(message: Message): Promise<Message> {
    ...
    let newMessage: Message = this.apiModel.parseDataToReceive(
      await this.httpRequest.httpRequest(BASE_URL + '/ms/api/task', POST_METHOD, {}, json),
      GET_SINGLE,
    );
    return newMessage;
  }
...
class APIMessageChatSingleObjectCrudRepository extends APIGenericSingleObjectCrudRepository<MessageChat> {
  ...
  async create(messageChat: MessageChat): Promise<MessageChat> {
    ...
    let newMessage: MessageChat = this.apiModel.parseDataToReceive(
      await this.httpRequest.httpRequest(BASE_URL + '/ms/api/task/workflow', POST_METHOD, {}, json),
      GET_MULTIPLE,
    );
    return newMessage;
    throw new ErrorResponse('0199');
...
class APIMessageSpecificMethodsRepository implements IMessageSpecificMethodsRepository {
  ...
  async getMessageCount(filter?: IFilter): Promise<number> {
    if (filter && filter.fields && filter.fields.get('type') == 'notificacion') {
      ...
      let result = await this.httpRequest.httpRequest(
        BASE_URL + '/ms/api/task/status/count?task_holder=' + localStorage.getItem('registry'),
        GET_METHOD,
        {},
        {},
      );
      ...
      let result2 = await this.httpRequest.httpRequest(
        BASE_URL + '/ms/api/task/status/count?task_holder=' + localStorage.getItem('registry'),
        GET_METHOD,
        {},
        {},
  ...
  async markAsReadNotification(element: Message): Promise<boolean> {
    if (element.Type == 'notificacion') {
      let result = await this.httpRequest.httpRequest(
        BASE_URL + '/ms/api/notification/mark-read-notification',
        POST_METHOD,
        {},
        { source_id: element.Id },
      );
      if (result.success && result.success == true) return true;
      else return false;
    } else throw new ErrorResponse('0199');
  }
}
...
export class Collection<T extends ICollectable> implements ICollection<T> {
  ...
  get(key: string): T {
    ...
    else throw new Error('Element not found in collection');
  }
  ...
  sort(filter: IFilter): void {
    ...
    filter.orderBy.forEach((key, value) => {
      if (this.toArray()[0].getSortableFields().get(value.toLowerCase()) == undefined)
        throw new Error('SORT ERROR: No campo a ordenar no existe o no se puede ordenar');
  ...
  filter(filter: IFilter): ICollection<T> {
    ...
    if (filter.intervalFields?.size != 0)
      filter.intervalFields?.forEach((value, key) => {
        filteredArray = filteredArray.filter((element) => value.end >= element.getFilterableFields().get(key.toLowerCase()) <= value.start);
      });
...
class ApiDocument extends Document implements IApiModel {
  getUrl(currentMethod: string, filter: IFilter): string[] {
    ...
    if (currentMethod == GET_MULTIPLE) {
      if (filter.fields?.has('path') && filter.fields.get('path')[0].toLowerCase() == '/a_contabilizar') return ['/ms/api/invoice?status=inbox'];
      if (filter.fields?.has('path') && filter.fields.get('path')[0].toLowerCase() == '/contabilizado') return ['/ms/api/invoice?status=inbox'];
      // return ['/ms/api/invoice?status=rejected'];
      if (filter.fields?.has('path') && filter.fields.get('path')[0].toLowerCase() == '/fiscal') return ['/ms/api/fiscal/models'];
      if (filter.fields?.has('path') && filter.fields.get('path')[0].toLowerCase() == '/papelera') return ['/ms/api/invoice?status=draft'];
  ...
  getUrl(currentMethod: string, filter: IFilter): string[] {
    ...
    if (currentMethod == GET_MULTIPLE) {
      if (filter && filter.fields && filter.fields?.has('workplace')) {
        ...
        return urls;
      } else return ['/ms/api/contract/employee/workplace'];
    }
    throw new ErrorResponse('0199');
  }
...
class ApiEnterprise extends Enterprise implements IApiModel {
  getUrl(currentMethod: string, filter: IFilter): string[] {
    if (currentMethod == GET_MULTIPLE) return ['/ms/api/company'];
    if (currentMethod == GET_SINGLE) return ['/ms/api/company/one?id=' + filter.fields?.get('id')];
    throw new ErrorResponse('0199');
  }

  getMethod(currentMethod: string, filter?: IFilter | undefined): string {
    if (currentMethod == GET_MULTIPLE) return GET_METHOD;
    if (currentMethod == GET_SINGLE) return GET_METHOD;
    throw new ErrorResponse('0199');
  }
  ...
  parseDataToReceive(data: any, currentMethod: string): any {
    if (currentMethod == GET_MULTIPLE) {
      ...
      enterprise.ProfilePhoto = enterprise.Province = data.address.province;
...
class ApiBank extends Bank implements IApiModel {
  getUrl(currentMethod: string): string[] {
    if (currentMethod == GET_MULTIPLE) return ['/ms/api/company/banks'];
    throw new ErrorResponse('0199');
  }
...
class ApiTaxModel extends TaxModel implements IApiModel {
  getUrl(currentMethod: string): string[] {
    if (currentMethod == GET_MULTIPLE) return ['/ms/api/fiscal/models'];
    throw new ErrorResponse('0199');
  }
...
class ApiMessage extends Message implements IApiModel {
  getUrl(currentMethod: string, filter: IFilter): string[] {
    if (currentMethod == GET_MULTIPLE) {
      if (filter && filter.fields?.has('type') && filter.fields.get('type') == TypeMessage.NOTIFICACION)
        ...
        return [
          '/ms/api/task?source=query&page=1&perPage=100&task_holder=' +
        ...
        return [
          '/ms/api/task?source=task&page=1&perPage=100&task_holder=' +
            localStorage.getItem('registry') +
            '&sender=' +
            localStorage.getItem('registry'),
        ];
        ...
        return [
          ...
          '/ms/api/task?source=query&page=1&perPage=100',
          '/ms/api/task?source=task&page=1&perPage=100',
        ];
    } else if (currentMethod == GET_SINGLE) {
      if (
        filter &&
        filter.fields?.has('id') &&
  ...
  parseDataToSend(data: Message, currentMethod: string) {
    if (currentMethod == UPDATE_SINGLE) {
      ...
      let status;
      if (data.Type == TypeMessage.NOTIFICACION) status = data.Status == StatusMessage.VISTA ? 1 : 0;
      else status = data.Status == StatusMessage.ABIERTA || data.Status == StatusMessage.PENDIENTE ? 'pending' : 'deleted';
  ...
  parseDataToReceive(data: any) {
    ...
    if (data.source && (data.source == 'task' || data.source == 'query')) {
      ...
      else message.Status = data.status && data.status == StatusMessage.PENDIENTE ? StatusMessage.ABIERTA : StatusMessage.CERRADA;
...
class ApiTaskHolder extends TaskHolder implements IApiModel {
  getUrl(currentMethod: string, filter?: IFilter | undefined): string[] {
    if (currentMethod == GET_MULTIPLE) return ['/ms/api/taskholder/list'];
    if (currentMethod == GET_SINGLE) return ['/ms/api/taskholder?id=' + filter?.fields?.get('id')];
    throw new Error('Method not implemented.');
  }
  getMethod(currentMethod: string, filter?: IFilter | undefined): string {
    if (currentMethod == GET_MULTIPLE || currentMethod == GET_SINGLE) return GET_METHOD;
    throw new Error('Method not implemented.');
  }
  parseDataToSend(data: any, currentMethod: string, filter?: IFilter | undefined) {
    throw new Error('Method not implemented.');
...
class ApiMessageChat extends MessageChat implements IApiModel {
  getUrl(currentMethod: string, filter: IFilter): string[] {
    if (currentMethod == GET_MULTIPLE && filter && filter.fields && filter.fields.has('idMessage'))
      return [
        '/ms/api/task/workflow?task=' +
          filter.fields.get('idMessage') +
          '&domainId=' +
...
class ApiDocumentTag extends DocumentTag implements IApiModel {
  ...
  parseDataToSend(data: any, currentMethod: string, filter?: IFilter | undefined) {
    if (currentMethod == CREATE_SINGLE) return { name: data.Name };
    if (currentMethod == UPDATE_SINGLE) {
  ...
  getUrl(currentMethod: string, filter?: IFilter | undefined): string[] {
    ...
    if (currentMethod == UPDATE_SINGLE) {
      return [
        '/ms/api/attachment/' + localStorage.getItem('domainName') + '/' + localStorage.getItem('login') + '/tag/edit/' + filter?.fields?.get('id'),
      ];
    }
    throw new ErrorResponse('0199');
  }
...
if (products.size() == 0) {
  products.add(
    new Product(
      productCodes.toArray()[0],
      'Producto 1',
      productCategories.toArray()[0],
      productClasses.toArray()[0],
      productCategories.toArray()[0],
      productTypes.toArray()[0],
      productStatuses.toArray()[0],
      irpf.toArray()[0],
      'bar code 1',
  ...
  products.add(
    new Product(
      productCodes.toArray()[1],
      'Producto 2',
      productCategories.toArray()[1],
      productClasses.toArray()[1],
      productCategories.toArray()[1],
      productTypes.toArray()[1],
      productStatuses.toArray()[1],
      irpf.toArray()[1],
      'bar code 2',
...
if (contacts.size() == 0) {
  contacts.add(
    new Contact(
      ...
      'www.123.com',
      '123456789',
      paymentMethods.toArray()[0],
      invoiceTransactionTypes.toArray()[0],
      false,
      false,
    ),
  );
  contacts.add(
    new Contact(
      ...
      'www.qwe.com',
      '123456789',
      paymentMethods.toArray()[1],
      invoiceTransactionTypes.toArray()[1],
      false,
      false,
    ),
  );
  contacts.add(
    new Contact(
      ...
      'www.asd.com',
      '123456789',
      paymentMethods.toArray()[2],
      invoiceTransactionTypes.toArray()[2],
      false,
      false,
    ),
  );
...
if (invoices.size() == 0) {
  invoices.add(
    new Invoice(
      invoiceSeries.toArray()[0],
      '1',
      new Date(),
      200,
      contacts.toArray()[0],
      invoiceCategories.toArray()[0],
      invoiceLines,
      invoiceTransactionTypes.toArray()[0],
      invoiceActivities.toArray()[0],
      false,
      false,
      false,
      taxes.toArray()[0],
...
if (certificates.size() == 0) {
  certificates.add(
    new Certificate(
      'Andrés Nava Carranza',
      'Persona Física',
      new Date('Fri Jul 28 2023 13:38:29 GMT+0100'),
      new Date('Thu Aug 31 2025 03:52:27 GMT+0100'),
      'And323',
      'Público',
      false,
      true,
  ...
  certificates.add(
    new Certificate(
      'Andrea Casarez Saldaña',
      'Persona Física',
      new Date('Tue Feb 28 2023 20:37:13 GMT+0100'),
      new Date('Thu May 23 2025 22:33:06 GMT+0100'),
      'Andrea',
      'Público',
      false,
      true,
  ...
  certificates.add(
    new Certificate(
      'María Elena Tirado Chacón',
      'Persona Física',
      new Date('Wed Mar 22 2023 15:48:30 GMT+0100 '),
      new Date('Fri Aug 02 2025 12:43:24 GMT+0100'),
      'Andrea',
      'Público',
      false,
      true,
...
if (registryEnterprises.size() == 0) {
  registryEnterprises.add(
    new RegistryEnterprise('B16880148', 'lorem ipsum ...', new Date(), new Date(), 'Esperanza', 'Protocolo 1', 'Inscripción 1'),
  );
  registryEnterprises.add(
    new RegistryEnterprise('U14241855', 'lorem ipsum ...', new Date(), new Date(), 'Jorge Luis', 'Protocolo 2', 'Inscripción 2'),
  );
...
if (messages.size() == 0) {
  messages.add(
    new Message(
      'Asesor1',
      'Asunto Asunto Asunto Asunto Asunto Asunto Asunto 1',
      'sunt in culpa qui officia deserunt sunt in culpa qui officia deserunt sunt in culpa qui officia deserunt sunt in culpa qui officia deseruntsunt in culpa qui officia deseruntsunt in culpa qui officia deserunt sunt in culpa qui officia deseruntsunt in culpa qui officia deserunt sunt in culpa qui officia deserunt sunt in culpa qui officia deseruntsunt in culpa qui officia deserunt sunt in culpa qui officia deserunt sunt in culpa qui officia deserunt sunt in culpa qui officia deserunt',
      new Date('2023-06-12 12:00'),
      TypeMessage.CONSULTA,
      StatusMessage.ABIERTA,
    ),
  );
  messages.add(
    new Message(
  ...
  messages.add(
    new Message(
      ...
      'Asunto 1',
      'sunt in culpa qui officia deserunt',
      new Date('2023-07-16 15:00'),
      TypeMessage.CONSULTA,
      StatusMessage.CERRADA,
    ),
  );
  messages.add(
    new Message(
      ...
      'Asunto 2',
      'sunt in culpa qui officia deserunt',
      new Date('2023-05-16 12:30'),
      TypeMessage.NOTIFICACION,
      StatusMessage.VISTA,
    ),
  );
  messages.add(
  ...
  messages.add(
    new Message(
      ...
      'Asunto 3',
      'sunt in culpa qui officia deserunt',
      new Date('2023-06-03 10:01'),
      TypeMessage.TAREA,
      StatusMessage.REALIZADA,
      new Date('2023-08-24 23:30'),
    ),
  );
  messages.add(
    new Message(
      ...
      'Asunto 3',
      'sunt in culpa qui officia deserunt',
      new Date('2023-06-19 12:25'),
      TypeMessage.TAREA,
      StatusMessage.REALIZADA,
      new Date('2023-08-28 22:30'),
    ),
  );
  messages.add(
    new Message(
      ...
      'Asunto 3',
      'sunt in culpa qui officia deserunt',
      new Date('2023-07-22 15:30'),
      TypeMessage.TAREA,
      StatusMessage.PENDIENTE,
      new Date('2023-09-05 21:30'),
    ),
  );
...
if (messageChats.size() == 0 && filteredMessages.size() != 0) {
  messageChats.add(
    new MessageChat(filteredMessages.toArray()[0].Key, 'Asesor1', 'sunt in culpa qui officia deserunt', new Date('2023-06-12 15:30'), 'enviado'),
  );
  messageChats.add(
    new MessageChat(filteredMessages.toArray()[0].Key, 'Asesor1', 'sunt in culpa qui officia deserunt', new Date('2023-06-12 15:31'), 'recibido'),
  );
  messageChats.add(
    new MessageChat(filteredMessages.toArray()[0].Key, 'Asesor1', 'sunt in culpa qui officia deserunt', new Date('2023-06-12 15:32'), 'recibido'),
  );
...
if (test) {
  ...
  documentFactory
    .createMultipleObjectCrud()
    .getCollection(filterBuider.getFilter())
    .then((response) => {
  ...
  messageFactory
    .createMultipleObjectCrud()
    .getCollection(filterMessage.getFilter())
    .then((response) => {
  ...
  messageFactory
    ...
    .then((response) => {
      messageFactory
        .createSingleObjectCrud()
        .getElement(response.result.toArray()[0].Id)
        .then((element) => {
  ...
  messageFactory
    ...
    .then((response) => {
      messageFactory
        .createMessageSpecificMethods()
        .archiveMessage(response.result.toArray()[0])
        .then((response) => {
      ...
      messageFactory
        .createMessageSpecificMethods()
        .reopenMessage(response.result.toArray()[0])
        .then((response) => {
  ...
  messageFactory
    .createMessageSpecificMethods()
    .getMessageCount(filterMessage.getFilter())
    .then((response) => {
  ...
  messageFactory2
    .createMultipleObjectCrud()
    .getCollection(filterMessage2.getFilter())
    .then((response) => {
      //response.result.toArray()[0].Id
      filterMessageChat.addField('idMessage', '29475');
      messageChatFactory
        .createMultipleObjectCrud()
        .getCollection(filterMessageChat.getFilter())
        .then((response) => {
  ...
  messageChatFactory
    .createSingleObjectCrud()
    .createElement(messageChatCreate)
    .then((response) => {
  ...
  taxFactory
    .createMultipleObjectCrud()
    .getCollection(filterTax.getFilter())
    .then((response) => {
  ...
  markFactory
    .createSpecificMethods()
    .getMarksOfOneUser(localStorage.getItem('registry') || '', markFilter.getFilter())
    .then((response) => {
