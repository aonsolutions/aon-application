...
public class InvoiceTextPrinter {
	...
	private InvoiceTextPrinter header(Invoice invoice, PrintStream out) {
		...
		buf.append(VERTICAL_BAR);
		buf.append("         ");
		buf.append(AonStringUtils.spaces(16));
		buf.append(VERTICAL_BAR);
		buf.append(AonStringUtils.spaces(19));
		buf.append(VERTICAL_BAR);
		buf.append("  Nº Referc.:");
		buf.append(AonStringUtils.abbreviate(invoice.getHeader().getReferenceCode(), 42));
		buf.append(AonStringUtils.leftPad(" ", getLineSize() - buf.length()));
		buf.append(VERTICAL_BAR);
		out.println(buf.toString());
	...
	private InvoiceTextPrinter details(Invoice invoice, PrintStream out) {
		...
		buf.append(VERTICAL_BAR);
		buf.append(AonStringUtils.SPACE);
		buf.append(AonStringUtils.rightPad("CONCEPTO", 35));
		buf.append(AonStringUtils.SPACE);
		buf.append(AonStringUtils.leftPad("CANTIDAD", 10));
		buf.append(AonStringUtils.SPACE);
		buf.append(AonStringUtils.leftPad("PRECIO", 12));
		buf.append(AonStringUtils.SPACE);
		buf.append(AonStringUtils.leftPad("DESCUENTO", 7));
		buf.append(AonStringUtils.SPACE);
		...
		invoice.detailStream()
			...
			.forEach(det -> detail(det, out));
		
		if (invoice.detailStream().anyMatch(det -> det.isPrepayment())) {
			buf = new StringBuilder();
			buf.append(AonStringUtils.SPACE);
			buf.append(VERTICAL_RIGHT_BAR);
			buf.append(AonStringUtils.repeat(HORIZONTAL_BAR, getLineSize() - buf.length()));
	...
	private void detail(InvoiceDetail detail, PrintStream out) {
		...
		buf.append(AonStringUtils.SPACE);
		buf.append(AonStringUtils.leftPad(FMT.format(detail.getDiscount() ), 6));
	...
	private void invoiceTax(InvoiceTax tax, PrintStream out) {
		...
		buf.append(VERTICAL_BAR);
		buf.append("   ");
		buf.append(AonStringUtils.leftPad(tax.getTaxType() == null ? "????" : "(" + tax.getTaxType().getName() + ")", 6));
		...
		buf.append("] ");
		if (tax.isVatType() &&  AonMathUtils.isZero( tax.getSurcharge() )) {
			buf.append("[% Ded:");
		...
		if (tax.isVatType() &&  AonMathUtils.isNotZero( tax.getSurcharge() )) {
			buf = new StringBuilder();
			buf.append(AonStringUtils.SPACE);
			buf.append(VERTICAL_BAR);
			buf.append("   ");
			buf.append(AonStringUtils.leftPad("(RE)",6));
			buf.append(AonStringUtils.leftPad(FMT.format(tax.getSurcharge()), 6));
			buf.append(AonStringUtils.PERCENT);
			buf.append(" [Base:");
			buf.append(AonStringUtils.leftPad(FMT1.format(tax.getBase()), 17));
			buf.append("] ");
			buf.append("[Cuota:");
			buf.append(AonStringUtils.leftPad(FMT.format(tax.getSurchargeQuota()), 13));
			buf.append("] ");
			buf.append("[% Ded:");
			buf.append(AonStringUtils.leftPad(FMT.format(tax.getDeductiblePercent()), 6));
			buf.append("] ");
	...
	private InvoiceTextPrinter vatBreakdown(Invoice invoice, PrintStream out) {
		...
		buf.append(AonStringUtils.leftPad("B.Imp.", 16));
		buf.append(AonStringUtils.leftPad("% IVA", 8));
	...
	private void vatInvoiceBreakdown(InvoiceBreakdown tax, PrintStream out) {
		...
		buf.append(AonStringUtils.leftPad(FMT.format(tax.getPercentage()), 7));
		buf.append(AonStringUtils.PERCENT);
	...
	private InvoiceTextPrinter withholding(Invoice invoice, PrintStream out) {
		...
		if (iw.isPresent()) {
			...
			buf.append(VERTICAL_BAR);
			buf.append(AonStringUtils.spaces(1));
			buf.append(AonStringUtils.rightPad(TaxType.RETENTION.getName(), 5));
			buf.append(AonStringUtils.rightPad(iw.get().getWithholdingType().name() , 15));
	...
	private InvoiceTextPrinter totals(Invoice invoice, PrintStream out) {
		...
		buf.append(VERTICAL_BAR);
		buf.append(AonStringUtils.center("OTROS", 17));
		buf.append(VERTICAL_BAR);
	...
	private InvoiceTextPrinter fiscal(Invoice invoice, PrintStream out) {
		...
		invoice.getFiscal()
			.ifPresent( invoiceFiscal -> {
				StringBuilder buff = new StringBuilder();
				buff.append(AonStringUtils.spaces(10));
				buff.append(VERTICAL_BAR);
				buff.append(AonStringUtils.spaces(5));
				buff.append(AonStringUtils.rightPad("Fecha factura (fiscal) ....: " 
				...
				buff.append(VERTICAL_BAR);
				buff.append(AonStringUtils.spaces(5));
				buff.append(AonStringUtils.rightPad("Fecha IVA (fiscal) ........: " 
					+ (invoiceFiscal.getIssueDate() == null
						?"<sin asignar>"
				...
				buff.append(VERTICAL_BAR);
				buff.append(AonStringUtils.spaces(5));
				buff.append(AonStringUtils.rightPad("Fecha expedición (fiscal) .: " 
	...
	public InvoiceTextPrinter messages(Invoice invoice, PrintStream out) {
		...
		invoice.messageStream()
			.forEach( e -> {
				StringBuilder buff = new StringBuilder();
				buff.append(AonStringUtils.spaces(15));
				buff.append(VERTICAL_BAR);
				if (e.getLevel() == InvoiceErrorLevel.ERR) buff.append(ConsoleColors.RED);
				if (e.getLevel() == InvoiceErrorLevel.WRN) buff.append(ConsoleColors.YELLOW);
				if (e.getLevel() == InvoiceErrorLevel.INF) buff.append(ConsoleColors.BLUE);
				buff.append(AonStringUtils.rightPad(e.getLevel() == null ? "" : e.getLevel().toString(), 4));
				buff.append(ConsoleColors.RESET);
				buff.append(VERTICAL_BAR);
				buff.append(AonStringUtils.rightPad(e.getKey() == null 
								?"" 
								:(e.getKey().toString() + (e.getLine() == null ? "" : (" (" + e.getLine() + ")")))
							, 25));
				buff.append(VERTICAL_BAR);
				buff.append(AonStringUtils.rightPad(AonStringUtils.abbreviate(AonStringUtils.defaultString(e.getMessage()),79),80));
				buff.append(VERTICAL_BAR);
				out.println(buff.toString());			
		});
