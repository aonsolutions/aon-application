...
//	,OTRAS_GANANCIAS_PATRIMONIALES	("Otras ganancias patrimoniales")
	...
	public static Optional<WithholdingTypeGroup> value( Byte i ) {
		if (i == null) return Optional.empty();
		return value( i.intValue() ); 
	}
	
	public static Optional<WithholdingTypeGroup> value( Integer i ) {
		if (i == null) return Optional.empty();
		if (i < 0 || i >= WithholdingTypeGroup.values().length) return Optional.empty();
		return Optional.of(WithholdingTypeGroup.values()[i]);
	}
	
	public static Optional<WithholdingTypeGroup> value( String s ) {
