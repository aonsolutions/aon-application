...
public class GridPanel extends Composite {
	...
	private void load(LinkedList<JsStockStat> linkedList) {
		dataGrid.setRowStyles(new RowStyles<JsStockStat>() {
			@Override
			public String getStyleNames(JsStockStat row, int rowIndex) {
				return AON.AON_CSS.aonClickableBlock();
			}
		});
		...
		DefaultKeyboardSelectionHandler<JsStockStat> selHandler = new DefaultKeyboardSelectionHandler<JsStockStat>(dataGrid){
			...
			public void onCellPreview(CellPreviewEvent<JsStockStat> event) {
				if(BrowserEvents.CLICK.equals(event.getNativeEvent().getType())){
					...
					final InlineLabel label =  new InlineLabel("Filtrando por producto...");
	...
	private void initTableColumns(final SelectionModel<JsStockStat> selectionModel, ListHandler<JsStockStat> sortHandler) {
		...
		/** Status Column **/
		Column<JsStockStat, String> selectionColumn = new Column<JsStockStat, String>(new TextCell()) {
			
			@Override	
			public void render(Context context, JsStockStat object, SafeHtmlBuilder sb) {
				FilterPanel filter = (FilterPanel)parent.getMainContent().northContent.getWidget();
				if(filter.selectedProduct!=null && object.getId()==filter.selectedProduct.getId())
					sb.appendHtmlConstant("<g:Label class=\"" + AON.AON_CSS.aonIconRowSelector() + "\" style=\"padding-left: 6px;\" >&nbsp;&nbsp;");
			}
		...
		dataGrid.setColumnWidth(selectionColumn, 35, Unit.PX);
