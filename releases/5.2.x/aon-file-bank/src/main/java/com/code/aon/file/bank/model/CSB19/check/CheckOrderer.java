...
public class CheckOrderer extends Check{
	...
	public static boolean parse(Orderer orderer,ArrayList exceptions){
		...
		if (orderer.getMakeDate()==null){
			exceptions.add( new Fd0Exception( getMessage("ERROR_ORDERER_7") ,orderer.toString()) );
			status = false;
		}
		if (orderer.getStartDate()==null){
			exceptions.add( new Fd0Exception( getMessage("ERROR_ORDERER_8") ,orderer.toString()) );
			status = false;
		}
		return status;
	}
