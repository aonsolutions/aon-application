...
public class MinoracionesNavarraListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		MinoracionesNavarra n = (MinoracionesNavarra) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(MinoracionesNavarra.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.MINORACIONES_NAVARRA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				MinoracionesNavarra cra = (MinoracionesNavarra) iter.next();
