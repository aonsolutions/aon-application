...
public class ConveniosColectivosControllerListener extends ControllerAdapter implements IPayrollConstants {
	...
	private void resetNivelModel() throws ControllerListenerException {
		LinesController nivelController = (LinesController) FormUtil.getController(NIVEL_CONTROLLER_NAME);
		try {
	...
	private void resetComplementosCotizaciones() throws ControllerListenerException {
		ConveniosComplementoController comlementoController = (ConveniosComplementoController) FormUtil.getController(COMPLEMENTO_CONTROLLER_NAME);
		try {
