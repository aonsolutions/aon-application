...
public class MinoracionAlavaController extends PayrollBasicController {
	...
	public void onSearch(ActionEvent event) {
		...
		try {
			if(searchFecini != null){
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.MINORACIONES_ALAVA_ID_FECINI), searchFecini);
			}
			if(searchFecfin != null){
				//getCriteria().addEqualExpression(getFieldName(event.getComponent().getId()), searchFecfin);
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.MINORACIONES_ALAVA_FECFIN), searchFecfin);
			}
