...
function aonResizeBody(){
	...
	var contentCalendarTabs=document.getElementById("aon-content-calendar-tabs");
	...
	var contentCalendarTabsHeight=0;
	...
	if(contentCalendarTabs!=null){
		var contentCalendarTabsHeight=contentCalendarTabs.offsetHeight;			
	}
			...
			for (var j = 0; j < controls.length; j++) {
				if(controls[j].disabled == false){
					...
					if (controls[j].tagName == 'SELECT') {
						if (controls[j].id.indexOf("Application_menus") < 0) {
							controls[j].focus();
				        		return;
				        	}
				        }
