...
public class CuotaRetencionVizcayaController extends PayrollBasicController {
	...
	public void onSearch(ActionEvent event) {
		...
		try {
			if(searchFecini != null){
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.CUOTA_RETENCION_VIZCAYA_ID_FECINI), searchFecini);
			}
			if(searchFecfin != null){
			
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.CUOTA_RETENCION_VIZCAYA_FECFIN), searchFecfin);
			}
