...
import com.code.aon.file.format.core.RegisterManager;
...
public abstract class AbstractFileFiller implements FileFiller {
	...
	protected void writeErrorsFile(){
		if (exceptions.size()>0){
			...
			Iterator<Exception> iterErr = exceptions.iterator();
	 ...
	 */
	protected LinesFiller createLine(String registryType, Map<String,Object> properties) {
	...
	static {
		try {
			Class.forName("com.code.aon.file.format.FormatFactory");
		}
		catch (Exception ex) {
			ex.printStackTrace();
		}
