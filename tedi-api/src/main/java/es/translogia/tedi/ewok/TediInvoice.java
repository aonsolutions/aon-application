...
public class TediInvoice implements Serializable{
	...
	public void ensureTax(TediInvoiceTax tediTax) {
		if (getTaxes() == null) {
			setTaxes( new LinkedList<TediInvoiceTax>());
		}
		TediInvoiceTax toAdd = null;
		for (TediInvoiceTax tax : getTaxes() ) {
			if (tax.getTaxType() == tediTax.getTaxType() && equals( tediTax.getPercentage(), tax.getPercentage()) ) {
				toAdd = tax;
				break;
			}
		}
		if (toAdd == null) {
			...
			toAdd.setTaxType(tediTax.getTaxType());
			toAdd.setPercentage(tediTax.getPercentage());
			toAdd.setBase(tediTax.getBase());
			toAdd.setQuota(tediTax.getQuota());
			getTaxes().add(toAdd);		
		}
	}
