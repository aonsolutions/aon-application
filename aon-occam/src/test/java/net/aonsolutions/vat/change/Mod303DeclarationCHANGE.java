...
public abstract class Mod303DeclarationCHANGE {
	
	public static Mod303Declaration getInstance( Mod303 mod) {
		
		if (mod.isAEAT()) return new AEAT_2017_Declaration();
		if (mod.isBizkaia()) return new BIZKAIA_2017_Declaration();
		if (mod.isAraba()) 	return new ARABA_2017_Declaration();
		if (mod.isGipuzkoa()) 	return new GIPUZKOA_2017_Declaration();
