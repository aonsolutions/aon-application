...
public class Mod303InsertQuarterlyTest extends Mod303AbstractTest {
	...
	public void test() {
		...
		for (Period period : Period.values()) {
			...
			if (period.isQuarterPeriod()) {
				mod303InsertQuarterly( params.setIssueDate(AonRandom.getRangeDate(start,end)) );
			}
		}
	}
	
	public void mod303InsertQuarterly(FiscalFakerParams params) {
		...
		if ( !aeat.isLastPeriod() ) {
			...
			Asserts.assertEqualsDouble("Gipuzkoa " + gipuzkoa.getModelFullName() + ". Resultado no coincide."
					, getMod303SuitableResult(aeat)
					, getMod303SuitableResult(gipuzkoa));
		}
