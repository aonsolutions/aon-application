...
public class Mod115RoundedAmountsTest extends AbstractOccamTest {
	
	@Test
	public void testRound() {
		MODEL115.getMod115s(getOccam())
			.stream()
			.map( model -> MODEL115.get(getOccam(), model.getId()))
			.flatMap(mod115 -> mod115.getMap().values().stream())
			.forEach( det -> {
