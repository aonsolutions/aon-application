...
public class Mod115ARABATest extends AbstractOccamTest {
	...
	public void mod115InsertAEAT() {
		...
		FiscalFakerParams sustParams = new FiscalFakerParams(ctx,getOccam())
			...
			.setReplacement( true )
			.setGenerateFromYearStart( true );
		Mod115 sust = FiscalFaker.getMod115(sustParams);
		sust.setMustIncludeInvoicesOnGeneration( true );
		sust = MODEL115.create(getOccam(), sust);
		MODEL115.save(getOccam(), sust);
		Mod115 actualSust = MODEL115.get(getOccam(), sust.getId());  
		Asserts.assertMod115(sust, actualSust);
