...
public class Mod111InsertQuarterlyComplementaryTest extends AbstractOccamTest {
	
	@Test
	public void mod111InsertQuarterlyComplementaryTest() {
	...
	private void test(Double quota) {
		...
		Asserts.assertEqualsDouble("Mod111 Trim (AEAT). Resultado y facturas no coincide.", quota, commonTerritory.getDeclarationResult());
		Asserts.assertEqualsDouble("Mod111 Trim (Araba). Resultado y facturas no coincide.", quota, araba.getDeclarationResult());
		Asserts.assertEqualsDouble("Mod111 Trim (Bizkaia). Resultado y facturas no coincide.", quota, bizkaia.getDeclarationResult());
		Asserts.assertEqualsDouble("Mod111 Trim (Gipuzkoa). Resultado y facturas no coincide.", quota, gipuzkoa.getDeclarationResult());
		Asserts.assertEqualsDouble("Mod111 Trim (Navarra). Resultado y facturas no coincide.", quota, navarra.getDeclarationResult());
	}
