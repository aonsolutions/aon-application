...
public class Mod123SimulateMonthlyTest extends AbstractOccamTest {
	
	@Test
	public void mod123InsertMonthlyTest() {
		Date today = new Date();
		for (Period period : Period.values()) {
			if (period.isMonthPeriod()) {
				...
				mod123InsertMonthly(AonRandom.getRangeDate(start,end));
			}
		}
	}
	
	public void mod123InsertMonthly(Date date) {
		System.out.println( "\t ---------------------");

		Mod123 aeat = simulateModel( Administration.COMMON_TERRITORY, date);
		Mod123 araba = simulateModel( Administration.ALAVA, date);
		Mod123 bizkaia = simulateModel( Administration.BIZKAIA, date);
		Mod123 gipuzkoa = simulateModel( Administration.GIPUZKOA, date);
		Mod123 navarra = simulateModel( Administration.NAVARRA, date);
	...
	private Mod123 simulateModel( Administration admon, Date date) {
		FiscalFakerParams params = new FiscalFakerParams(ctx,getOccam())
			.setIssueDate(date)
			.setMonthly(true)
			.setAdministration(admon)
			;
		Mod123 mod123 = FiscalFaker.simulateMod123(params);
		mod123 = MODEL123.calculate(getOccam(), mod123);
		FiscalTestSuite.printModel(mod123);
		return mod123;
	}
}
