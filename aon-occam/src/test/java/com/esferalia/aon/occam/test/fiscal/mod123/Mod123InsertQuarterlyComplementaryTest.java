...
public class Mod123InsertQuarterlyComplementaryTest extends AbstractOccamTest {

	@Test
	public void mod123InsertQuarterlyComplementaryTest() {
		ctx.getDslContext().transaction( config -> {
			...
			for (Period period : Period.values()) {
				if (period.isQuarterPeriod()) {
					...
					mod123InsertQuarterlyComplementary(AonRandom.getRangeDate(start,end));
				}
			}
		});
	}
	
	public void mod123InsertQuarterlyComplementary(Date date) {
		System.out.println( "\t ---------------------");
	...
	private Mod123 insertModel( Administration admon, Date date) {
		...
		Mod123TestSuite.printModel(actual);
		return actual;
	}
}
