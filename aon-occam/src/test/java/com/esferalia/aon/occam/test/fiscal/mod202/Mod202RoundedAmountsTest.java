...
public class Mod202RoundedAmountsTest extends AbstractOccamTest {
	
	@Test
	public void testRound() {
		MODEL202.getMod202s(getOccam())
			.stream()
			.map( model -> MODEL202.get(getOccam(), model.getId()))
			.flatMap(mod202 -> mod202.getMap().values().stream())
			.forEach( det -> {
