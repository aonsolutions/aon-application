...
public class OfficeTest {
	...
	public void testCreate() {
		...
		TagRecord tag1 = ctx.getDslContext().insertInto(TAG)
				...
				.set(TAG.NAME, "Et. 1")
				.set(TAG.TYPE, TagType.NOTICE.value())
				.returning().fetchOne();
		...
		.set(NOTICE.NOTICE_, newNotice.getValue(NOTICE.ID))
		.execute();	
		...
		.set(NOTICE.DATE,
				new java.sql.Timestamp(newDate().getTime()))
		.set(NOTICE.SUBJECT, title2)
		.set(NOTICE.PHONE, phone2)
		.set(NOTICE.STATUS, NoticeStatus.OPEN.value())
		.set(NOTICE.TYPE, NoticeType.TICKET.value())
		.set(NOTICE.PRIORITY, Priority.NORMAL.value())
		.returning().fetchOne();
		...
		.set(NOTICE.NOTICE_, notice2.getValue(NOTICE.ID))
		.execute();
		...
		.set(NOTICE.DATE,
				new java.sql.Timestamp(newDate().getTime()))
		.set(NOTICE.SUBJECT, title3)
		.set(NOTICE.PHONE, phone3)
		.set(NOTICE.STATUS, NoticeStatus.OPEN.value())
		.set(NOTICE.TYPE, NoticeType.TICKET.value())
		.set(NOTICE.PRIORITY, Priority.NORMAL.value())
		.returning().fetchOne();
		...
		.set(NOTICE.NOTICE_, notice3.getValue(NOTICE.ID))
		.execute();
		...
		System.out.println(" *************  ========== Notices creados correctamente " );
	...
	public void testOpenNotices() {		
		...
		ctx.getDslContext().selectFrom(NOTICE)
				.where(NOTICE.DOMAIN.eq(domainId)
						.and(NOTICE.TYPE.eq(NoticeStatus.OPEN.value()).or(
								NOTICE.TYPE.eq(NoticeStatus.REOPEN.value())))
	...
	public static void testRemove() {
		...
		for (TagRecord tag : tagList) {
			...
			.where(TAG.ID.eq(tag.getValue(TAG.ID)))
			.execute();
			
			System.out.println(tag.getValue(TAG.NAME) + " BORRADA CORRECTAMENTE .... ");
		}
