...
public class OfficeTest {
	...
	public void testCreate() {
		...
		Integer user_id = 1651;
		// CREANDO ETIQUETAS....

		System.out.println("Creando etiquetas ....");
		TagRecord tag1 = ctx.getDslContext().insertInto(TAG)
				.set(TAG.DOMAIN, DOMAIN_ID).set(TAG.NAME, "Et. 1")
				.set(TAG.TYPE, TagType.NOTICE.value()).returning().fetchOne();
		System.out.println("Etiqueta 1 creada correctamente .... ");

		TagRecord tag2 = ctx.getDslContext().insertInto(TAG)
				.set(TAG.DOMAIN, DOMAIN_ID).set(TAG.NAME, "Et. 2")
				.set(TAG.TYPE, TagType.NOTICE.value()).returning().fetchOne();
		System.out.println("Etiqueta 2 creada correctamente .... ");

		TagRecord tag3 = ctx.getDslContext().insertInto(TAG)
				.set(TAG.DOMAIN, DOMAIN_ID).set(TAG.NAME, "Et. 3")
				.set(TAG.TYPE, TagType.NOTICE.value()).returning().fetchOne();
		System.out.println("Etiqueta 3 creada correctamente .... ");

		System.out
				.println("OK!  ======== >> Etiquetas insertadas correctamente");
		...
		String title = "Titulo: Abriendo Notice 1 desde OfficeTest";
		String body = "Cuerpo del mensaje para el notice 1 abierto dede OfficeTest";
		String phone = "555-000.000";
		...
		NoticeRecord newNotice = ctx.getDslContext().insertInto(NOTICE)
				...
				.set(NOTICE.SUBJECT, title).set(NOTICE.PHONE, phone)
				.set(NOTICE.STATUS, NoticeStatus.OPEN.value())
				.set(NOTICE.TYPE, NoticeType.TICKET.value())
				.set(NOTICE.PRIORITY, Priority.NORMAL.value()).returning()
				.fetchOne();

		ctx.getDslContext().insertInto(NOTICE)
				.set(NOTICE.DOMAIN, newNotice.getValue(NOTICE.DOMAIN))
				.set(NOTICE.SENDER, newNotice.getValue(NOTICE.SENDER))
				.set(NOTICE.DATE, newNotice.getValue(NOTICE.DATE))
				.set(NOTICE.SUBJECT, body)
				.set(NOTICE.PHONE, newNotice.getValue(NOTICE.PHONE))
				.set(NOTICE.STATUS, newNotice.getValue(NOTICE.STATUS))
				.set(NOTICE.TYPE, newNotice.getValue(NOTICE.TYPE))
				.set(NOTICE.PRIORITY, newNotice.getValue(NOTICE.PRIORITY))
				.set(NOTICE.NOTICE_, newNotice.getValue(NOTICE.ID)).execute();

		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, newNotice.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag1.getValue(TAG.ID))
				.set(NOTICE_TAG.START_DATE, newNotice.getValue(NOTICE.DATE))
				.execute();
		...
		String title2 = "Titulo del mensaje 2 para evaluar";
		String body2 = "En un lugar de la mancha de cuyo nombre no quiero acordarme, "
				+ "residia un hombre cuya prepotencia y carisma nada altruista "
				+ "hacia que fuera solitario y amante de la bebida.";
		String phone2 = "555-000.000";
		...
		ctx.getDslContext().insertInto(NOTICE)
				.set(NOTICE.DOMAIN, notice2.getValue(NOTICE.DOMAIN))
				.set(NOTICE.SENDER, notice2.getValue(NOTICE.SENDER))
				.set(NOTICE.DATE, notice2.getValue(NOTICE.DATE))
				.set(NOTICE.SUBJECT, body2)
				.set(NOTICE.PHONE, notice2.getValue(NOTICE.PHONE))
				.set(NOTICE.STATUS, notice2.getValue(NOTICE.STATUS))
				.set(NOTICE.TYPE, notice2.getValue(NOTICE.TYPE))
				.set(NOTICE.PRIORITY, notice2.getValue(NOTICE.PRIORITY))
				.set(NOTICE.NOTICE_, notice2.getValue(NOTICE.ID)).execute();

		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, notice2.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag1.getValue(TAG.ID))
				.set(NOTICE_TAG.START_DATE, notice2.getValue(NOTICE.DATE))
				.execute();
		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, notice2.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag2.getValue(TAG.ID))
				.set(NOTICE_TAG.START_DATE, notice2.getValue(NOTICE.DATE))
				.execute();
		...
		String title3 = "Titulo del mensaje 3 kjasljipqjwekñajsp oaskpqasjñajs iasñjkañs";
		String body3 = "The fact that you are seeing this page indicates that the website "
		...
		ctx.getDslContext().insertInto(NOTICE)
				.set(NOTICE.DOMAIN, notice3.getValue(NOTICE.DOMAIN))
				.set(NOTICE.SENDER, notice3.getValue(NOTICE.SENDER))
				.set(NOTICE.DATE, notice3.getValue(NOTICE.DATE))
				.set(NOTICE.SUBJECT, body3)
				.set(NOTICE.PHONE, notice3.getValue(NOTICE.PHONE))
				.set(NOTICE.STATUS, notice3.getValue(NOTICE.STATUS))
				.set(NOTICE.TYPE, notice2.getValue(NOTICE.TYPE))
				.set(NOTICE.PRIORITY, notice3.getValue(NOTICE.PRIORITY))
				.set(NOTICE.NOTICE_, notice3.getValue(NOTICE.ID)).execute();

		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, notice3.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag1.getValue(TAG.ID))
				.set(NOTICE_TAG.START_DATE, notice3.getValue(NOTICE.DATE))
				.execute();
		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, notice3.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag2.getValue(TAG.ID))
		...
		ctx.getDslContext().insertInto(NOTICE_TAG)
				.set(NOTICE_TAG.NOTICE, notice3.getValue(NOTICE.ID))
				.set(NOTICE_TAG.TAG, tag3.getValue(TAG.ID))
				.set(NOTICE_TAG.START_DATE, notice3.getValue(NOTICE.DATE))
				.execute();
	...
	public void testOpenNotices() {
		...
		ctx.getDslContext().selectFrom(NOTICE)
				.where(NOTICE.DOMAIN.eq(DOMAIN_ID)
				...
				.forEach(record -> {
					...
					int domain = record.getValue(NOTICE.DOMAIN);
		...
		for (Notice notice : notices) {
			...
			System.out.println("Subject: " + notice.getTitle());
			System.out.println("Body: " + notice.getBody());
			System.out.println("PriorityName: " + notice.getPriority());
			System.out.println("UserId: " + notice.getSender().getId());
			System.out.println("userLogin: " + notice.getSender().getName());
	...
	private void setBody2Notice(AONContext ctx, Notice notice) {
		...
		if (record == null)
			notice.setBody(notice.getTitle());
		else {
	...
	public static void testRemove() {
		...
		for (NoticeRecord record : noticeList) {
			...
			ctx.getDslContext().delete(NOTICE_TAG)
					.where(NOTICE_TAG.NOTICE.eq(record.getValue(NOTICE.ID)))
					.execute();

			ctx.getDslContext().delete(NOTICE)
					.where(NOTICE.NOTICE_.eq(record.getValue(NOTICE.ID)))
					.execute();
		...
		for (TagRecord tag : tagList) {
			...
			System.out.println(
					tag.getValue(TAG.NAME) + " BORRADA CORRECTAMENTE .... ");
		}
