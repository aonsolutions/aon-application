...
public class BOEBalancePYMESScript extends BalanceScript {
	...
	private static final long serialVersionUID = -2073081836397905630L;
	...
	private static enum AccBOEBalancePYMESKey implements IBalanceKey{
		...
		,AC(0,HEADER0,"B)","ACTIVO CORRIENTE",null,"ACI+ACII+ACIII+ACIV+ACV+ACVI")
			...
			,ACII(1,HEADER1,"II","Deudores comerciales y otras cuentas a cobrar",null,"ACII1+ACII2+ACII3")
				,ACII1(2,LEAF,"1","Clientes por ventas y prestaciones de servicios","sdb({430,431,432,433,434,435,436})-sab({437,490,493})",null)
			...
			,ACIII(1,LEAF,"III","Inversiones en empresas del grupo y asociadas a corto plazo"
				,"sdb({5303,5304,5313,5314, 5323, 5324,5333, 5334, 5343,5344, 5353, 5354})"
				+"+sdbPositivo({5523})"
				+"-sab({5393,5394,593,5943,5944,5953,5954})"
				+"+sdbPositivo({5524})"
				,null)
			,ACIV (1,LEAF,"IV","Inversiones financieras a corto plazo"
				,"sdb({5305,540,5325,5345,542,543,547,5315,5335,541,546,5590,5593,5355,545,548,5525,565,566})"
				+"-sab({5955,598,5945,597,5395,549})"
				+"+sdbPositivo({551}) "
				,null)
	...
	//PASIVO Y PATRIMONIO
		...
		,PC(0,HEADER0,"C)","PASIVO CORRIENTE",null,"PCI+PCII+PCIII+PCIV+PCV+PCVI")
			...
			,PCII(1,HEADER1,"III","Deudas a corto plazo",null,"PCII1+PCII2+PCII3")
				...
				,PCII3(2,LEAF,"3","Otras deudas a corto plazo"
					,"sab({194,500,501,505,506,509,5115,5135,5145,521,522,523,525,526,528,525,555,5565,5566,5595,5598,560,561,569})"
					+"-sdb({1034,1044,190,192})"
					+"+sabPositivo({551})"
					,null)
			,PCIII(1,LEAF,"IV","Deudas con empresas del grupo y asociadas a corto plazo"
					,"sab({5103,5104,5113,5114,5123,5124,5133,5134,5143,5144,5563,5564})"
					+"+sabPositivo({5523})"
					+"+sabPositivo({5524})"
					,null)
