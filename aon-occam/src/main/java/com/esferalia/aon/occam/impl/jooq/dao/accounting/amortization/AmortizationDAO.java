...
public class AmortizationDAO {
	...
	private static final com.esferalia.aon.jooq.tables.AmortizationDetail AMORTIZATION_DETAIL_SUM = AMORTIZATION_DETAIL
			.as("ad2");
	private static final Field<BigDecimal> ACCUMULATED_SUM_FIELD = DSL.coalesce(
			DSL.select(DSL.sum(AMORTIZATION_DETAIL_SUM.ALLOCATION)).from(AMORTIZATION_DETAIL_SUM)
					.where(AMORTIZATION_DETAIL_SUM.AMORTIZATION.eq(AMORTIZATION_DETAIL.AMORTIZATION))
					.and(AMORTIZATION_DETAIL_SUM.FROM_DATE.le(AMORTIZATION_DETAIL.FROM_DATE)).asField(),
			BigDecimal.ZERO);
	...
	private static SelectConditionStep<Record> select(AONContext ctx, Integer domain) {
		return ctx.getDslContext().select().from(AMORTIZATION).leftOuterJoin(INVEST_ASSET)
				.on(AMORTIZATION.INVEST_ASSET.eq(INVEST_ASSET.ID)).leftOuterJoin(FIXED_ASSET_ACCOUNT)
				.on(AMORTIZATION.FIXED_ASSET_ACCOUNT.eq(FIXED_ASSET_ACCOUNT.ID)).leftOuterJoin(ACCUMULATED_ACCOUNT)
				.on(AMORTIZATION.ACCUMULATED_ACCOUNT.eq(ACCUMULATED_ACCOUNT.ID)).leftOuterJoin(ALLOCATION_ACCOUNT)
				.on(AMORTIZATION.ALLOCATION_ACCOUNT.eq(ALLOCATION_ACCOUNT.ID)).where(AMORTIZATION.DOMAIN.eq(domain));
	}
	...
	private static SelectConditionStep<Record> selectFull(AONContext ctx, Integer domain) {
		return ctx.getDslContext().select().from(AMORTIZATION).leftOuterJoin(INVEST_ASSET)
				...
				.on(AMORTIZATION.FIXED_ASSET_ACCOUNT.eq(FIXED_ASSET_ACCOUNT.ID)).leftOuterJoin(ACCUMULATED_ACCOUNT)
				.on(AMORTIZATION.ACCUMULATED_ACCOUNT.eq(ACCUMULATED_ACCOUNT.ID)).leftOuterJoin(ALLOCATION_ACCOUNT)
				.on(AMORTIZATION.ALLOCATION_ACCOUNT.eq(ALLOCATION_ACCOUNT.ID)).leftOuterJoin(AMORTIZATION_DETAIL)
				.on(AMORTIZATION_DETAIL.AMORTIZATION.eq(AMORTIZATION.ID)).where(AMORTIZATION.DOMAIN.eq(domain))

		;
	}

	public static Optional<Amortization> get(AONContext ctx, Integer domain, Integer id) {
		return selectFull(ctx, domain).and(AMORTIZATION.ID.eq(id))
				.orderBy(AMORTIZATION.DESCRIPTION, AMORTIZATION_DETAIL.FROM_DATE).collect(toAmortizationMap()).values()
				.stream().findFirst().map(am -> checkLinkedInvoices(ctx, domain, am)).map(am -> calculateDetails(am));
	}

	public static Stream<Amortization> stream(AONContext ctx, AmortizationParams params) {
		return select(ctx, getDomain(params)).and(getCondition(params)).orderBy(getOrderBy(params, false))
				.limit(params.getOffset(), params.getLimit()).fetch().stream().map(r -> AmortizationFiller.build(r));
	}

	public static Stream<AmortizationDetailFlat> detailStream(AONContext ctx, AmortizationParams params) {
		return ctx.getDslContext().select().from(AMORTIZATION_DETAIL).innerJoin(AMORTIZATION)
				.on(AMORTIZATION_DETAIL.AMORTIZATION.eq(AMORTIZATION.ID)).innerJoin(FIXED_ASSET_ACCOUNT)
				.on(AMORTIZATION.FIXED_ASSET_ACCOUNT.eq(FIXED_ASSET_ACCOUNT.ID)).innerJoin(ACCUMULATED_ACCOUNT)
				.on(AMORTIZATION.ACCUMULATED_ACCOUNT.eq(ACCUMULATED_ACCOUNT.ID)).innerJoin(ALLOCATION_ACCOUNT)
				.on(AMORTIZATION.ALLOCATION_ACCOUNT.eq(ALLOCATION_ACCOUNT.ID)).leftOuterJoin(INVEST_ASSET)
				.on(AMORTIZATION.INVEST_ASSET.eq(INVEST_ASSET.ID)).and(getCondition(params))
				.orderBy(getOrderBy(params, true)).limit(params.getOffset(), params.getLimit()).fetch().stream()
				.map(r -> new AmortizationDetailFlat().setAmortization(AmortizationFiller.build(r))
						.setDetail(AmortizationDetailFiller.build(r)));
	}

	private static Integer getDomain(AmortizationParams params) {
		return params.getDomain().orElseThrow(() -> new AonCoreException(AonError.EMPTY_DOMAIN.getMessage()));
	}
	...
	private static Condition getCondition(AmortizationParams params) {
		...
		c = params.getDeadlineFilled().filter(f -> params.getFromDeadline().isEmpty())
				.filter(f -> params.getToDeadline().isEmpty())
				.map(f -> f ? AMORTIZATION.DEADLINE.isNotNull() : AMORTIZATION.DEADLINE.isNull()).map(c::and).orElse(c);
		return c.and(when(params.getFromInitialDate(), d -> AMORTIZATION.INITIAL_DATE.ge(AonDateUtils.toSql(d))))
	...
	private static Collector<Record, Map<Integer, Amortization>, Map<Integer, Amortization>> toAmortizationMap() {
		...
		return Collector.of(LinkedHashMap::new, (map, r) -> {
			Amortization a = map.computeIfAbsent(r.getValue(AMORTIZATION.ID), id -> AmortizationFiller.build(r));
	...
	public static Amortization save(AONContext ctx, Amortization a) {
		if (a.isDirty()) {
			...
			Amortization amo = (a.getId() == null) ? insert(ctx, a) : update(ctx, a);
	...
	private static Amortization insert(AONContext ctx, Amortization a) {
		Integer id = ctx.getDslContext().insertInto(AMORTIZATION).set(AMORTIZATION.DOMAIN, a.getDomain())
				.set(AMORTIZATION.DESCRIPTION, a.getDescription())
				.set(AMORTIZATION.INITIAL_DATE, AonDateUtils.toSql(a.getInitialDate()))
				.set(AMORTIZATION.DEADLINE, AonDateUtils.toSql(a.getDeadline())).set(AMORTIZATION.AMOUNT, a.getAmount())
				.set(AMORTIZATION.FEE_PERIOD, a.getFeePeriod() != null ? (byte) a.getFeePeriod().ordinal() : null)
				.set(AMORTIZATION.SALE_AMOUNT, a.getSaleAmount()).set(AMORTIZATION.COMMENTS, a.getComments())
				.set(AMORTIZATION.INVEST_ASSET, a.getInvestAsset() != null ? a.getInvestAsset().getId() : null)
				.set(AMORTIZATION.FIXED_ASSET_ACCOUNT,
						a.getFixedAssetAccount() != null ? a.getFixedAssetAccount().getId() : null)
				.set(AMORTIZATION.ACCUMULATED_ACCOUNT,
						a.getAccumulatedAccount() != null ? a.getAccumulatedAccount().getId() : null)
				.set(AMORTIZATION.ALLOCATION_ACCOUNT,
						a.getAllocationAccount() != null ? a.getAllocationAccount().getId() : null)
				.set(AMORTIZATION.PERCENTAGE, a.getPercentage())
				.set(AMORTIZATION.SECURITY_LEVEL,
						a.getSecurityLevel() != null ? (byte) a.getSecurityLevel().ordinal() : null)
				.returning(AMORTIZATION.ID).fetchOne().getValue(AMORTIZATION.ID);
		a.setId(id);
		AmortizationManager am = new AmortizationManager();
		am.generateDetailsAndFill(ctx, a);
		saveDetails(ctx, a);
	...
	private static Amortization update(AONContext ctx, Amortization a) {
		ctx.getDslContext().update(AMORTIZATION).set(AMORTIZATION.DESCRIPTION, a.getDescription())
				.set(AMORTIZATION.INITIAL_DATE, AonDateUtils.toSql(a.getInitialDate()))
				.set(AMORTIZATION.DEADLINE, AonDateUtils.toSql(a.getDeadline())).set(AMORTIZATION.AMOUNT, a.getAmount())
				.set(AMORTIZATION.FEE_PERIOD, a.getFeePeriod() != null ? (byte) a.getFeePeriod().ordinal() : null)
				.set(AMORTIZATION.SALE_AMOUNT, a.getSaleAmount()).set(AMORTIZATION.COMMENTS, a.getComments())
				...
				.set(AMORTIZATION.SECURITY_LEVEL,
						a.getSecurityLevel() != null ? (byte) a.getSecurityLevel().ordinal() : null)
				.where(AMORTIZATION.ID.eq(a.getId())).execute();
		return a;
	}
	...
	public static void delete(AONContext ctx, Amortization a) {
		...
		ctx.getDslContext().deleteFrom(AMORTIZATION_DETAIL).where(AMORTIZATION_DETAIL.AMORTIZATION.eq(a.getId()))
				.execute();
		ctx.getDslContext().deleteFrom(AMORTIZATION).where(AMORTIZATION.ID.eq(a.getId())).execute();
	}
	...
	static class AmortizationFiller extends Filler implements Function<Record, Amortization> {
		...
		public static Amortization build(Record r) {
			return new Amortization().setId(getValue(r, AMORTIZATION.ID)).setDomain(getValue(r, AMORTIZATION.DOMAIN))
					.setDescription(getValue(r, AMORTIZATION.DESCRIPTION))
					.setInitialDate(getValue(r, AMORTIZATION.INITIAL_DATE))
					.setDeadline(getValue(r, AMORTIZATION.DEADLINE)).setAmount(getValue(r, AMORTIZATION.AMOUNT))
					...
					.setComments(getValue(r, AMORTIZATION.COMMENTS)).setInvestAsset(InvestAssetFiller.build(r))
	...
	public static Amortization saveFiscalAllocation(AONContext ctx, AmortizationDetail detail) {
		...
		// Se calcula la dotación fiscal para cada periodo, la última se calcula por
		// diferencia.
		...
		am.detailStream()
				...
				.forEach(d -> {
					...
					ctx.getDslContext().update(AMORTIZATION_DETAIL)
							.set(AMORTIZATION_DETAIL.FISCAL_ALLOCATION, d.getFiscalAllocation())
							.where(AMORTIZATION_DETAIL.ID.eq(d.getId())).execute();
				});
	...
	private static Query getDeleteDetailStmt(AONContext ctx, AmortizationDetail detail) {
		return ctx.getDslContext().deleteFrom(AMORTIZATION_DETAIL).where(AMORTIZATION_DETAIL.ID.eq(detail.getId()));
	}

	private static InsertSetMoreStep<AmortizationDetailRecord> getInsertDetailStmt(AONContext ctx,
			AmortizationDetail d) {
		return ctx.getDslContext().insertInto(AMORTIZATION_DETAIL).set(AMORTIZATION_DETAIL.DOMAIN, d.getDomain())
	...
	static class AmortizationDetailFiller extends Filler implements Function<Record, AmortizationDetail> {
		...
		public static AmortizationDetail build(Record r) {
			return new AmortizationDetail().setId(getValue(r, AMORTIZATION_DETAIL.ID))
					...
					.setStatus(
							AmortizationDetailStatus.safeValueOf(getValue(r, AMORTIZATION_DETAIL.STATUS)).orElse(null))
	...
	private static class AmortizationAutoComplete {
		...
		private static final Consumer<AmortizationContext> COMPLETE_PERCENTAGE = ac -> {
			if (ac.a.getAmortizationType() == null)
				return;
			if (AonMathUtils.isZero(ac.a.getPercentage()))
				ac.a.setPercentage(ac.a.getAmortizationType().getPercentage());
		};
		...
		private static final Consumer<AmortizationContext> COMPLETE_FIXED_ASSET_ACCOUNT = ac -> {
			...
			if (ac.a.getFixedAssetAccount() == null || ac.a.getFixedAssetAccount().getId() != null) {
				...
				Account account = new Account().setDomain(ac.a.getDomain()).setCode(code)
						.setDescription(ac.a.getDescription()).setActive(true);
		...
		private static final Consumer<AmortizationContext> COMPLETE_SECURITY_LEVEL = ac -> {
			if (ac.a.getSecurityLevel() == null)
				ac.a.setSecurityLevel(SecurityLevel.OFFICIAL);
		};

		static void complete(AONContext ctx, Amortization a) throws AonCoreException {
			COMPLETE_PERCENTAGE.andThen(COMPLETE_FIXED_ASSET_ACCOUNT).andThen(COMPLETE_ACCUMULATED_ACCOUNT)
					.andThen(COMPLETE_ALLOCATION_ACCOUNT).andThen(COMPLETE_SECURITY_LEVEL)
	...
	private static class AmortizationValidation {
		...
		private static final Consumer<AmortizationContext> OVERFLOW_DESCRIPTION = ivc -> {
			if (AonStringUtils.length(ivc.a.getDescription()) > AMORTIZATION.DESCRIPTION.getDataType().length())
				throw new AonCoreException(
						AonError.INVALID_LENGTH.format("DESCRIPTION", AMORTIZATION.DESCRIPTION.getDataType().length()));
		};
		...
		private static final Consumer<AmortizationContext> INVALID_PERCENTAGE = ivc -> {
			if (AonMathUtils.isLessThanOrEqual(ivc.a.getPercentage(), 0)
					|| AonMathUtils.isGreaterThan(ivc.a.getPercentage(), 100))
		...
		static void validate(AONContext ctx, Amortization a) throws AonCoreException {
			EMPTY_DOMAIN.andThen(EMPTY_DESCRIPTION).andThen(EMPTY_INITIAL_DATE).andThen(EMPTY_AMOUNT)
					.andThen(OVERFLOW_DESCRIPTION).andThen(EMPTY_AMORTIZATION_TYPE).andThen(INVALID_PERCENTAGE)
					.andThen(EMPTY_PERIOD).andThen(EMPTY_FIXED_ASSET_ACCOUNT).andThen(EMPTY_ACCUMULATED_ACCOUNT)
					.andThen(EMPTY_ALLOCATION_ACCOUNT).accept(new AmortizationContext(ctx, a));
		}
		...
		private static final Consumer<AmortizationContext> SCORED_DETAILS = ivc -> ivc.ctx.getDslContext()
				.select(AMORTIZATION_DETAIL.ID).from(AMORTIZATION_DETAIL)
				.where(AMORTIZATION_DETAIL.AMORTIZATION.eq(ivc.a.getId()))
				.and(AMORTIZATION_DETAIL.STATUS.eq(AmortizationDetailStatus.SCORED.value())).fetch().stream().findAny()
				.ifPresent(r -> {
					throw new AonCoreException(AonError.AMORTIZATION_DELETE_SCORED_DETAILS.getMessage());
				});

		private static final Consumer<AmortizationContext> LINKED_INVOICES = ivc -> ivc.ctx.getDslContext()
				.select(AMORTIZATION_INVOICE.ID).from(AMORTIZATION_INVOICE)
				.where(AMORTIZATION_INVOICE.AMORTIZATION.eq(ivc.a.getId())).fetch().stream().findAny().ifPresent(r -> {
		...
		public static void validateDeletion(AONContext ctx, Amortization a) {
			EMPTY_ID.andThen(SCORED_DETAILS).andThen(LINKED_INVOICES).accept(new AmortizationContext(ctx, a));
		}

		private static final Consumer<AmortizationContext> WRONG_DEADLINE = ivc -> {
			if (DateUtils.isSameDay(ivc.a.getInitialDate(), ivc.a.getDeadline())
					|| ivc.a.getInitialDate().after(ivc.a.getDeadline()))
		...
		private static final Consumer<AmortizationContext> NOT_PENDING_ALLOCATIONS = ivc -> {
			...
			ivc.ctx.getDslContext().select(AMORTIZATION_DETAIL.ID).from(AMORTIZATION_DETAIL)
					.where(AMORTIZATION_DETAIL.AMORTIZATION.eq(ivc.a.getId()))
					.and(AMORTIZATION_DETAIL.TO_DATE.gt(AonDateUtils.toSql(cancelDate)))
					.and(AMORTIZATION_DETAIL.STATUS.notEqual(AmortizationDetailStatus.PENDING.value())).fetch().stream()
					.findAny().ifPresent(r -> {
...
//			.andThen(EMPTY_DEADLINE)
					.andThen(WRONG_DEADLINE).andThen(WRONG_SALE_AMOUNT).andThen(NOT_PENDING_ALLOCATIONS)
					.accept(new AmortizationContext(ctx, a));
		}
	}
	...
	public static AmortizationDetail blockDetail(AONContext ctx, AmortizationDetail detail) {
		...
		ctx.getDslContext().update(AMORTIZATION_DETAIL).set(AMORTIZATION_DETAIL.STATUS, detail.getStatus().value())
				.where(AMORTIZATION_DETAIL.ID.eq(detail.getId())).execute();
		return detail;
	}
	...
	// ------------------------------------------------------ [ACCOUNTING] ---
	public static void recordAmortizationDetails(AONContext ctx, Integer domain, Integer[] ids) {
		AonCollectionUtils.stream(ids).map(id -> getAccountingAmortizations(ctx, domain, id)).flatMap(Optional::stream)
				.forEach(aa -> recordAllocation(ctx, aa.getAmortization(), aa.getDetail()));
	}
	...
	public static AmortizationDetail recordAllocation(AONContext ctx, Amortization am, AmortizationDetail detail) {
		...
		AccountEntry entry = new AccountEntry().setDomain(am.getDomain()).setPeriod(period.getId())
				.setEntryDate(accountEntryDate).setSecurityLevel(am.getSecurityLevel())
				.setEntryType(AccountEntryType.AMORTIZATION).setActivity(activity);
		...
		entry.addDetail(new AccountEntryDetail().setLine(0).setAccount(allocation).setConcept(concept)
				.setDebit(detail.getAllocation()).setBalancingAccount(accumulated));

		entry.addDetail(new AccountEntryDetail().setLine(1).setAccount(accumulated).setConcept(concept)
				.setCredit(detail.getAllocation()).setBalancingAccount(allocation));

		Integer entryId = AccountEntryDAO.save(ctx, entry);
		detail.setAccountEntry(entryId).setStatus(AmortizationDetailStatus.SCORED);
		ctx.getDslContext().update(AMORTIZATION_DETAIL).set(AMORTIZATION_DETAIL.ACCOUNT_ENTRY, detail.getAccountEntry())
	...
	public static void unrecordAmortizationDetails(AONContext ctx, Integer domain, Integer[] ids) {
		AonCollectionUtils.stream(ids).map(id -> getAccountingAmortizations(ctx, domain, id)).flatMap(Optional::stream)
				.forEach(aa -> unrecordAllocation(ctx, aa.getDetail()));
	}
	...
	public static AmortizationDetail unrecordAllocation(AONContext ctx, AmortizationDetail detail) {
		...
		return ctx.getDslContext().select().from(AMORTIZATION_DETAIL).where(AMORTIZATION_DETAIL.ID.eq(detail.getId()))
				.fetch().stream().findFirst().map(r -> AmortizationDetailFiller.build(r))
	...
	public static void unrecord(AONContext ctx, Integer entryId) {
		ctx.getDslContext().update(AMORTIZATION_DETAIL).setNull(AMORTIZATION_DETAIL.ACCOUNT_ENTRY)
				.set(AMORTIZATION_DETAIL.STATUS, AmortizationDetailStatus.PENDING.value())
				.where(AMORTIZATION_DETAIL.ACCOUNT_ENTRY.eq(entryId)).execute();
	}
	...
	public static Amortization sale(AONContext ctx, Amortization a) {
		...
		am.detailStream().filter(d -> d.getStatus() == AmortizationDetailStatus.PENDING)
				.filter(d -> d.getToDate().after(cancelDate)).forEach(d -> {
	...
	private static Stream<AmortizationInvoice> getInvoices(AONContext ctx, Integer domain, Amortization am) {
		if (am == null)
			throw new AonCoreException(AonError.EMPTY_DATA.format("Amortization"));
		return ctx.getDslContext().select().from(AMORTIZATION_INVOICE)
				.where(AMORTIZATION_INVOICE.AMORTIZATION.eq(am.getId())).fetch().stream()
				.map(r -> new AmortizationInvoice().setId(r.getValue(AMORTIZATION_INVOICE.ID)).setDomain(domain)
						.setAmortization(am)
	...
	public static LinkedList<AmortizationInvoice> getInvoices(AONContext ctx, Integer domain, Integer amortizationId) {
		if (domain == null)
			throw new AonCoreException(AonError.EMPTY_DOMAIN.getMessage());
		if (amortizationId == null)
			throw new AonCoreException(AonError.EMPTY_ID.getMessage());
		Amortization am = get(ctx, domain, amortizationId)
				.orElseThrow(() -> new AonCoreException(AonError.AMORTIZATION_NOT_FOUND.getMessage()));
		return getInvoices(ctx, domain, am).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	private static AmortizationInvoice fillAccountEntry(AONContext ctx, AmortizationInvoice ami) {
		if (ami != null && ami.getInvoice() != null) {
			ctx.getDslContext().select(ACCOUNT_ENTRY_INVOICE.ACCOUNT_ENTRY).from(ACCOUNT_ENTRY_INVOICE)
					.where(ACCOUNT_ENTRY_INVOICE.INVOICE.eq(ami.getInvoice().getId())).fetch().stream()
					.map(r -> r.getValue(ACCOUNT_ENTRY_INVOICE.ACCOUNT_ENTRY)).findFirst()
	...
	public static void linkInvoices(AONContext ctx, Integer domain, Integer amortizationId, Integer[] invoiceIds) {
		if (invoiceIds == null || invoiceIds.length == 0)
			throw new AonCoreException(AonError.EMPTY_DATA.format("Facturas"));
		AonCollectionUtils.stream(invoiceIds).forEach(id -> linkInvoice(ctx, domain, amortizationId, id));
	}
	...
	private static void linkInvoice(AONContext ctx, Integer domain, Integer amortizationId, Integer invoiceId) {
		if (domain == null)
			throw new AonCoreException(AonError.EMPTY_DOMAIN.getMessage());
		if (amortizationId == null)
			throw new AonCoreException(AonError.EMPTY_DATA.format("Amortizaci\u00F3n"));
		if (invoiceId == null)
			throw new AonCoreException(AonError.EMPTY_DATA.format("Factura"));
		...
		if (invoice.isSales())
			linkOutputInvoice(ctx, amortization, invoice);
		else
			linkInputInvoice(ctx, amortization, invoice);
	}
	...
	private static void linkOutputInvoice(AONContext ctx, Amortization amortization, Invoice invoice) {
		if (amortization.isNotDisposed()) {
			...
			Double saleAmount = AonMathUtils.round(inv.detailStream().filter(d -> !d.isPrepayment())
					.filter(d -> AonMathUtils.isNotZero(d.getTaxableBase())).mapToDouble(d -> d.getTaxableBase())
					.sum());
	...
	public static void unlinkInvoice(AONContext ctx, Integer domain, Integer amortizationId, Integer invoiceId) {
		ctx.getDslContext().deleteFrom(AMORTIZATION_INVOICE).where(AMORTIZATION_INVOICE.DOMAIN.eq(domain))
				.and(AMORTIZATION_INVOICE.AMORTIZATION.eq(amortizationId))
				.and(AMORTIZATION_INVOICE.INVOICE.eq(invoiceId)).execute();
	}

	private static void insertAmortizationInvoice(AONContext ctx, Integer domain, Integer amortizationId,
			Integer invoiceId) {
		ctx.getDslContext().insertInto(AMORTIZATION_INVOICE).set(AMORTIZATION_INVOICE.DOMAIN, domain)
				.set(AMORTIZATION_INVOICE.AMORTIZATION, amortizationId).set(AMORTIZATION_INVOICE.INVOICE, invoiceId)
				.execute();
	}
	...
	private static SortField<?>[] getOrderBy(AmortizationParams params, boolean linesFields) {
		SortField<?>[] fields = params.getOrderBy().orElse(AmortizationParamsOrderBy.DESCRIPTION_ASC)
		...
		if (linesFields) {
			...
			for (int i = 0; i < DETAIL_FIELDS.length; i++)
				ret[fields.length + i] = DETAIL_FIELDS[i];
			return ret;
		}
		return fields;
	}
	...
	public static Amortization checkLinkedInvoices(AONContext ctx, Integer domain, Amortization am) {
		double totalBases = ctx.getDslContext().select(INVOICE_DETAIL.TAXABLE_BASE).from(AMORTIZATION_INVOICE)
				.join(INVOICE).on(INVOICE.ID.eq(AMORTIZATION_INVOICE.INVOICE)).join(INVOICE_DETAIL)
				.on(INVOICE_DETAIL.INVOICE.eq(INVOICE.ID)).where(AMORTIZATION_INVOICE.AMORTIZATION.eq(am.getId()))
				.and(AMORTIZATION_INVOICE.DOMAIN.eq(domain)).and(INVOICE.TYPE.ne(InvoiceType.SALES.value()))
				.and(INVOICE_DETAIL.PREPAYMENT.eq((byte) 0)).fetch().stream()
				.mapToDouble(r -> r.getValue(INVOICE_DETAIL.TAXABLE_BASE)).sum();
		if (totalBases >= INVESTMENT_LIMIT) {
			ctx.getDslContext().selectOne().from(AMORTIZATION_INVOICE).join(INVOICE)
					.on(AMORTIZATION_INVOICE.INVOICE.eq(INVOICE.ID))
					.where(AMORTIZATION_INVOICE.AMORTIZATION.eq(am.getId())).and(AMORTIZATION_INVOICE.DOMAIN.eq(domain))
					.and(INVOICE.TYPE.ne(InvoiceType.SALES.value())).and(INVOICE.INVESTMENT.eq((byte) 0)).fetch()
					.stream().findAny()
					.ifPresent(x -> am.addMessage("La base imponible de las facturas recibidas vinculadas supera "
							+ INVESTMENT_LIMIT + " \u20AC: la ficha tiene la consideraci\u00F3n de bien de"
			...
			ctx.getDslContext().selectOne().from(AMORTIZATION_INVOICE).join(INVOICE)
					.on(AMORTIZATION_INVOICE.INVOICE.eq(INVOICE.ID))
					.where(AMORTIZATION_INVOICE.AMORTIZATION.eq(am.getId())).and(AMORTIZATION_INVOICE.DOMAIN.eq(domain))
					.and(INVOICE.TYPE.ne(InvoiceType.SALES.value())).and(INVOICE.INVESTMENT.eq((byte) 1)).fetch()
					.stream().findAny()
					.ifPresent(ami -> am.addMessage("La base imponible de las facturas recibidas vinculadas no supera "
							+ INVESTMENT_LIMIT + " \u20AC: la ficha no tiene la consideraci\u00F3n de bien de"
	...
	public static Optional<AccountingAmortization> getAccountingAmortizations(AONContext ctx, Integer domain,
			Integer detailId) {
		return getAccountingAmortizations(ctx, domain, new AmortizationParams().setDomain(domain).setDetailId(detailId))
				.findFirst();
	}

	public static Stream<AccountingAmortization> getAccountingAmortizations(AONContext ctx, Integer domain,
			AmortizationParams params) {
		if (domain == null)
			throw new AonCoreException(AonError.EMPTY_DOMAIN.getMessage());
		return ctx.getDslContext()
				.select(AMORTIZATION.asterisk(), AMORTIZATION_DETAIL.asterisk(), FIXED_ASSET_ACCOUNT.asterisk(),
						ACCUMULATED_ACCOUNT.asterisk(), ALLOCATION_ACCOUNT.asterisk(), INVEST_ASSET.asterisk(),
						ACCUMULATED_SUM_FIELD)
				.from(AMORTIZATION_DETAIL).innerJoin(AMORTIZATION)
				.on(AMORTIZATION.ID.eq(AMORTIZATION_DETAIL.AMORTIZATION)).innerJoin(FIXED_ASSET_ACCOUNT)
				.on(AMORTIZATION.FIXED_ASSET_ACCOUNT.eq(FIXED_ASSET_ACCOUNT.ID)).innerJoin(ACCUMULATED_ACCOUNT)
				.on(AMORTIZATION.ACCUMULATED_ACCOUNT.eq(ACCUMULATED_ACCOUNT.ID)).innerJoin(ALLOCATION_ACCOUNT)
				.on(AMORTIZATION.ALLOCATION_ACCOUNT.eq(ALLOCATION_ACCOUNT.ID)).leftOuterJoin(INVEST_ASSET)
				.on(AMORTIZATION.INVEST_ASSET.eq(INVEST_ASSET.ID)).where(getAccountingCondition(params))
				.orderBy(getOrderBy(params, true)).limit(params.getOffset(), params.getLimit()).fetch().stream()
				.map(r -> {
	...
	private static Condition getAccountingCondition(AmortizationParams params) {
		...
		c = params.getDeadlineFilled().filter(f -> params.getFromDeadline().isEmpty())
				.filter(f -> params.getToDeadline().isEmpty())
				.map(f -> f ? AMORTIZATION.DEADLINE.isNotNull() : AMORTIZATION.DEADLINE.isNull()).map(c::and).orElse(c);
		return c.and(when(params.getId(), AMORTIZATION.ID::eq)).and(when(params.getFromId(), AMORTIZATION.ID::ge))
				.and(when(params.getToId(), AMORTIZATION.ID::le))
				.and(when(params.getDetailId(), AMORTIZATION_DETAIL.ID::eq))
