...
public class BOEPyGPYMESScript extends BalanceScript {
	
	private static final long serialVersionUID = -9102763252110798473L;
	
	private IAccMiningKeyAccept accepter;
	
	private static enum AccBOEPyGPYMESKey implements IBalanceKey{
		...
		private AccBOEPyGPYMESKey(int level
		...
		if (accepter == null) {
			accepter = new IAccMiningKeyAccept() {
					...
					public boolean acceptKey(Object key) {
						try {
							return (AccBOEPyGPYMESKey.valueOf((String) key) != null);
	...
	@Override
	public List<AccBOEPyGPYMESKey> getKeyList() {
		return new LinkedList<AccBOEPyGPYMESKey>( Arrays.asList(AccBOEPyGPYMESKey.values()));
	}
