...
public class InvoiceTextPrinter {
	...
	public static void print(PrintStream out, Invoice invoice, boolean abbrv) {
		new InvoiceTextPrinter(abbrv).header(invoice, out).details(invoice, out).vatBreakdown(invoice, out)
				.withholding(invoice, out).totals(invoice, out).fiscal(invoice, out);
		out.flush();
	}
