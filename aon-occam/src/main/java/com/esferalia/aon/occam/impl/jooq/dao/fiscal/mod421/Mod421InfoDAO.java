...
public class Mod421InfoDAO extends FiscalModelDAO {
	private static final DecimalFormat DEC2 = new DecimalFormat("#,##0.00");		
	
	private Mod421InfoDAO() {
	}
	
	public static String getInfo(AONContext ctx, Mod421 mod421, IModelScript<Mod421Key> script, FiscalModelKeyInfo infoKey) {
		...
		return  Stream.of(script)
			...
			.map(keyDAO -> 	infoKey.visit( new IFiscalModelKeyInfoVisitor<String>() {
					...
					public String visitCompute() {
						return Objects.requireNonNullElse(getExpression(mod421, script),new JSONArray()).toString();
					}
					
					@Override 
					public String visitComputeKey() {
						return getComputeKey(ctx, mod421, script, keyDAO);
					}
					
					@Override 
					public String visitDiffInvoice() {
						return getDiffInvoicesInfo(ctx, mod421, script, keyDAO).toString();
					}
					...
					public String visitModelOutVatAccrualInvoice() {
						return Objects.requireNonNullElse(
								getModelOutVatAccrualInvoicesInfo(ctx, mod421,keyDAO)
									.map( VatContextJSON::toJSON)
									.collect(JSONArray::new,JSONArray::put,JSONArray::put)
									,new JSONArray()).toString();
					}
	...
	public static Stream<VatContext> getModelInvoicesInfo(AONContext ctx, final Mod421 mod421, Mod421Key key) {
		Mod421Declaration dec = Mod421Declaration.getInstance(mod421);
		return getModelInvoicesInfo(ctx, mod421, dec.getKey(key)); 
	}
	
	private static Stream<VatContext> getModelInvoicesInfo(AONContext ctx, final Mod421 mod421, IMod421KeyDAO keyDAO) {
		if (mod421.isManualDeclaration() || !mod421.hasInvoicesBound()) {
			return VATDAO.getVatBreakdown(ctx, mod421)
					.filter( br ->  keyDAO.acceptValue(mod421, br));
		}
		return VATDAO.getModelVatBreakdown(ctx, mod421)
			.filter( br ->  keyDAO.acceptValue(mod421, br));
	}
	
	private static Stream<VatContext> getModelOutVatAccrualInvoicesInfo(AONContext ctx, final Mod421 mod421, IMod421KeyDAO keyDAO) {
		return VATDAO.getModelCritCajaInvoices(ctx, mod421)
			.filter( VatContext::isSales )	
			.filter( br ->  keyDAO.acceptValue(mod421, br));
	}
		
	private static Stream<VatContext> getModelInVatAccrualInvoicesInfo(AONContext ctx, final Mod421 mod421, IMod421KeyDAO keyDAO) {
		return VATDAO.getModelCritCajaInvoices(ctx, mod421)
	...
	private static class Mod421MVELContext extends com.esferalia.aon.occam.impl.jooq.dao.Mod421MVELContext {
		public Mod421MVELContext(final Mod421 mod421) {
			super(mod421);
			for (String keyValue : mod421.getMap().keySet()) {
				Mod421Key mod421Key = Mod421Key.getKey(keyValue);
				if (mod421Key != null) {
					FiscalModelDetail detail = mod421.getMap().get(keyValue);
					put(mod421Key.toString(), detail==null?0.0:detail.getAmount());
				}
			}
		}
	}

	private static class Mod421MVELExpressionContext extends Mod421MVELContext {
		
		private final LinkedList<Mod421Key> keys = new LinkedList<>();
		private String expression;
		public Mod421MVELExpressionContext(String expression, final Mod421 mod421) {
			super(mod421);
		...
		public Object get(Object keyString) {
			Mod421Key key = Mod421Key.valueOf(keyString.toString());
			keys.add(key);
		...
		public String getFormula() {
			 return MessageFormat.format(expression,keys.stream().map(Mod421Key::getBoxFormatted).toArray());
		}
		public List<Mod421Key> getKeys() {
			return keys;
		}
		...
		public String getResult() {
			 return MessageFormat.format(expression,
					 keys.stream()
					 .map(k -> " ["+ format(getMod421().getAmount(k)) +"] " )
					 .toArray());
		}
		
	}
	
	private static String getExpression(Mod421 mod421, IModelScript<Mod421Key> script) {
		Mod421Declaration dec = Mod421Declaration.getInstance(mod421);
		JSONArray array = new JSONArray();
		for (Mod421Key key : script.getKeys() ) {
			IMod421KeyDAO keyDAO = dec.getKey(key);
			if (keyDAO != null) {
				Mod421MVELExpressionContext mvelCtx = new Mod421MVELExpressionContext(keyDAO.getExpression(),mod421);
				...
				for (Mod421Key k : mvelCtx.getKeys()) {
					keysArray.put(k);
					keysValues.put( DEC2.format(mod421.getAmount(k)));
				}
	...
	private static String getComputeKey(AONContext ctx, Mod421 mod421, IModelScript<Mod421Key> script,IMod421KeyDAO keyDAO) {
		Mod421Declaration dec = Mod421Declaration.getInstance(mod421);
		...
		for (Mod421Key key : script.getKeys() ) {
			if (key != null) {
				if (Arrays.stream(dec.getCompensationExplainKeys()).anyMatch(k -> k == key)) {
					...
					mvelCtx.put("mod", mod421);
					mvelCtx.put("periodModels", Mod421DAO.getSamePeriodEffectiveModels(ctx, mod421).collect(Collectors.toCollection(LinkedList::new)));
					mvelCtx.put("lastPeriodModels", Mod421DAO.getLastPeriodEffectiveModels(ctx, mod421).collect(Collectors.toCollection(LinkedList::new)));
					mvelCtx.put("models", Mod421DAO.getMod421s(ctx, ctx.getDomainId()).collect(Collectors.toCollection(LinkedList::new)));
	...
	private static String getDiffInvoicesInfo(AONContext ctx, Mod421 mod421, IModelScript<Mod421Key> script,IMod421KeyDAO keyDAO) {
		StringBuilder buf = new StringBuilder();
				...
				buf.append("<div style=\"" 
					...
					.append("<tr>")
						...
						.append( MessageFormat.format(styledTag, "td",  bold+textRight+width150+border) )
							.append(DEC2.format(mod421.getAccumulatedAmount(key)))
						.append("</td>")
					.append("</tr>");
					
					double keyTotal = Mod421DAO.getPreviousEffectiveModels(ctx, mod421)
							.map( fm ->  {
								buf.append("<tr>")
					...
					buf.append("<tr>")
						...
						.append( MessageFormat.format(styledTag, "td",  bold+textRight+width150+fontLarger+border) )
							.append(DEC2.format(AonMathUtils.round(mod421.getAccumulatedAmount(key) - keyTotal)))
						.append("</td>")
					.append("</tr>");
