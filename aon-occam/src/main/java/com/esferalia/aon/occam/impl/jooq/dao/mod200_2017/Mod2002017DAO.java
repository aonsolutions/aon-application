...
import static com.esferalia.aon.occam.impl.jooq.dao.mod200_2017.Mod2002017Initialization.INITIALIZE_EXPRESSION_MAP;
...
public class Mod2002017DAO  {
	
	@FunctionalInterface
	private static interface IPopulater {
		boolean populate(Mod2002017 mod,FsModel200RegistryRecord reg);
	}
	...
	private static enum Mod2002017RegistryType {
		...
		,REPRESENTATIVE( 
			(mod,reg) -> mod.getRepresentatives().add(new LegalRepresentative()					
				...
				.setNotaryDate(AonDateUtils.toDate(reg.getNotaryDate()))))
		,UTE_PARTICIPATION( 
		...
		private Mod2002017RegistryType( IPopulater populater){
		...
		private void _populate(Mod2002017 mod,FsModel200RegistryRecord reg) {
			if ( accept(reg) ) {
				this.populater.populate(mod, reg);
			}
		}
		
		public static void populate(Mod2002017 mod,FsModel200RegistryRecord reg) {
			Mod2002017RegistryType type = Mod2002017RegistryType.safeValueOf(reg.getType());
			if (type != null) {
				type._populate(mod, reg);
			}
		}
		
		private static Mod2002017RegistryType safeValueOf( Byte i ) {
			if (i == null) return null;
			return safeValueOf( i.intValue() ); 
		}
		private static Mod2002017RegistryType safeValueOf( Integer i ) {
			if (i == null) return null;
			if (i < 0 || i >= Mod2002017RegistryType.values().length) return null;
			return Mod2002017RegistryType.values()[i];
		}
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				return (Mod2002017Key.valueOf((String) key) != null);	
	...
	public static Mod2002017 save(AONContext ctx, Mod2002017 mod200) {
		try {
			ctx.log().info("------ [START] SAVE MOD 200");
			if (mod200.getPeriodType() == 1) {
				mod200.setPeriodStart(AonDateUtils.getYearFirstDay(2017));
				mod200.setPeriodEnd(AonDateUtils.getYearLastDay(2017));
			}
	...
	private static Mod2002017 insert(AONContext ctx, Mod2002017 mod200)  {
	...
	private static void insertAdministrator(AONContext ctx,Mod2002017 mod200) {
		...
		if (mod200.getAdministrators() != null) {
			for ( Mod200CompanyAdministrator ca : mod200.getAdministrators() ) {
				...
				detail.setType( Mod2002017RegistryType.ADMINISTRATOR.byteValue());
		...
		if (mod200.getParticipationsOut() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsOut() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.PARTICPATION_OUT.byteValue());
		...
		if (mod200.getParticipationsIn() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsIn() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.PARTICPATION_IN.byteValue());
				detail.setDocument(AonStringUtils.substring(cp.getDocument(),0,9));
				detail.setName(AonStringUtils.substring(cp.getName(),0,45));
				detail.setProvince( (byte) cp.getProvince() );
				detail.setCountry( cp.getCountry() );
		...
		if (mod200.getRepresentatives() != null) {
			for ( LegalRepresentative lr : mod200.getRepresentatives() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.REPRESENTATIVE.byteValue());
		...
		if (mod200.getUteParticipations() != null) {
			for ( UteParticipation ute : mod200.getUteParticipations() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.UTE_PARTICIPATION.byteValue());
		...
		if (mod200.getUteForeign() != null) {
			for ( UteForeign ute : mod200.getUteForeign() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.UTE_FOREIGN.byteValue());
		...
		if (mod200.getUteBases() != null) {
			for ( UteBase ute : mod200.getUteBases() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002017RegistryType.UTE_BASE.byteValue());
				detail.setNominalValue(ute.getBase());
				detail.setPercent(ute.getPercent());
				list.add(detail);
			}
		...
		if (mod200.getGroupEntities() != null) {
			for ( String ge : mod200.getGroupEntities() ) {
				if (AonStringUtils.isNotBlank(ge)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002017RegistryType.GROUP_ENTITIES.byteValue());
		...
		if (mod200.getEstablishments() != null) {
			for ( String es : mod200.getEstablishments() ) {
				if (AonStringUtils.isNotBlank(es)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002017RegistryType.ESTABLISHMENTS.byteValue());
	...
	private static void insertDetail(AONContext ctx, Mod2002017 mod200) {
		LinkedList<FsModel200DetailRecord> list = new LinkedList<FsModel200DetailRecord>();
		FsModel200DetailRecord detail = null;
		for (Mod2002017Key k : Mod2002017Key.values()) {
			DoubleVariable2017 dv = null;	
	...
	private static Mod2002017 update(AONContext ctx, Mod2002017 mod200)  {
	...
	public static Mod2002017 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
				.selectFrom(FS_MODEL200)
				.where(FS_MODEL200.ID.equal(id))
				.fetchOne();
		Mod2002017 mod200 =  populateMod200(ctx,record);
		initializeActiveMap(mod200);
		return mod200;
	}
	
	public static Mod2002017 createNewMod200(AONContext ctx, int year) {
		try {
	...
	public static Mod2002017 getByYear(AONContext ctx, int year) {
		return getByYear(ctx, year, true);
	}
	
	public static Mod2002017 getByYear(AONContext ctx, int year, boolean initialize) {
	...
	private static Mod2002017 getMod200(FsModel200Record record) {
		
		Mod2002017 mod200 = new Mod2002017();
		mod200 = new Mod2002017();
	...
	private static Mod2002017 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002017 mod200 = null;
		if (record != null) {
			mod200 = getMod200(record);
			fillDetail(mod200,ctx);
			final Mod2002017 mod = mod200;
			ctx.getDslContext() 
				.selectFrom(FS_MODEL200_REGISTRY)
				.where(	FS_MODEL200_REGISTRY.FS_MODEL200.equal(mod200.getId()))
				.fetch()
				.stream()
				.forEach(reg -> Mod2002017RegistryType.populate(mod, reg));
		}
		return mod200;
	}
	
	private static void fillDetail(Mod2002017 mod200,AONContext ctx) {
		...
		for (FsModel200DetailRecord det : result) {
			try {
				Mod2002017Key.valueOf(det.getKey());
		...
		DoubleVariable2017 v = null;
		for (Mod2002017Key key : Mod2002017Key.values() ) {
			v = new DoubleVariable2017( key );
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002017Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002017Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002017Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		if (map.containsKey( Mod2002017Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002017Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002017Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setPygType( bt );
	}
	
	public static Mod2002017 initializeNewMod200(AONContext ctx, Mod2002017 mod200) {
		Mod2002016 old= Mod2002016DAO.getByYear(ctx, 2016);
		if (old != null && old.getId() != null) { 
			mod200.setEnterprise(old.getEnterprise());
			Mod2002017Import2016.import2016(mod200,old);
	...
	public static Mod2002017 initializeMod200(AONContext ctx, Mod2002017 mod200) {
		try {
			...
			Mod2002017MVELContext mvelCtx = new Mod2002017MVELContext( mod200, ACCEPTER );		
			...
			DoubleVariable2017 dv = null;
			for (Mod2002017Key k : INITIALIZE_EXPRESSION_MAP.keySet()) {
	...
	private static void fillMod202(AONContext ctx,Mod2002017 mod200) {
		Mod202DAO.getMod202s(ctx, mod200.getDomain())
			...
			.forEach( mod -> {
				...
				if (mod202.getPeriod() == Period.T1) {
					...
					key = Mod2002017Key.BN603;
				} if (mod202.getPeriod() == Period.T3) {
					key = Mod2002017Key.BN605;
				}
				if (key != null) {
	...
	public static Mod2002017 calculate(Mod2002017 mod200) {
		return calculate(mod200,true);
	}
	
	public static Mod2002017 calculate(Mod2002017 mod200, boolean addToDraft) {
		try {
			Mod2002017MVELContext ctx = new Mod2002017MVELContext( mod200, ACCEPTER );
			ctx.setExpressionMap(Mod2002017Compute.COMPUTE_EXPRESSION_MAP);
			for (DoubleVariable2017 dv : mod200.getKeysMap().values()) {
				ctx.put(dv.getKey().toString(), dv.getValue());
			}
			addCharacters(ctx,mod200);
			addBalanceCharacters(ctx,mod200);
			
			DoubleVariable2017 d = null;
			for (Mod2002017Key key : mod200.getDraftMap().keySet() ) {
			...
			DoubleVariable2017 v = null;
			for (Mod2002017Key k : Mod2002017Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				String stringKey = k.toString();
				DoubleVariable2017 existingVariable = mod200.getVariable(k);
				Double existingValue = ( existingVariable == null )?0.0:existingVariable.getValue();
				ctx.put(stringKey, existingValue);
				Object ret = ctx.evaluateExpression(k,Mod2002017Compute.COMPUTE_EXPRESSION_MAP.get(k));
				if (ret instanceof Double) {
					Double calculated = (Double) ret;
					ctx.put(stringKey, calculated);
					if ( !AonMathUtils.equals( existingValue , calculated ) ) {
						v = new DoubleVariable2017( k );
			...
			v = mod200.getVariable(Mod2002017Key.BN621);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002017 mod200) throws AonCoreException {
...
//		}
	...
	private static void addCharacters(Mod2002017MVELContext ctx, Mod2002017 mod200) {
		for (Mod2002017Key key : Mod2002017Character.CHARACTERS_KEYS) {
			DoubleVariable2017 dv = mod200.getKeysMap().get(key);
			ctx.put(key.toString(), (dv != null && dv.getBooleanValue() )); 
		}
		
	}
	
	private static void initializeActiveMap(Mod2002017 mod200) {
		Mod2002017MVELContext ctx = new Mod2002017MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002017Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		addBalanceCharacters(ctx,mod200);
		for (Mod2002017Key key : Mod2002017Key.values() ) {
			if (Mod2002017Activation.ACTIVE_EXPRESSION_MAP.containsKey(key) ) {
	...
	private static void addBalanceCharacters(Mod2002017MVELContext ctx, Mod2002017 mod200) {
		ctx.put(Mod2002017Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002017Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002017Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);

		ctx.put(Mod2002017Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002017Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002017Key.C0055.toString(), mod200.getPygType() == BalanceType.PYMES);
	}
	
	public static Mod2002017 validate(Mod2002017 mod200) {
		mod200.setMessages(new LinkedList<ValidationMessage2017>());
//		LinkedList<ValidationMessage2017> list = mod200.getMessages();
		Mod2002017MVELContext ctx = new Mod2002017MVELContext( mod200, ACCEPTER );
		for (DoubleVariable2017 dv : mod200.getKeysMap().values()) {
			ctx.put(dv.getKey().toString(), dv.getValue());
		}
		...
		addCharacters(ctx,mod200);
		Mod2002017Validation.validate(mod200);
//		for (ValidationMessage2017 validation : VALIDATION_EXPRESSION_LIST) {
...
//		}
	...
	public static String dumpAEAT(Mod2002017 mod200)  {
		try {
	...
	public static Mod2002017 importMod2002016(AONContext ctx, Mod2002017 mod200) {
		Mod2002016 old= Mod2002016DAO.getByYear(ctx, 2016);
		return Mod2002017Import2016.import2016(old);
	}	
}
