...
import static com.esferalia.aon.occam.impl.jooq.dao.mod200_2019.Mod2002019Initialization.INITIALIZE_EXPRESSION_MAP;
...
import com.esferalia.aon.occam.api.model.fiscal.mod200_2019.Mod2002019Character;
...
import com.esferalia.aon.occam.impl.jooq.dao.mod200_2019.jaxb.Mod2002019toMOD2002019;
import com.esferalia.aon.occam.server.fiscal.format.Mod2002019Import2018;
...
public class Mod2002019DAO  {
	
	@FunctionalInterface
	private static interface IPopulater {
		boolean populate(Mod2002019 mod,FsModel200RegistryRecord reg);
	}

	private static enum Mod2002019RegistryType {
		 ADMINISTRATOR ( 
		...
		private Mod2002019RegistryType( IPopulater populater){
		...
		private void _populate(Mod2002019 mod,FsModel200RegistryRecord reg) {
			if ( accept(reg) ) {
				this.populater.populate(mod, reg);
			}
		}
		
		public static void populate(Mod2002019 mod,FsModel200RegistryRecord reg) {
			Mod2002019RegistryType type = Mod2002019RegistryType.safeValueOf(reg.getType());
			if (type != null) {
				type._populate(mod, reg);
			}
		}
		
		private static Mod2002019RegistryType safeValueOf( Byte i ) {
			if (i == null) return null;
			return safeValueOf( i.intValue() ); 
		}
		private static Mod2002019RegistryType safeValueOf( Integer i ) {
			if (i == null) return null;
			if (i < 0 || i >= Mod2002019RegistryType.values().length) return null;
			return Mod2002019RegistryType.values()[i];
		}
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				// Comprobar si la clave está en Mod2002019Key
				return (Mod2002019Key.valueOf((String) key) != null);	
			} catch (IllegalArgumentException e) {
				try {
					// Si no lo está, comprobar si está en Mod2002019KeyDC (Detalle correcciones)
					return (Mod2002019KeyDC.valueOf((String) key) != null);	
	...
	public static Mod2002019 save(AONContext ctx, Mod2002019 mod200) {
		try {
			ctx.log().info("------ [START] SAVE MOD 200");
			if (mod200.getPeriodType() == 1) {
				mod200.setPeriodStart(AonDateUtils.getYearFirstDay(2019));
				mod200.setPeriodEnd(AonDateUtils.getYearLastDay(2019));
			}
	...
	private static Mod2002019 insert(AONContext ctx, Mod2002019 mod200)  {
	...
	private static void insertRegistry(AONContext ctx,Mod2002019 mod200) {
		...
		if (mod200.getAdministrators() != null) {
			for ( CompanyAdministrator ca : mod200.getAdministrators() ) {
				...
				detail.setType( Mod2002019RegistryType.ADMINISTRATOR.byteValue());
		...
		if (mod200.getParticipationsOut() != null) {
			for ( CompanyParticipation cp : mod200.getParticipationsOut() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.PARTICPATION_OUT.byteValue());
		...
		if (mod200.getParticipationsIn() != null) {
			for ( CompanyParticipation cp : mod200.getParticipationsIn() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.PARTICPATION_IN.byteValue());
				detail.setDocument(AonStringUtils.substring(cp.getDocument(),0,9));
				detail.setName(AonStringUtils.substring(cp.getName(),0,45));
				detail.setProvince( (byte) cp.getProvince() );
				detail.setCountry( cp.getCountry() );
		...
		if (mod200.getMinorEntities() != null) {
			for ( MinorEntity me : mod200.getMinorEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.MINOR_ENTITIES.byteValue());
		...
		if (mod200.getRepresentatives() != null) {
			for ( LegalRepresentative lr : mod200.getRepresentatives() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.REPRESENTATIVE.byteValue());
		...
		if (mod200.getUteParticipations() != null) {
			for ( UteParticipation ute : mod200.getUteParticipations() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.UTE_PARTICIPATION.byteValue());
		...
		if (mod200.getUteForeign() != null) {
			for ( UteForeign ute : mod200.getUteForeign() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.UTE_FOREIGN.byteValue());
		...
		if (mod200.getUteBases() != null) {
			for ( UteBase ute : mod200.getUteBases() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.UTE_BASE.byteValue());
				detail.setNominalValue(ute.getBase());
				detail.setPercent(ute.getPercent());
				list.add(detail);
			}
		...
		if (mod200.getGroupEntities() != null) {
			for ( GroupEntitie ge : mod200.getGroupEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002019RegistryType.GROUP_ENTITIES.byteValue());
		...
		if (mod200.getEstablishments() != null) {
			for ( String es : mod200.getEstablishments() ) {
				if (AonStringUtils.isNotBlank(es)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002019RegistryType.ESTABLISHMENTS.byteValue());
	...
	private static void insertDetail(AONContext ctx, Mod2002019 mod200) {
		LinkedList<FsModel200DetailRecord> list = new LinkedList<FsModel200DetailRecord>();
		FsModel200DetailRecord detail = null;
		// Insertamos los valores de Mod2002019Key
		for (Mod2002019Key k : Mod2002019Key.values()) {
			DoubleVariable2019 dv = null;	
		...
		// Insertamos los valores de Mod2002019KeyDC (Detalle correcciones)
		for (Mod2002019KeyDC k : Mod2002019KeyDC.values()) {
			DoubleVariable2019 dv = null;	
			if (mod200.getDraftMap().containsKey(k)) {
				dv = mod200.getDraftMap().get(k);
			} else {
	...
	private static Mod2002019 update(AONContext ctx, Mod2002019 mod200)  {
	...
	public static Mod2002019 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
				.selectFrom(FS_MODEL200)
				.where(FS_MODEL200.ID.equal(id))
				.fetchOne();
		Mod2002019 mod200 =  populateMod200(ctx,record);
		initializeActiveMap(mod200);
		return mod200;
	}
	
	public static Mod2002019 createNewMod200(AONContext ctx, int year) {
		try {
	...
	public static Mod2002019 getByYear(AONContext ctx, int year) {
		return getByYear(ctx, year, true);
	}
	
	public static Mod2002019 getByYear(AONContext ctx, int year, boolean initialize) {
	...
	private static Mod2002019 getMod200(FsModel200Record record) {
		
		Mod2002019 mod200 = new Mod2002019();
		mod200 = new Mod2002019();
	...
	private static Mod2002019 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002019 mod200 = null;
		if (record != null) {
			mod200 = getMod200(record);
			fillDetail(mod200,ctx);
			final Mod2002019 mod = mod200;
			ctx.getDslContext() 
				.selectFrom(FS_MODEL200_REGISTRY)
				.where(	FS_MODEL200_REGISTRY.FS_MODEL200.equal(mod200.getId()))
				.fetch()
				.stream()
				.forEach(reg -> Mod2002019RegistryType.populate(mod, reg));
		}
		return mod200;
	}
	
	private static void fillDetail(Mod2002019 mod200,AONContext ctx) {
		...
		for (FsModel200DetailRecord det : result) {
			try {
				// Comprobamos si la clave es de Mod2002019Key
				Mod2002019Key.valueOf(det.getKey());
				map.put(det.getKey(),det.getValue());
			} catch (IllegalArgumentException e) {
				try {
					// Comprobamos si la clave es de Mod2002019KeyDC (Detalle correcciones)
					Mod2002019KeyDC.valueOf(det.getKey());
					map.put(det.getKey(),det.getValue());
				} catch (IllegalArgumentException e2) {
		...
		DoubleVariable2019 v = null;
		// Cargamos las claves de Mod2002019Key
		for (Mod2002019Key key : Mod2002019Key.values() ) {
			v = new DoubleVariable2019( key );
		...
		// Cargamos las claves de Mod2002019KeyDC (Detalle correcciones)
		for (Mod2002019KeyDC key : Mod2002019KeyDC.values() ) {
			v = new DoubleVariable2019( key );
			if (map.containsKey(key.toString())) {
				v.setValue( map.get(key.toString()));
			} else {
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002019Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002019Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002019Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		
		EcpnType et = EcpnType.NO_CONSTA;
		if (map.containsKey( Mod2002019Key.C0075.toString() )) {
			et = EcpnType.NORMAL;
		} else if (map.containsKey( Mod2002019Key.C0076.toString() )) {
			et = EcpnType.ABREVIADO;
		} else if (map.containsKey( Mod2002019Key.C0077.toString() )) {
			et = EcpnType.PYMES;
		}
		mod200.setEcpnType(et);
		
		bt = null;
		if (map.containsKey( Mod2002019Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002019Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002019Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setPygType( bt );
	}
	
	public static Mod2002019 initializeNewMod200(AONContext ctx, Mod2002019 mod200) {
		Mod2002018 old = Mod2002018DAO.getByYear(ctx, 2018);
		if (old != null && old.getId() != null) { 
			mod200.setEnterprise(old.getEnterprise());
			Mod2002019Import2018.import2018(mod200,old);
	...
	public static Mod2002019 initializeMod200(AONContext ctx, Mod2002019 mod200) {
		try {
			...
			// Inicialización estados contables (solo afecta a claves de Mod2002019Key)
			Mod2002019MVELContext mvelCtx = new Mod2002019MVELContext( mod200, ACCEPTER );		
			...
			DoubleVariable2019 dv = null;
			for (Mod2002019Key k : INITIALIZE_EXPRESSION_MAP.keySet()) {
	...
	private static void fillMod202(AONContext ctx,Mod2002019 mod200) {
	...
	public static Mod2002019 calculate(Mod2002019 mod200) {
		return calculate(mod200,true);
	}
	
	public static Mod2002019 calculate(Mod2002019 mod200, boolean addToDraft) {
		try {
			Mod2002019MVELContext ctx = new Mod2002019MVELContext( mod200, ACCEPTER );
			ctx.setExpressionMap(Mod2002019Compute.COMPUTE_EXPRESSION_MAP);
			for (DoubleVariable2019 dv : mod200.getKeysMap().values()) {
				ctx.put(dv.getKey().toString(), dv.getValue());
			}
			addCharacters(ctx,mod200);
			addBalanceCharacters(ctx,mod200);
			
			DoubleVariable2019 d = null;
			...
			// Actualmente las casillas calculadas solo son de Mod2002019Key
			DoubleVariable2019 v = null;
			for (Mod2002019Key k : Mod2002019Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				String stringKey = k.toString();
				DoubleVariable2019 existingVariable = mod200.getVariable(k);
				Double existingValue = ( existingVariable == null )?0.0:existingVariable.getValue();
				ctx.put(stringKey, existingValue);
				Object ret = ctx.evaluateExpression(k,Mod2002019Compute.COMPUTE_EXPRESSION_MAP.get(k));
				if (ret instanceof Double) {
					Double calculated = (Double) ret;
					ctx.put(stringKey, calculated);
					if ( !AonMathUtils.equals( existingValue , calculated ) ) {
						v = new DoubleVariable2019( k );
			...
			v = mod200.getVariable(Mod2002019Key.BN621);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002019 mod200) throws AonCoreException {
	...
	private static void addCharacters(Mod2002019MVELContext ctx, Mod2002019 mod200) {
		// Caracteres de la declaración, solo están en Mod2002019Key
		for (Mod2002019Key key : Mod2002019Character.CHARACTERS_KEYS) {
			DoubleVariable2019 dv = mod200.getKeysMap().get(key);
			ctx.put(key.toString(), (dv != null && dv.getBooleanValue() )); 
		}		
	}
	
	private static void initializeActiveMap(Mod2002019 mod200) {
		// Actualmente la visualizacion de determinadas casillas solo afecta a Mod2002019Key
		Mod2002019MVELContext ctx = new Mod2002019MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002019Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		addBalanceCharacters(ctx,mod200);
		for (Mod2002019Key key : Mod2002019Key.values() ) {
			if (Mod2002019Activation.ACTIVE_EXPRESSION_MAP.containsKey(key) ) {
	...
	private static void addBalanceCharacters(Mod2002019MVELContext ctx, Mod2002019 mod200) {
		ctx.put(Mod2002019Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002019Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002019Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);
		
		ctx.put(Mod2002019Key.C0075.toString(), mod200.getEcpnType() == EcpnType.NORMAL);
		ctx.put(Mod2002019Key.C0076.toString(), mod200.getEcpnType() == EcpnType.ABREVIADO);
		ctx.put(Mod2002019Key.C0077.toString(), mod200.getEcpnType() == EcpnType.PYMES);

		ctx.put(Mod2002019Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002019Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002019Key.C0055.toString(), mod200.getPygType() == BalanceType.PYMES);
	}

	// YA NO SE USA ESTA VALIDACION, SE USA LA DE LA AGENCIA TRIBUTARIA
	public static Mod2002019 validate(Mod2002019 mod200) {
		mod200.setMessages(new LinkedList<ValidationMessage2019>());
		Mod2002019MVELContext ctx = new Mod2002019MVELContext( mod200, ACCEPTER );
		for (DoubleVariable2019 dv : mod200.getKeysMap().values()) {
			ctx.put(dv.getKey().toString(), dv.getValue());
		}
		...
		addCharacters(ctx,mod200);
		Mod2002019Validation.validate(mod200);
		return mod200;
	}
	
	public static String dumpAEAT(Mod2002019 mod200)  {
		try {
	...
	public static Mod2002019 importMod2002018(AONContext ctx, Mod2002019 mod200) {
		Mod2002018 old= Mod2002018DAO.getByYear(ctx, 2018);
		return Mod2002019Import2018.import2018(old);
	}	
}
