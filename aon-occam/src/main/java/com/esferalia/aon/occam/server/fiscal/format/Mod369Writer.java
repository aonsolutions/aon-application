...
public class Mod369Writer {
	...
	private enum Mod369File {
		...
		,T36903 (mod369 -> mod369.getRegime() == Mod369Regime.OUTSIDE, new IPropertyFiller[] {				
				...
				,(wr, mod) -> wr.append(" ")                          // Complementaria
	...
	// Los códigos de país seguiran el estándar ISO-3166 de dos letras. Se utilizará "EL" para Grecia y "XI" para Irlanda del Norte, que en determinados casos actuará como un EM
	private static String getCountry(Country country) {
		if (country != null && country == Country.GR)
			return "EL";
		else 
			return Country.safeIso2(country);
	}
	...
	private static void addHeader(Writer wr, Mod369 mod, boolean isComplementary) throws IOException {
		...
		wr.append(AonFiscalFileUtils.text(mod.getPeriod().toString(), 3));        // 2. Ejercicio y período. Tipo de periodo y Periodo
	...
	private static void addDetail(Writer wr, LinkedList<Mod369Detail> details, int index) throws IOException {
		...
		if (index < details.size()) {
			wr.append(AonFiscalFileUtils.text(getCountry(details.get(index).getCountry()),2));  // Código de país/EM de consumo 
	...
	private static void addDetailOther(Writer wr, LinkedList<Mod369DetailOther> details, int index) throws IOException {
		...
		if (index < details.size()) {
			wr.append(AonFiscalFileUtils.text(getCountry(details.get(index).getOtherCountry()),2));  // Código de país envío o EP
	...
	private static void addCorrection(Writer wr, LinkedList<Mod369DetailCorrection> corrections, int index) throws IOException {
		...
		if (index < corrections.size()) {
			wr.append(AonFiscalFileUtils.text(getCountry(corrections.get(index).getCountry()),2)); // Código País EM de consumo
			wr.append(AonFiscalFileUtils.year(corrections.get(index).getYear()));                  // Ejercicio
			wr.append(AonFiscalFileUtils.text(corrections.get(index).getPeriod().toString(),3));   // Tipo de periodo y Periodo
