...
public class InvoiceCalculator {
	...
	public static void calculate(AccountingInvoice ai, InvoiceVAT vat) {
		if (vat.isPrepayment()) {
			...
			vat.setDirectTaxQuotaEdited(false);
		}
	...
	public static double getDirectTaxQuota(InvoiceVAT vat) {
		double percent = AonMathUtils.round(100 - vat.getDirectTaxPercent());
		return AonMathUtils.round(vat.getBase() *  percent / 100);		
	}

	public static double getDirectTaxQuotaGap(InvoiceVAT vat, Double directTaxQuota) {
		if (directTaxQuota == null) directTaxQuota = 0.0;
		return AonMathUtils.round(directTaxQuota - getDirectTaxQuota(vat));		
	}
		...
		if (duaInvoice != null) {
			...
			if (AonMathUtils.isNotZero( totalQuota )) {
				...
				vats.add( new InvoiceVAT()
						...
						.setExpAccountId(vatAccount==null?null:vatAccount.getId())
						.setExpAccountCode(vatAccount==null?null:vatAccount.getCode())
						.setExpAccountDescription(vatAccount==null?null:vatAccount.getDescription())
						.setBase( totalQuota  )
						);
			}
			if ( AonMathUtils.isNotZero( duaInvoice.getInfo().getDutyTotal() )) {
				...
				vats.add( new InvoiceVAT()
						.setPrepayment(true)
						.setAutoGenerated(true)
						.setExpAccountId(dutyAccount==null?null:dutyAccount.getId())
						.setExpAccountCode(dutyAccount==null?null:dutyAccount.getCode())
						.setExpAccountDescription(dutyAccount==null?null:dutyAccount.getDescription())
