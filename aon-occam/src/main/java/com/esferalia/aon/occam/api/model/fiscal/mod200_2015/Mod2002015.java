...
public class Mod2002015 implements IFiscalModel {
	
	//private static final long serialVersionUID = 4332206787096572914L;
	private static final long serialVersionUID = -6025903716384386043L;
	...
	private EnumMap<Mod2002015Key,Boolean> visibleMap = new EnumMap<Mod2002015Key,Boolean>(Mod2002015Key.class);

	private List<ValidationMessage2015> messages;
	...
	public EnumMap<Mod2002015Key, DoubleVariable2015> getKeysMap() {
		return keysMap;
	}
	public void setKeysMap(EnumMap<Mod2002015Key, DoubleVariable2015> keysMap) {
		this.keysMap = keysMap;
	}
	public EnumMap<Mod2002015Key, DoubleVariable2015> getDraftMap() {
		return draftMap;
	}
	public void setDraftMap(EnumMap<Mod2002015Key, DoubleVariable2015> draftMap) {
		this.draftMap = draftMap;
	}
	public EnumMap<Mod2002015Key, Boolean> getVisibleMap() {
		return visibleMap;
	}
	...
	public List<ValidationMessage2015> getMessages() {
		return messages;
	}
	public void setMessages(List<ValidationMessage2015> messages) {
		this.messages = messages;
	}
	public DoubleVariable2015 getKey(Mod2002015Key key) {
		return keysMap.get(key);
	}
	public boolean isChecked(Mod2002015Key key) {
		DoubleVariable2015 dv = keysMap.get(key); 
		return dv != null && (AonMathUtils.round(dv.getValue()) == 1.0);
	}
	
	public void addVariable(Mod2002015Key key, Number d) {
		DoubleVariable2015 dv = new DoubleVariable2015(key);
		dv.setValue(d==null?0:d.doubleValue());
		keysMap.put( dv.getKey(),dv );
	}

	public void addVariable(DoubleVariable2015 t) {
		keysMap.put( t.getKey(), t);
	}
	public void removeVariable(DoubleVariable2015 t) {
		keysMap.remove(t.getKey());
	}
	
	public void addDraftVariable(DoubleVariable2015 t) {
		draftMap.put( t.getKey(), t);
	}
	public void removeDraftariable(DoubleVariable2015 t) {
		draftMap.remove(t.getKey());
	}

	public DoubleVariable2015 getVariable(Mod2002015Key key) {
		DoubleVariable2015 var = getDraftMap().get(key);
		if (var == null ) {
	...
	public Double getDoubleValue(Mod2002015Key key) {
		...
		DoubleVariable2015 v = keysMap.get(key);
		if (v==null) {
			return new Double(0);
		}
		Object o = v.getValue();
	...
	public void listDraftVariables() {
		for (DoubleVariable2015 dv : getDraftMap().values()) {
