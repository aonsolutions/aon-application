...
public class TaxBreakdown implements Serializable {
	...
	public Optional<InvoiceWithholding> getInvoiceWithholding() {
		InvoiceBreakdown withholding = ibs.stream().filter(ib -> ib.isWithholding() )
			.findFirst()
			.orElse( null );
		if (withholding != null) {
			return Optional.of(
			new InvoiceWithholding()
				.setBase(withholding.getBase())
				.setQuota(withholding.getQuota())
				.setPercentage(withholding.getPercentage())
				.setWithholdingType(withholding.getWithholdingType())
			);
		}
