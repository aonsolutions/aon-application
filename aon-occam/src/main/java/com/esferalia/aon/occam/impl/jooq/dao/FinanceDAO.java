	...
	public static Integer insert(AONContext ctx, Finance finance) {
		...
		FinanceRecord record = ctx.getDslContext()
			.insertInto(FINANCE)
				...
				.set(FINANCE.PAY_METHOD,finance.getPayMethod()==null?null:finance.getPayMethod().getId())
	...
	public static class FullFinanceFiller  implements Function<Record,Finance> {
		...
		public Finance apply(Record record) {
			return new Finance()
				...
				.setPayMethod(record.getValue(FINANCE.PAY_METHOD) == null?null:new FullPayMethodFiller().apply(record))
