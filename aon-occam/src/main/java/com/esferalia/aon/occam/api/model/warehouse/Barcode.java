...
public class Barcode {
	...
	public Map<GS1128Codes, String> parseGS1128() {
		...
		while(barcode.length() > 0) {
			...
			} else if(barcode.contains("\u001d") || barcode.contains("\f") || barcode.contains("\312")) {
				Integer index = 0;
				if(barcode.contains("\u001d")) index = barcode.indexOf('\u001d');
				else if(barcode.contains("\f")) index = barcode.indexOf('\f');
				else if(barcode.contains("\312")) index = barcode.indexOf('\312');
