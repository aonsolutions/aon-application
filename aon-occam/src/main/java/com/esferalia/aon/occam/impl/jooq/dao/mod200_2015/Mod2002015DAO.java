...
import static com.esferalia.aon.occam.impl.jooq.dao.mod200_2015.Mod2002015Initialization.INITIALIZE_EXPRESSION_MAP;
import static com.esferalia.aon.occam.impl.jooq.dao.mod200_2015.Mod2002015Validation.VALIDATION_EXPRESSION_LIST;
...
import com.esferalia.aon.occam.api.model.fiscal.mod200_2015.Mod2002015Character;
...
public class Mod2002015DAO  {
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				return (Mod2002015Key.valueOf((String) key) != null);	
	...
	private static Mod2002015 insert(AONContext ctx, Mod2002015 mod200)  {
	...
	private static void insertAdministrator(AONContext ctx,Mod2002015 mod200) {
	...
	private static void insertDetail(AONContext ctx, Mod2002015 mod200) {
		List<FsModel200DetailRecord> list = new LinkedList<FsModel200DetailRecord>();
		FsModel200DetailRecord detail = null;
		for (Mod2002015Key k : Mod2002015Key.values()) {
			DoubleVariable2015 dv = null;	
	...
	private static Mod2002015 update(AONContext ctx, Mod2002015 mod200)  {
	...
	public static Mod2002015 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
	...
	public static Mod2002015 createNewMod200(AONContext ctx, int year) {
	...
	private static Mod2002015 getMod200(FsModel200Record record) {
		
		Mod2002015 mod200 = new Mod2002015();
		mod200 = new Mod2002015();
	...
	private static Mod2002015 populateMod200(AONContext ctx, FsModel200Record record) {
	...
	private static void fillDetail(Mod2002015 mod200,AONContext ctx) {
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002015Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002015Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002015Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		if (map.containsKey( Mod2002015Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002015Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002015Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setPygType( bt );
	}
	
	private static void fillRegistryLists(Mod2002015 mod200, AONContext ctx) {
	...
	public static Mod2002015 initializeNewMod200(AONContext ctx, Mod2002015 mod200) {
		Mod2002014 old= Mod2002014DAO.getByYear(ctx, 2014);
		if (old != null && old.getId() != null) { 
			mod200.setEnterprise(old.getEnterprise());
			Mod2002015Import2014.import2014(mod200,old);
	...
	public static Mod2002015 initializeMod200(AONContext ctx, Mod2002015 mod200) {
		...
		Mod2002015MVELContext mvelCtx = new Mod2002015MVELContext( mod200, ACCEPTER );		
	...
	public static Mod2002015 calculate(Mod2002015 mod200, boolean addToDraft) {
		try {
			Mod2002015MVELContext ctx = new Mod2002015MVELContext( mod200, ACCEPTER );
			ctx.setExpressionMap(Mod2002015Compute.COMPUTE_EXPRESSION_MAP);
			for (DoubleVariable2015 dv : mod200.getKeysMap().values()) {
				ctx.put(dv.getKey().toString(), dv.getValue());
			}
			addCharacters(ctx,mod200);
			addBalanceCharacters(ctx,mod200);
			
			DoubleVariable2015 d = null;
			for (Mod2002015Key key : mod200.getDraftMap().keySet() ) {
			...
			v = mod200.getVariable(Mod2002015Key.BN621);
...
//			e.printStackTrace();
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002015 mod200) throws AonCoreException {
...
//		}
	...
	private static void addCharacters(Mod2002015MVELContext ctx, Mod2002015 mod200) {
		for (Mod2002015Key key : Mod2002015Character.CHARACTERS_KEYS) {
	...
	private static void initializeActiveMap(Mod2002015 mod200) {
		Mod2002015MVELContext ctx = new Mod2002015MVELContext( mod200, ACCEPTER );
	...
	private static void addBalanceCharacters(Mod2002015MVELContext ctx, Mod2002015 mod200) {
		ctx.put(Mod2002015Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002015Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002015Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);

		ctx.put(Mod2002015Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002015Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002015Key.C0055.toString(), mod200.getPygType() == BalanceType.PYMES);
	}
	
	public static Mod2002015 validate(Mod2002015 mod200) {
		List<ValidationMessage2015> list = new LinkedList<ValidationMessage2015>();
		...
		for (ValidationMessage2015 validation : VALIDATION_EXPRESSION_LIST) {
	...
	private static void validateComplementary(List<ValidationMessage2015> list, Mod2002015 mod200) {
		if (mod200.isComplementary() ) {
			if (AonStringUtils.isEmpty(mod200.getComplementaryReceipt() )) {
				list.add(new ValidationMessage2015(PAGE00,"Si marca Decl. Complementaria, debe indicar un n. de justificante anterior."));
			} else {
				if (mod200.getComplementaryReceipt().length() != 13) {
					list.add(new ValidationMessage2015(PAGE00,"El n. de justificante anterior debe tener 13 caracteres."));
				}
				if (!mod200.getComplementaryReceipt().startsWith("200") && !mod200.getComplementaryReceipt().startsWith("206")) {
					list.add(new ValidationMessage2015(PAGE00,"El n. de justificante anterior debe empezar por 200 o 206."));
				}
			}
			
		} else if (!mod200.isComplementary() && !AonStringUtils.isEmpty(mod200.getComplementaryReceipt())) {
			list.add(new ValidationMessage2015(PAGE00,"Si no marca Decl. Complementaria, no debe indicar un n. de justificante anterior."));
		}
	}

	private static void validateDocument(List<ValidationMessage2015> list, Mod2002015 mod200) {
		if (!AonDocumentUtil.isValid(mod200.getEnterpriseDocument())) {
			list.add(new ValidationMessage2015(PAGE00,"NIF de la declaraci\u00F3n incorrecto."));
		}
		
	}
	private static void validateCNAE(List<ValidationMessage2015> list, Mod2002015 mod200) {
		if (AonStringUtils.isEmpty(mod200.getCnae())) {
			list.add(new ValidationMessage2015(PAGE00,"Rellene el CNAE de la empresa."));
		} else if (CNAE2009.valueOfCode(mod200.getCnae()) == null) {
			list.add(new ValidationMessage2015(PAGE00,"CNAE de la empresa, no válido."));	
		}
	}
	private static void validateAdministrators(List<ValidationMessage2015> list,Mod2002015 mod200) {
		if (mod200.getAdministrators() == null || mod200.getAdministrators().size() == 0 ) {
			...
			for (int i = 0; i < mod200.getAdministrators().size(); i++ ) {
				...
				if (AonStringUtils.isEmpty(ca.getName())) {
					list.add(new ValidationMessage2015(PAGE01,"Falta nombre del administrador nº "+(i+1) +". ["+ca.getDocument()+"]"));
				}
			}
		}
	}
	
	private static void validateSecretary(List<ValidationMessage2015> list, Mod2002015 mod200) {
		if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			if (mod200.getSecretary() == null) {
				list.add(new ValidationMessage2015(PAGE01,"Para personas jur\u00EDdicas, debe rellenar los datos del secretario"));
			} else {
				if (!AonDocumentUtil.isValid(mod200.getSecretary().getDocument())) {
					list.add(new ValidationMessage2015(PAGE01,"NIF del secretario incorrecto."));
				}
				if ( AonStringUtils.isEmpty(mod200.getSecretary().getName())) {
					list.add(new ValidationMessage2015(PAGE01,"Falta nombre del secretario."));
				} else if (mod200.getSecretary().getName().length() > 25) {
					list.add(new ValidationMessage2015(PAGE01,"Longitud excedida en el nombre del secretario. Debe limitarse a 25 caracteres."));	
				}
				if (mod200.getSecretary().getIrnr() == null && (mod200.isChecked(Mod2002015Key.C0021) || mod200.isChecked(Mod2002015Key.C0046)) ) {
					list.add(new ValidationMessage2015(PAGE01,"Falta fecha IRNR."));
				}
			}
		}
	}

	private static void validateRepresentatives(List<ValidationMessage2015> list,Mod2002015 mod200) {
		if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			if (mod200.getRepresentatives() == null || mod200.getRepresentatives().size() == 0 ) {
				list.add(new ValidationMessage2015(PAGE01,"Para personas jur\u00EDdicas, debe rellenar al menos un representante."));
			} else {
				for (int i = 0; i < mod200.getRepresentatives().size(); i++ ) {
					...
					if (!AonDocumentUtil.isValid(lr.getDocument())) {
						list.add(new ValidationMessage2015(PAGE01,"NIF del representante legal nº "+(i+1) +" incorrecto ["+lr.getDocument()+"]"));		
					}
					if (AonStringUtils.isEmpty(lr.getName())) {
						list.add(new ValidationMessage2015(PAGE01,"Falta nombre del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					}
					if (AonStringUtils.isEmpty(lr.getNotary())) {
						list.add(new ValidationMessage2015(PAGE01,"Falta el dato de la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					} else if (lr.getNotary().length() > 20) {
						list.add(new ValidationMessage2015(PAGE01,"Longitud excedida en la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]. Debe limitarse a 20 caracteres."));	
					}
					if (lr.getNotaryDate() == null) {
						list.add(new ValidationMessage2015(PAGE01,"Falta el dato fecha de la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					}
				}
			}
		}
			...
			if (participations == null || participations.size() == 0) {
				...
				for (int i = 0; i < participations.size(); i++ ) {
					...
					if (AonStringUtils.isEmpty(cp.getName())) {
						list.add(new ValidationMessage2015(PAGE02,"Falta nombre de la participaci\u00F3n en la declarante nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
					if (cp.getPercent() < 0 || cp.getPercent() > 100) {
						list.add(new ValidationMessage2015(PAGE02,"Porcentaje no correcto en la participaci\u00F3n en la declarante nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
				}
			}
		}
		 ...
		 if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			...
			if (participations == null || participations.size() == 0) {
				...
				for (int i = 0; i < participations.size(); i++ ) {
					...
					if (AonStringUtils.isEmpty(cp.getName())) {
						list.add(new ValidationMessage2015(PAGE02,"Falta nombre de la participaci\u00F3n de la declarante en otras nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
					if (cp.getPercent() < 0 || cp.getPercent() > 100) {
						list.add(new ValidationMessage2015(PAGE02,"Porcentaje no correcto en la participaci\u00F3n de la declarante en otras nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
				}
			}
		}
	...
	public static String dumpAEAT(Mod2002015 mod200)  {
		try {
	...
	public static Mod2002015 importMod2002014(AONContext ctx, Mod2002015 mod200) {
		Mod2002014 old= Mod2002014DAO.getByYear(ctx, 2014);
		return Mod2002015Import2014.import2014(old);
	}	
}
