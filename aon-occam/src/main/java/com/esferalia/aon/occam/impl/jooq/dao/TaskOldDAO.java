...
public class TaskOldDAO {
	...
	public static OldTask getTask(AONContext ctx, TaskFilter filter){
		return ctx.getDslContext().select().from(TASK).where(TASK_PROPERTIES.getConditions(filter)).fetchInto(TASK)
				.stream().map(new FullTaskFiller()).findFirst().orElse(new OldTask());
	}
	...
	public static Stream<OldTask> getDuplicateTaskStream(AONContext ctx, Integer parent){
	...
	public static Stream<OldTask> getTaskStream(AONContext ctx, TaskFilter filter){	
		return ctx.getDslContext().selectDistinct().from(TASK).where(TASK_PROPERTIES.getConditions(filter))
				.orderBy(TASK.NUMBER.desc()).fetchInto(TASK).stream().map(new FullTaskFiller());
	}
	
	@Deprecated
	public static Stream<OldTask> getTaskStream(AONContext ctx, TaskFilter filter, IssueFilter issueFilter){
		Boolean tagBool = (issueFilter.getLabels() != null && !issueFilter.getLabels().equals(""))
				|| (issueFilter.getType() != null && !issueFilter.getType().equals(""));
		Boolean commentBool = issueFilter.getTitle() != null && !issueFilter.getTitle().equals("");
		Boolean eventBool = false;
	...
	public static Integer createTask(AONContext ctx, OldTask task) {
	...
	public static long getTaskHolderCount(AONContext ctx, TaskHolderFilter filter){
		return ctx.getDslContext().select(TASK_HOLDER.REGISTRY)
		.from(TASK_HOLDER).join(REGISTRY).on(REGISTRY.ID.eq(TASK_HOLDER.REGISTRY))
		.join(UserAppRole.USER_APP_ROLE).on(UserAppRole.USER_APP_ROLE.USER_ID.eq(TASK_HOLDER.USER_ID))
		.join(DOMAIN).on(DOMAIN.ID.eq(TASK_HOLDER.DOMAIN))
		.where(TASK_HOLDER_PROPERTIES.getConditions(filter))
		.and(UserAppRole.USER_APP_ROLE.ROLE.eq(AonRole.EMPLOYEE.value())).fetch().stream().count();
	}
	...
	public static class FullTaskFiller implements Function<TaskRecord, OldTask> {
		@Override
		public OldTask apply(TaskRecord r) {
			return new OldTask().setId(r.getId())
