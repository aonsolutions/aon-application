...
public class Mod347Writer {
	...
	private enum Mod347File2014 {
		...
		,TYPE_2_DEC (new IPropertyFiller[] { 
		   ...
		   ,(line, mod347, declared, asset) -> line.append(mod347.getYear() >= 2025 ? (mod347.isNavarra() && AonStringUtils.isBlank(declared.getBdns()) ? AonStringUtils.spaces(6) : AonFiscalFileUtils.unsigned(declared.getBdns(),6,0)) : AonStringUtils.repeat(' ',6))  // Número de convocatoria BDNS (a partir del ejercicio 2025) (Navarra si el campo está vacio deben ponerse blancos, no ceros)
		   ,(line, mod347, declared, asset) -> line.append(AonStringUtils.repeat(' ',195))							                    // Blancos
		   ,(line, mod347, declared, asset) -> line.append("\r\n") 												                    	// FIN DE REGISTRO. CONSTANTE CRLF
		})
		...
		// CANARIAS ---------------------------------------------------------
		 ...
		 // Registro de Declarante (Canarias)
		,TYPE_1_CANARIAS (new IPropertyFiller[] { 
			(line, mod347, declared, asset) -> line.append("1") 											       // TIPO DE REGISTRO
		   ,(line, mod347, declared, asset) -> line.append("415") 										           // MODELO DE DECLARACIÓN
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned(mod347.getYear(), 4,0)) 	   // EJERCICIO
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(mod347.getDocument(),9))	 	   // N.I.F. DEL OBLIGADO TRIBUTARIO O DECLARANTE		   
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(mod347.getName(),40))	 	   // APELLIDOS Y NOMBRE O DENOMINACIÓN O RAZÓN SOCIAL
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.A).count()), 9, 0))                                    // Clave A - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.A).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave A - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.B).count()), 9, 0))                                    // Clave B - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.B).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave B - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.C).count()), 9, 0))                                    // Clave C - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.C).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave C - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.D).count()), 9, 0))                                    // Clave D - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.D).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave D - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.E).count()), 9, 0))                                    // Clave E - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.E).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave E - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.F).count()), 9, 0))                                    // Clave F - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.F).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave F - Importe total de las operaciones
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned((int)(mod347.getDeclared()==null ? 0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.G).count()), 9, 0))                                    // Clave G - Número total de personas o entidades
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.signedSpace((mod347.getDeclared()==null ? 0.0 : mod347.getDeclared().stream().filter(det -> det.getType() == Mod347Key.G).mapToDouble(det -> det.getAmount() + det.getRentalAmount() + det.getAssetAmount()).sum()), 16, 2)) // Clave G - Importe total de las operaciones
		...
		,TYPE_3_CANARIAS (new IPropertyFiller[] {
		   ...
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(asset.getCadasdralReference(),25))              	// REFERENCIA CATASTRAL
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(getStreetType2(asset.getAssetStreetType()), 5)) 	// TIPO DE VÍA (LAS SIGLAS VAN SOLO CON 2 CARACTERES)
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(asset.getAssetStreet(), 50))              		// NOMBRE VÍA PÚBLICA
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(asset.getAssetStreetNumberType(), 3))     		// TIPO DE NUMERACIÓN
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.unsigned(asset.getAssetStreetNumber(), 5, 0))   		// NÚMERO DE CASA
		   ,(line, mod347, declared, asset) -> line.append(AonFiscalFileUtils.text(asset.getAssetStreetNumberSuffix(), 3))   		// CALIFICADOR DEL NÚMERO
	...
	public static void fillWriter(Mod347 mod347, Writer writer) throws IOException {
		...
		if (mod347.getAdministration() == Administration.CANARIAS) {
			// Canarias
			Mod347File2014.TYPE_1_CANARIAS.fillPage(mod347, null, null, writer);		
			for (Mod347Declared declared : mod347.getDeclared()) {
				Mod347File2014.TYPE_2_CANARIAS.fillPage(mod347, declared, null, writer);
			}
			for (Mod347Asset asset : mod347.getAssets()) {
				Mod347File2014.TYPE_3_CANARIAS.fillPage(mod347, null,  asset, writer);
			}
		} else if (mod347.getAdministration() == Administration.GIPUZKOA) {
			// Gipuzkoa
			provinceCodeDeponent = getRegistryMainAddressProvince(mod347); // Necesitamos el codigo de provincia del declarante para el fichero, se obtiene de la dirección principal
