...
public class IsolateDomain {
	...
	private static void isolateDomain(ConsoleParams params) {
		Optional<Schema> schema = params.getDslContext().meta().getSchemas().stream()
				.filter(s -> s.getName().equals(params.getDatabase())).findFirst();
		if (schema.isPresent()) {
			log(params,"** Start domain isolation!");
			try {
				params.setSchema(schema.get())
			...
			if (fkOldIntegerId != null) {
				...
				if ( fkNewId != null) {
					...
					if (toRec != null
						...
						if (toRec != null && getDomainValue(toTable,toRec).intValue() != 0) {
