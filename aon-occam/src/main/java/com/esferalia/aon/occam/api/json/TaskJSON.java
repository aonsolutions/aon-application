...
//			.setActivityType(JsonUtils.getInteger(json, IJsonNames.ACTIVITY_TYPE))
			...
			.setStartDate(JsonUtils.getDateTime(json, IJsonNames.START_DATE2))
	...
	public static JSONObject toJSON(Task task) {
		...
		return new JSONObject()
			...
			.put(IJsonNames.SENDER, task.getSender()!=null?TaskHolderJSON.toJSON(task.getSender()) : null)
			.put(IJsonNames.TASK_HOLDER, task.getTaskHolder()!=null?TaskHolderJSON.toJSON(task.getTaskHolder()) : null)
			.put(IJsonNames.REGISTRY, task.getRegistry()!=null  ? RegistryJSON.toJSON(task.getRegistry()) : null)
			.put(IJsonNames.WORKGROUP, task.getWorkgroup()!=null?WorkgroupJSON.toJSON(task.getWorkgroup()) : null)
			...
			.put(IJsonNames.START_DATE2, task.getStartDate()!=null ?  task.getStartDate().getTime() : null)
			.put(IJsonNames.END_DATE2, task.getEndDate()!=null ?  task.getEndDate().getTime() : null)
