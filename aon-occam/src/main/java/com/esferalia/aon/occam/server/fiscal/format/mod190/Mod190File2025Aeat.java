...
enum Mod190File2025Aeat {
	...
	,TYPE_2 (new IPropertyFiller[] { 
	   ...
	   ,(wr, mod190,detail) -> wr.append(detail.isPermanentDisabilityPension() ? "1" : "0") // Tipos de prestaciones de la Clave B.01 percibidas en el ejercicio: 03. Pensión por incapacidad permanente, total o parcial.
	   ,(wr, mod190,detail) -> wr.append(detail.isNonContributoryPension() ? "1" : "0") 	// Tipos de prestaciones de la Clave B.01 percibidas en el ejercicio: 04. Pensión NO contributiva por invalidez o jubilación.
	   ,(wr, mod190,detail) -> wr.append(detail.isOtherNonExemptPensions() ? "1" : "0") 	// Tipos de prestaciones de la Clave B.01 percibidas en el ejercicio: 05. Resto de prestaciones del artículo 17.2.a).1ª de la Ley IRPF, no exentas, distintas de las anteriores.
	...
	private Mod190File2025Aeat(IPropertyFiller[] pf) {
	...
	static void fill(Mod190 mod190, Writer wr) throws IOException {
		Mod190File2025Aeat.TYPE_1.fillPage(mod190, null, wr);
		for (Mod190Detail detail : mod190.getDetails()) {
			Mod190File2025Aeat.TYPE_2.fillPage(mod190, detail, wr);	
		}
	}
}
