...
public class Mod421InfoUtil extends DeclarationInfoUtil {
	
	private Mod421InfoUtil() {
		super();
	}
	
//	public static String getRegularizationExplain(AONContext ctx, Mod421 mod421, Mod421Key key) {
...
//				Mod421 m421 = (Mod421) fm;
//				setSomething(true);
//				sum(fm.getDeclarationResult());
//				double percent = m421.getProratePercent();	
//
//				Mod421Declaration dec =  Mod421Declaration.getInstance(m421);
//				MutableDouble sumProrratedMustDeclared = new MutableDouble();
//				MutableDouble sumUnprorratedMustDeclared = new MutableDouble();
//				VATDAO.getModelVatBreakdown(ctx, m421)
//					.filter( br -> Arrays.stream(dec.getProrateKeys())
//							.map(dec::getKey )
//							.anyMatch(kd -> kd.acceptValue(m421, br)))
//					.forEach(br -> {
//						if (Mod421Declaration.mustApplyProrrate(m421,br)) {
...
//						if (dec.isProrrated( Mod421Key.getKey(key))) {
...
//				
//				double mustProrrated = AonMathUtils.round(prorratedQuota * mod421.getProratePercent() / 100);
...
//						.append(DEC2.format(mod421.getProratePercent()))
//							.append(" %")
//						.append("</td>")
//						.append( MessageFormat.format(styledTag, "td", textRight+width150+border+noWrap+colorLightGreen) )				
