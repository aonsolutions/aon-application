...
abstract class Mod303CANARIAS extends Mod303Declaration {
	...
	double getResult(Mod303 mod) {
		return mod.getAmount(Mod303Key.CA_C045);
	}
	...
	protected Set<Alcatraz> createVatAccrualKeysFromInvoices(AONContext ctx, Mod303 mod303) {
		...
		stream.forEach( vc -> {
			if (vc.isSales()) {
				add(Mod303Key.CA_C048, mod303, vc.getBase());
				add(Mod303Key.CA_C049, mod303, vc.getDeductibleQuota());
			} else {
				add(Mod303Key.CA_C050, mod303, vc.getBase());
				add(Mod303Key.CA_C051, mod303, vc.getDeductibleQuota());
			}
			invoices.add(
				new Alcatraz()
		...
		add(Mod303Key.CA_X03, mod303, AonMathUtils.isZero(mod303.getAmount(Mod303Key.CA_C051)) ? (0.0) : (1.0));
		return invoices;
	}
