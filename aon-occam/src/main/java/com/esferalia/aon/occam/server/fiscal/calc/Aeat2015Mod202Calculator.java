...
public class Aeat2015Mod202Calculator {

	public static Mod202 calculate(AONContext ctx, Mod202 mod202) {
		int year = mod202.getInitialDate() == null?2015: AonDateUtils.getYear( mod202.getInitialDate() );
		...
		if (x00 == 0) {
			...
			double c33 = 0;
			if (x09 > 1) {
				boolean x10 = mod202.getCheck(Mod202Key.X10);
				if (!x10) {
					c33 = AonMathUtils.round(((c04 * 12.0) * c29 / 100) - c30 -c31);	
				} else {
					c33 = AonMathUtils.round(((c04 * 6.0) * c29 / 100) - c30 -c31);	
				}
			}
			mod202.putAmount(Mod202Key.C33, c33);
