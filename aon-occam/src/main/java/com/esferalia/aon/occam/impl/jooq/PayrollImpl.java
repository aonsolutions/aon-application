...
public class PayrollImpl implements IPayroll {
	...
	public ContractExtendedData getContractById(AONContext ctx, ContractExtendedDataFilter filter, Integer id) {
		return ctx.getDslContext().transactionResult(configuration ->
		ContractDAO.getContractById(ctx, filter, id));
	}
	...
	public List<AuxSalaryInfo> getEmployeeSalary(AONContext ctx, SalaryNewPortalFilter filter, Integer page, Integer per_page) {
		return ctx.getDslContext().transactionResult(configuration ->
