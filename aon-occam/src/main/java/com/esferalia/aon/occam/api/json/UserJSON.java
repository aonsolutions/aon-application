...
public class UserJSON {
	...
	public static List<User> fromJSON(JSONArray jsonArray) {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			users.add(UserJSON.fromJSON(jsonObject));
		}
		return users;
	}
	...
	public static User fromJSON(JSONObject json) {
		...
		if (json.optBoolean(IJsonNames.PORTAL) != false)
		...
		user.setDomain(new Domain().setId(json.optInt(IJsonNames.DOMAIN)));
		if (json.optJSONArray("taskHolders") != null)
			user.setTaskHolders(TaskHolderJSON.fromJSON(json.optJSONArray("taskHolders")));
		else
			user.setTaskHolders(null);
	...
	public static JSONObject toJSON(User user) {
		...
		return json.put(IJsonNames.ID, user.getId()).put(IJsonNames.DOMAIN, user.getDomain().getId())
				...
				.put(IJsonNames.PORTAL, user.isPortal()).put(IJsonNames.SHARED, user.isShared())
				.put(IJsonNames.LOGIN, user.getLogin()).put(IJsonNames.ACTIVE, user.isActive())
