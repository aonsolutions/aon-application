...
import static com.esferalia.aon.occam.impl.jooq.dao.Mod200Initialization.INITIALIZE_EXPRESSION_MAP;
import static com.esferalia.aon.occam.impl.jooq.dao.Mod200Validation.VALIDATION_EXPRESSION_LIST;
...
import com.esferalia.aon.occam.api.model.fiscal.mod200.Mod200Character;
...
public class Mod200DAO  {
	...
	public static Mod200 save(AONContext ctx, Mod200 mod200) {
	...
	private static Mod200 insert(AONContext ctx, Mod200 mod200)  {
	...
	private static void insertAdministrator(AONContext ctx,Mod200 mod200) {
	...
	private static void insertDetail(AONContext ctx, Mod200 mod200) {
	...
	private static Mod200 update(AONContext ctx, Mod200 mod200)  {
	...
	public static Mod200 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
	...
	public static Mod200 getByYear(AONContext ctx, int year) {
		FsModel200Record record = ctx.getDslContext()
				.selectFrom(FS_MODEL200)
				.where(FS_MODEL200.DOMAIN.equal(ctx.getDomainId()))
				.and(FS_MODEL200.YEAR.equal(year))
				.fetchOne();
		Mod200 mod200 = populateMod200(ctx,record);
		if (mod200 == null) {
			mod200 = new Mod200();
	...
	private static Mod200 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod200 mod200 = null;
		if (record != null) {
	...
	private static void fillDetail(Mod200 mod200,AONContext ctx) {
	...
	private static void fillRegistryLists(Mod200 mod200, AONContext ctx) {
	...
	public static Mod200 initializeNewMod200(AONContext ctx, Mod200 mod200) {
	...
	public static Mod200 initializeMod200(AONContext ctx, Mod200 mod200) {
	...
	public static Mod200 calculate(Mod200 mod200) {
		return calculate(mod200,true);
	}
	
	public static Mod200 calculate(Mod200 mod200, boolean addToDraft) {
		try {
			Mod200MVELContext ctx = new Mod200MVELContext( mod200, ACCEPTER );
			ctx.setExpressionMap(Mod200Compute.COMPUTE_EXPRESSION_MAP);
			...
			DoubleVariable v = null;
			for (String stringKey : Mod200Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				Mod200Key k = Mod200Key.valueOf(stringKey.toString());
				String expression = Mod200Compute.COMPUTE_EXPRESSION_MAP.get(stringKey);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod200 mod200) throws AonCoreException {
	...
	private static void addCharacters(Mod200MVELContext ctx, Mod200 mod200) {
		for (Mod200Key key : Mod200Character.CHARACTERS_KEYS) {
	...
	private static void initializeActiveMap(Mod200 mod200) {
		Mod200MVELContext ctx = new Mod200MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod200Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		for (Mod200Key key : Mod200Key.values() ) {
			if (Mod200Activation.ACTIVE_EXPRESSION_MAP.containsKey(key.toString()) ) {
	...
	private static void addBalanceCharacters(Mod200MVELContext ctx, Mod200 mod200) {
	...
	public static Mod200 validate(Mod200 mod200) {
	...
	private static void validateComplementary(List<ValidationMessage> list, Mod200 mod200) {
	...
	private static void validateDocument(List<ValidationMessage> list, Mod200 mod200) {
	...
	private static void validateCNAE(List<ValidationMessage> list, Mod200 mod200) {
	...
	private static void validateAdministrators(List<ValidationMessage> list,Mod200 mod200) {
	...
	private static void validateSecretary(List<ValidationMessage> list, Mod200 mod200) {
	...
	private static void validateRepresentatives(List<ValidationMessage> list,Mod200 mod200) {
		if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
	...
	private static void validateParticipationsIn(List<ValidationMessage> list,Mod200 mod200) {
		 if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
	...
	private static void validateParticipationsOut(List<ValidationMessage> list,Mod200 mod200) {
		 if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
	...
	public static String dumpAEAT(Mod200 mod200)  {
		try {
