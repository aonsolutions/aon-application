...
class Mod303WriterAEAT2026 implements IMod303Writer{
	...
	private enum Mod303File {
		...
		,T30301000 (mod303 -> true ,new IPropertyFiller[] {
		   ...
		   ,(wr, mod) -> wr.append(AonStringUtils.repeat(' ', 521))
		...
		,T30302001 (mod303 -> mod303.getAmount(Mod303Key.CT_A02) < 2 ,new IPropertyFiller[] {
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(  mod.getAmount(Mod303Key.CT_S47)  ,17,2)) // Suma de ingresos a cuenta del conjunto de actividades
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.signedZero(mod.getAmount(Mod303Key.CT_S53)  ,17,2)) // IVA devengado por inversión del sujeto pasivo
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.signedZero(mod.getAmount(Mod303Key.CT_S58)  ,17,2)) // Resultado Régimen Simplificado
		   
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S1X1) ,3,0)) //		   90	1105	3	Num	C	Liquidación (3) - RS - (B) Actividades en RS (exc. a, g y f) - Actividad 1 - ACTIVIDAD DE TEMPORADA: nº de días en los que se ejerció la actividad en el año anterior - 1T/2T/3T		3 enteros
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S1X2) ,2,0)) //		   91	1108	2	Num	C	Liquidación (3) - RS - (B) Actividades en RS (exc. a, g y f) - Actividad 1 - Número de días de ejercicio de la actividad en el trimestre - 1T/2T/3T		2 enteros
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(0.0,3,0)) // Reservado para la AEAT
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(!mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S1Y1) ,3,0)) //		   93	1113	3	Num	C	Liquidación (3) - RS - (B) Actividades en RS (exc. a, g y f) - Actividad 1 - Actividad de temporada. Número de días de ejercicio - 4T		3 enteros
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(0.0,4,0)) // Reservado para la AEAT
...
//		   117	1228	3	Num	C	Liquidación (3) - RS - (B) Actividades en RS (exc. a, g y f) - Actividad 1 - Módulo Mesas - Días - 4T		3 enteros
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S2X1) ,3,0))  // Actividad 2 - ACTIVIDAD DE TEMPORADA: nº de días en los que se ejerció la actividad en el año anterior - 1T/2T/3T
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S2X2) ,2,0))  // Actividad 2 - Número de días de ejercicio de la actividad en el trimestre (1T/2T/3T)
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(0.0,3,0))                                                       // Reservado para la AEAT
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(!mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S2Y1) ,3,0)) // Actividad 2 - Actividad de temporada. Nº Días de ejercicio (4T)
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(0.0,4,0))  // Reservado para la AEAT
		...
		,T30302002 (mod303 -> mod303.getAmount(Mod303Key.CT_A02) < 2
			   ...
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.zeros(3))                                                                // Reservado para la AEAT
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(!mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S3Y1) ,3,0)) // Actividad 3: Actividad de temporada. Nº Días de ejercicio (4T)
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.zeros(4))                                                                // Reservado para la AEAT
			   ...
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.zeros(3))                                                                // Reservado para la AEAT
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.unsigned(!mod.isLastPeriod()?0.0:mod.getAmount(Mod303Key.CT_S4Y1) ,3,0)) // Actividad 4: Actividad de temporada. Nº Días de ejercicio (4T) 
			   ,(wr, mod) -> wr.append(AonFiscalFileUtils.zeros(4))                                                                // Reservado para la AEAT
		...
		,T30303000 (mod303 -> true ,new IPropertyFiller[] {
			   ...
			   ,(wr, mod) -> wr.append(AonStringUtils.spaces(546))		                                  // Reservado para la AEAT
			   ,(wr, mod) -> wr.append("</T30303000>")
		})
		...
		,T30305000 (IFiscalModel::isLastPeriod ,new IPropertyFiller[] {
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.text (mod.getDescription(Mod303Key.CT_P1C),4)) // Prorratas - 1 - Código CNAE-2025
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.text (mod.getDescription(Mod303Key.CT_P2C),4)) // Prorratas - 2 - Código CNAE-2025
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.text (mod.getDescription(Mod303Key.CT_P3C),4)) // Prorratas - 3 - Código CNAE-2025
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.text (mod.getDescription(Mod303Key.CT_P4C),4)) // Prorratas - 4 - Código CNAE-2025
		   ...
		   ,(wr, mod) -> wr.append(AonFiscalFileUtils.text (mod.getDescription(Mod303Key.CT_P5C),4)) // Prorratas - 5 - Código CNAE-2025
