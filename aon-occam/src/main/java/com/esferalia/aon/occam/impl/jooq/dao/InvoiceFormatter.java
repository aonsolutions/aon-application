...
public class InvoiceFormatter {
	...
	public static final SimpleDateFormat FMT = new SimpleDateFormat("dd/MM/yy");
	...
	public static String formatInvoices(String title, String subtitle, LinkedList<InvoiceDetail> list) {
		...
		String header = AonStringUtils.repeat(" ", 2)
				...
				+ AonStringUtils.leftPad("PR. MEDIO" ,15)
		...
		for (InvoiceDetail det : list) {
			...
			double price = AonMathUtils.isZero( det.getQuantity())
					?0.0
					:AonMathUtils.round(det.getTaxableBase() / det.getQuantity());
			buf.append(MessageFormat.format(DIV_MSG
					 ,AonStringUtils.repeat(" ", 2)
					+ AonStringUtils.rightPad(det.getInvoice().getDocumentNumber(),15)
					+ AonStringUtils.rightPad(FMT.format(det.getInvoice().getIssueDate()),9)					
					+ AonStringUtils.rightPad(
							AonStringUtils.length(det.getInvoice().getRegistryDocument()) > 9
								?AonStringUtils.abbreviate(det.getInvoice().getRegistryDocument(),9)
