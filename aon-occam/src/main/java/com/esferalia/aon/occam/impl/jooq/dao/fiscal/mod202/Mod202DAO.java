...
//			}
	...
	// SE OBTIENE EL CNAE DE LA ACTIVIDAD PRINCIPAL DE LA EMPRESA (CNAE2009 HASTA 2025 Y CNAE2025 A PARTIR DE 2026)
	static String getMainActivityCNAE(AONContext ctx, Mod202 mod) {
		...
		EnterpriseActivity activity  = CompanyDAO.getEnterpriseActivities(ctx, ctx.getDomainId(), atDate)
			...
			.findFirst()
			.orElse(null);
		// CNAE2005 a partir de 2026, antes CNAE2009, si CNAE2009 no está indicado en la actividad, se coge el equivalente del CNAE2025 
		String cnaeCode = activity == null ? null : mod.getYear() >= 2026 ? activity.getCnae25Code() : activity.getCnaeCode() == null ? activity.getCnae2509Code() : activity.getCnaeCode();
		if (cnaeCode != null) {
			cnaeCode = AonStringUtils.substring(cnaeCode,0,2) + "." + AonStringUtils.substring(cnaeCode,2,4);
		}
		
		if (mod.getYear() >= 2026) {
			// A partir del ejercicio 2026 se graba el CNAE2025
			CNAE2025 cnae2025 = CNAE2025.valueOfCode(cnaeCode);
