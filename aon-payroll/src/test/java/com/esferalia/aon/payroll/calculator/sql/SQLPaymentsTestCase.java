...
public class SQLPaymentsTestCase extends AbstractSQLTestCase {
	...
	public void testSalaryInKindIRPF() throws ExpressionException, SQLException,
		...
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00, salary.getIrpfBase(), 0.001);
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00 + (1200.00 * 9.55 / 100.00), salary.getTotalPayment(), 0.001);
		
		salary.getSalaryDeductions().forEach( p -> {
			System.out.println( p.getDescription() + ": " + p.getAmount() );
		});

		org.junit.assertEquals( 5, salary.getSalaryDeductions().size(), 0.001);
		
		org.junit.assertEquals( salary.getCommonBase() * ( 4.7 + 0.10 + 1.55 ) / 100.00 + salary.getIrpfBase() * 9.55 / 100.00 + 1200.00 * 9.55 / 100.00 , 
				salary.getSalaryDeductions().stream().collect(Collectors.summingDouble(d -> d.getAmount() )), 
				0.001);
		
		org.junit.assertEquals( 1000.00 * 13/12, salary.getRemuneration(), 0.001);
	...
	public void testSalaryInKindIRPFII() throws ExpressionException, SQLException,
		...
		calculator.setSalaryBuilder(new SalaryBuilder() {
			...
			public void addPayment(Double amount, Double quote, Double tax, String description,
				...
				if ( "INGR_CTA_ESP".equals(payment.getName())) {
					...
					org.junit.assertEquals(1200.00, baseCtaEsp.getValue(baseCtaEsp.getPeriod()));
				}
				super.addPayment(amount, quote, tax, description, startDate, endDate, payment, context);
			}
		});
		...
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00, salary.getIrpfBase(), 0.001);
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00 + (1200.00 * irpf / 100.00), salary.getTotalPayment(), 0.001);
		...
		org.junit.assertEquals( salary.getCommonBase() * ( 4.7 + 0.10 + 1.55 ) / 100.00 + salary.getIrpfBase() * irpf / 100.00 + 1200.00 * irpf / 100.00 , 
				salary.getSalaryDeductions().stream().collect(Collectors.summingDouble(d -> d.getAmount() )), 
				0.001);
	...
	public void testSalaryInKindIRPFIII() throws ExpressionException, SQLException,
		...
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00, salary.getIrpfBase(), 0.001);
		org.junit.assertEquals( 1000.00 * 13.00/12.00  + 1200.00 , salary.getTotalPayment(), 0.001);
		
		salary.getSalaryDeductions().forEach( p -> {
			System.out.println( p.getDescription() + ": " + p.getAmount() );
		});

		org.junit.assertEquals( 4, salary.getSalaryDeductions().size(), 0.001);
		
		org.junit.assertEquals( salary.getCommonBase() * ( 4.7 + 0.10 + 1.55 ) / 100.00 + salary.getIrpfBase() * irpf / 100.00  , 
				salary.getSalaryDeductions().stream().collect(Collectors.summingDouble(d -> d.getAmount() )), 
				0.001);
