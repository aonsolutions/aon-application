...
public class SQLCretaTestCase extends AbstractSQLTestCase {
    ...
    public void testCretaPPE() throws ExpressionException, SQLException, SalaryException, JAXBException, IOException, EmptyBasesException, XMLStreamException, FactoryConfigurationError {
		...
		org.junit.assertEquals(1, bases.size());
    ...
    public void testCretaPPEIT() throws ExpressionException, SQLException, SalaryException, JAXBException, IOException, EmptyBasesException, XMLStreamException, FactoryConfigurationError {
		...
		org.junit.assertEquals(3, bases.size());
		assertDato(bases.get(0).getDatosTramo().getDato(), "C", "500");
		assertDato(bases.get(0).getDatosTramo().getDato(), "C", "601");
		assertDato(bases.get(0).getDatosTramo().getDato(), "C", "301");
		double baseCgc = getDato(bases.get(0).getDatosTramo().getDato(), "C", "500");
    ...
    public void testCretaUnpaidMonthly() throws ExpressionException, SQLException, SalaryException, JAXBException, IOException, EmptyBasesException, XMLStreamException, FactoryConfigurationError {
		...
		org.junit.assertEquals(1, trabajadoresTramos.size());
		...
		org.junit.assertEquals( 3 , bases.get(0).getDatosTramo().getDato().size());
    ...
    public void testCretaUnpaidDaily() throws ExpressionException, SQLException, SalaryException, JAXBException, IOException, EmptyBasesException, XMLStreamException, FactoryConfigurationError {
		...
		org.junit.assertEquals(2, trabajadoresTramos.size());
		...
		org.junit.assertEquals(2, bases.size());
		org.junit.assertEquals( 1 , bases.get(0).getDatosTramo().getDato().size());
	...
	org.junit.assertEquals(1, datos.size());
	org.junit.assertEquals(c763 * 100.00, Double.parseDouble(datos.get(0).getValor()), DELTA);
	...
	List<Dato> datos = liquidacion.getDatosLiquidacion().getDato();
	org.junit.assertEquals(1, datos.size());
	org.junit.assertEquals(33300.00, Double.parseDouble(datos.get(0).getValor()), DELTA);
	...
	try {
	    List<net.aonsolutions.core.tgss.creta.jaxb.bases.Tramo> bases = getTramosBases(connection, startDate,
		    endDate, ccc, contract);
	    org.junit.fail("Bases must be empty");
	...
	org.junit.assertEquals(4, datos.size());
	// 500 , 601 , 737 , 06
...
//	assertDatosSolicitado(datoSolicitados, "C", "301", "P");
	...
	org.junit.assertEquals(2, datos.size());
	// 500 , 601 

	double c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(10000, c500, DELTA);

	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(10000, c601, DELTA);
	...
	org.junit.assertEquals(175000 * 15 / monthDays, c500, DELTA);

	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(175000 * 15 / monthDays, c601, DELTA);

	double c737 = datos.stream().filter(d -> d.getCodigo().equals("737")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(15.00 * 1000.00, c737, DELTA);

	double h6 = datos.stream().filter(d -> d.getCodigo().equals("06")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(15.00, h6, DELTA);
	...
	org.junit.assertEquals(175000 * (monthDays - 15) / monthDays, c500, 1);

	c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(175000 * (monthDays - 15) / monthDays, c601, 1);

	c737 = datos.stream().filter(d -> d.getCodigo().equals("737")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals((monthDays - 15) * 1000.00, c737, DELTA);

	h6 = datos.stream().filter(d -> d.getCodigo().equals("06")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals((monthDays - 15), h6, DELTA);
	...
	org.junit.assertEquals(175000 * 1 / 30, c500, DELTA);

	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(175000 * 1 / 30, c601, DELTA);

	double c737 = datos.stream().filter(d -> d.getCodigo().equals("737")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1.00 * 1000.00, c737, DELTA);

	double h6 = datos.stream().filter(d -> d.getCodigo().equals("06")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1.00, h6, DELTA);
    ...
    public void testCretaFormacionEnAlternanciaNormalTutoriaTramosIII()
    	...
    	double c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(175000 * firstPeriodDays / 30.00, c500, DELTA);

    	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(175000 * firstPeriodDays / 30.00, c601, DELTA);

    	double h6 = datos.stream().filter(d -> d.getCodigo().equals("06")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(Math.floor(34.00 * firstPeriodDays / ( monthDays - 5.00)), h6, DELTA);

    	double c737 = datos.stream().filter(d -> d.getCodigo().equals("737")).map(d -> d.getValor())
        		.collect(Collectors.summingDouble(Double::parseDouble));
        	org.junit.assertEquals(( 68.00 / 34.00 * h6 ) * 100.00 , c737, DELTA);
    	...
    	c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(175000 * (monthDays- 5 - firstPeriodDays) / 30.00, c500, 1);

    	c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(175000 * (monthDays - 5 - firstPeriodDays) / 30.00, c601, 1);
    	
    	double previousH6 = h6; 
    	h6 = datos.stream().filter(d -> d.getCodigo().equals("06")).map(d -> d.getValor())
    		.collect(Collectors.summingDouble(Double::parseDouble));
    	org.junit.assertEquals(34.00 - previousH6 , h6, DELTA);
	...
	org.junit.assertEquals(7, datos.size());

	double c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1750.00 * 0.40 * 100.00, c500, DELTA);

	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1750.00 * 0.40 * 100.00, c601, DELTA);

	double c536 = datos.stream().filter(d -> d.getCodigo().equals("536")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1750.00 * 0.60 * 100.00, c536, DELTA);

	double c636 = datos.stream().filter(d -> d.getCodigo().equals("636")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(1750.00 * 0.60 * 100.00, c636, DELTA);
	...
	org.junit.assertEquals(400, h5, DELTA);
	...
	org.junit.assertEquals(Math.ceil(100.00 * 0.75 * 5 * 100), c563, DELTA);
	...
	org.junit.assertEquals(Math.ceil(100.00 * 0.75 * (days) * 100), c563, DELTA);
	...
	org.junit.assertEquals(3, datos.size());

	double c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * 10 / 30 * 100.00), c500, DELTA);

	double c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * 10 / 30 * 100.00), c601, DELTA);
	...
	org.junit.assertEquals(Math.round(100.00 / workedDays * 10.00 * 100.00), h4, DELTA);
	...
	double c509 = datos.stream().filter(d -> d.getCodigo().equals("509")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * (30 - workedDays) / 30 * 100.00), c509, DELTA);

	double c603 = datos.stream().filter(d -> d.getCodigo().equals("603")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * (30 - workedDays) / 30 * 100.00), c603, DELTA);
	...
	c500 = datos.stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * (workedDays - 10) / 30 * 100.00), c500, DELTA);

	c601 = datos.stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(Math.round(1750.00 * (workedDays - 10) / 30 * 100.00), c601, DELTA);

	h4 = datos.stream().filter(d -> d.getCodigo().equals("737")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(Math.round(100.00 / workedDays * (workedDays - 10) * 100.00), h4, DELTA);
	...
	org.junit.assertEquals(1, tramosBases.size());

	tramosBases.get(0).getDatosTramo().getDato().forEach(d -> {
	    org.junit.assertNotEquals("51", d.getCodigo());
	...
	org.junit.assertEquals(_500 * 0.75, _563, DELTA);
...
//		int salaries = calculateAndSave(connection, ctx);
	...
	//@formatter:off
		BonusConceptRecord concept = addBonusConcept(aonContext, 
				...
				"/*read-only*/"+
				"FIN_BONIF=A�O(INICIO_CONTRATO,2);"+
				"TRAMO(FIN_BONIF);"+
				"0.00"+
				"/**/");
...
//		assertEquals(1, nonStructuralBases.size());
	...
	//@formatter:off
		BonusConceptRecord concept = addBonusConcept(aonContext, 
				BonusType.SOCIAL_SECURITY,
				"/*read-only*/"+
				"FIN_BONIF=DIA(A�O(INICIO_CONTRATO,2),9);"+
				"TRAMO(FIN_BONIF);"+
				"0.00"+
				"/**/");
		addBonus(aonContext, contract, concept, null);
...
//		assertEquals(endDate, nonStructuralBases.get(0).getEndDate());
	...
	double _300 = tramoI.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("300"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_300, (1750.00 * 6 / 30) * 100.00, DELTA);
	...
	_300 = tramoII.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("300"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_300, (int) ((1750.00 * 7.00 / 30.00) * 100.00), DELTA);
...

	{
	    ...
	    double _300 = tramoI.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("300"))
		    .map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	    org.junit.assertEquals(_300, (int) ((1750.00 / 30.00) * 100.00), DELTA);
	}
	{
	    net.aonsolutions.core.tgss.creta.jaxb.bases.Tramo tramoII = tramos.get(1);
...
//	}
	...
	double _500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500, (1750.00 * 0.25) * 100, DELTA);
	double _601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601, (1750.00 * 0.25) * 100, DELTA);
	...
	_500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500, (int) (1750.00 * 0.25 / 30.00 * 100.00), DELTA);
	double _603 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("603"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_603, (int) (1750.00 * 0.25 / 30.00 * 100.00), DELTA);
    }
	...
	double _500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500, (1750.00) * 100, DELTA);
	double _601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601, (1750.00) * 100, DELTA);

	tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("01")).findAny()
		.ifPresent(d -> org.junit.fail("H 01 Dato solicitado proporcionado no requerido"));
	;
	...
	double _500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500, Math.round((1750.00 / 30 * 0.03125) * 100), DELTA);
	double _601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601, Math.round((1750.00 / 30 * 0.03125) * 100), DELTA);

	double _01 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("01"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));

	org.junit.assertEquals(_01, 1.00, DELTA);
    }
	...
	double _500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500, (1750.00) * 75, DELTA);
	double _601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601, (1750.00) * 75, DELTA);
	...
	double _500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500 / 100.00, (1750.00 * 0.25 / 30.00 * 29), DELTA);
	double _601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601"))
		.map(d -> d.getValor()).collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601 / 100.00, (1750.00 * 0.25 / 30.00 * 29), DELTA);
	...
	_500 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("500")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_500 / 100.00, (1750.00 * 0.50 / 30.00), DELTA * 1000);
	_601 = tramo.getDatosTramo().getDato().stream().filter(d -> d.getCodigo().equals("601")).map(d -> d.getValor())
		.collect(Collectors.summingDouble(Double::parseDouble));
	org.junit.assertEquals(_601 / 100.00, (1750.00 * 0.50 / 30.00), DELTA * 1000);
...
//		addPayment(aonContext, contract, pagaExtra, "ROUND(INPUT(\"/*user*/P_0 + P_1/**/\",\"...\"),2)");
	...
	tramo1.getDatosTramo().getDato().stream().filter(d -> !d.getCodigo().equals("500"))
		...
		.ifPresent(d -> org.junit.fail("Dato solicitado proporcionado no requerido"));
	;
...
//		}
	...
	AON.getSalaryData(aonContext,
		...
		.forEach(salary -> {
		    ...
		    assertEquals(startIt, datas.get(1).getStartDate());
		    assertEquals(endIt, datas.get(1).getEndDate());
		    org.junit.assertTrue(Double.parseDouble(datas.get(1).getExpression()) > 0.00);
	...
	assertEquals(2, tramosBases.size());
	org.junit.assertEquals("01", tramosBases.get(0).getFechaDesde().getDia());
	org.junit.assertEquals("01", tramosBases.get(0).getFechaHasta().getDia());

	org.junit.assertEquals("02", tramosBases.get(1).getFechaDesde().getDia());
    }
	...
	org.junit.assertEquals(Integer.parseInt(anho),
		Integer.parseInt(liquidacion.getPeriodoDesde().getAnho()));
	org.junit.assertEquals(Integer.parseInt(mes), Integer.parseInt(liquidacion.getPeriodoDesde().getMes()));
	org.junit.assertEquals(Integer.parseInt(anho),
		Integer.parseInt(liquidacion.getPeriodoHasta().getAnho()));
	org.junit.assertEquals(Integer.parseInt(mes), Integer.parseInt(liquidacion.getPeriodoHasta().getMes()));

	org.junit.assertEquals(2, liquidacion.getLiquidacionMes().size());
	...
	org.junit.assertEquals(Integer.parseInt(mes),
	...
	org.junit.assertEquals(1, trabajador.getTramos().getTramo().size());

	Tramo tramo0 = trabajador.getTramos().getTramo().get(0);
	org.junit.assertEquals(Integer.parseInt(mes), Integer.parseInt(tramo0.getFechaDesde().getMes()));
	org.junit.assertEquals(Integer.parseInt(anho), Integer.parseInt(tramo0.getFechaDesde().getAnho()));
	...
	org.junit.assertEquals(Integer.parseInt(anho1),
		Integer.parseInt(liquidacionMes.getMesLiquidativo().getAnho()));
	org.junit.assertEquals(Integer.parseInt(mes1),
		Integer.parseInt(liquidacionMes.getMesLiquidativo().getMes()));
	...
	org.junit.assertEquals(calendar.get(Calendar.MONTH) + 1,
		Integer.parseInt(tramo0.getFechaDesde().getMes()));
	org.junit.assertEquals(calendar.get(Calendar.YEAR), Integer.parseInt(tramo0.getFechaDesde().getAnho()));
	...
	org.junit.assertEquals(1, liquidacion.getLiquidacionMes().size());
	...
	org.junit.assertEquals(year, Integer.parseInt(liquidacionMes.getMesLiquidativo().getAnho()));
	org.junit.assertEquals(month, Integer.parseInt(liquidacionMes.getMesLiquidativo().getMes()));
	...
	org.junit.assertEquals(calendar.get(Calendar.YEAR), Integer.parseInt(tramo1.getFechaDesde().getAnho()));
	...
	List<net.aonsolutions.core.tgss.creta.jaxb.bases.Tramo> bases = getBases(connection, trabajadoresTramos);
	org.junit.assertEquals(1, bases.size());
	org.junit.assertEquals(1, Integer.parseInt(bases.get(0).getFechaDesde().getDia()));
	org.junit.assertEquals(get(endDate, DAY_OF_MONTH),
		Integer.parseInt(bases.get(0).getFechaHasta().getDia()));
	org.junit.assertEquals(175000,
	...
	org.junit.assertEquals(2, bonusBases.size());

	org.junit.assertEquals(1, Integer.parseInt(bonusBases.get(0).getFechaDesde().getDia()));
	org.junit.assertEquals(9, Integer.parseInt(bonusBases.get(0).getFechaHasta().getDia()));

	org.junit.assertEquals(10, Integer.parseInt(bonusBases.get(1).getFechaDesde().getDia()));
	org.junit.assertEquals(get(endDate, DAY_OF_MONTH),
		Integer.parseInt(bonusBases.get(1).getFechaHasta().getDia()));

	org.junit.assertEquals(Math.round(175000.00 * 9 / get(endDate, DAY_OF_MONTH)),
	...
	List<net.aonsolutions.core.tgss.creta.jaxb.bases.Tramo> bases = getBases(connection, trabajadoresTramos);
	org.junit.assertEquals(4, bases.size());
	org.junit.assertEquals(1, Integer.parseInt(bases.get(0).getFechaDesde().getDia()));
...
//		net.aonsolutions.core.tgss.creta.jaxb.bases.Tramo tramo3 = bTramos.get(3);
	...
	org.junit.fail("EmptyBasesException expected");
	...
	org.junit.assertEquals(5, tramo.getDatosTramo().getDatoSolicitado().size());
    ...
    public void testCretaHoursAndDrop() throws ExpressionException, SQLException, SalaryException, JAXBException,
		...
		org.junit.assertEquals(salaryHours, Double.parseDouble(_1.getValor()), 0.00);
...
//				"TRACE('DIAS_TRABAJADOS=%f\r\n', DIAS_TRABAJADOS); 0.00;"
	...
	// PARTE JORNADA EN SITUACI�N DE DESCANSO
	assertDatosSolicitado(datoSolicitados, "C", "535", "B");
	try {
	    assertDatosSolicitado(datoSolicitados, "C", "635", "B");
	} catch (AssertException e) {
