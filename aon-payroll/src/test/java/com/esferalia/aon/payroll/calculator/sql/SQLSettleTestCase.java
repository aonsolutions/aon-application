...
public class SQLSettleTestCase extends AbstractSQLTestCase {
	...
	public void testSettleSeniority() throws ExpressionException, SQLException, SalaryException {
		...
		double fixFeb = Math.min(1, Math.abs(get(seniority, DAY_OF_MONTH) - get(getToday(), DAY_OF_MONTH))); //0;
		...
		for ( ITimedResult<Object> result:  ctx.getExpressionContext().eval("AÑOS_TRABAJADOS", seniority, getToday()))
			Assert.assertEquals( (2.00 + 2/12.00 + fixFeb/12.00), result.getValue());
		...
		for ( ITimedResult<Object> result:  ctx.getExpressionContext().eval("12.00 * AÑOS_TRABAJADOS * SALARIO_DIA", seniority, getToday())) {
			Assert.assertEquals( seniority, result.getPeriod().getStart());
			Assert.assertEquals( 12.00 * (2.00 + 2/12.00+ fixFeb/12.00) * br , result.getValue());
		}
		...
		Assert.assertEquals( 12 * (2.00 + 2/12.00 + fixFeb/12.00) * br, settle.getTotalPayment(), DELTA);
		Assert.assertEquals( 12 * ( 2 + 2/12.00 + fixFeb/12.00) * br, settle.getTotalLiquid(), DELTA);
	}
	...
	public void testSettleContractEnd() throws ExpressionException, SQLException, SalaryException {
		...
		double fixFeb = Math.min(1, Math.abs(get(contractStart, DAY_OF_MONTH) - get(getToday(), DAY_OF_MONTH)));
		Assert.assertEquals( 12 * (2+fixFeb)/12.00 * br, settle.getTotalPayment(), DELTA);
		Assert.assertEquals( 12 * (2+fixFeb)/12.00 * br , settle.getTotalLiquid(), DELTA);
	}
	...
	public void testSettleObjective() throws ExpressionException, SQLException, SalaryException {
		...
		double fixFeb = Math.min(1, Math.abs(get(contractStart, DAY_OF_MONTH) - get(getToday(), DAY_OF_MONTH)));
		Assert.assertEquals( 20 * (2+fixFeb)/12.00 * br, settle.getTotalPayment(), DELTA);
		
		System.out.println( (20 * (2+fixFeb)/12.00 * br ) + " = " + settle.getTotalPayment() );
		
		Assert.assertEquals( 20 * (2+fixFeb)/12.00 * br, settle.getTotalLiquid(), DELTA);
	}
...
//		settle.getSalaryPayments().stream().forEach(p->System.out.println(p.getExpression() + " = "  + p.getAmount() ));
		...
		double fixFeb = Math.min(1, Math.abs(get(contractStart, DAY_OF_MONTH) - get(getToday(), DAY_OF_MONTH)));
		Assert.assertEquals( 20 * (2+fixFeb)/12.00 * br + ( br * 4 ), settle.getTotalPayment(), DELTA);
...
//		settle.getSalaryPayments().stream().forEach(p->System.out.println(p.getExpression() + " = "  + p.getAmount() ));
	...
	public void testSettleVacations2Month() throws ExpressionException, SQLException, SalaryException {
		...
		double fixFeb = Math.min(1, Math.abs(get(contractStart, DAY_OF_MONTH) - get(getToday(), DAY_OF_MONTH)));
		Assert.assertEquals( 20 * (2+fixFeb)/12.00 * br + ( br * (10 + noHolidays) ), settle.getTotalPayment(), DELTA);
