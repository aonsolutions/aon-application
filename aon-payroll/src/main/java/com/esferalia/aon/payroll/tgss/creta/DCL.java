...
import static com.esferalia.aon.payroll.tgss.creta.Bases.toCalendar;
...
public class DCL {
	...
	private static class LineaSalary {
	...
	private static boolean checkEquals(LineaDCL lineaDCL, LineaSalary lineaSalary, int delta, String ansiSuccess, String ansiError, String ansiWarn){
	...
	private static boolean isCotizacion(Deduction  deduction){
		if ( ! (deduction.isJobTraining() ||
				deduction.isUnemployment() ||
				deduction.isCommonContingency() ||
				deduction.isProfesionalContingency()) )
		System.out.println(deduction.getDescription() +" " + deduction.getAmount() + "(" + 
				(deduction.isJobTraining() ||
						deduction.isUnemployment() ||
						deduction.isCommonContingency() ||
						deduction.isProfesionalContingency())
		+ ")");
		
		return  deduction.isJobTraining() ||
				deduction.isUnemployment() ||
				deduction.isCommonContingency() ||
				deduction.isProfesionalContingency()
				;
	...
	private static double getCotizaciones(Salary salary) {
		...
		importe = salary.getDeductions().stream()
		.filter(DCL::isCotizacion)
		.collect(Collectors.summingDouble(deduction-> deduction.getAmount() * 100.00 ));
		importe += salary.getCosts().stream()
	...
	private static double getBonificaciones(Salary salary) {
		...
		importe = salary.getBonuses().stream()
		.collect(Collectors.summingDouble(bonus-> bonus.getAmount() * 100.00 ));
		...
		for( DeductionType type : types ) {
			importe += type.accept(new DeductionType.Visitor<Double>() {
				...
				private Double visit(Predicate<Deduction> filter) {
					return salary.getDeductions().stream()
					.filter( filter )
					.collect(Collectors.summingDouble(deduction-> deduction.getAmount()*100.00 ));
				}
			});
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException, JAXBException, IOException {
		...
		Option delta = OptionBuilder.withArgName("delta")
				.hasArg()
				.withType(Integer.class)
				.withLongOpt("delta")
				.withDescription("Asserts that two values are equal concerning a delta.")
				.create();
		...
		try {
			...
			InputStream is = cmd.getArgList().isEmpty() ? 
					System.in
					: new FileInputStream(cmd.getArgs()[0]);

			
			net.aonsolutions.tgss.creta.jaxb.dcl.DCL dcl = 
					Utils.unmarshal(net.aonsolutions.tgss.creta.jaxb.dcl.DCL.class, is);
			...
			lineasSalary.put(LIQUIDO_DE_ACCIDENTES_DE_TRABAJO, new LineaSalary());
			...
			Calendar desde = toCalendar(dcl.getLiquidacion().getPeriodoDesde());
			desde.set(Calendar.DAY_OF_MONTH,
					desde.getActualMinimum(Calendar.DAY_OF_MONTH));
			Date startDate = desde.getTime();
			
			Calendar hasta = toCalendar(dcl.getLiquidacion().getPeriodoHasta());
			...
			LineaSalary totalLiquidLineaSalary = lineasSalary.get(LIQUIDO_DE_TOTALES); 
			...
			.forEach(salary->{
				cgcLineaSalary.base += salary.getCommonContingenciesBase() * 100.00;
				cgcLineaSalary.importe += getImporte(salary, DeductionType.COMMON_CONTINGENCY);
				
				itLineaSalary.base += salary.getProfessionalContingenciesBase() * 100.00;
				itLineaSalary.importe += getImporte(salary, DeductionType.IT);

				imsLineaSalary.base += salary.getProfessionalContingenciesBase() * 100.00;
				imsLineaSalary.importe += getImporte(salary, DeductionType.IMS);

				otherLineaSalary.base += salary.getCommonContingenciesBase() * 100.00;
				otherLineaSalary.importe += getImporte(salary, 
						DeductionType.JOB_TRAINING ,
						DeductionType.UNEMPLOYMENT,
						DeductionType.FOGASA);
				
				totalLiquidLineaSalary.importe += getCotizaciones(salary) -  getBonificaciones(salary); 
			});
			;
			LineaSalary atLineaSalary = lineasSalary.get(LIQUIDO_DE_ACCIDENTES_DE_TRABAJO); 
			atLineaSalary.importe = itLineaSalary.importe + imsLineaSalary.importe;

			String ansiError = cmd.hasOption(pretty.getLongOpt()) ? ANSI_RED:"";
			String ansiSucces = cmd.hasOption(pretty.getLongOpt()) ? ANSI_GREEN:"";
			...
			for( LineaDCL lineaDCL: dcl.getLiquidacion().getDatosCuerpoDCL().getLineasDCL().getLineaDCL() )
				if ( lineasSalary.containsKey(lineaDCL.getDescripcionLDCL()) )
					checkEquals(lineaDCL, lineasSalary.get(lineaDCL.getDescripcionLDCL()), deltaValue ,ansiSucces, ansiError, ansiWarning);
