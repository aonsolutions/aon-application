	...
	private static final Map<String, CretaData> CONTEXT_VARIABLE_MAP = new HashMap<String, CretaData>() {
		{
			...
			put("01", new DistributeHCretaData(SALARY_HOURS.getName()) {
				...
				public Double getDefaultHours(Salary salary, Fecha desde, Fecha hasta)
						...
						UnMatchedVariableException {
					try {
						double salaryHours =  super.get(salary, desde, hasta);
						return Math.max(1.00, salaryHours);
					} catch (NoSuchVariableException e) {
					}
					
					double workedHours = getWorkedHours(salary, desde, hasta);
					return Math.max(1.00, workedHours);
				};
