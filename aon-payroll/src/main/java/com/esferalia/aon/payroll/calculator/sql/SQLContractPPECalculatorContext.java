...
public class SQLContractPPECalculatorContext extends SQLContractSalaryCalculatorContext {
	...
	public SQLContractPPECalculatorContext(Connection connection, java.util.Date startDate, java.util.Date endDate,
			java.util.Date issueDate, java.util.Date chargeDate, Criteria criteria) throws SQLException, ExpressionException {
	...
	public Collection<IContractPayment> getContractPayments() throws AonException {
		readDelayCause();
		return new FilterCollection<IContractPayment>(p -> ContextVariable.PPE.equals(p.getName()),super.getContractPayments()) {
			@Override
	...
	private void readDelayCause() {
		...
		try {
			...
			getExpressionContext().putVariable(
					...
					new ExpressionVariable<>(PaymentType.CRA_0033, 
