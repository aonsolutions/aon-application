...
public class SQLContractSettleCalculatorContext extends SQLContractSalaryCalculatorContext {
	...
	private SystemPayment salaryDayPayment;
	...
	private double calculateDaySalary() throws ExpressionException, SQLException, SalaryException {
		...
		salaryDayPayment.setExpression(String.format(Locale.US, "HIDE(\""
				+ "<div>No existe nomina emitida para este perido, por favor recuerde emitirla.</div>"
		...
		contractEnd.setTime(this.settleEndDate);
	...
	private double getDaySalaryDB() {
		...
		Result<Record> salary = dslContext.select().from(SALARY)
			...
			.and(SALARY.START_DATE.le(new java.sql.Date(this.settleEndDate.getTime())))
			.and(SALARY.END_DATE.ge(new java.sql.Date(this.settleEndDate.getTime())))
			.fetch();
	...
	public Collection<IContractPayment> getContractPayments() throws AonException {
		salaryDayPayment = salaryDayPayment();
		return new CompositePayments<IContractPayment>(super.getContractPayments(), Collections.singleton(salaryDayPayment));
	}
	
	private SystemPayment salaryDayPayment() {
		SystemPayment salaryDayPayment = new SystemPayment();
		
		salaryDayPayment.setType(PaymentType.CRA_0000);
		salaryDayPayment.setSalaryType(SalaryType.SETTLE);
		salaryDayPayment.setStartDate(getStart());
		salaryDayPayment.setEndDate(getEnd());
		salaryDayPayment.setExpression("HIDE()");
		
		return salaryDayPayment;
	}
