...
public class CalendarController_ extends BasicController {

	private static final Logger LOGGER = LoggerFactory.getLogger(CalendarController_.class.getName());
...
//	private Integer sourceId;
	...
	public String getSourceFullName() {
		String name=null;
		if(getContract()!=null){
			name = getContract().getPerson().getRegistry().getFullName();
		} else if(getWorkPlace()!=null){
			name = getWorkPlace().getDescription();
		} else if(getEnterprise()!=null){
			name = getEnterprise().getRegistry().getFullName();
		}
...
//	}
	...
	public void loadSource() throws ManagerBeanException {
		...
		if(c.getTo()!=null){
			setEnterprise((Enterprise) c.getTo());
		}
		c = FormUtil.getController(ICompanyConstants.WORK_PLACE_CONTROLLER_NAME);
		if(c.getTo()!=null){
			setWorkPlace((WorkPlace) c.getTo());
		}
		c = FormUtil.getController(ICompanyConstants.CONTRACT_CONTROLLER_NAME);
		if(c.getTo()!=null){
			setContract((Contract) c.getTo());
		}
