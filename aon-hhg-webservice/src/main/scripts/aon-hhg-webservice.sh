...
for jar in /usr/share/java/aon-hhg-webservice/*.jar ; do 
...
case $1 in
	help)
  		...
  		echo "aon hhg webservice command-line client."
	...
	get-nonce) 
    		$JAVA -classpath $CLASSPATH com.code.aon.hhg.webservice.rpm.GetNonceRPM "${@:2}" 
    	;;

	hello) 
    		$JAVA -classpath $CLASSPATH com.code.aon.hhg.webservice.rpm.HelloRPM "${@:2}" 
    	;;

	booking-harvest) 
    		$JAVA -classpath $CLASSPATH com.code.aon.hhg.webservice.rpm.BookingHarvestRPM "${@:2}" 
    	;;

	auth) 
    		$JAVA -classpath $CLASSPATH com.code.aon.hhg.webservice.rpm.AuthRPM "${@:2}" 
    	;;
