...
public class CuotaRetencionNavarraListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		CuotaRetencionNavarra n = (CuotaRetencionNavarra) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(CuotaRetencionNavarra.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.CUOTA_RETENCION_NAVARRA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				CuotaRetencionNavarra cra = (CuotaRetencionNavarra) iter.next();
