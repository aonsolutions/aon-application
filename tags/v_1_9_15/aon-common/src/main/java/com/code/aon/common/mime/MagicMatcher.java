...
{
    private static Logger log = Logger.getLogger(MagicMatcher.class.getName());
    ...
    {
        log.fine("setSubMatchers(): for match '" + match.getDescription() + "'");
        subMatchers.clear();
        ...
        throws IOException, UnsupportedTypeException
    {
        ...
        log.fine("test(File): testing '" + f.getName() + "' for '" + description + "'");
        ...
        try {
            ...
            log.fine("test(File): stream size is '" + buf.length + "'");
        ...
        throws IOException, UnsupportedTypeException
    {
        ...
        log.fine("test(byte[]): testing byte[] data for '" + description + "'");
        ...
        byte[] buf = new byte[length];
        log.fine("test(byte[]): offset=" + offset + ",length=" + length + ",data length=" +
            data.length);

        if ((offset + length) < data.length) {
            System.arraycopy(data, offset, buf, 0, length);

            log.fine("test(byte[]): stream size is '" + buf.length + "'");
            ...
            if (testInternal(buf)) {
                ...
                log.fine("test(byte[]): testing matched '" + description + "'");

                // set the data on this match
                if ((onlyMimeMatch == false) && (subMatchers != null) && (subMatchers.size() > 0)) {
                    log.fine("test(byte[]): testing " + subMatchers.size() + " submatches for '" +
                        description + "'");

                    for (int i = 0; i < subMatchers.size(); i++) {
                        log.fine("test(byte[]): testing submatch " + i);

                        MagicMatcher m = (MagicMatcher) subMatchers.get(i);

                        if ((submatch = m.test(data, false)) != null) {
                            log.fine("test(byte[]): submatch " + i + " matched with '" +
                                submatch.getDescription() + "'");
                            match.addSubMatch(submatch);
                        } else {
                            log.fine("test(byte[]): submatch " + i + " doesn't match");
                        }
                    }
                }
            }
    ...
    private boolean testInternal(byte[] data)
    {
        ...
        if ((type != null) && (test != null) && (test.length() > 0)) {
            if (type.equals("string")) {
                ...
                log.severe("testInternal(byte[]): invalid test type '" + type + "'");
            }
        } else {
            log.severe("testInternal(byte[]): type or test is empty for '" + mimeType + " - " +
                description + "'");
        }
    ...
    private boolean testByte(ByteBuffer data)
    {
        ...
        byte b = data.get(0);
        b = (byte) (b & bitmask);
        log.fine("testByte(): decoding '" + test + "' to byte");

        int tst = Integer.decode(test).byteValue();
        byte t = (byte) (tst & 0xff);
        log.fine("testByte(): applying bitmask '" + bitmask + "' to '" + tst + "', result is '" +
            t + "'");
        log.fine("testByte(): comparing byte '" + b + "' to '" + t + "'");
    ...
    private boolean testString(ByteBuffer data)
    {
        ...
        for (i = 0; i < t.length; i++) {
            log.fine("testing byte '" + b[i] + "' from '" + new String(data.array()) +
    ...
    private boolean testShort(ByteBuffer data)
    {
        ...
        log.fine("testShort(): testing '" + Long.toHexString(val) + "' against '" +
    ...
    private boolean testLong(ByteBuffer data)
    {
        ...
        log.fine("testLong(): testing '" + Long.toHexString(val) + "' against '" + test +
            "' => '" + Long.toHexString(tst) + "'");
    ...
    private boolean testRegex(String text)
    {
        ...
        log.fine("testRegex(): searching for '" + test + "'");
    ...
    {
        log.fine("testDetector()");

        String detectorClass = new String(match.getTest().array());

        try {
            log.fine("loading class: " + detectorClass);
            ...
            log.log( Level.SEVERE, "failed to load detector: " + detectorClass, e);
        } catch (InstantiationException e) {
            log.log( Level.SEVERE, "specified class is not a valid detector class: " + detectorClass, e);
        } catch (IllegalAccessException e) {
            log.log( Level.SEVERE, "specified class cannot be accessed: " + detectorClass, e);
        }
