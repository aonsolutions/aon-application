...
public class MinoracionesGuipuzcoaListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		MinoracionesGuipuzcoa n = (MinoracionesGuipuzcoa) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(MinoracionesGuipuzcoa.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.MINORACIONES_GUIPUZCOA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				MinoracionesGuipuzcoa cra = (MinoracionesGuipuzcoa) iter.next();
