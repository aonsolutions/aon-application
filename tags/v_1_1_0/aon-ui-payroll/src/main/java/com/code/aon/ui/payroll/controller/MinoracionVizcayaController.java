...
public class MinoracionVizcayaController extends PayrollBasicController {
	...
	public void onSearch(ActionEvent event) {
		...
		try {
			if(searchFecini != null){
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.MINORACIONES_VIZCAYA_ID_FECINI), searchFecini);
			}
			if(searchFecfin != null){
				//getCriteria().addEqualExpression(getFieldName(event.getComponent().getId()), searchFecfin);
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.MINORACIONES_VIZCAYA_FECFIN), searchFecfin);
			}
