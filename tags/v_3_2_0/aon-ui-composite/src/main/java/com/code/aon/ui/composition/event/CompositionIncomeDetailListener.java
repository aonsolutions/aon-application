		...
		try {
			...
			if(composition != null){
				...
				while(iter.hasNext()){
                    ...
                    ((IncomeDetail)event.getController().getTo()).setDescription(compositionDetail.getDescription() + "( " + composition.getItem().getProduct().getName() + " )");
					...
					if(composition.isPriceInDetails()){
						((IncomeDetail)event.getController().getTo()).setPrice(compositionDetail.getPrice());
					}else{
						((IncomeDetail)event.getController().getTo()).setPrice(0.0);
					}
