...
public class CreditorLoaderManager {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(CreditorLoaderManager.class.getName());
	...
	private static final String[] SUPPORTED_ENTITIES = {ACR};
	...
	public CreditorLoaderManager(Scope scope, SecurityLevel securityLevel,WorkPlace workPlace,PrintWriter log) {
		this.log = log;
		this.scope = scope;
	...
	public void loadMetadata(InputStream input) throws AonException {
		...
		try {
			...
			if (!getColumns().containsKey(ACR)) {
				log("Meta: CLI entity metadata not found in file. Default used;");
				getColumns().put(ACR, SUPPORTED_COLUMNS[0]);
			}
	...
	private void insert(Object bean) throws AonException {
		if (bean instanceof LoadedCreditor) {
			insertCreditor( (LoadedCreditor) bean);
		}
	}

	private Object getTargetBean(String entity) {
		if (ACR.equals(entity)) {
	...
	private void insertCreditor(LoadedCreditor loaded) throws ManagerBeanException {
	...
	private RegistryBank insertRegistryBank(Registry registry,LoadedCreditor loaded) throws ManagerBeanException {
	...
	private void insertRegistryPayMethod(Registry registry,RegistryBank rbank,LoadedCreditor loaded) throws ManagerBeanException {
	...
	private void insertRegistryAddress(Registry registry, LoadedCreditor loaded) throws ManagerBeanException {
