...
node {
   ...
   if ( MAVEN_RELEASE == 'true' ) {
      ...
      sh "echo yes | ${mvnHome}/bin/mvn  -DdevelopmentVersion=${mavenRelease['developmentVersion']} -DreleaseVersion=${mavenRelease['releaseVersion']} -Dusername=${mavenRelease['username']} -Dpassword=${mavenRelease['password']} -Dtag=${mavenRelease['tag']} -Dresume=false -DdryRun=${mavenRelease['dryRun']} -DscmCommentPrefix=${mavenRelease['scmCommentPrefix']} -Dproject.rel.net.aonsolutions:sii=${mavenRelease['releaseVersion']} -Dproject.dev.net.aonsolutions:sii=${mavenRelease['developmentVersion']} -Dproject.rel.net.aonsolutions:aeat=${mavenRelease['releaseVersion']} -Dproject.dev.net.aonsolutions:aeat=${mavenRelease['developmentVersion']} -Darguments='-Drpm.release=true -DskipTests=true -Dgwt.localWorkers=4' release:prepare"
