...
public class ProductionUnloadManager implements IDataLoadConstants {
	...
	public void processProductionList(Parameters params) {
		...
		try {
			...
			for (Produccion prod : produccionList.getProduccion()) {
				BandejaImportesProduccion importes = factory.createBandejaImportesProduccion();
				importes.setKey("0");
				importes.setIdentificativoHotel(prod.getIdentificativoHotel());
				importes.setFechaProduccion(prod.getFechaProduccion());
				importes.setConcepto("FIN");
				importes.setSubconcepto("FIN");
				importes.setCantidad(BigDecimal.valueOf(0));
				importes.setDescripcion("FIN");
				importes.setImporte(BigDecimal.valueOf(0));
				prod.getBandejaImportesProduccion().getBandejaImportesProduccion().add(importes);
	...
	private void removePreviousData(Parameters params) throws Exception {
		String sql = "DELETE FROM data_attach WHERE source = " + DataAttachmentSource.PRODUCTION.ordinal() + " AND source_id = ?";
		PreparedStatement stmt = null;
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setObject(1, params.getProductionId(), Types.INTEGER);
			stmt.executeUpdate();
		} catch (SQLException ex) {
			throw ex;
		} finally {
