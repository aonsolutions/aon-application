...
public class ItemAlternativeControllerListener extends ControllerAdapter {
	...
	private	Integer calculateNextPriority(Item item) throws ManagerBeanException {
		...
		criteria.addEqualExpression(itemAlternativeBean.getFieldName(IEntityAlias.ITEM_ALTERNATIVE_ITEM_ID), item.getId());
		Projection projection = Projection.max(itemAlternativeBean.getFieldName(IEntityAlias.ITEM_ALTERNATIVE_PRIORITY));
