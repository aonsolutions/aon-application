#/bin/sh
#
# aon-google-calendar     This shell script takes care of manage google drive
#
...
case $1 in
  help)
  	...
  	echo "aon Google Calendar command-line client."
    ...
    ;;
  sync) 
    $JAVA -classpath $CLASSPATH com.code.aon.google.apis.calendar.SynchronizeEvents "${@:2}" 
    ;;
  search) 
    $JAVA -classpath $CLASSPATH com.code.aon.google.apis.calendar.SearchEvents "${@:2}" 
    ;;
  delete)
    $JAVA -classpath $CLASSPATH com.code.aon.google.apis.calendar.DeleteEvents "${@:2}" 
    ;;
  deleteCal)
    $JAVA -classpath $CLASSPATH com.code.aon.google.apis.calendar.DeleteCalendars "${@:2}" 
    ;;
  *) 
