	...
	public HashMap<String, String> getValues(String domainName, Integer domainId, Integer drId){
		...
		if(idOptional.isPresent()){
			...
			if("0.0".equals(map.get(QualitySheetCode.UFQC2.getName()))){
				...
				LinkedList<DataResponseDetail> dddd = AON.getDataResponseDetailStream(domainName, domainId, login, f -> 
				f.getDataResponseProperty().eq(drId).and(f.getDataVariableProperty().eq(QualitySheetCode.UFQC2.getName())))
				...
				if(dddd.size()> 1) {
					...
					for(Integer j = 1; j < dddd.size(); j++) {
						Integer n = j;
						AON.deleteDataResponseDetail(domainName, domainId, login, f -> f.getIdProperty().eq(dddd.get(n).getId()));
					}
				}
			}
 		}
