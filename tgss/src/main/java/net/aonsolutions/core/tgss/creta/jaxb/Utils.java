...
public class Utils {
	...
	public static <T> T unmarshal(Class<T> clazz, InputStream in) throws JAXBException {
		try {
			...
			return (T) newUnmarshaller(clazz).unmarshal(ss);
		} catch (SAXException | ParserConfigurationException e1) {
			throw new JAXBException(e1);
		}
//		return (T) newUnmarshaller(clazz).unmarshal(in);
	}

	@SuppressWarnings("unchecked")
	public static <T> T unmarshal(Class<T> clazz, Reader reader) throws JAXBException {
		try {
			XMLReader xmlreader = new NamespaceFilterXMLReader();
			InputSource is = new InputSource(reader);
			SAXSource ss = new SAXSource(xmlreader, is);
			return (T) newUnmarshaller(clazz).unmarshal(ss);
		} catch (SAXException | ParserConfigurationException e1) {
			throw new JAXBException(e1);
		}
//		return (T) newUnmarshaller(clazz).unmarshal(reader);
	...
	private static Marshaller newMarshaller(Class classToBeBound) throws JAXBException {
		Marshaller marshaller = newJAXBContext(classToBeBound).createMarshaller();
		// marshaller.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.TRUE);
		marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
		return marshaller;
	}

	private static Unmarshaller newUnmarshaller(Class classToBeBound) throws JAXBException {
	...
	public static String toString(CtaCot ctaCot) {
		return String.format("%s%s%s", ctaCot.getProvincia(), ctaCot.getRegimen(), ctaCot.getNumero());
