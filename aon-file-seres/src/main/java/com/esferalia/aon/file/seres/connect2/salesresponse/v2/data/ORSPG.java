...
public class ORSPG {
	...
	private static Pattern PATTERN_ORSPG_codigoLugarDeEntrega = Pattern.compile("^.{6}(.{25}).*");
	private static Pattern PATTERN_ORSPG_agenciaResponsableListaDeCodigos = Pattern.compile("^.{31}(.{3}).*");
	private static Pattern PATTERN_ORSPG_lugarDeEntrega_textoLibre = Pattern.compile("^.{34}(.{70}).*");
	private static Pattern PATTERN_ORSPG_fecha_horaDeEntregaRequerida_2_ = Pattern.compile("^.{104}(.{12}).*");
	private static Pattern PATTERN_ORSPG_cantidadEnvioDividida_11_ = Pattern.compile("^.{116}(.{16}).*");
	private static Pattern PATTERN_ORSPG_calificadorUnidadDeMedida = Pattern.compile("^.{132}(.{6}).*");
	
	public void parse(String value) {
		Matcher m;
		if((m = PATTERN_ORSPG_codigoLugarDeEntrega.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setCodigoLugarDeEntrega(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ORSPG_agenciaResponsableListaDeCodigos.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setAgenciaResponsableListaDeCodigos(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ORSPG_lugarDeEntrega_textoLibre.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setLugarDeEntrega_textoLibre(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ORSPG_fecha_horaDeEntregaRequerida_2_.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setFecha_horaDeEntregaRequerida_2_(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ORSPG_cantidadEnvioDividida_11_.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setCantidadEnvioDividida_11_(Double.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ORSPG_calificadorUnidadDeMedida.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
	...
	/**
	 * 4 - Lugar de entrega, texto libre
	 */
	...
	/**
	 * 7 - Calificador unidad de medida
	 */
