...
/** 
 * This is an AUTOMATICALLY GENERATED class for represent EDI ERE1E entity.
 * <br>
 ...
 * 	</tr>
 * 	<tr>
 * 		 <td>ERE1E</td> <td>Descuentos y cargos línea</td> <td>Opcional</td> <td>N</td>
 * 	</tr>
 * </table>
 */ 
...
public class ERE1E {
	...
	private static Pattern PATTERN_ERE1E_numeroDescuento_Cargo = Pattern.compile("^.{6}(.{2}).*");
	private static Pattern PATTERN_ERE1E_indicadorDescuento_Cargo_A_C_ = Pattern.compile("^.{8}(.{1}).*");
	private static Pattern PATTERN_ERE1E_indicadorSecuenciaDeCalculo = Pattern.compile("^.{9}(.{3}).*");
	private static Pattern PATTERN_ERE1E_tipoDescuento_Cargo = Pattern.compile("^.{12}(.{3}).*");
	private static Pattern PATTERN_ERE1E_descripcionDescuento_Cargo = Pattern.compile("^.{15}(.{70}).*");
	private static Pattern PATTERN_ERE1E_porcentajeDescuento_Cargo = Pattern.compile("^.{85}(.{9}).*");
	private static Pattern PATTERN_ERE1E_importeDescuento_Cargo = Pattern.compile("^.{94}(.{18}).*");
	private static Pattern PATTERN_ERE1E_importeTotalSujetoAAplicacion_13_ = Pattern.compile("^.{112}(.{18}).*");
	private static Pattern PATTERN_ERE1E_descuentosMonetariosPorUnidad = Pattern.compile("^.{130}(.{18}).*");
	private static Pattern PATTERN_ERE1E_unidadDeMedida = Pattern.compile("^.{148}(.{6}).*");
	private static Pattern PATTERN_ERE1E_cantidadDescuento_Cargo = Pattern.compile("^.{154}(.{16}).*");

	public void parse(String value) {
		Matcher m;
		if((m = PATTERN_ERE1E_numeroDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setNumeroDescuento_Cargo(Integer.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_indicadorDescuento_Cargo_A_C_.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setIndicadorDescuento_Cargo_A_C_(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_indicadorSecuenciaDeCalculo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setIndicadorSecuenciaDeCalculo(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_tipoDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setTipoDescuento_Cargo(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_descripcionDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setDescripcionDescuento_Cargo(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_porcentajeDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setPorcentajeDescuento_Cargo(Double.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_importeDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setImporteDescuento_Cargo(Double.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_importeTotalSujetoAAplicacion_13_.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setImporteTotalSujetoAAplicacion_13_(Double.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_descuentosMonetariosPorUnidad.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setDescuentosMonetariosPorUnidad(Double.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_unidadDeMedida.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
			setUnidadDeMedida(String.valueOf(m.group(1).trim()));
		}
		if((m = PATTERN_ERE1E_cantidadDescuento_Cargo.matcher(value)).find() && StringUtils.isNotBlank(m.group(1))) {
	...
	public enum ERE1E_3 {
		...
		public static ERE1E_3 enumByValue(String value) {
	...
	public enum ERE1E_4 {
		...
		public static ERE1E_4 enumByValue(String value) {
			return Arrays.asList(values()).stream()
				.filter(o -> (o.getValue().equalsIgnoreCase(value)))
				.findFirst().orElse(null);
		}
	...
	public enum ERE1E_5 {
		...
		public static ERE1E_5 enumByValue(String value) {
			return Arrays.asList(values()).stream()
				.filter(o -> (o.getValue().equalsIgnoreCase(value)))
				.findFirst().orElse(null);
		}
