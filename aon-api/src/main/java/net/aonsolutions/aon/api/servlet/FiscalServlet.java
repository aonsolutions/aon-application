...
public class FiscalServlet extends AonApiHttpServlet{
	...
	private JSONArray getFiscalModels(AonApiData api) {
		try ( CloseableAONContext ctx = AONContext.getAONContext(api.getDomain().getName(), api.getDomain().getId(), api.getUser().getLogin())){
			...
			// Comprobar si est� configurado "Presentaci�n autom�tica de modelos" y "Entorno de Pruebas de la AEAT"
			...
			models.forEach(model-> {
				try {
					jsonModels.put(FiscalModelJSON.toJSON(model)
							.put("presModelAuto", model.getAdministration() == Administration.COMMON_TERRITORY ? presModelAutoEnabled : 0) // Presentaci�n autom�tica del modelo (solo modelos de la Agencia Tributaria)							
	...
	private JSONObject markAsFinished(AonApiData api) {
		try ( final CloseableAONContext ctx = AONContext.getAONContext(api.getDomain().getName(), api.getDomain().getId(), api.getUser().getLogin())) {
			...
			if (declarationType != null) {
				...
				int presModelAuto = reject || declarationType == FiscalModelDeclarationType.DEFERRAL ? 0 : JsonUtils.getInt(params, "presModelAuto");  // Presentaci�n autom�tica del modelo (solo si no ha sido Rechazado por el Cliente y No es aplazamiento)				
				...
				// Comprobar si hay presentacion autom�tica del modelo y no se ha seleccionado certificado
				if (presModelAuto == 1 && certi == null) {
					throw new AonApiException("ERROR: Debe seleccionar certificado.");
				}
				
				// Comprobar si hay presentaci�n autom�tica, es ingreso y no se ha indicado NRC
				...
				// Comprobar si hay presentaci�n aut�matica, es domiciliaci�n o devoluci�n y no se ha indicado IBAN
				...
				// Par�metros para la posible presentaci�n autom�tica del modelo 
				...
				if(reject) {
					...
					// Presentaci�n autom�tica del modelo 
					if (presModelAuto == 1) {
...
//			params.put("FIRNOMBRE", aeatParams.getName());
			...
			// A�adir los parametros FIRNIF y FIRNOMBRE cuyos valores se obtienen en la llamada a getKeyManagers
			params.put("FIRNIF", aeatParams.getDocument());
			params.put("FIRNOMBRE", aeatParams.getName());
	...
	// Se utilizar� para devolver los errores que se han producido en la presentaci�n, es decir cuando la llamada al 
	// servicio de presentaci�n del modelo es correcta, pero la Agencia Tributaria devuelve mensajes de error
	...
	// Se utilizar� para devolver cualquier otro error que se produzca en la llamada a la presentaci�n del modelo 
