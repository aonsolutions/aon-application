package com.code.aon.ui.supplier.event.servlet;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if(integer != null){
				...
				criteria.addEqualExpression(supplierBean.getFieldName(ISupplierAlias.SUPPLIER_ID),integer);
				...
				if(list.size() == 0){
					criteria = new Criteria();
					criteria.addEqualExpression(supplierBean.getFieldName(ISupplierAlias.SUPPLIER_REGISTRY_ALIAS),req.getParameter(VALUE_PARAMETER));
					list = supplierBean.getList(criteria);
					if (list.size() > 0) {
						ILookupObject ito = (ILookupObject) list.get(0);
						renderResponse (res,ito,ids);
