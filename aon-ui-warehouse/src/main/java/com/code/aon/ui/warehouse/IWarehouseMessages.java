package com.code.aon.ui.warehouse;

public interface IWarehouseMessages {

	/** ResourceBundle name. */
	String BUNDLE_KEY = "warehouseBundle";

	String WAREHOUSE_DELIVERY_EMAIL_SUBJECT = "warehouse_delivery_email_subject";
	String WAREHOUSE_DELIVERY_EMAIL_BODY = "warehouse_delivery_email_body";
	
	String WAREHOUSE_DELIVERY_SOURCE = "warehouse_delivery_source";
	String WAREHOUSE_DELIVERY_DETAIL_LINE = "warehouse_delivery_detail_line";
	
	String WAREHOUSE_INCOME_SOURCE = "warehouse_income_source";
	String WAREHOUSE_INCOME_DETAIL_LINE = "warehouse_income_detail_line";
	
	String STOCK_BY_WAREHOUSE_KEY = "warehouse_stock_by_warehouse_module";
	String STOCK_BY_ITEM_KEY = "warehouse_stock_by_item_module";
	String STOCK_BY_WAREHOUSE_VALUED_KEY = "warehouse_stock_by_warehouse_valued_module";
	String STOCK_BY_ITEM_VALUED_KEY = "warehouse_stock_by_item_valued_module";
	
}

