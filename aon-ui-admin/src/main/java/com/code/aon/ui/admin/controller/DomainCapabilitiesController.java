...
import static com.code.aon.ui.admin.controller.DomainController.DEFAULT_MAX_DOCUMENT_SIZE;
import static com.code.aon.ui.admin.controller.DomainController.DEFAULT_MAX_TOTAL_DOCUMENT_SIZE;
import static com.code.aon.ui.admin.controller.IAdminConstants.ADMIN_CONTROLLER_NAME;
import static com.code.aon.ui.admin.controller.IAdminConstants.DEFAULT_SUBDOMAIN_SUFFIX;
...
import static com.code.aon.ui.admin.controller.IAdminConstants.SHOW_DOMAIN_MANAGEMENT;
...
import com.code.aon.admin.enumeration.DomainCapability;
...
public class DomainCapabilitiesController {
	...
	public List<DomainCapability> getCapabilities( Domain domain ) {
	...
	public void setCapabilities( Domain domain, List<DomainCapability> list ) {
		domain.setDocumentManagement( list.contains(DomainCapability.DOCUMENTAL) );
		domain.setUserManagement( list.contains(DomainCapability.MULTI_USER) );
		domain.setDomainManagement( list.contains(DomainCapability.MULTI_DOMAIN) );
	}
	
	
	public void onInit(ActionEvent event) {
		this.model = new ListDataModel( getCapabilities(getDomainController().getDomain()) );
		resetCapabilities();
		resetTo();
	}
	
	private boolean isShowDomainManagement() {
		String value = getAdmin().getConfig().getProperty(SHOW_DOMAIN_MANAGEMENT);
	...
	private void resetCapabilities() {
		...
		List<DomainCapability> list = getCapabilities(domain);
	...
	private void capabilityChanged( Domain domain, DomainCapability capability, boolean removed ) throws ManagerBeanException {
		if ( DomainCapability.DOCUMENTAL == capability ) {
			getAdmin().getLogger().documental(domain);
			...
			getAdmin().getLogger().multiDomain(domain);
			...
			getAdmin().getLogger().multiUser(domain);			
		}
	}
