...
if [ "$1" = 'catalina.sh' -a -z "$wantHelp" ]; then
	...
	cat << EOF > $AON_SOLUTIONS_ANGULAR_CONF/connection
...
EOF
	...
	CLASSPATH=$CLASSPATH:`find $AON_SOLUTIONS_ANGULAR_HOME -name 'dbutils-*.jar'`
	CLASSPATH=$CLASSPATH:`find $AON_SOLUTIONS_ANGULAR_HOME -name 'aon-master-*.jar'`
