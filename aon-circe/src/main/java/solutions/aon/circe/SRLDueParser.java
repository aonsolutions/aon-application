...
//			System.out.println(text);
	...
	public static void parse(String text, DueParserListener listener)
	...
	protected static final Pattern REGISTRO_ENTRADA_PAE = Pattern.compile("^REGISTRO\\s*ENTRADA\\s*Y\\s*PAE$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern PERSONAS_TRABAJADORAS = Pattern.compile("^PERSONAS\\s*TRABAJADORAS$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern DOMICILIO = Pattern.compile("^DOMICILIO||CENTRO\\s*[a-zA_Z:]*\\s*(?<domicilio>.*)$",
			Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern COMUNICACION_NOTIFICACION = Pattern.compile("^COMUNICACIONES\\s*NOTIFICACI.NES$", Pattern.CASE_INSENSITIVE);

	// NOTIFICACIÓN TGSS
	protected static final Pattern NOTIFICACION_TGSS = Pattern.compile("^NOTIFICACI.N\\s*TGSS$", Pattern.CASE_INSENSITIVE);
	...
	// NOTIFICACIÓN AEAT
	protected static final Pattern NOTIFICACION_AEAT = Pattern.compile("^NOTIFICACI.N\\s*AEAT$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern MEDIO_NOTIFICACION_SOCIEDAD = Pattern.compile("^MEDIO\\s*NOTIFICACI.N\\s*SOCIEDAD$", Pattern.CASE_INSENSITIVE);


	// COMUNICACIONES DE LA DIRECCIÓN GENERAL DE INDUSTRIA Y PYME
	protected static final Pattern COMUNICACIONES = Pattern.compile("^COMUNICACIONES\\s*DE\\s*LA\\s*DIRECCI.N\\s*GENERAL\\s*DE\\s*INDUSTRIA\\s*Y\\s*PYME$", Pattern.CASE_INSENSITIVE);
	...
	// CENTRO DE ACTIVIDAD
	protected static final Pattern CENTRO_DE_ACTIVIDAD = Pattern.compile("^CENTRO\\s*DE\\s*ACTIVIDAD$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern CNAE = Pattern.compile("^CALSIFICACI.N\\s*NACIONAL\\s*ACTIVIDADES\\s*ECON.MICAS\\s*\\(CNAE\\)$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern OTRA_ACTIVIDAD = Pattern.compile("^OTRAS\\s*ACTIVIDADES\\s*:\\s*(?<otrasActividades>.*)$", Pattern.CASE_INSENSITIVE);
	
	// IMPUESTO ACTIVIDAD ECONÓMICA (IAE)
	protected static final Pattern IAE = Pattern.compile("^IMPUESTO\\s*ACTIVIDAD\\s*ECON.MICA\\s*\\(IAE\\)$", Pattern.CASE_INSENSITIVE);
	...
	// DECLARACIÓN CENSAL
	protected static final Pattern DECLARACION_CENSAL = Pattern.compile("^DECLARACI.N\\s*CENSAL$", Pattern.CASE_INSENSITIVE);
	...
	// Causa de la representación: Legal Clave: Personas Jurídicas residentes o constituidas en España
	protected static final Pattern CAUSA_REPRESENTACION = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Tipo de representación: Individual Título de la representación: Documento público
	protected static final Pattern TIPO__TITULO_REPRESENTACION = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// TRABAJADOR/A POR CUENTA PROPIA
	protected static final Pattern TRABAJADOR_CUENTA_PROPIA = Pattern.compile("^TRABAJADOR\\/A\\s*POR\\s*CUENTA\\s*PROPIA$", Pattern.CASE_INSENSITIVE);

	// ALTA DE PERSONA TRABAJADORA
	protected static final Pattern ALTA_PERSONA_TRABAJADORA = Pattern.compile("^ALTA\\s*DE\\s*PERSONA\\s*TRABAJADORA$", Pattern.CASE_INSENSITIVE);

	// Fecha de Alta: 26/04/2024 ¿Ejerce funciones de Dirección y Gerencia?: Sí
	protected static final Pattern FECHA_ALTA_DIRECCION_GERENCIA = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// CENTRO DE TRABAJO
	protected static final Pattern CENTRO_TRABAJO = Pattern.compile("^CENTRO\\s*DE\\s*TRABAJO$", Pattern.CASE_INSENSITIVE);

	// RÉGIMEN DE ENCUADRAMIENTO
	protected static final Pattern REGIMEN_ENCUADRAMIENTO = Pattern.compile("^R.GIMEN\\s*DE\\s*ENCUADRAMIENTO$", Pattern.CASE_INSENSITIVE);

	// Régimen: RETA TRL: NO Subgrupo: N/A
	protected static final Pattern REGIMEN_SUBGRUPO = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Grupo: 0521- RÉGIMEN ESPECIAL DE TRABAJADORES POR CUENTA PROPIA O AUTÓNOMOS.
	protected static final Pattern GRUPO = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// CNAE: 7021 - Relaciones públicas y comunicación
	protected static final Pattern CNAE_DATOS = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// 7311 - Agencias de publicidad
	protected static final Pattern CNAE_SECOND = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Mutua de IT: FREMAP
	protected static final Pattern MUTUA_IT = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// ¿Ha tramitado ya su alta en RETA?: No ¿Contingencias AT / EP?: Sí Acogerse a Cese de Actividad: Sí
	protected static final Pattern TRAMITAR_ALTA_CONTINGENCIAS = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Fecha Real Alta: 26/04/2024
	protected static final Pattern FECHA_REAL_ALTA = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Opcion CA FP: Sí
	protected static final Pattern OPCION_CA_FP = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// BASE COTIZACIÓN
	protected static final Pattern BASE_COTIZACION = Pattern.compile("^BASE\\s*COTIZACI.N$", Pattern.CASE_INSENSITIVE);

	// Base Cotización: 950,98 Rendimientos netos anuales en promedio mensual: 166,70
	protected static final Pattern BASE_COTIZACION_RENDIMIENTOS = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// DATOS PARA LA DOMICILIACIÓN DE PAGO DE CUOTAS
	protected static final Pattern DATOS_DOMICILIACION = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Cuenta: 2100 0293 24 0200443673
	protected static final Pattern CUENTA = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Observaciones TGSS: ES71
	protected static final Pattern OBSERVACIONES_TGSS = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern PERSONA_CITA_TELEFONO = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);

	// Fecha: 05/04/2024 Hora: 10:30
	protected static final Pattern FECHA_HORA = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);
	...
	protected static final Pattern NOMBRE_APELLIDOS = Pattern.compile("^$", Pattern.CASE_INSENSITIVE);
