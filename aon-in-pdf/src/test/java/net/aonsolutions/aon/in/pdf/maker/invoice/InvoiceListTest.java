...
public class InvoiceListTest {
	...
	public void testOneEntry() throws FileNotFoundException {
		...
		try (OutputStream os = OutputStream.nullOutputStream()) {
			InvoiceListTemplate template = new InvoiceListTemplate(nullProbability(faker.company().name(), 20),
					entries);
			template.print(os);
		} catch (CanNotCreatePdfException e) {
			fail();
	...
	public void testPageLimitEntries() throws FileNotFoundException {
		...
		for (int i = 0; i < numberOfEntries; i++) {
			int min = -99999;
			int max = 99999;
			InvoiceListEntry entry = new InvoiceListEntry().setBase(faker.number().randomDouble(4, min, max))
					.setDate(faker.date().past(50 * 365, TimeUnit.DAYS))
					.setIrpf(faker.number().randomDouble(4, min, max)).setIva(faker.number().randomDouble(4, min, max))
					.setName(faker.company().name()).setNif(AonStringUtils.substring(faker.idNumber().valid(), 0, 10))
					.setNumber(faker.code().imei()).setTotal(faker.number().randomDouble(4, min, max));
			entries.add(entry);
		}
	...
	public void testPageLimitEntriesPlusOne() throws FileNotFoundException {
		...
		try (ByteArrayOutputStream os = new ByteArrayOutputStream()/*
																	 * OutputStream os = new
																	 * FileOutputStream("InvoiceListTemplateTest.pdf")
																	 */) {
			InvoiceListTemplate template = new InvoiceListTemplate(nullProbability(faker.company().name(), 20),
					entries);
			template.print(os);
	...
	private void singleTest() throws FileNotFoundException {
		...
		try (OutputStream os = OutputStream
				.nullOutputStream()/* new FileOutputStream("InvoiceListTemplateTest.pdf") */) {
			InvoiceListTemplate template = new InvoiceListTemplate(nullProbability(faker.company().name(), 20),
					entries);
			template.print(os);
		} catch (CanNotCreatePdfException e) {
	...
	private void singleTest2() throws FileNotFoundException {
		...
		for (int i = 0; i < 106; i++) {
			...
			Invoice invoice = new Invoice()
					.setTaxableBase(
							AonNumberUtils.zeroIfNull(nullProbability(faker.number().randomDouble(4, min, max), 20)))
		...
		try (OutputStream os = OutputStream
				.nullOutputStream()/* new FileOutputStream("InvoiceListTemplateTest2.pdf") */) {
