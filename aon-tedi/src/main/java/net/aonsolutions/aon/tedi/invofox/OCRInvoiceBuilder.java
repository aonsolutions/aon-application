...
//		}   
	...
	private static boolean mustImportFromBreakdown(OCRInvoice ocrInvoice) {
		// Mientras los se devuelvan las l�nea correctamente, se importa siempre el BreakDown
		// En otro caso descomentar el m�todo.
		return Boolean.TRUE;
		//----------------
	...
	private static void guessItemsOrAccounts(OCRContextDetail ocr) {
		...
		if ( ocr.getDetail().getItem() == null && ocr.getDetail().getAccountId() == null) {
			if (ocr.getConfig() != null && ocr.getConfig().getOcrDefaultItem() != null) {
				...
				throw new AonCoreException("No existe producto por defecto definido en la configuraci�n");
			}
		}
	}
	...
	private static void guessItemsOrAccounts(AONContext ctx, Invoice invoice, InvoiceDetail invoiceDetail) {
		if (invoice != null && invoice.getRegistry() != null) {
			// Se busca el �ltimo item del registry que se trata
	...
	// Buscar en facturas anteriores para suponer el tipo de retenci�n con mas seguridad.
