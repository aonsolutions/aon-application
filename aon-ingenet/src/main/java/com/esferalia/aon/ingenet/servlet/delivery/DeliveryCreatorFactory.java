...
@Deprecated
public class DeliveryCreatorFactory implements Serializable {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DeliveryCreatorFactory.class.getName());
	...
	public DeliveryCreatorFactory(String domain, Integer domainId, String user) {
		this.domain = domain;
		...
		if (lineasAlbaran != null && lineasAlbaran.size() > 0) {
			...
			.forEach(
					linea -> {
						Item item;
						try {
							item = ProductUtils.obtainItem(ctx,
									linea.getPRODUCTO());
