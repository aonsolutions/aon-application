...
import javax.tools.Tool;
	...
	public static Collection<Employee> getTotalEmployees(final InputStream certificateInputStream, final String certificatePassword,
		...
		try {
			...
			try {
				employees.addAll(getEmployees(new ByteArrayInputStream(cert), certificatePassword, certificateType, regimen, ccc));
			} catch (Exception e) {
				e.printStackTrace();
			}
			try {
				employees.addAll(getPrevEmployees(new ByteArrayInputStream(cert), certificatePassword, certificateType, regimen, ccc));
	...
	//GET PDF INFO
	public static byte[] getCccLaboralLife(InputStream certificateInputStream, String certificatePassword, String certificateType, String regime, String ccc, Date from, Date to) throws FailingHttpStatusCodeException, IOException, SegSocialException {

		try (WebClient webClient = HtmlUnitToolkit.getWebClient(certificateInputStream, certificatePassword, certificateType)) {

			HtmlPage document = 			webClient.getPage("https://w2.seg-social.es/Xhtml?JacadaApplicationName=SGIRED&TRANSACCION=ACR71&E=I&AP=AFIR");
			HtmlInput regime_in = 			document.querySelector("#SDFREGCCO");
			HtmlInput ccc_1_in = 			document.querySelector("#SDFTESCCO");
			HtmlInput ccc_2_in = 			document.querySelector("#SDFNYCCCO");
			HtmlInput from_dd_in = 			document.querySelector("#SDFDIADESDEM");
			HtmlInput from_mm_in = 			document.querySelector("#SDFMESDESDEM");
			HtmlInput from_yyyy_in = 		document.querySelector("#SDFAODESDEM");
			HtmlInput to_dd_in = 			document.querySelector("#SDFDIAHASTAM");
			HtmlInput to_mm_in = 			document.querySelector("#SDFMESHASTAM");
			HtmlInput to_yyyy_in = 			document.querySelector("#SDFAOHASTAM");
			HtmlOption online_opt = 		document.querySelector("#ListaTipoImpresion option:nth-child(3)");
			HtmlSubmitInput continue_btn = 	document.querySelector("#Sub2207001009");

			Integer[] from_arr = 			Toolkit.getDateArray(from);
			Integer[] to_arr = 				Toolkit.getDateArray(to);
			ArrayList<String> ccc_arr = 	Toolkit.splitString_m(ccc,2);

			regime_in.setAttribute("value", regime );
			ccc_1_in.setAttribute("value", ccc_arr.get(0));
			ccc_2_in.setAttribute("value", ccc_arr.get(1));

			from_dd_in.setAttribute("value", String.valueOf(from_arr[0]));
			from_mm_in.setAttribute("value", String.valueOf(from_arr[1]));
			from_yyyy_in.setAttribute("value", String.valueOf(from_arr[2]));

			to_dd_in.setAttribute("value", String.valueOf(to_arr[0]));
			to_mm_in.setAttribute("value", String.valueOf(to_arr[1]));
			to_yyyy_in.setAttribute("value", String.valueOf(to_arr[2]));

			online_opt.click();
			try{
				UnexpectedPage doc = 	continue_btn.click();
