...
//		String authorized = "127770";
		...
		Date date = new Date();
		String nss = "411022570585";
		String ident = "136118951G";
		String regimen = "0111";
		String ccc = "11120424045";
		String cno = "1234";
		...
		try {
			...
			AonSegSocialJuanma.setCnoCertificate(certificateInputStream, certificatePassword, certificateType, nss,
					ident, regimen, ccc, cno);
	...
	// DATA TO USE : NSS : 41 1022570585 DNI: 36118951G CUENTA COT: 11 120424045
	// REGIM 0111
	private static void setCnoCertificateP(final InputStream certificateInputStream, final String certificatePassword,
			final String certificateType, String nss, String ident, String regimen, String ccc, String cno)
			throws Exception {
		...
		try (WebClient webClient = HtmlUnitToolkit.getWebClient(certificateInputStream, certificatePassword,
			...
			form.getInputByName("txt_SDFTESCTACOT").setValue(ccc.substring(0, 2));
			form.getInputByName("txt_SDFCTACOT").setValue(ccc.substring(2));
			...
			String readOnly = form2.getInputByName("txt_SDFDREAL").getReadOnlyAttribute();
			System.out.println(readOnly);
			
			String dayOfChange = form2.getInputByName("txt_SDFDREAL").getValue();
			String monthOfChange = form2.getInputByName("txt_SDFMREAL").getValue();
			String yearOfChange = form2.getInputByName("txt_SDFAREAL").getValue();
			String fecha =parseDate(new Date());
			
			System.out.println(
					"La ultima fecha de cambio de C.N.O es : " + dayOfChange + "/" + monthOfChange + "/" + yearOfChange);
			
			String cnOcupacion = form2.getInputByName("txt_SDFCNOCUP_ayuda").getValue();

			System.out.println("Su C.N.O es : " + cnOcupacion);

			form2.getInputByName("txt_SDFCNOCUP_ayuda").setValue(cno);
			
			htmlPage = formSubmit.click();
			
			cnOcupacion = form2.getInputByName("txt_SDFCNOCUP_ayuda").getValueAttribute();
			
			System.out.println("Su C.N.O ahora es : " + cnOcupacion + " y fue modificado el "+fecha);
//			HtmlUnitToolkit.manageStatusCode(htmlPage);
	...
	public static String parseDate(Date date) {
		...
		try {
			fCambio = dateFormat.format(date);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return fCambio;
