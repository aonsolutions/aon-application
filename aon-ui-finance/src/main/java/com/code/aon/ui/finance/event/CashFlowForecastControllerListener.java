	...
	public void beforeModelInitialized(ControllerEvent event) throws ControllerListenerException {
		try {
			...
			if (params.getRegistryBank() != null && params.getRegistryBank().getId() != null) {
				c.addEqualExpression(cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_REGISTRY_BANK_ID), params.getRegistryBank().getId());
			}
			if (StringUtils.isNotEmpty(params.getDescription())) {
				c.addExpression(cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_DESCRIPTION), params.getDescription());
			}
			if (params.getAmount() != null) {
				c.addEqualExpression(cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_AMOUNT), params.getAmount());
			}
			if (params.getPaymentDay() != null) {
				c.addEqualExpression(cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_PAYMENT_DAY), params.getPaymentDay());
			}
			if (params.getStartDate() != null) {
				c.addGreaterThanOrEqualExpression(cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_START_DATE), params.getStartDate());
			}
			if (params.getDueDate() != null) {
				String alias = cf.getFieldName(IEntityAlias.CASH_FLOW_FORECAST_DUE_DATE); 
