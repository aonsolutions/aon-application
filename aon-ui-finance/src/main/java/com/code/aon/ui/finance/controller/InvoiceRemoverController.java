...
public class InvoiceRemoverController extends BasicController implements IProgression{
		...
		try {
			...
			while(iter.hasNext()){
				...
				try {
					...
					String msg =  "Error deleting invoice: " + invoice.getReferenceCode() + " " + e.getMessage();
					LOGGER.error(msg, e);
	...
	public void checkInvoiceTbai(Invoice inv) {
		...
		if(icc.isTbai()) {
			DataResponseSource source = icc.isTest() ? DataResponseSource.TBAI_TEST: DataResponseSource.TBAI;
