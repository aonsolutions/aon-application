...
public class RectifiedInvoiceController {
	...
	public void onSearch(ActionEvent event ) {
		...
		try {
			...
			stmt.append("SELECT i.id,sum(f.amount)");
		...
		try {
			...
			for ( InvoiceWrapper iw : getInvoiceList() ) {
				if (iw.isEnabled()) {
					...
					int number = obtainMaxRectificationNumber( getRectificationSeries() );
					Invoice rectifier = rectificationManager.specialRectifyInvoice(invoice, getRectificationSeries(), number, getRectificationDate(), getRectificationCause(),iw.getPercent());
				}
			}
