...
export class TediPdfParser {
  ...
  public static extract(buffer: Buffer): Observable<Invoice> {
    ...
    }).pipe(map((result: PDFExtractResult) => TediPdfParser.precog(result)));
  }
  ...
  public static parse(info: TediImportInvoicesInfo): Promise<Invoice> {
    ...
    return new Promise((resolve, reject) =>
      pdfExtract.extractBuffer(buffer, options, (err: Error | null, result: PDFExtractResult | undefined) => {
        if (err !== null) {
          reject(err);
  ...
  public static precog(result: PDFExtractResult): Invoice {
