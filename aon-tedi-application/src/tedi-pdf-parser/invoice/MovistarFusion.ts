...
// |   Línea fija:999999999 Línea móvil:999999999                                                             |
// |                                                                           							  			          |
...
// |                                                                           																|
// |   Las líneas móviles facturadas en este documento con                                                    |
// |   importes correspondientes a conceptos no incluidos en su       Resumen de factura                      |
// |   servicio Movistar Fusión son: 999999999                        ........................................|
// |                                                                           																|
// |                                                                    Cuotas Mensuales           999.9999 € |
// |                                                                           																|
// |                                                                           																|
// |                                                                    Otros Conceptos            999.9999 € |
// |                                                                           																|
// |                                                                           																|
// |                                                                    Impuestos                  999.9999 € |
...
export class MovistarFusion {
  public static extract(pdf: PDFExtractResult): Invoice {
    ...
    for (i = 0; i < pages.length; i++) {
      ...
      try {
        ...
        if (content0) {
          ...
          if (line0) {
            // Madrid,1 Febrero 2019 Factura Fusión TA62E0184664
            match = line0.str.match(/^Madrid\s*,\s*\d+\s*[a-z\.]+\s*\d+\s*Factura\s*Fusi.n\s*/i);
            if (match) {
              break;
            }
          }
          if (line0) {
            // Madrid,1 Noviembre 2018 | Factura Fusión TA60G0026117 | Identificador
            match = line0.str.match(/^Madrid\s*,\s*\d+\s*[a-z\.]+\s*\d+[|\s]*Factura\s*(Fusi.n)?\s*(\w*)[|\s]*identificador/i);
    ...
    if (i === pages.length || !line0 || !content0) {
      throw new TediUnknownInvoiceFormatError('Not Movistar Fusion invoice');
    }
    ...
    do {
      ...
      if (line) {
        index = line.index;
        match = line.str.match(/Factura\s*(Fusi.n)?\s*(\w+)/i);
        if (match) {
          invoice.reference = match[2];
          break;
        }
      }
    } while (line);
    ...
    do {
      ...
      if (line) {
        ...
        if (match) {
          ...
          invoice.finances[0].amount = invoice.total;
          break;
        }
      }
    } while (line);
