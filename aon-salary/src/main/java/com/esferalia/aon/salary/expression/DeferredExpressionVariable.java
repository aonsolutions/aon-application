...
public class DeferredExpressionVariable implements IExpressionVariable <Object> {
	...
	public DeferredExpressionVariable(ExpressionContext ctx, IExpression expression, Period period) {
		this.ctx = ctx;
		this.period = period;
		this.expression = expression;
	}

	public DeferredExpressionVariable(ExpressionContext ctx, IExpression expression, Date start, Date end) {
		this.ctx = ctx;
		this.expression = expression;
	...
	public Object getValue(Period period) {
		try {
			List<ITimedResult<Object>> results = ctx.eval(expression.getExpression(),
