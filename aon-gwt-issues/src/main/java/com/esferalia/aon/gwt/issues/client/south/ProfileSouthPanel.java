...
public class ProfileSouthPanel extends SouthPanel {

    public ProfileSouthPanel(AonJsArray<JsProfile> items) {   
    	super();
        vertical.setWidth("100%");
        if(items.length() > 0){
        	items.stream().forEach(js -> {
        		PaperItem pi = buildQuestion(js);
    ...
    public PaperItem buildQuestion(JsProfile js){
    ...
    public PaperItem buildPaperItem(JsObject js){
    	PaperItem pi = new PaperItem();
    	IronIcon ironIcon = new IronIcon();
    	ironIcon.setIcon("account-box");
    	ironIcon.addStyleName(AON.AON_CSS.aonMinWidth24());
