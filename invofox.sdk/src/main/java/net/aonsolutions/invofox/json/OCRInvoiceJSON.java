...
public class OCRInvoiceJSON {
	...
	public static OCRInvoice from(JSONObject json) {
		...
		return new OCRInvoice()
			...
			.setSupplier(OCRRegistryJSON.from(OCRJSONUtils.getObject(json, OCRNames.SUPPLIER)))
			.setCustomer(OCRRegistryJSON.from(OCRJSONUtils.getObject(json, OCRNames.CUSTOMER)))
	...
	public static JSONObject to(OCRInvoice error) {
		...
		return new JSONObject()
				...
				.putOpt(OCRNames.SUPPLIER, OCRRegistryJSON.to(error.getSupplier().orElse(null)))
				.putOpt(OCRNames.CUSTOMER, OCRRegistryJSON.to(error.getCustomer().orElse(null)))
