...
import net.aonsolutions.infovox.model.OCRAddress;
...
public class OCRAddressJSON {
	...
	public static JSONObject to(OCRAddress address) {
		...
		return new JSONObject()
			.putOpt(OCRNames.TEXT, address.getText())
			.putOpt(OCRNames.ADDRESS_NUMBER, address.getAddressNumber())
			.putOpt(OCRNames.COUNTRY, address.getCountry())
			.putOpt(OCRNames.MUNICIPALITY, address.getMunicipality())
			.putOpt(OCRNames.NEIGHBORHOOD, address.getNeighborhood())
			.putOpt(OCRNames.POSTAL_CODE, address.getPostalCode())
			.putOpt(OCRNames.REGION, address.getRegion())
			.putOpt(OCRNames.STREET, address.getStreet())
			.putOpt(OCRNames.SUB_REGION, address.getSubRegion())
		;
	}
}
