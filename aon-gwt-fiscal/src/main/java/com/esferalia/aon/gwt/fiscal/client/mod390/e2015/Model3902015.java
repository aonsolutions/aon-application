...
public class Model3902015 extends DockLayoutPanel  {
	...
	protected static final Mod3902015ServiceAsync MOD3902015_SERVICE;
	static {
	...
	class Model3902015Callback {
		void markAsDirty() {
			Model3902015.this.markAsDirty();
		}
		Mod3902015  getModel() {
			return Model3902015.this.getModel(); 
		}
	}
	...
	public Model3902015(final Model390Callback mod390Callback, Mod390 mod390) {
		super(Unit.PX);
		...
		popup.center();
		try {
			MOD3902015_SERVICE.get(getCallback().getOptions().getOccam(), mod390, new AsyncCallback<Mod3902015>() {
				@Override
	...
	public Mod3902015 getModel() {
		return model;
	}
	public void setModel(Mod3902015 model) {
		this.model = model;
	}
	...
	private Widget getToolbar() {
		...
		saveButton.addClickHandler(event -> {
			...
			try {
				...
				MOD3902015_SERVICE.save(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Mod3902015>() {
					@Override
		...
		deleteButton.addClickHandler(event -> {
			...
			cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					MOD3902015_SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
						@Override
	...
	private Widget getLinksPanel() {
		...
		Model3902015Callback cbk = new Model3902015Callback();
	...
	private AonToolbar getDeclarationToolbarPanel() {
		...
		markAsFinishedButton.addClickHandler(event -> {
			...
			MOD3902015_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod3902015>() {
				@Override
				public void onSuccess(Mod3902015 result) {
					select(result);
				}
		...
		markAsSentButton.addClickHandler(event -> {
			...
			MOD3902015_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.SENT, new AsyncCallback<Mod3902015>() {
				@Override
				public void onSuccess(Mod3902015 result) {
					select(result);
				}
		...
		markAsPendingButton.addClickHandler(event -> {
			...
			MOD3902015_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod3902015>() {
				@Override
				public void onSuccess(Mod3902015 result) {
					select(result);
				}
