...
public class Model130  implements EntryPoint {
	...
	private Model130Table model130Table;
	...
	public void onModuleLoad(Model130ModuleOptions options) {
		...
		model130Table = new Model130Table( new Model130Callback() );
		model130Table.addSelectionHandler( this::onSelectionChange );
		declarationContainer.setWidget(model130Table);
	...
	private void newModel(Mod130 newModel) {
		SERVICE.initialize(getOptions().getOccam(),newModel, new AsyncCallback<Mod130>() {
			@Override
			public void onSuccess(Mod130 m130) {
				tabLayout.selectTab(INFORMATION_TAB);
				closeFootPanel();
	...
	enum Mod130Declarations {
		AEAT {
			...
			@Override
			public Widget getDeclarationWidget(Mod130 mod130, Model130Callback cbk) {
				return new Model130AEAT(mod130, cbk);
			}
		},
		BIZKAIA {
			...
			@Override
			public Widget getDeclarationWidget(Mod130 mod130, Model130Callback cbk) {
				return new Model130Bizkaia(mod130,cbk);
			}
		},
		;
		public abstract boolean accept(Mod130 mod130);
		public abstract Widget getDeclarationWidget(Mod130 mod130, Model130Callback cbk);
	}

	private void select(Mod130 selected) {
		aonLayout.hideErrorPanel();
		Widget declaration = null;
		for (Mod130Declarations dec : Mod130Declarations.values()) {
			if (dec.accept(selected)) {
				declaration = dec.getDeclarationWidget(selected, new Model130Callback());
			}
		}
		if (declaration != null) {
	...
	private void onSelectionChange(SelectionEvent<Mod130> event) {
		Mod130 sel = event.getSelectedItem();
		SERVICE.getMod130(getOptions().getOccam(),sel.getId(), new AsyncCallback<Mod130>() {
			@Override
			public void onSuccess(Mod130 selected) {
				if (selected == null) {
	...
	public static void run() {
		GWT.runAsync(Model130.class, new RunAsyncCallback() {
			...
			public void onFailure(Throwable reason) {
				Window.alert(AON.MSG.loadError("Modelo 130"));
			}
