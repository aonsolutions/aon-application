...
class Model390HFTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod390HF>{
	
	private static final Logger LOGGER = Logger.getLogger(Model390HFTable.class.getName());
	static {
	...
	protected Model390HFTable(Model390HFCallback cbk) {
	...
	public void refresh(Model390HFCallback cbk) {
		...
		Model390HF.MOD_SERVICE.getMod390HFs(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod390HF> result) {
		for ( Mod390HF mod390HF : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod390HF.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod390HF.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod390HF.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod390HF.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model390HFTable.this, mod390HF));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod390HF)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod390HF.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod390HF.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod390HF.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod390HF.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod390HF.getStatus()) );
			row.add( statusCell );
