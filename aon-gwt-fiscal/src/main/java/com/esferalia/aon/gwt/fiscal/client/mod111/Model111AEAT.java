...
public class Model111AEAT extends Model111Base {
	...
	final Mod111ServiceAsync MOD111Service = GWT.create(Mod111Service.class);
	
	
	public Model111AEAT(IFiscalModelCallback<Mod111> callback, AonData aonData) {
	...
	public FlowPanel getAdministrationPanel() {
				...
				CertificationPopup certPopup = new CertificationPopup(getAPI(), getModel().getName(), getModel().getDocument(), showNRC) {
					...
					protected void onAccept() {
						if(getModel().isSent()) {
							...
							submitAEAT(MODEL111_PRINT_AEAT, getCert(), getPass(), getName(), getDocument(), showNRC ? getNRC() : null);
							getCallback().showVisorAEAT();
						} else {
							getCallback().showInfoPanel("Para generar el fichero debe finalizar la confecci\u00F3n del modelo.");
						}	
		...
		MOD111Service.presentationFile(getCallback().getDomainName(), getCallback().getDomain(), getCallback().getUser(), getModel().getId(), new AsyncCallback<Integer>() {
