...
public class Model180Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod180>{
	...
	protected Model180Table(Model180Callback cbk) {
	...
	private Widget getToolbarPanel(Model180Callback cbk) {
		AonToolbar toolbar = new AonToolbar( "Modelo 180");
	...
	public void refresh(Model180Callback cbk) {
		...
		Model180.SERVICE.getMod180s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod180> result) {
		for ( Mod180 mod180 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod180.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod180.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod180.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod180.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model180Table.this, mod180));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod180)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod180.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod180.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod180.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod180.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod180.getStatus()) );
			row.add( statusCell );
			
			row.addCell( comp , AON.CSS.aonTextCenter())
				.addCell( sust , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod180.getDocument()))
				.addCell( new InlineLabel(mod180.getFullName()))
				;
		}
