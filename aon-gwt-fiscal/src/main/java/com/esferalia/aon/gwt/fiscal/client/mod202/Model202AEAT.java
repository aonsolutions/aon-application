...
public class Model202AEAT extends Model202Base {
	...
	public Model202AEAT(Model202Callback callback) {
		super(callback);
	}
	...
	protected void paintParticularyRow(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		if (script == Model202AEATScript.R01) {
			paintDateRow(table,callback, script);
		} else if (script == Model202AEATScript.R02) {
			paintCNAERow(table,callback, script);
		} else if (script == Model202AEATScript.R04) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R05) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R06) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R07) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R08) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R09) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R10) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R17) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R18) {
			paintR18(table,callback, script);
		} else if (script == Model202AEATScript.R19) {
			paintR19(table,callback, script);
		} else if (script == Model202AEATScript.R21) {
			paintR21(table,callback, script);
		} else if (script == Model202AEATScript.R61) {
			paintCheckRow(table,callback, script);
		} else if (script == Model202AEATScript.R62) {
			paintR62(table,callback, script);
		}
	}
	
	private void paintCNAERow(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
	...
	private void paintDateRow(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
		table.setWidget(row, 0, new Label(script.getLabel()));
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonBorderBottom() );
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonPaddingLeft() );
	...
	private void paintCheckRow(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
	...
	private void paintR18(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
		final Mod202Key key = script.getKeys()[0];
		table.setWidget(row, 0, new Label(script.getLabel()));
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonBorderBottom() );
	...
	private void paintR19(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
	...
	private void paintR21(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
		Mod202Key key = script.getKeys()[0];
		table.setWidget(row, 0, new Label(script.getLabel()));
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonBorderBottom() );
		...
		r21Box.addChangeHandler( event -> {
			callback.getModel().ensureDetail(Mod202Key.X00).setAmount(r21Box.getSelectedIndex());
	...
	private void paintR62(FlexTable table, Model202Callback callback, IModelScript<Mod202Key> script) {
		int row = table.getRowCount();
		Mod202Key key = script.getKeys()[0];
