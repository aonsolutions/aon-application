...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190BIZKAIADetail2022.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902022BizkaiaKey;
...
public class Model190BIZKAIA2022DetailPanel extends SimpleLayoutPanel implements Focusable {
	
	private int tabIndex; 
	private DocumentTextBox document;
	
	public Model190BIZKAIA2022DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902022BizkaiaKey k : Mod1902022BizkaiaKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190BIZKAIA2022DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(new ChangeHandler() {
			@Override
			public void onChange(ChangeEvent event) {
				subkey.clear();
				Mod1902022BizkaiaKey keyEnum = Mod1902022BizkaiaKey.values()[key.getSelectedIndex()];
				...
				Model190BIZKAIA2022DetailPanel.enableOrDisableAdditionalDataPanel(key, subkey, additionalDataPanel);
				callback.onValueChanged(detail);
			}
		});
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902022BizkaiaKey.A.toString());
		}
		Mod1902022BizkaiaKey keyEnum = Mod1902022BizkaiaKey.valueOf(detail.getKey());
	...
	private static void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		panel.setVisible( 
				Mod1902022BizkaiaKey.A == keyEnum
				|| Mod1902022BizkaiaKey.B == keyEnum 
				|| Mod1902022BizkaiaKey.C == keyEnum
				|| (Mod1902022BizkaiaKey.E == keyEnum && "01".equals(subk))
			);
	}
