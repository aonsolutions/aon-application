...
public class Model3902023 extends DockLayoutPanel {
	
	private static final Integer DEFAULT_YEAR = 2023;
	private static final String MOD390_2023_DRAFT = "/aon_gwt_fiscal/Model3902023Draft";
			
	protected static final Mod3902023ServiceAsync MOD3902023_SERVICE;
	static {
		Mod3902023ServiceAsync mod3902023ServiceRaw = GWT.create(Mod3902023Service.class);
		MOD3902023_SERVICE = new Mod3902023ServiceAsyncDecorator(mod3902023ServiceRaw);
	}
	
	class Model3902023Callback {
		void markAsDirty() {
			Model3902023.this.markAsDirty();
		}
		Mod3902023 getModel() {
			return Model3902023.this.getModel(); 
		}
		void setModel(Mod3902023 model) {
			Model3902023.this.setModel(model);
		}
		void refreshDeclarationToolbarPanel() {
			Model3902023.this.refreshDeclarationToolbarPanel();			
		}
	}
	
	private SimpleLayoutPanel contentContainer = new SimpleLayoutPanel();
	private final Model390Callback callback; 
	private Mod3902023 model;
	...
	public Model3902023(final Model390Callback mod390Callback, Mod390 mod390) {
		super(Unit.PX);
		...
		popup.center();
		try {
			MOD3902023_SERVICE.get(getCallback().getOptions().getOccam(), mod390, new AsyncCallback<Mod3902023>() {
				@Override
	...
	public Mod3902023 getModel() {
		return model;
	}
	public void setModel(Mod3902023 model) {
		this.model = model;
	}
	...
	private Widget getToolbar() {
		...
		saveButton.addClickHandler(event -> {
			...
			try {
				...
				MOD3902023_SERVICE.save(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Mod3902023>() {
					@Override
					public void onSuccess(Mod3902023 result) {
						select(result);
						popup.hide();
		...
		deleteButton.addClickHandler(event -> {
			...
			cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					MOD3902023_SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
						@Override
		...
		draftButton.addClickHandler(event -> submitForm(MOD390_2023_DRAFT));
	...
	public void select(Mod3902023 m390) {
		if ("INVALID".equals(m390.getXmlFormat()) ) {
			...
			cd.confirm( "La declaraci\u00F3n est\u00E1 creada con un formato anterior al del ejercicio 2018 y es inv\u00E1lida. \u00BFDesea borrarla?"
				...
				@Override
				public void onAccept() {
					MOD3902023_SERVICE.delete(getCallback().getOptions().getOccam(),
	...
	private Widget getLinksPanel() {
		...
		Model3902023Callback cbk = new Model3902023Callback();
	...
	private AonToolbar getDeclarationToolbarPanel() {
		...
		markAsFinishedButton.addClickHandler(event -> {
			...
			MOD3902023_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod3902023>() {
				@Override
				public void onSuccess(Mod3902023 result) {
		...
		markAsSentButton.addClickHandler(event -> {
			...
			MOD3902023_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.SENT, new AsyncCallback<Mod3902023>() {
				@Override
				public void onSuccess(Mod3902023 result) {
					getCallback().reload(result.getId());
				}
		...
		markAsPendingButton.addClickHandler(event -> {
			...
			MOD3902023_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod3902023>() {
				@Override
				public void onSuccess(Mod3902023 result) {
					getCallback().reload(result.getId());
				}
