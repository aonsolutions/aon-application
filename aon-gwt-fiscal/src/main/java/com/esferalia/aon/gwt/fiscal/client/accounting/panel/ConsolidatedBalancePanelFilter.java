...
public class ConsolidatedBalancePanelFilter extends SimpleLayoutPanel implements HasValueChangeHandlers<AccountingReportParams>{
	...
	private FlowPanel getMinMaxButtonsPanel() {
		...
		maximize.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				MaximizeEvent.fire(ConsolidatedBalancePanelFilter.this);
			}
		});
		...
		minimize.addClickHandler(new ClickHandler() {
			...
			@Override
			public void onClick(ClickEvent event) {
				MinimizeEvent.fire(ConsolidatedBalancePanelFilter.this);
			}
		});
		min.add(minimize);
		return min;
	...
	private Widget getFilterTab(AonConfiguration config, AccountingReportParams params) {
		...
		// **********************************************************************  Empresas
		tab.setWidget(2, 0, new Label("Empresas a consolidar"));
		...
		for (String d : selectedDomains.values()) {
			if (names.length() > 200) {
				remainder++;
			} else {
				names = names + "[" + d + "] , ";	
		...
		if (remainder > 0) {
			names = names + " y " + remainder + " m\u00E1s.";
		}
		if (AonStringUtils.isBlank(names)) {
			names = "[Seleccionar empresas]";
		}
		Label domains = new Label( names );
		String title = "Selecci\u00F3n de empresas";
		domains.setTitle(title);
		domains.setStyleName(AON.AON_CSS.aonClickable());
		tab.setWidget(2, 1, domains );
		tab.getCellFormatter().setStyleName(2,1, AON.AON_CSS.aonPanelGridEven());
		tab.getFlexCellFormatter().setColSpan(2, 1, 6);
		domains.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				selectDialog.setGlassEnabled(true);
				selectDialog.setModal(true);
				selectDialog.setCaption(title);
				DockLayoutPanel rootPanel = new DockLayoutPanel(Unit.PX);
				FlowPanel buttonsPanel = new FlowPanel();
				buttonsPanel.setStyleName(AON.AON_CSS.aonTextCenter());
				buttonsPanel.addStyleName(AON.AON_CSS.aonPaddingTop());
				rootPanel.addSouth(buttonsPanel, 32);
					...
					box.addClickHandler(new ClickHandler() {
						...
						public void onClick(ClickEvent event) {
							if (box.getValue()) {
								...
								selectedDomains.remove(d.getId());
							}
						}
					});
	...
	public AccountingReportParams getWidgetParams() {
		...
		for (Integer id : selectedDomains.keySet()) {
