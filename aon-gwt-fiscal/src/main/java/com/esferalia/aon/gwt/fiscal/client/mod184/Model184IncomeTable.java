...
public class Model184IncomeTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod184Income> {
	...
	private static class Model184DetailCellTable extends CellTable<Mod184Income> {
		private static final CellTable.Resources TABLE_STYLE = GWT.create(AonCellTable.class);
	
		private SingleSelectionModel<Mod184Income> model;
		
		public Model184DetailCellTable(ProvidesKey<Mod184Income> providesKey) {
			...
			model = new SingleSelectionModel<Mod184Income>(Model184.MOD184_INCOME_PROVIDES_KEY);
			this.setSelectionModel(model);
			this.setEmptyTableWidget(new HTML(AON.MSG.noData()));
		}
	
		private void addKeyColumn() {
			final TextColumn<Mod184Income> nameColumn = new TextColumn<Mod184Income>() {
				@Override
				public String getValue(Mod184Income detail) {
					return AonStringUtils.defaultIfBlank(detail.getKey(), AON.MSG.newIncome());
				}
				@Override
				public void render(Context context, Mod184Income income, SafeHtmlBuilder sb) {
					...
					sb.appendEscaped( getValue(income) );
					sb.appendHtmlConstant("</div>");
				} 
				
			};
			this.addColumn(nameColumn, "CLAVE");
			this.setColumnWidth(nameColumn, "40px");
		}
	
		private void addSubkeyColumn() {
			final TextColumn<Mod184Income> nameColumn = new TextColumn<Mod184Income>() {
				@Override
				public String getValue(Mod184Income detail) {
					return AonStringUtils.defaultIfBlank(detail.getSubKey(), "");
				}
				@Override
				public void render(Context context, Mod184Income income, SafeHtmlBuilder sb) {
					sb.appendHtmlConstant("<div style='");
			...
			this.addColumn(nameColumn, "SUBCLAVE");
			this.setColumnWidth(nameColumn, "auto");
		}

		public Mod184Income getSelected() {
	...
	public Model184IncomeTable(Model184BaseCallback cbk, Integer selectedIndex) {
		table = new Model184DetailCellTable(Model184.MOD184_INCOME_PROVIDES_KEY);
		if (table.getSelectionModel() != null) {
			table.getSelectionModel().addSelectionChangeHandler(new com.google.gwt.view.client.SelectionChangeEvent.Handler() {
				...
				public void onSelectionChange(SelectionChangeEvent event) {
					SelectionEvent.fire(Model184IncomeTable.this, table.getSelected());
				}
			});
		}
	...
	private Widget getToolbarPanel(Model184BaseCallback cbk) {
		...
		addSelectionHandler(new SelectionHandler<Mod184Income>() {
		...
		newDetailButton.setTitle(AON.MSG.newIncome());
	...
	private void newIncome(Model184BaseCallback cbk) {
		...
		table.setRowData(cbk.getMod184().getIncomes());
		table.redraw();
		int i = cbk.getMod184().getIncomes().size() - 1;
		table.getSelectionModel().setSelected(cbk.getMod184().getIncomes().get(i),true);
	...
	public Integer getSelectionIndex() {
		if ( table.getSelected() != null) {
			int i = 0;
			for ( Mod184Income det : table.getVisibleItems()) {
