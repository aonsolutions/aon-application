...
class Model111Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod111>{
	
	private static final Logger LOGGER = Logger.getLogger(Model111Table.class.getName());
	static {
	...
	protected Model111Table(Model111Callback cbk) {
	...
	@Override
	public HandlerRegistration addSelectionHandler(SelectionHandler<Mod111> handler) {
		return super.addHandler(handler, SelectionEvent.getType());
	}

	public void refresh(Model111Callback cbk) {
		...
		Model111.SERVICE.getMod111s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod111> result) {
		for ( Mod111 mod111 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod111.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod111.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod111.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod111.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model111Table.this, mod111));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod111)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod111.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod111.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod111.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod111.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod111.getStatus()) );
			row.add( statusCell );
			
			InlineLabel acc = new InlineLabel();
			acc.setTitle( AON.MSG.recorded());
			acc.setStyleName(AON.CSS.aonIconLabel());
			acc.addStyleName( mod111.isRecorded()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel declarationResult = new InlineLabel();
			if (mod111.getDeclarationResult() != null) {
				declarationResult.setText(AON.FMT.format(mod111.getDeclarationResult()));
			}
			row.addCell( comp , AON.CSS.aonTextCenter())
				.addCell( sust , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod111.getDocument()))
				.addCell( new InlineLabel(mod111.getFullName()))
				.addCell( declarationResult, AON.CSS.aonTextRight())
				.addCell( new InlineLabel(mod111.getDeclarationResultType() == null ? "" : mod111.getDeclarationResultType().getDescription()))
				.addCell( new InlineLabel(
						(mod111.getFinance() != null && mod111.getFinance().getFinanceStatus() != null)
							?mod111.getFinance().getFinanceStatus().getDescription()
							:""
