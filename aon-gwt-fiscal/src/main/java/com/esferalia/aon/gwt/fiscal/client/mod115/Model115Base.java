...
public abstract class Model115Base extends DockLayoutPanel {
	...
	private EnumMap<Mod115Key,AonDoubleBox> fieldsMap;
	...
	protected Model115Base(Mod115 mod115, Model115Callback callback) {
		...
		AonFiscalModelHeader modelHeader = new AonFiscalModelHeader( mod115 );
	...
	protected Mod115 getModel() {
		return model;
	}
	public void setModel(Mod115 mod115) {
		this.model = mod115;
	}
	
	private void select( Mod115 mod115) {
		setModel(mod115);
		refreshToolbarState();
		styleStatusLabel(mod115);
	}
	
	protected void selectAndPopulate( Mod115 mod115) {
		select(mod115);
		populate(mod115);
		showPaymentInfo(mod115);
		...
		commentsButton.addClickHandler( event -> {
			if (toast == null || toast.getParent() == null) {
				...
				comment.addValueChangeHandler(event1 -> {
					...
					styleCommentsButton();
					Model115.SERVICE.saveComments( getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod115>() {
						@Override
	...
	private void styleStatusLabel(Mod115 mod115) {
		statusLabel.setText(mod115.getStatus().getName());
		statusLabel.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB( mod115.getStatus() ));
		statusLabel.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB( mod115.getStatus() ));
	...
	private void save(AsyncCallback<Mod115> cbk) {
		...
		popup.center();
		Model115.SERVICE.save(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod115>() {
			@Override
			public void onSuccess(Mod115 result) {
				setDirty( false );
				select(result);
	...
	private void delete() {
		...
		cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
			...
			@Override
			public void onAccept() {
				Model115.SERVICE.delete(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Void>() {
					@Override
	...
	private void doReopenDeclaration() {
		...
		popup.setAnimationEnabled(true);
		popup.center();
		Model115.SERVICE.markAsPending(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod115>() {
					@Override
					public void onSuccess(Mod115 result) {
						setDirty(false);
	...
	private void markAsSent() {
		...
		Model115.SERVICE.markAsSent(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod115>() {
					@Override
					public void onSuccess(Mod115 result) {
						setDirty(false);
						selectAndPopulate(result);
	...
	private void markAsFinished() {
		...
		Model115.SERVICE.initializeForFinish(getCallback().getOptions().getOccam(),getModel(),
	...
	private void showFinalizePopup(Mod115 model) {
		AonFinishDeclarationPopup<Mod115,Model115ModuleOptions> finalizeDialog = new AonFinishDeclarationPopup<>(
			...
			new IFinishDeclarationPopupCallback<Mod115>() {
				...
				@Override
				public void onAccept(Mod115 t) {
					Model115Base.this.doMarkAsFinished();
				}

				@Override
				public void onCancel(Mod115 t) {
					// Nothing
					
				}

				@Override
				public void onCustomerCheck(Mod115 t) {
					Model115Base.this.markAsCustomerCheck();
				}
		});
	...
	private void markAsCustomerCheck() {
		...
		popup.setAnimationEnabled(true);
		popup.center();
		Model115.SERVICE.markAsCustomerCheck(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod115>() {
			@Override
			public void onSuccess(Mod115 result) {
				setDirty(false);
				select(result);
				...
				FiscalModelUtils.sendEmail(getCallback(), result); // Si ha ido bien el cambio de estado, entonces se envía email de notificación
			}
	...
	private void showPaymentInfo(Mod115 mod) {
...
//			Label label = new Label( buff.toString() );
	...
	private void paintRow(FlexTable table,final Model115Callback callback, IModelScript<Mod115Key> script) {
	...
	protected void paintLabel( FlexTable table, int row, IModelScript<Mod115Key> script) {
	...
	private int paintBox(FlexTable table, int row, int col, Mod115Key key) {
		table.setWidget(row, col, new AonBoxLabel(key.getBox()));
		return ++col;
	}

	private int paintField(FlexTable table, int row, int col, final Model115Callback callback, IModelScript<Mod115Key> script, final Mod115Key key) {
	...
	private void paintInfoCol(FlexTable table, int row, int col, final Model115Callback callback, final IModelScript<Mod115Key> script) {
	...
	public void calculateAndRefresh(final Model115Callback callback) {
		Model115.SERVICE.calculate(callback.getOptions().getOccam(),getModel(),
				new AsyncCallback<Mod115>() {
