...
public class Model3902024 extends DockLayoutPanel {
	
	private static final Integer DEFAULT_YEAR = 2024;
	private static final String MOD390_2024_DRAFT = "/aon_gwt_fiscal/Model3902024Draft";
			
	protected static final Mod3902024ServiceAsync MOD3902024_SERVICE;
	static {
		Mod3902024ServiceAsync mod3902024ServiceRaw = GWT.create(Mod3902024Service.class);
		MOD3902024_SERVICE = new Mod3902024ServiceAsyncDecorator(mod3902024ServiceRaw);
	}
	
	class Model3902024Callback {
		void markAsDirty() {
			Model3902024.this.markAsDirty();
		}
		Mod3902024 getModel() {
			return Model3902024.this.getModel(); 
		}
		void setModel(Mod3902024 model) {
			Model3902024.this.setModel(model);
		}
		void refreshDeclarationToolbarPanel() {
			Model3902024.this.refreshDeclarationToolbarPanel();			
		}
	}
	
	private SimpleLayoutPanel contentContainer = new SimpleLayoutPanel();
	private final Model390Callback callback; 
	private Mod3902024 model;
	...
	public Model3902024(final Model390Callback mod390Callback, Mod390 mod390) {
		super(Unit.PX);
		...
		popup.center();
		try {
			MOD3902024_SERVICE.get(getCallback().getOptions().getOccam(), mod390, new AsyncCallback<Mod3902024>() {
				@Override
	...
	public Mod3902024 getModel() {
		return model;
	}
	public void setModel(Mod3902024 model) {
		this.model = model;
	}
	...
	private Widget getToolbar() {
		...
		saveButton.addClickHandler(event -> {
			...
			try {
				...
				MOD3902024_SERVICE.save(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Mod3902024>() {
					@Override
					public void onSuccess(Mod3902024 result) {
						setDirty(false);
						select(result);
						popup.hide();
		...
		deleteButton.addClickHandler(event -> {
			...
			cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					MOD3902024_SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
						@Override
		...
		draftButton.addClickHandler(event -> submitForm(MOD390_2024_DRAFT));
	...
	public void select(Mod3902024 m390) {
		if ("INVALID".equals(m390.getXmlFormat()) ) {
			AonConfirmDialog cd = new AonConfirmDialog();
			cd.confirm( "La declaraci\u00F3n est\u00E1 creada con un formato incorrecto y es inv\u00E1lida. \u00BFDesea borrarla?"
					, new AonConfirmDialogCallback() {

				@Override
				public void onAccept() {
					MOD3902024_SERVICE.delete(getCallback().getOptions().getOccam(),
	...
	private Widget getLinksPanel() {
		...
		Model3902024Callback cbk = new Model3902024Callback();
	...
	private AonToolbar getDeclarationToolbarPanel() {
		...
		markAsFinishedButton.addClickHandler(event -> {
			...
			MOD3902024_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod3902024>() {
				@Override
				public void onSuccess(Mod3902024 result) {
		...
		markAsSentButton.addClickHandler(event -> {
			...
			MOD3902024_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.SENT, new AsyncCallback<Mod3902024>() {
				@Override
				public void onSuccess(Mod3902024 result) {
					getCallback().reload(result.getId());
				}
		...
		markAsPendingButton.addClickHandler(event -> {
			...
			MOD3902024_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod3902024>() {
				@Override
				public void onSuccess(Mod3902024 result) {
					getCallback().reload(result.getId());
				}
