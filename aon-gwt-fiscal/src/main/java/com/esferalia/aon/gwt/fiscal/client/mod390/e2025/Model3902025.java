...
public class Model3902025 extends DockLayoutPanel {
	
	private static final Integer DEFAULT_YEAR = 2025;
	private static final String MOD390_2025_DRAFT = "/aon_gwt_fiscal/Model3902025Draft";
			
	protected static final Mod3902025ServiceAsync MOD3902025_SERVICE;
	static {
		Mod3902025ServiceAsync mod3902025ServiceRaw = GWT.create(Mod3902025Service.class);
		MOD3902025_SERVICE = new Mod3902025ServiceAsyncDecorator(mod3902025ServiceRaw);
	}
	
	class Model3902025Callback {
		void markAsDirty() {
			Model3902025.this.markAsDirty();
		}
		Mod3902025 getModel() {
			return Model3902025.this.getModel(); 
		}
		void setModel(Mod3902025 model) {
			Model3902025.this.setModel(model);
		}
		void refreshDeclarationToolbarPanel() {
			Model3902025.this.refreshDeclarationToolbarPanel();			
		}
	}
	
	private SimpleLayoutPanel contentContainer = new SimpleLayoutPanel();
	private final Model390Callback callback; 
	private Mod3902025 model;
	...
	public Model3902025(final Model390Callback mod390Callback, Mod390 mod390) {
		super(Unit.PX);
		...
		popup.center();
		try {
			MOD3902025_SERVICE.get(getCallback().getOptions().getOccam(), mod390, new AsyncCallback<Mod3902025>() {
				@Override
	...
	public Mod3902025 getModel() {
		return model;
	}
	public void setModel(Mod3902025 model) {
		this.model = model;
	}
	...
	private Widget getToolbar() {
		...
		saveButton.addClickHandler(event -> {
			...
			try {
				...
				MOD3902025_SERVICE.save(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Mod3902025>() {
					@Override
					public void onSuccess(Mod3902025 result) {
						setDirty(false);
						select(result);
						popup.hide();
		...
		deleteButton.addClickHandler(event -> {
			...
			cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					MOD3902025_SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
						@Override
		...
		draftButton.addClickHandler(event -> submitForm(MOD390_2025_DRAFT));
	...
	public void select(Mod3902025 m390) {
		if ("INVALID".equals(m390.getXmlFormat()) ) {
			...
			cd.confirm( "La declaraci\u00F3n est\u00E1 creada con un formato incorrecto y es inv\u00E1lida. \u00BFDesea borrarla?"
				...
				@Override
				public void onAccept() {
					MOD3902025_SERVICE.delete(getCallback().getOptions().getOccam(),
	...
	private Widget getLinksPanel() {
		...
		Model3902025Callback cbk = new Model3902025Callback();
	...
	private AonToolbar getDeclarationToolbarPanel() {
		...
		markAsFinishedButton.addClickHandler(event -> {
			...
			MOD3902025_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod3902025>() {
				@Override
				public void onSuccess(Mod3902025 result) {
		...
		markAsSentButton.addClickHandler(event -> {
			...
			MOD3902025_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.SENT, new AsyncCallback<Mod3902025>() {
				@Override
				public void onSuccess(Mod3902025 result) {
					getCallback().reload(result.getId());
				}
		...
		markAsPendingButton.addClickHandler(event -> {
			...
			MOD3902025_SERVICE.changeStatus(getCallback().getOptions().getOccam(),getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod3902025>() {
				@Override
				public void onSuccess(Mod3902025 result) {
					getCallback().reload(result.getId());
				}
