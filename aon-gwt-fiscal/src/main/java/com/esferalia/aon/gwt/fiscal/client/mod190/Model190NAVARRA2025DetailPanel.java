...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190NAVARRADetail2025.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902025NavarraKey;
...
public class Model190NAVARRA2025DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	private AonIntegerBox birthYear;
	private ListBox contract;
	private AonIntegerBox lessThan3Descendent;
	...
	private AonDoubleBox foodAnnuality;
	private ListBox titConvivivencia;
	private AonDocumentTextBox spouseDocument;
	private ListBox compInfancia;
	
	public Model190NAVARRA2025DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902025NavarraKey k : Mod1902025NavarraKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190NAVARRA2025DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(event -> {
			subkey.clear();
			Mod1902025NavarraKey keyEnum = Mod1902025NavarraKey.values()[key.getSelectedIndex()];
			...
			enableOrDisableIlPanel(key, subkey, ilPanel);
			enableOrDisableAdditionalDataPanel(key, subkey, additionalDataPanel);
			enableOrDisableAdministrationPanel(key, administrationPanel);
			callback.onValueChanged(detail);
		});
		...
		CheckBox permanentDisabilityPension = new CheckBox("03. Incapacidad permanente, total o parcial");
		...
		CheckBox nonContributoryPension = new CheckBox("04. Pensiones no contributivas por invalidez o jubilaci\u00F3n");
		...
		CheckBox otherNonExemptPensions = new CheckBox("05. Resto de prestaciones art.14.2.a).1\u00AA Texto Refundido Ley Foral del Impuesto, no exentas, distintas de las anteriores");
		...
		// Panel administraciones
		
		FlexTable tab31 = new FlexTable();
		tab31.getColumnFormatter().setWidth(0, "200px");
		tab31.getColumnFormatter().setWidth(1, "200px");
		tab31.getColumnFormatter().setWidth(2, "200px");
		tab31.getColumnFormatter().setWidth(3, "200px");
		tab31.getColumnFormatter().setWidth(4, "200px");
		...
		tab4.getColumnFormatter().setWidth( 0, "150px"); 
		tab4.getColumnFormatter().setWidth( 1, "150px"); 
		...
		tab4.setWidget(1, 2, new Model190SmallerLabel("Desc. menores de 3 a\u00F1os"));
		tab4.setWidget(1, 3, new Model190SmallerLabel("Desc. resto"));
		tab4.setWidget(1, 4, new Label());		
		
		birthYear = new AonIntegerBox();
		...
		lessThan3Descendent = new AonIntegerBox();
		...
		tab4.setWidget(2, 2, lessThan3Descendent);		
		...
		tab4.setWidget(2, 3, otherDescendent);
		tab4.setWidget(2, 4, new Label());
		
		tab4.setWidget(3, 0, new Model190SmallerLabel(AON.MSG.applicableReduction()));
		tab4.setWidget(3, 1, new Model190SmallerLabel(AON.MSG.deducibleExpense()));
		tab4.setWidget(3, 2, new Model190SmallerLabel(AON.MSG.compensatoryPension()));
		tab4.setWidget(3, 3, new Model190SmallerLabel(AON.MSG.foodAnnuality()));
		tab4.setWidget(3, 4, new Label());
		...
		tab4.setWidget(4, 0, applicableReduction);
		...
		tab4.setWidget(4, 1, deducibleExpense);
		...
		tab4.setWidget(4, 2, compensatoryPension);

		foodAnnuality = new AonDoubleBox();
		...
		tab4.setWidget(4, 3, foodAnnuality);
		
		tab4.setWidget(4, 4, new Label());
		
		tab4.setWidget(5, 0, new Model190SmallerLabel("Tit. unidad conviv."));
		...
		tab4.setWidget(6,0 , titConvivivencia);
		
		tab4.setWidget(5, 1, new Model190SmallerLabel("NIF titular ud. conv."));
		...
		tab4.setWidget(6, 1, spouseDocument);

		tab4.setWidget(5, 2, new Model190SmallerLabel("Compl. ayuda infancia"));
		...
		tab4.setWidget(6, 2, compInfancia);
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902025NavarraKey.A.toString());
		}
		Mod1902025NavarraKey keyEnum = Mod1902025NavarraKey.valueOf(detail.getKey());
	...
	private void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		boolean a = (Mod1902025NavarraKey.A == keyEnum);
		boolean abc = (Mod1902025NavarraKey.A == keyEnum || Mod1902025NavarraKey.B == keyEnum || Mod1902025NavarraKey.C == keyEnum);
		boolean e01 = (Mod1902025NavarraKey.E == keyEnum && "01".equals(subk));
		boolean l0510 = (Mod1902025NavarraKey.L == keyEnum && ("05".equals(subk) || "10".equals(subk)));
		boolean l2829 = (Mod1902025NavarraKey.L == keyEnum && ("28".equals(subk) || "29".equals(subk)));
		boolean l29 = (Mod1902025NavarraKey.L == keyEnum && "29".equals(subk));
		boolean abcefghi = (Mod1902025NavarraKey.A == keyEnum || Mod1902025NavarraKey.B == keyEnum || Mod1902025NavarraKey.C == keyEnum || Mod1902025NavarraKey.E == keyEnum || Mod1902025NavarraKey.F == keyEnum || Mod1902025NavarraKey.G == keyEnum || Mod1902025NavarraKey.H == keyEnum || Mod1902025NavarraKey.I == keyEnum);
		
		panel.setVisible( abcefghi || e01 || l0510 || l2829);
		
		if (panel.isVisible()) {
			birthYear.setEnabled(abc);                         // Año nacimiento: Claves A, B, C 
			contract.setEnabled(a);                            // Tipo de relacion: Clave A
			lessThan3Descendent.setEnabled(abc);               // Descendientes menores de 3 años: Claves A, B, C
			otherDescendent.setEnabled(abc);                   // Descendientes resto: Claves A, B, C
			applicableReduction.setEnabled(abcefghi);          // Reducciones: Claves A, B, C, E, F(subclaves 01 a 06), G (subclaves 01 a 06, 08 y 09), H e I 
			deducibleExpense.setEnabled(abc || e01 || l0510);  // Gastos: Claves A, B, C, E01, L05 y L10
			compensatoryPension.setEnabled(abc); 			   // Pensiones compensatorias: Claves A, B, C 
			foodAnnuality.setEnabled(abc);  				   // Anualidades alimentos: Claves A, B, C
			titConvivivencia.setEnabled(l2829); 			   // Titular convivencia: Claves L28 y L29
			spouseDocument.setEnabled(l2829); 				   // NIF titular convivencia: Claves L28 y L29
			compInfancia.setEnabled(l29);           		   // Complemento infancia: Clave L29
		}
		
	}
	
	private void enableOrDisableIlPanel(ListBox key, ListBox subKey, Panel  panel) {
		Mod1902025NavarraKey keyEnum = Mod1902025NavarraKey.values()[key.getSelectedIndex()];
		String subk = ((subKey.getSelectedIndex() == -1) ? null : subKey.getValue(subKey.getSelectedIndex()));
		panel.setVisible( 
				    Mod1902025NavarraKey.A == keyEnum
				|| (Mod1902025NavarraKey.B == keyEnum && "01".equals(subk))
			);
	}
	
	private void enableOrDisableAdministrationPanel(ListBox key, Panel panel) {
		Mod1902025NavarraKey keyEnum = Mod1902025NavarraKey.values()[key.getSelectedIndex()];
		panel.setVisible(Mod1902025NavarraKey.E == keyEnum);
	}
