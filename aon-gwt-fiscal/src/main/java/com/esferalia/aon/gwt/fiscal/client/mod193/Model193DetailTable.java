...
public class Model193DetailTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod193Detail> {
	...
	private static class Model193DetailCellTable extends CellTable<Mod193Detail> {
		private static final CellTable.Resources TABLE_STYLE = GWT.create(AonCellTable.class);
	
		private SingleSelectionModel<Mod193Detail> model;
		
		public Model193DetailCellTable(ProvidesKey<Mod193Detail> providesKey) {
			...
			model = new SingleSelectionModel<Mod193Detail>(Model193.MOD193_DETAIL_PROVIDES_KEY);
			this.setSelectionModel(model);
			this.setEmptyTableWidget(new HTML(AON.MSG.noData()));
		}
	
		private void addNameColumn() {
			final TextColumn<Mod193Detail> nameColumn = new TextColumn<Mod193Detail>() {
				@Override
				public String getValue(Mod193Detail detail) {
					return AonStringUtils.defaultIfBlank(detail.getName(), AON.MSG.newPerceptor());
				}
				@Override
				public void render(Context context, Mod193Detail detail, SafeHtmlBuilder sb) {
		...
		public Mod193Detail getSelected() {
			return model.getSelectedObject();
		}
	}
	
	private Model193DetailCellTable table;
	private ScrollPanel tablePanel;
	
	public Model193DetailTable(Model193BaseCallback cbk, Integer selectedIndex) {
		table = new Model193DetailCellTable(Model193.MOD193_DETAIL_PROVIDES_KEY);
		if (table.getSelectionModel() != null) {
			table.getSelectionModel().addSelectionChangeHandler(new com.google.gwt.view.client.SelectionChangeEvent.Handler() {
				...
				public void onSelectionChange(SelectionChangeEvent event) {
					SelectionEvent.fire(Model193DetailTable.this, table.getSelected());
				}
			});
		}
		...
		if (table.getVisibleItemCount() == 0) {
			...
			table.getSelectionModel().setSelected(cbk.getMod193().getDetails().get(selectedIndex), true);	
		} else {
	...
	private Widget getToolbarPanel(Model193BaseCallback cbk) {
		...
		addSelectionHandler(new SelectionHandler<Mod193Detail>() {
	...
	private void newPerceptor(Model193BaseCallback cbk) {
		...
		table.setRowData(cbk.getMod193().getDetails());
		table.redraw();
		int i = cbk.getMod193().getDetails().size() - 1;
		table.getSelectionModel().setSelected(cbk.getMod193().getDetails().get(i),true);
	...
	public Integer getSelectionIndex() {
		if ( table.getSelected() != null) {
			int i = 0;
			for ( Mod193Detail det : table.getVisibleItems()) {
