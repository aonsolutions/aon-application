...
public class Mod115SendAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod115SendAEAT.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			byte[] fileContent = ModelAdmonUtils.getModelFile(mod115);
			
			JSONObject params = new JSONObject();
			params.put("MODELO", "115");
			params.put("EJERCICIO", AonNumberUtils.toString( mod115.getYear()));
			params.put("PERIODO", mod115.getPeriod().getName());
			params.put("NRC", (mod115.isStrictToDeposit()?aeatParams.getNrc() : ""));
			params.put("IDI", "ES");
...
//				: "DESACTIVADO_https://www1.agenciatributaria.gob.es/wlpl/PFTW-PICW/PresBasica");
			...
			if (response.statusCode() == 302) {
				...
				if (AonStringUtils.contains(ct, MimeType.JSON.getName())) {
					ModelAdmonUtils.manageJSONContent( resp, aeatParams, mod115 ,response.body() );
