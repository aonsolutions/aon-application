...
public abstract class PageAbs extends ResizeComposite {
	...
	private EnumMap<Mod2002017Key, DoubleBox> inputs = new EnumMap<Mod2002017Key, DoubleBox>(Mod2002017Key.class);
	private EnumMap<Mod2002017Key, BoxLabel> labels = new EnumMap<Mod2002017Key, BoxLabel>(Mod2002017Key.class);
		...
		callback.getMod200Object().register( new IMod200ChangeListener() {
			...
			public void mod200Changed(Mod2002017 mod200) {
				for ( Mod2002017Key key : inputs.keySet() ) {
					DoubleVariable2017 var = mod200.getDraftMap().get(key);
	...
	public Map<Mod2002017Key, DoubleBox> getInputs() {
		return inputs;
	}
	public Map<Mod2002017Key, BoxLabel> getLabels() {
		return labels;
	}
	
	protected int paintKey(FlexTable tab,final Mod2002017Key key,int row) {
	...
	protected void paintKeyDescription(FlexTable tab, Mod2002017Key key, int row,int col) {
	...
	protected void paintKeyField(FlexTable tab,final Mod2002017Key key,int row, int col) {
		paintKeyField(tab, key, row, col, DoubleBox.VISIBLE_LENGTH);
	}
	protected void paintKeyField(FlexTable tab,final Mod2002017Key key,int row, int col, int fieldLength) {
		...
		if (show) {
			BoxLabel code = new BoxLabel(codeId, Model2002017.BOX_LENGTH);
	...
	protected boolean isTitle(Mod2002017Key key) {
		Boolean[] behaviour = BEHAVIOUR_KEYS_MAP.get(key);
	...
	protected void dump() {
		for (Mod2002017Key key : callback.getMod200Object().getMod200().getDraftMap().keySet()) {
			if (inputs.containsKey(key)) {
				DoubleBox input = inputs.get(key);
				DoubleVariable2017 var = callback.getMod200Object().getMod200().getDraftMap().get(key);
