...
public class Page09 extends PageAbs {
		...
		for (Mod2002020Key key : Mod2002020Constants.LIQUIDATION_II_KEYS) {
			if (callback.getMod200Object().isVisible(key)) {				
				...
				if (margin) {
     			...
     			if (key == Mod2002020Key.LQ554) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.LQ554,Mod2002020LQ554Key.values(),HEADERS_1);
				}
				if (key == Mod2002020Key.LQ561) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.LQ561,Mod2002020LQ561Key.values(),HEADERS_2);
				}
    			if (key == Mod2002020Key.LQ579) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.LQ579,Mod2002020LQ579Key.values(),null);
				}
    			if (key == Mod2002020Key.LQ1032 
					...
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.LQ1032,Mod2002020LQ1032Key.values(),HEADERS_3);
				}
    			if (key == Mod2002020Key.LQ547 && !callback.getMod200Object().getMod200().isCooperativa()) {
					row = paintKeyBreakdownLink(table,row,Mod2002020Key.LQ547,Mod2002020LQ547Key.values(),HEADERS_2);
				}
    			if (key == Mod2002020Key.LQ1033) {
					row = paintKeyBreakdownLinkLQ1033(table,row,Mod2002020Key.LQ1033.getDescription());
				}
			}
		}
	}
	...
	protected int paintKeyBreakdownLinkLQ1033(final FlexTable tab,int row,final String label) {
		...
		int r = 1;
		int col = 0;
		for (IMod200KeysProvider key : Mod2002020LQ1033_1Key.values()) {
			...
			for (final IMod200Key k : key.getKeys() ) {
				if (k != null && k!= Mod2002020Key.LQ1033){
		...
		r = 1;
		col = 0;
		for (IMod200KeysProvider key : Mod2002020LQ1033_2Key.values()) {
			Label desc = new Label(key.getDescription() );
	...
	protected boolean isDisabled(IMod200Key key) {
		...
		if (key == Mod2002020Key.LQ562 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002020Key.C0030) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002020Key.C0047))) {
			return false;
		}
		if ((key == Mod2002020Key.LQ520 || key == Mod2002020Key.LQ521) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002020Key.C0064))) {
			return true;
		}
