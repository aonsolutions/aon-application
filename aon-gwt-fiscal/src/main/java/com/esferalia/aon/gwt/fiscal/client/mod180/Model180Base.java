...
abstract class Model180Base extends DockLayoutPanel {
	...
	private static final String MODEL180_DRAFT = "/aon_gwt_fiscal/ms/Model180Draft";
	protected static final String MODEL180_FILE = "/aon_gwt_fiscal/ms/Model180File";
	...
	private Model180Callback callback;
	private boolean dirty;

	protected FiscalModelAdmonPanel<Mod180, Model180ModuleOptions> admonPanel;
	...
	protected final AonToolbarButton certificateButton = new AonToolbarButton(AON.MSG.mod180CertificatePrint(),AON.CSS.aonIconPdf());
	...
	public Model180Base(Model180Callback cbk, Mod180 mod180) {
		super(Unit.PX);
		this.callback = cbk;
		
		select( mod180 );
		
		AonFiscalModelHeader modelHeader = new AonFiscalModelHeader(this.mod180);
	...
	public Model180Callback getCallback() {
		return callback;
	}
	protected Mod180 getModel() {
		return mod180;
	}
	public void setModel(Mod180 mod180) {
		this.mod180 = mod180;
	}

	public IModel180Detail getDetailManager() {
		return detailManager;
	}
	public void setDetailManager(IModel180Detail detailManager) {
		...
		commentsButton.addClickHandler( event -> {
			if (toast == null || toast.getParent() == null) {
				...
				comment.addValueChangeHandler(event1 -> {
					...
					styleCommentsButton();
					Model180.SERVICE.saveComments( getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod180>() {
						@Override
	...
	protected void save(AsyncCallback<Mod180> cbk) {
		...
		popup.center();
		Model180.SERVICE.save(getCallback().getOptions().getOccam(),
				getModel(), new AsyncCallback<Mod180>() {
					@Override
					public void onSuccess(Mod180 result) {
						popup.hide();
		...
		if (isDirty()) {
			...
			cd.confirm(AON.MSG.confirmDeclarationCancelAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					if (getCallback().getOptions().isBackButtonVisible() && getCallback().getOptions().hasExternalCallback()) {
						getCallback().getOptions().getExternalCallback().onExit(mod180);
					} else {
	...
	private void delete() {
		...
		cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
			...
			@Override
			public void onAccept() {
				Model180.SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
					@Override
					public void onSuccess(Void result) {
						deleteButton.setEnabled(true);
						getCallback().onRemove(mod180);
					}
	...
	private void markAsFinished() {
		...
		Model180.SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod180>() {
			@Override
			public void onSuccess(Mod180 result) {
				getCallback().onSelect(result , detailManager.getSelectedPerceptorIndex() );
			}
	...
	private void markAsSent() {
		...
		Model180.SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.SENT, new AsyncCallback<Mod180>() {
			@Override
			public void onSuccess(Mod180 result) {
	...
	private void markAsPending() {
		...
		Model180.SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod180>() {
			@Override
			public void onSuccess(Mod180 result) {
				callback.onSelect(result, detailManager.getSelectedPerceptorIndex());
			}
	...
	private void certificate() {
		if (isDirty()) {
			new AonConfirmDialog().confirm(AON.MSG.mod180CertificatePrint(),AON.MSG.mod180CertificatePrintNote() 
					, new AonConfirmDialogCallback() {
	...
	protected void paintPerceptorsTab(TabLayoutPanel tabPanel, Integer selectedIndex) {
		...
		if ( getModel().getYear() < 2020 ) {
			detailManager = new Model180Detail2017( getCallback() , getModel(), selectedIndex );
		} else {
			detailManager = new Model180Detail2020( getCallback() , getModel(), selectedIndex );
		}
		tabPanel.add( (Widget) detailManager,  AON.MSG.receiverList() );
