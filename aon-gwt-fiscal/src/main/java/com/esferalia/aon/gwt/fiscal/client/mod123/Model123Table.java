...
class Model123Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod123>{
	
	private static final Logger LOGGER = Logger.getLogger(Model123Table.class.getName());
	static {
	...
	protected Model123Table(Model123Callback cbk) {
	...
	@Override
	public HandlerRegistration addSelectionHandler(SelectionHandler<Mod123> handler) {
		return super.addHandler(handler, SelectionEvent.getType());
	}

	public void refresh(Model123Callback cbk) {
		...
		Model123.SERVICE.getMod123s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod123> result) {
		for ( Mod123 mod123 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod123.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod123.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod123.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod123.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model123Table.this, mod123));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod123)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod123.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod123.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod123.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod123.getStatus()) );
