...
public class Mod111ValidatePrintAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod111ValidatePrintAEAT.class.getName()); 
	...
	private enum AeatUrl {
		URL_2021_2_SEMESTER {
			...
			@Override
			protected boolean accept(Mod111 mod111) {
				return mod111.getYear() <= 2024;
			}
			...
			@Override
			protected String getUrlParameters(Mod111 mod111) throws IOException {
				...
				return MessageFormat.format("MOD=111&EJF={0}&FIC={1}&IDI=ES"
						,AonNumberUtils.toString( mod111.getYear())
						,ModelAdmonUtils.getEncodedFile(output.toByteArray(),StandardCharsets.ISO_8859_1));
			}
		};
		private static AeatUrl getAeatUrl(Mod111 mod111) {
			for (AeatUrl aeatUrl : AeatUrl.values()) {
				if (aeatUrl.accept(mod111)) {
					return aeatUrl;
				}
			}
		...
		protected abstract boolean accept( Mod111 mod111);
		protected abstract String getUrl();
		protected abstract String getUrlParameters(Mod111 mod111) throws IOException;
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (mod111.getYear() >= 2025) {
				ModelAdmonUtils.serValiDos(resp, aeatParams, mod111);
			} else {
				AeatUrl aeatURL = AeatUrl.getAeatUrl(mod111);
				HttpRequest request = HttpRequest.newBuilder()
					.uri(URI.create( aeatURL.getUrl() ))
					.POST(HttpRequest.BodyPublishers.ofString(aeatURL.getUrlParameters(mod111)))
