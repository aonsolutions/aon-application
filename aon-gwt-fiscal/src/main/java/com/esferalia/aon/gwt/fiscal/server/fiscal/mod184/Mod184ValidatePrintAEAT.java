...
@WebServlet(name = "Mod184 Validate Print AEAT", urlPatterns = { "/aon_gwt_fiscal/ms/Mod184ValidatePrintAEAT" })
public class Mod184ValidatePrintAEAT extends HttpServlet {

	private static final Logger LOGGER = Logger.getLogger(Mod184ValidatePrintAEAT.class.getName()); 
	...
	private enum AeatUrl {
		URL_1841 {
			...
			@Override
			protected boolean accept(Mod184 mod184) {
				return true;
			}
			...
			@Override
			protected String getUrlParameters(Mod184 mod184) throws IOException {
				...
				return MessageFormat.format("MOD=184&EJF={0}&FIC={1}&IDI=ES"
						,AonNumberUtils.toString( mod184.getYear())
						,ModelAdmonUtils.getEncodedFile(output.toByteArray(),StandardCharsets.ISO_8859_1));
			}
		};
		private static AeatUrl getAeatUrl(Mod184 mod184) {
			for (AeatUrl aeatUrl : AeatUrl.values()) {
				if (aeatUrl.accept(mod184)) {
					return aeatUrl;
				}
			}
		...
		protected abstract boolean accept( Mod184 mod184);
		protected abstract String getUrl();
		protected abstract String getUrlParameters(Mod184 mod184) throws IOException;
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			AeatUrl aeatURL = AeatUrl.getAeatUrl(mod184);
			HttpRequest request = HttpRequest.newBuilder()
				.uri(URI.create( aeatURL.getUrl() ))
				.POST(HttpRequest.BodyPublishers.ofString(aeatURL.getUrlParameters(mod184)))
