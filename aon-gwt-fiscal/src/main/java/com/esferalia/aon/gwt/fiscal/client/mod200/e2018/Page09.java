...
import com.esferalia.aon.occam.api.model.fiscal.mod200_2018.Mod2002018LQ579Key;
...
/**/			if (key == Mod2002018Key.LQ554) {
					row = paintKeyBreakdownLink(table,row,Mod2002018Key.LQ554,Mod2002018LQ554Key.values(),HEADERS_1);
				}
				if (key == Mod2002018Key.LQ561) {
					row = paintKeyBreakdownLink(table,row,Mod2002018Key.LQ561,Mod2002018LQ561Key.values(),HEADERS_2);
				}
/**/			if (key == Mod2002018Key.LQ579) {
					row = paintKeyBreakdownLink(table,row,Mod2002018Key.LQ579,Mod2002018LQ579Key.values(),null);
				}
/**/			if (key == Mod2002018Key.LQ1032 
					...
					row = paintKeyBreakdownLink(table,row,Mod2002018Key.LQ1032,Mod2002018LQ1032Key.values(),HEADERS_3);
				}
/**/			if (key == Mod2002018Key.LQ547 && !callback.getMod200Object().getMod200().isCooperativa()) {
					row = paintKeyBreakdownLink(table,row,Mod2002018Key.LQ547,Mod2002018LQ547Key.values(),HEADERS_2);
				}
/**/			if (key == Mod2002018Key.LQ1033) {
					row = paintKeyBreakdownLinkLQ1033(table,row,Mod2002018Key.LQ1033.getDescription());
				}
			}
		}
	}
	...
	protected int paintKeyBreakdownLinkLQ1033(final FlexTable tab,int row,final String label) {
		...
		int r = 1;
		int col = 0;
		for (IMod200KeysProvider key : Mod2002018LQ1033_1Key.values()) {
			...
			for (final Mod2002018Key k : key.getKeys() ) {
				if (k != null && k!= Mod2002018Key.LQ1033){
		...
		r = 1;
		col = 0;
		for (IMod200KeysProvider key : Mod2002018LQ1033_2Key.values()) {
			Label desc = new Label(key.getDescription() );
	...
	protected boolean isDisabled(Mod2002018Key key) {
		...
		if (key == Mod2002018Key.LQ562 
				&& (callback.getMod200Object().getMod200().isChecked(Mod2002018Key.C0030) 
				|| callback.getMod200Object().getMod200().isChecked(Mod2002018Key.C0047))) {
			return false;
		}
		if ((key == Mod2002018Key.LQ520 || key == Mod2002018Key.LQ521) && 
			(callback.getMod200Object().getMod200().isChecked(Mod2002018Key.C0064))) {
			return true;
		}
