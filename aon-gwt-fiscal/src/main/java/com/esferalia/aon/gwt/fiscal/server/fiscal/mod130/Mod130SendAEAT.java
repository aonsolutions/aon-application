...
public class Mod130SendAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod130SendAEAT.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			byte[] fileContent = ModelAdmonUtils.getModelFile(mod130);
			
			JSONObject params = new JSONObject();
			params.put("MODELO", "130");
			params.put("EJERCICIO", AonNumberUtils.toString( mod130.getYear()));
			params.put("PERIODO", mod130.getPeriod().getName());
			params.put("NRC", (mod130.isStrictToDeposit()?aeatParams.getNrc() : ""));
			params.put("IDI", "ES");
...
//				: "DESACTIVADO_https://www1.agenciatributaria.gob.es/wlpl/PFTW-PICW/PresBasica");
			...
			if (response.statusCode() == 302) {
				...
				if (AonStringUtils.contains(ct, MimeType.JSON.getName())) {
					ModelAdmonUtils.manageJSONContent( resp, aeatParams, mod130 ,response.body() );
