...
public class Page16 extends PageAbs {
	...
	protected void initializeTable() {
		...
		++row;
		for (int i = 0; i < Mod2002019Constants.CANARIAS_KEYS.length; i++) {
			Mod2002019Key[] keys = Mod2002019Constants.CANARIAS_KEYS[i];
			paintDescription(table, Mod2002019Constants.CANARIAS_KEYS_DESCRIPTIONS[i], row, 0, false);
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && (callback.getMod200Object().getMod200().isChecked(Mod2002019Key.C0029));
		return av;
	}
	
	@Override
	protected boolean isDisabled(IMod200Key key) {
		if (key == Mod2002019Key.RC020 || key == Mod2002019Key.RC021)  {
			return callback.getMod200Object().getMod200().isNotChecked(Mod2002019Key.C0045);
		}
