...
import com.esferalia.aon.gwt.fiscal.client.mod193.Model193AEATDetail2025.IModel193DetailCallback;
...
public class Model193AEAT2025DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model193AEAT2025DetailPanel(Mod193Detail detail, IModel193DetailCallback callback) {
		...
		tab2.getColumnFormatter().setWidth(7, "100px");
		...
		tab2.getFlexCellFormatter().setColSpan(0, 0, 9);
		...
		Model193AEAT2025DetailPanel.setValue(key, nature, detail);
		...
		keyCode.addItem("4 - La identificaci\u00F3n se realiza mediante el c\u00F3digo ISIN y el NIF del emisor.");
		...
		// Código ISIN
		AonTextBox isin = new AonTextBox();
		isin.setVisibleLength(12);
		isin.setMaxLength(12);
		isin.setValue(detail.getIsin());
		isin.addValueChangeHandler(event -> {
			detail.setIsin(isin.getValue());
			callback.onValueChanged(detail);
		});
		tab2.setWidget(2, 6, isin);
		...
		tab2.setWidget(2, 8, ceutaMelillaPalma);
