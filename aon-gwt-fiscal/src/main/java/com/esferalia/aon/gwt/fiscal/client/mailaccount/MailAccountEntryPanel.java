...
public class MailAccountEntryPanel extends AonCustomDockLayout {
	...
	public MailAccountEntryPanel(RegistryModuleOptions options) {
		super("Cuentas Correo");
		...
		addKeyUpHandler(e -> {
			...
			if(AonStringUtils.isNotBlank(value) && value.length() > 2) {
				mailAccountTable.setSearchPattern(value);
				mailAccountTable.onSearch();
			} else if(AonStringUtils.isBlank(value)) {
				mailAccountTable.setSearchPattern(null);
				mailAccountTable.onSearch();
			}
		});
		...
		type.getListBox().addChangeHandler(event -> {
			mailAccountTable.setTypeFilter(type.getValue());
			mailAccountTable.onSearch();
		});
		...
		mailAccountTable = new MailAccountTable(this.options.getDomainName(), this.options.getDomain(), this.options.getUser(), false) {
		...
		container.add(mailAccountTable);
	...
	protected void onClearFilter() {
		...
		mailAccountTable.setSearchPattern(null);
		mailAccountTable.setTypeFilter("all");
		mailAccountTable.onSearch();
	...
	private void createToolbar() {
		...
		createButton.addClickHandler(e -> {
			mailAccountTable.createMailAccount();
		});
