...
public class Page18 extends PageAbs {
	...
	public Page18( Model200PageCallback callback ) {
		...
		baseContainerPanel.setStyleName(AON.AON_CSS.aonFiscalContainer());
			...
			gr1.setStyleName(AON.AON_CSS.aonGroup());
				...
				groupEntitiesProvider = new ListDataProvider<String>();
				...
				newGroupEntity.addClickHandler(new ClickHandler() {
					...
					public void onClick(ClickEvent event) {
						groupEntitiesProvider.getList().add(new String());
		...
		baseContainerPanel.add(gr1);	
			...
			gr2.setStyleName(AON.AON_CSS.aonGroup());
				...
				establishmentsProvider = new ListDataProvider<String>();
		    ...
		    public void update(int index, String ca, String value) {
		    	...
		    	cd.confirm(AON.MSG.confirmDeleteAction(), new ConfirmDialogCallback() {
					...
					public void onAccept() {
						callback.getMod200Object().getMod200().getGroupEntities().remove(index);
						groupEntitiesProvider = new ListDataProvider<String>(callback.getMod200Object().getMod200().getGroupEntities());
						groupEntitiesProvider.addDataDisplay(groupEntitiesTable);
						groupEntitiesTable.redraw();
					}
				});
		...
		col.setFieldUpdater(new FieldUpdater<String, String>() {
		    ...
		    public void update(int index, String ca, String value) {
		    	...
		    	cd.confirm(AON.MSG.confirmDeleteAction(), new ConfirmDialogCallback() {
					...
					@Override
					public void onAccept() {
						callback.getMod200Object().getMod200().getEstablishments().remove(index);
						establishmentsProvider = new ListDataProvider<String>(callback.getMod200Object().getMod200().getEstablishments());
						establishmentsProvider.addDataDisplay(establishmentsTable);
						establishmentsTable.redraw();
					}
				});
	...
	protected void populate() {
		...
		for (String cp : groupEntitiesProvider.getList()) {
			listIn.add(cp);
		}
		callback.getMod200Object().getMod200().setGroupEntities(listIn);
		
		LinkedList<String> list2 = new LinkedList<String>();
		for (String cp : establishmentsProvider.getList()) {
			list2.add(cp);
		}
		callback.getMod200Object().getMod200().setEstablishments(list2);
