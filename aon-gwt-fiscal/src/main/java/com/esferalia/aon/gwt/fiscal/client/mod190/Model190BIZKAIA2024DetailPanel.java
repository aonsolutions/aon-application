...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190BIZKAIADetail2024.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902024BizkaiaKey;
...
public class Model190BIZKAIA2024DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190BIZKAIA2024DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902024BizkaiaKey k : Mod1902024BizkaiaKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190BIZKAIA2024DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(event -> {
			subkey.clear();
			Mod1902024BizkaiaKey keyEnum = Mod1902024BizkaiaKey.values()[key.getSelectedIndex()];
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902024BizkaiaKey.A.toString());
		}
		Mod1902024BizkaiaKey keyEnum = Mod1902024BizkaiaKey.valueOf(detail.getKey());
	...
	private void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		boolean abc = (Mod1902024BizkaiaKey.A == keyEnum) ||
					  (Mod1902024BizkaiaKey.B == keyEnum && ("01".equals(subk) || "03".equals(subk))) ||
				      (Mod1902024BizkaiaKey.C == keyEnum);
		boolean e = (Mod1902024BizkaiaKey.E == keyEnum);
		boolean fghi = (Mod1902024BizkaiaKey.F == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk))) ||
				  	   (Mod1902024BizkaiaKey.G == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk) || "04".equals(subk) || "05".equals(subk) || "06".equals(subk) || "08".equals(subk))) ||
				  	   (Mod1902024BizkaiaKey.H == keyEnum) ||
				  	   (Mod1902024BizkaiaKey.I == keyEnum);
		boolean l051027 = (Mod1902024BizkaiaKey.L == keyEnum && ("05".equals(subk) || "10".equals(subk) || "27".equals(subk)));
		boolean l29 = (Mod1902024BizkaiaKey.L == keyEnum && "29".equals(subk));
	...
	private void enableOrDisableAdministrationPanel(ListBox key, Panel panel) {
		Mod1902024BizkaiaKey keyEnum = Mod1902024BizkaiaKey.values()[key.getSelectedIndex()];
		panel.setVisible(Mod1902024BizkaiaKey.E == keyEnum);
	}
