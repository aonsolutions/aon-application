...
public class ProductCatalogueBooking extends HTMLPanel {
	...
	private void createFee(Product product) {
		if(product.isComposition()) {
			List<Integer> packItemIds = products.stream().filter(p -> p.isComposition()).map(p -> p.getItem().getId()).collect(Collectors.toList());
	...
	private Button createServiceButton(Product packProduct) {
		...
		bookBtn.getElement().getStyle().setProperty("width", "6rem");
