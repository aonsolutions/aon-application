...
public class Model184Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod184>{
	...
	protected Model184Table(Model184Callback cbk) {
	...
	private Widget getToolbarPanel(Model184Callback cbk) {
		AonToolbar toolbar = new AonToolbar( "Modelo 184");
	...
	public void refresh(Model184Callback cbk) {
		...
		Model184.SERVICE.getMod184s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod184> result) {
		for ( Mod184 mod184 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod184.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod184.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod184.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod184.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model184Table.this, mod184));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod184)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod184.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod184.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod184.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod184.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod184.getStatus()) );
			row.add( statusCell );
			
			row.addCell( comp , AON.CSS.aonTextCenter())
				.addCell( sust , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod184.getDocument()))
				.addCell( new InlineLabel(mod184.getFullName()))
				;
		}
