...
public class Model3902014 extends ResizeComposite {
	...
	public Model3902014(final IMod390CallBack mod390CallBack) {
		...
		errorPage.addSelectionChangeHandler(new Handler() {
			...
			public void onSelectionChange(SelectionChangeEvent event) {
				ValidationMessage2014 v =  errorPage.getSelected();
				if (v != null) {
					showPage(v.getPage());
				}
			}
	...
	void onNewButtonClick(ClickEvent event) {
		cleanErrorMessage();
		fiscalService.initializeMod390(getCurrentDomainName(),domain, DEFAULT_YEAR,
				new AsyncCallback<Mod3902014>() {
			@Override
	...
	private void cleanErrorMessage() {
		errorPage.addErrorMsg(new LinkedList<ValidationMessage2014>());
		closeFootPanel();
	}
...
//	}
	...
	private static enum Messages {
		// PAGE00
		 EMPTY_YEAR		(new ValidationMessage2014(0, AON.MSG.requiredField(AON.MSG.fiscalYear())))
		,EMPTY_DOCUMENT	(new ValidationMessage2014(0, AON.MSG.requiredField(AON.MSG.document())))
		,WRONG_DOCUMENT	(new ValidationMessage2014(0, "El NIF/DNI no es correcto"))
		,REQ_NAME 		(new ValidationMessage2014(0, "Para personas f\u00EDsicas, el nombre es obligatorio."))
		,REQ_SURNAME 	(new ValidationMessage2014(0, "Para personas f\u00EDsicas, el primer apellido es obligatorio."))
		,EMPTY_NAME 	(new ValidationMessage2014(0, "No se ha indicado el nombre del declarante."))
		// PAGE01
		,EMPTY_ACTI 	(new ValidationMessage2014(1, "No se ha indicado actividad principal."))
		// PAGE02
		,EMPTY_REPR 	(new ValidationMessage2014(2, "Indique datos del represante."))
		,EMPTY_REPR_DOC	(new ValidationMessage2014(2, "Para personas f\u00EDsicas, el NIF/DNI del representante es obligatorio."))
		,WRONG_REPR_DOC	(new ValidationMessage2014(2, "El NIF/DNI del representante no es correcto."))
		,LG1_WRONG_DOC	(new ValidationMessage2014(2, "El NIF del primer representante para personas jur\u00EDdicas no es correcto."))
		,LG2_WRONG_DOC	(new ValidationMessage2014(2, "El NIF del segundo representante para personas jur\u00EDdicas no es correcto."))
		,LG3_WRONG_DOC	(new ValidationMessage2014(2, "El NIF del tercer representante para personas jur\u00EDdicas no es correcto."))
		;
		
		private ValidationMessage2014 msg;
		private Messages(ValidationMessage2014 msg) {
			this.msg = msg;
		}
		public ValidationMessage2014 getMsg() {
			return msg;
		}
	};  
	
	private void validate(Mod3902014 m390) {
		LinkedList<ValidationMessage2014> msg = new LinkedList<ValidationMessage2014>();
		if (m390.getYear() != 2014 && m390.getYear() != 2015) msg.add(Messages.EMPTY_YEAR.getMsg());
		if (!m390.isLegalEntity()) {
			if (AonStringUtils.isEmpty(m390.getDocument())) msg.add(Messages.EMPTY_DOCUMENT.getMsg());
			if (!AonDocumentUtil.isValid(m390.getDocument())) msg.add(Messages.WRONG_DOCUMENT.getMsg());
			if (AonStringUtils.isEmpty(m390.getName())) msg.add(Messages.REQ_NAME.getMsg());
			if (AonStringUtils.isEmpty(m390.getFirstSurname())) msg.add(Messages.REQ_SURNAME.getMsg());
		} else {
			if (AonStringUtils.isEmpty(m390.getName())) msg.add(Messages.EMPTY_NAME.getMsg());
		}
		if (m390.getMainActivity() == null || AonStringUtils.isEmpty(m390.getMainActivity().getKey())) 
			msg.add(Messages.EMPTY_ACTI.getMsg());
		if (!m390.isLegalEntity()) {
			if (m390.getAddress() == null) msg.add(Messages.EMPTY_REPR.getMsg());
			if (AonStringUtils.isEmpty(m390.getAddress().getRdocument())) msg.add(Messages.EMPTY_REPR_DOC.getMsg());
			if (!AonDocumentUtil.isValid(m390.getAddress().getRdocument())) msg.add(Messages.WRONG_REPR_DOC.getMsg());
		} else {
			if (m390.getLegalRepr1() != null &&
				!AonDocumentUtil.isValid(m390.getLegalRepr1().getDocument())) msg.add(Messages.LG1_WRONG_DOC.getMsg());
			if (m390.getLegalRepr2() != null &&
				!AonDocumentUtil.isValid(m390.getLegalRepr2().getDocument())) msg.add(Messages.LG2_WRONG_DOC.getMsg());
			if (m390.getLegalRepr3() != null &&
				!AonDocumentUtil.isValid(m390.getLegalRepr3().getDocument())) msg.add(Messages.LG3_WRONG_DOC.getMsg());
		}
		if (!msg.isEmpty()) {
			errorPage.addErrorMsg( msg );
			resultsPanel.setWidget(errorPage);
