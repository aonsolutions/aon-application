...
public class OfferExcelAction extends AbsExcelAction implements Consumer<OfferDetail> {
	...
	protected void headerRow() {
		...
		CellUtil.createCell(row, cellCount, "Nº Presupuesto", headerCellStyle);
	...
	@Override
	public void accept(OfferDetail detail) {
		row = sheet.createRow(rowCount++);
		cellCount = 0;
		alignCenter( addCell( detail.getOffer().getType().getDescription() ) );
		alignCenter( addCell( detail.getOffer().getStatus().getDescription() ) );
