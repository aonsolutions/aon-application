...
public class Page16 extends PageAbs {
	...
	protected void initializeTable() {
		...
		++row;
		for (int i = 0; i < Mod2002017Constants.CANARIAS_KEYS.length; i++) {
			Mod2002017Key[] keys = Mod2002017Constants.CANARIAS_KEYS[i];
			paintDescription(table, Mod2002017Constants.CANARIAS_KEYS_DESCRIPTIONS[i], row, 0, false);
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && (callback.getMod200Object().getMod200().isChecked(Mod2002017Key.C0029));
		return av;
	}
	
	@Override
	protected boolean isDisabled(Mod2002017Key key) {
		if (key == Mod2002017Key.RC020 || key == Mod2002017Key.RC021)  {
			return callback.getMod200Object().getMod200().isNotChecked(Mod2002017Key.C0045);
		}
