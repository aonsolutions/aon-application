...
public class Mod131SendAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod131SendAEAT.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			byte[] fileContent = ModelAdmonUtils.getModelFile(mod131);
			
			JSONObject params = new JSONObject();
			params.put("MODELO", "131");
			params.put("EJERCICIO", AonNumberUtils.toString( mod131.getYear()));
			params.put("PERIODO", mod131.getPeriod().getName());
			params.put("NRC", (mod131.isStrictToDeposit()?aeatParams.getNrc() : ""));
			params.put("IDI", "ES");
...
//				: "DESACTIVADO_https://www1.agenciatributaria.gob.es/wlpl/PFTW-PICW/PresBasica");
			...
			if (response.statusCode() == 302) {
				...
				if (AonStringUtils.contains(ct, MimeType.JSON.getName())) {
					ModelAdmonUtils.manageJSONContent( resp, aeatParams, mod131 ,response.body() );
