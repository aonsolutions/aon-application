...
public class AonCheckItBankBox extends ResizeComposite implements HasValue<String>
	, HasDescription, Focusable, HasSelectionHandlers<CheckItBank>, HasAllFocusHandlers
	,HasAllKeyHandlers {
	
	private static final Logger LOGGER = Logger.getLogger(AonCheckItBankBox.class.getName());
	static {
	...
	public static class LoginPanel extends FlowPanel {
		public LoginPanel(Integer bankId) {
					...
					if (banks != null) {
						for (final CheckItBank bank : banks) {
							if (containsWords(bank.getName(), request.getQuery())) {
						        ...
						        AccountSuggestion as = new AccountSuggestion(bank, bank.getName(), bld.toSafeHtml().asString());
						        if (AonStringUtils.containsIgnoreCase(bank.getName(), request.getQuery()))
						        	suggestions.addFirst(as);
						        else	
						        	suggestions.add(as);
					        }
						}
...
//			}
	...
	private void select(String accountCode) {
		if (AonValidationUtil.isValidAccount(accountCode,isRequired())) {
			if (!isRequired() && AonStringUtils.isEmpty(accountCode)) {
				...
				reset();
				SelectionEvent.fire(AonCheckItBankBox.this, null );
			} else {
				...
				if (result != null) {
					...
					AonCheckItBankBox.this.setFocus(true);
				}
	...
	private void select(CheckItBank result) {
		...
		SelectionEvent.fire(AonCheckItBankBox.this, result );
	}
...
//		simplePanel.add(new InlineLabel("no lo se"));
//		
//		dialog.hide();
...
//		
//		dialog.add( simplePanel );
//		dialog.center();
//		dialog.show();
//	}
	
	private boolean containsWords(String main, String search) {
		if (main == null || search == null)
			return false;
		String[] mainWords = main.split("\\s");
		String[] words = search.split("\\s");
		
		if (AonStringUtils.containsIgnoreCase(main, search))
			return true;
		
		for (String word : words) {
			if (Arrays.stream(mainWords).filter(w -> AonStringUtils.equalsIgnoreCase(w, word)).findAny().isPresent())
				return true;
		}
