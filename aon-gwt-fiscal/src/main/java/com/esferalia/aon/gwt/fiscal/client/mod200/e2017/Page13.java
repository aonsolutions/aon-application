...
public class Page13 extends PageAbs {
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (final Mod2002017Key key : Mod2002017Constants.COMBINED_TAXATION_1) {
			if (callback.getMod200Object().isVisible(key)) {
				row = paintKey(table,key,row);
			}
		}
		
		paintTable(table1, Mod2002017Constants.COMBINED_TAXATION_2);

		getFlexTable(table2,0,null, new String[]{"","ARABA","GIPUZKOA","BIZKAIA","NAVARRA","TOTAL"});
		table2.getColumnFormatter().setWidth(0, "auto");
		row = 1;
		for (Mod2002017Key[] keys : Mod2002017Constants.COMBINED_TAXATION_3) {
	...
	private void paintTable(FlexTable table, Mod2002017Key[] keys) {
		table.setWidth("100%");
		table.setCellSpacing(0);
		table.getColumnFormatter().setWidth(1, "200px");	
		int row = 0;
		for (Mod2002017Key key : keys) {
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && (callback.getMod200Object().getMod200().isChecked(Mod2002017Key.C0028));
		return av;
	}
