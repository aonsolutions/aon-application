...
public class Mod111SendAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod111SendAEAT.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			byte[] fileContent = ModelAdmonUtils.getModelFile(mod111);
			
			JSONObject params = new JSONObject();
			params.put("MODELO", "111");
			params.put("EJERCICIO", AonNumberUtils.toString( mod111.getYear()));
			params.put("PERIODO", mod111.getPeriod().getName());
			params.put("NRC", (mod111.isStrictToDeposit()?aeatParams.getNrc() : ""));
			params.put("IDI", "ES");
...
//				: "DESACTIVADO_https://www1.agenciatributaria.gob.es/wlpl/PFTW-PICW/PresBasica");
			...
			if (response.statusCode() == 302) {
				...
				if (AonStringUtils.contains(ct, MimeType.JSON.getName())) {
					ModelAdmonUtils.manageJSONContent( resp, aeatParams, mod111 ,response.body() );
