...
public class Mod123SendAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod123SendAEAT.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			byte[] fileContent = ModelAdmonUtils.getModelFile(mod123);
			
			JSONObject params = new JSONObject();
			params.put("MODELO", "123");
			params.put("EJERCICIO", AonNumberUtils.toString( mod123.getYear()));
			params.put("PERIODO", mod123.getPeriod().getName());
			params.put("NRC", (mod123.isStrictToDeposit()?aeatParams.getNrc() : ""));
			params.put("IDI", "ES");
...
//				: "DESACTIVADO_https://www1.agenciatributaria.gob.es/wlpl/PFTW-PICW/PresBasica");
			...
			if (response.statusCode() == 302) {
				...
				if (AonStringUtils.contains(ct, MimeType.JSON.getName())) {
					ModelAdmonUtils.manageJSONContent( resp, aeatParams, mod123 ,response.body() );
