...
public class Model111Araba2016 extends Model111Base {
	...
	public Model111Araba2016(Mod111 mod111, Model111Callback callback) {
	...
	private void paintRow907(FlexTable table, IModelScript<Mod111Key> script) {
		int row = table.getRowCount();
		final FiscalModelDetail ar907 = getModel().ensureDetail(Mod111Key.AR_907);
	...
	private void paintRow908(FlexTable table, IModelScript<Mod111Key> script) {
		int row = table.getRowCount();
		final  FiscalModelDetail ar908 = getModel().ensureDetail(Mod111Key.AR_908);
		table.setWidget(row, 0, new Label(script.getLabel()));
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonTextRight() );
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonPaddingRight() );
		table.getFlexCellFormatter().setColSpan(row, 0, 5);
	...
	private void paintRow909(FlexTable table, IModelScript<Mod111Key> script) {
		int row = table.getRowCount();
		final FiscalModelDetail ar909 = getModel().ensureDetail(Mod111Key.AR_909);
		table.setWidget(row, 0, new Label(script.getLabel()));
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonTextRight() );
		table.getFlexCellFormatter().addStyleName(row, 0,AON.CSS.aonPaddingRight() );
		table.getFlexCellFormatter().setColSpan(row, 0, 5);
		...
		IFiscalModelAdmonPanelCallback<Mod111, Model111ModuleOptions> cbk = 
			new IFiscalModelAdmonPanelCallback<Mod111, Model111ModuleOptions>() {
				...
				public Mod111 getModel() {
					return Model111Araba2016.this.getModel();
				}
