...
public class Page13 extends PageAbs {
	...
	private static String[] KEY_DESCRIPTIONS = new String[] {
		...
		,"Nueva declaraci\u00F3n art. 12.2.b) RDLey 19/2020 no complementaria: importe que result\u00F3 a ingresar/devoluci\u00F3n solicitada en la declaraci\u00F3n originaria"		
	...
	protected void initializeTable() {
		...
		int row = 0;
		for (final Mod2002019Key key : Mod2002019Constants.COMBINED_TAXATION_1) {
			if (callback.getMod200Object().isVisible(key)) {
				row = paintKey(table,key,row);
			}
		}
		
		paintTable(table1, Mod2002019Constants.COMBINED_TAXATION_2);

		getFlexTable(table2,0, new String[]{"","ARABA","GIPUZKOA","BIZKAIA","NAVARRA","TOTAL"});
		table2.getColumnFormatter().setWidth(0, "auto");
		row = 1;
		for (Mod2002019Key[] keys : Mod2002019Constants.COMBINED_TAXATION_3) {
			boolean paintDescription = true;	
			for (int i = 0; i< keys.length; i++) {
				if (keys[i] != null && callback.getMod200Object().isVisible(keys[i])) {
					if (paintDescription) {
						paintDescription(table2, KEY_DESCRIPTIONS[row], row, 0, (row==1 || row == 5 || row == 13));
	...
	private void paintTable(FlexTable table, Mod2002019Key[] keys) {
		table.setWidth("100%");
		table.setCellSpacing(0);
		table.getColumnFormatter().setWidth(1, "200px");	
		int row = 0;
		for (Mod2002019Key key : keys) {
	...
	protected boolean isAvailable() {
		boolean av = super.isAvailable()
  		  && (callback.getMod200Object().getMod200().isChecked(Mod2002019Key.C0028));
		return av;
	}
