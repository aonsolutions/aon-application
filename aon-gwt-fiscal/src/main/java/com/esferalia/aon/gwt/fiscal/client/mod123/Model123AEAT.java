...
public class Model123AEAT extends Model123Base {
	...
	final Mod123ServiceAsync MOD123Service = GWT.create(Mod123Service.class);

	public Model123AEAT(IFiscalModelCallback<Mod123> callback, AonData aonData) {
	...
	public FlowPanel getAdministrationPanel() {
				...
				CertificationPopup certPopup = new CertificationPopup(getAPI(), getModel().getName(), getModel().getDocument(), showNRC) {
					...
					protected void onAccept() {
						if(getModel().isSent()) {
							...
							submitAEAT(MODEL123_PRINT_AEAT, getCert(), getPass(), getDocument(), getName(), showNRC ? getNRC() : null);
							getCallback().showVisorAEAT();
						} else {
							getCallback().showInfoPanel("Para generar el fichero debe finalizar la confecci\u00F3n del modelo.");
						}	
		...
		MOD123Service.presentationFile(getCallback().getDomainName(), getCallback().getDomain(), getCallback().getUser(), getModel().getId(), new AsyncCallback<Integer>() {
