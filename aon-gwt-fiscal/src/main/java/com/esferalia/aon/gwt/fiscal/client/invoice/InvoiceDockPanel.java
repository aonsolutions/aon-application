...
public class InvoiceDockPanel extends AonDockLayout {
	
	public interface InvoiceDockPanelCallback {
		Invoice getInvoice();
	...
	private static final Logger LOGGER = Logger.getLogger(InvoiceDockPanel.class.getName());
	static {
	...
	public InvoiceDockPanel(InvoiceModuleOptions options, Invoice invoice, InvoiceDockPanelCallback callback) {
	...
	private boolean checkLoad(InvoiceModuleOptions options) {
		...
		if ( AonNumberUtils.notEquals(optionsDomain, configurationDomain) ) {
			showMainError( "Diferentes dominios en 'options' y 'configuration'", InvoiceDockPanel.this::clear);
			return false;
		} else  if ( AonNumberUtils.notEquals(invoiceDomain, configurationDomain) ) {
			showMainError( "Diferentes dominios en 'factura' y 'configuration'", InvoiceDockPanel.this::clear);
			return false;
		}
		return true;
	}
	
	private void load(InvoiceModuleOptions options, Invoice invoice, InvoiceDockPanelCallback callback) {
		if (checkLoad(options)) {
			paint(options, invoice, callback);
		}
	}

	private void paint(InvoiceModuleOptions options, Invoice invoice, InvoiceDockPanelCallback callback) {
		resetWidget();
		fillTooolbar(options, invoice, callback);
		fillContent(options, invoice);
	}

	private void fillTooolbar(InvoiceModuleOptions options, Invoice invoice, InvoiceDockPanelCallback callback) {
		// -----------------------------------------------------------------
		// ------------------------------------------------- Save button ---
		// -----------------------------------------------------------------
	...
	private void finalizeAction(AonToolbarButton button, String message) {
		...
		AonMessagePanel.showSuccess(getMessagePanel(), message);
	}
	private Panel getMessagePanel() {
		return this.getToolbar().getMessagePanel();
	}
	private void hideMessages() {
		this.getToolbar().hideMessages();
	}

	private void finalizeAction(AonToolbarButton button, Throwable caught) {
		finalizeAction(button);
		AonMessagePanel.showError(getMessagePanel(), caught.getMessage());
	}
	...
	private void fillContent(InvoiceModuleOptions options, Invoice invoice) {
		...
		InvoicePanel invoicePanel = new InvoicePanel(options, invoice, new InvoicePanelCallback() {

			@Override
			public void onNatureChange(Invoice invoice) {
				StringBuilder buf = new StringBuilder(AON.MSG.invoice());
				if (invoice != null && invoice.getType() != null) {
					invoice.getType().visit(invoice, new IInvoiceTypeVisitor<StringBuilder>() {
						@Override public StringBuilder visitPurchase(Invoice invoice) {return buf.append(" DE COMPRA");}
						@Override public StringBuilder visitSales(Invoice invoice) {return buf.append(" DE VENTA");}
						@Override public StringBuilder visitExpenses(Invoice invoice) {return buf.append(" DE GASTOS");}
						@Override public StringBuilder visitUndeductible(Invoice invoice) {return buf.append(" DE GASTOS NO DEDUCIBLES");}
					});
				}
				getToolbar().setTitle( buf.toString() );
			}
			
			@Override public void onRegistryChange(Invoice invoice) { /* Nothing to do */ }
