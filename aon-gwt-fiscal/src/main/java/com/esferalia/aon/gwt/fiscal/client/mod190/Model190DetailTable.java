...
public class Model190DetailTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod190Detail> {
	...
	public Model190DetailTable(Model190BaseCallback cbk, Integer selectedIndex) {
	...
	private void newPerceptor(Model190BaseCallback cbk) {
		cbk.getMod190().getDetails().add(
				new Mod190Detail()
					.setDirty(true)
					.setTempId((cbk.getMod190().getDetails().size() * (-1)))
			);
		table.setRowData(cbk.getMod190().getDetails());
		table.redraw();
		int i = cbk.getMod190().getDetails().size() - 1;
		table.getSelectionModel().setSelected(cbk.getMod190().getDetails().get(i),true);
	...
	public Integer getSelectionIndex() {
		if ( table.getSelected() != null) {
			int i = 0;
			for ( Mod190Detail det : table.getVisibleItems()) {
