...
class InvoiceIntegrityCheck extends OptionBase {
	...
	private static final String REPORT_URL = URL.encode(GWT.getModuleBaseURL() + "roms/InvoiceIntegrityStreamServlet");
	...
	protected InvoiceIntegrityCheck(final FinanceUtilitiesModuleOptions options, Domain domain) {
	...
	public String getOptionDescription() {
		return AonStringUtils.BULLET + " Integridad de facturas.";
	}
	...
	public void run( AccountingReportParams params ) {
		...
		content.setWidget(new InvoiceIntegrityCheckPanel( getOptions(), params));		

	}

	class InvoiceIntegrityCheckPanel extends ScrollPanel{
		
		InvoiceIntegrityCheckPanel(FinanceUtilitiesModuleOptions options, AccountingReportParams params) {
...
//			grid.addSelectionHandler(event -> showInvoice(options, event.getSelectedItem()));
			grid.addSelectionHandler(event -> showEntry(options, event.getSelectedItem()));
			setWidget(grid);
			...
			xhr.setOnReadyStateChange(new ReadyStateChangeHandler() {
				...
				public void onReadyStateChange(XMLHttpRequest xhr) {
					...
					if (state == XMLHttpRequest.LOADING || state == XMLHttpRequest.DONE) {
						...
						try {
							for (JsInvoiceIntegrity invoice = read(text); text != null; invoice = read(text)) {
								grid.addRow(invoice);
							}
				...
				private JsInvoiceIntegrity  read(String text) {
		...
		private void showInvoice(FinanceUtilitiesModuleOptions options, JsInvoiceIntegrity br) {
		...
		private void showEntry(FinanceUtilitiesModuleOptions options, JsInvoiceIntegrity br) {
			if ( br.getAccountEntryId() > 0) {
				Integer entryId = br.getAccountEntryId();
				...
				module.onModuleLoad( new AccountEntryModuleOptions()
					...
					.setExternalCallback( new ModuleCallback() {
						...
						private static final long serialVersionUID = -6824269430786138261L;
	...
	class JsInvoiceGridPanel extends FlowPanel implements HasSelectionHandlers<JsInvoiceIntegrity>{
		...
		@Override
		public HandlerRegistration addSelectionHandler(SelectionHandler<JsInvoiceIntegrity> handler) {
