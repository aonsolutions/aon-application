...
public class Mod130ValidatePrintAEAT extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(Mod130ValidatePrintAEAT.class.getName()); 
	...
	private enum AeatUrl {
		URL_2021_2_SEMESTER {
			...
			@Override
			protected String getUrlParameters(Mod130 mod130) throws IOException {
				...
				return MessageFormat.format("MOD=130&EJF={0}&FIC={1}&IDI=ES"
						,AonNumberUtils.toString( mod130.getYear())
						,ModelAdmonUtils.getEncodedFile(output.toByteArray(),StandardCharsets.ISO_8859_1));
			}
		};
		private static AeatUrl getAeatUrl(Mod130 mod130) {
			for (AeatUrl aeatUrl : AeatUrl.values()) {
				if (aeatUrl.accept(mod130)) {
					return aeatUrl;
				}
			}
		...
		protected abstract boolean accept( Mod130 mod130);
		protected abstract String getUrl();
		protected abstract String getUrlParameters(Mod130 mod130) throws IOException;
	}
