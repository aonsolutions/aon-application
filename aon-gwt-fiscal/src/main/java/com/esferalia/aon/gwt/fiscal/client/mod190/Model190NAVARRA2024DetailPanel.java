...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190NAVARRADetail2024.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902024NavarraKey;
...
public class Model190NAVARRA2024DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190NAVARRA2024DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902024NavarraKey k : Mod1902024NavarraKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190NAVARRA2024DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(event -> {
			subkey.clear();
			Mod1902024NavarraKey keyEnum = Mod1902024NavarraKey.values()[key.getSelectedIndex()];
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902024NavarraKey.A.toString());
		}
		Mod1902024NavarraKey keyEnum = Mod1902024NavarraKey.valueOf(detail.getKey());
	...
	private void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		boolean a = (Mod1902024NavarraKey.A == keyEnum);
		boolean abc = (Mod1902024NavarraKey.A == keyEnum || Mod1902024NavarraKey.B == keyEnum || Mod1902024NavarraKey.C == keyEnum);
		boolean e01 = (Mod1902024NavarraKey.E == keyEnum && "01".equals(subk));
		boolean l0510 = (Mod1902024NavarraKey.L == keyEnum && ("05".equals(subk) || "10".equals(subk)));
		boolean l2829 = (Mod1902024NavarraKey.L == keyEnum && ("28".equals(subk) || "29".equals(subk)));
		boolean l29 = (Mod1902024NavarraKey.L == keyEnum && "29".equals(subk));
		
		panel.setVisible( abc || e01 || l0510 || l2829);
		...
		if (panel.isVisible()) {
			...
			applicableReduction.setEnabled(abc);               // Reducciones: Claves A, B, C 
	...
	private void enableOrDisableIlPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		String subk = ((subKey.getSelectedIndex() == -1) ? null : subKey.getValue(subKey.getSelectedIndex()));
		panel.setVisible( 
				    Mod1902024NavarraKey.A == keyEnum
				|| (Mod1902024NavarraKey.B == keyEnum && "01".equals(subk))
			);
	}
	
	private void enableOrDisableAdministrationPanel(ListBox key, Panel panel) {
		Mod1902024NavarraKey keyEnum = Mod1902024NavarraKey.values()[key.getSelectedIndex()];
		panel.setVisible(Mod1902024NavarraKey.E == keyEnum);
	}
