...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190BIZKAIADetail2025.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902025BizkaiaKey;
...
public class Model190BIZKAIA2025DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190BIZKAIA2025DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902025BizkaiaKey k : Mod1902025BizkaiaKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190BIZKAIA2025DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(event -> {
			subkey.clear();
			Mod1902025BizkaiaKey keyEnum = Mod1902025BizkaiaKey.values()[key.getSelectedIndex()];
		...
		tab2.setWidget(4, 0, entrepreneurship);
		tab2.getFlexCellFormatter().setColSpan(4, 0, 8);
		...
		tab4.setWidget(1, 1, new Model190SmallerLabel("Cantidad no integrada"));
		...
		tab4.setWidget(1, 4, new Model190SmallerLabel("Total descendientes"));
		...
		tab4.setWidget(2, 4, otherDescendent);
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902025BizkaiaKey.A.toString());
		}
		Mod1902025BizkaiaKey keyEnum = Mod1902025BizkaiaKey.valueOf(detail.getKey());
	...
	private void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		// DATOS ADICIONALES (Solo en las claves y subclaves A, B, C, E, F subclaves 01 a 03, G subclaves 01 a 06 y 08, H, I y L subclaves 05, 10, 27 y 29)
		Mod1902025BizkaiaKey keyEnum = Mod1902025BizkaiaKey.values()[key.getSelectedIndex()];
		String subk = ((subKey.getSelectedIndex() == -1) ? null : subKey.getValue(subKey.getSelectedIndex()));
		
		boolean abc = (Mod1902025BizkaiaKey.A == keyEnum) ||
					  (Mod1902025BizkaiaKey.B == keyEnum) ||
				      (Mod1902025BizkaiaKey.C == keyEnum);
		boolean e = (Mod1902025BizkaiaKey.E == keyEnum);
		boolean fghi = (Mod1902025BizkaiaKey.F == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk))) ||
				  	   (Mod1902025BizkaiaKey.G == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk) || "04".equals(subk) || "05".equals(subk) || "06".equals(subk) || "08".equals(subk))) ||
				  	   (Mod1902025BizkaiaKey.H == keyEnum) ||
				  	   (Mod1902025BizkaiaKey.I == keyEnum);
		boolean l051027 = (Mod1902025BizkaiaKey.L == keyEnum && ("05".equals(subk) || "10".equals(subk) || "27".equals(subk)));
		boolean l29 = (Mod1902025BizkaiaKey.L == keyEnum && "29".equals(subk));
		
		panel.setVisible(abc || e || fghi || l051027 || l29);
		
		disability.setEnabled(abc);                       // Discapacidad: Claves A, B, C.
		applicableReduction.setEnabled(abc || e || fghi); // Cantidad no integrada: Claves A, B, C, E, F (subclaves 01 a 03), G (subclaves 01 a 06 y 08), H e I.
		deducibleExpense.setEnabled(abc || l051027);  	  // Gastos deducibles: claves A, B, C, E, L05, L10 y L27.
		...
		titConvivivencia.setEnabled(l29); 				  // Titular unidad de convivencia: Clave L29
		spouseDocument.setEnabled(l29); 				  // NIF titular unidad de convivencia: Clave L29
		compInfancia.setEnabled(l29);  					  // Complemento ayuda infancia: Clave L29
		
	}
	
	private void enableOrDisableAdministrationPanel(ListBox key, Panel panel) {
		Mod1902025BizkaiaKey keyEnum = Mod1902025BizkaiaKey.values()[key.getSelectedIndex()];
		panel.setVisible(Mod1902025BizkaiaKey.E == keyEnum);
	}
