...
public class Model2002019 extends ResizeComposite  {
	...
	protected interface Model200PageCallback {
		public Mod2002019Object getMod200Object();
	}
	
	private PageAbs[] PAGES = new PageAbs[20];
	private int P00 = 0;
	
	interface Model2002019Binder extends
			UiBinder<Widget, Model2002019> {
	}

	private static final Model2002019Binder binder = GWT
			.create(Model2002019Binder.class);

	protected Mod2002019Object mod200Object;
	...
	public Model2002019(Model200ModuleOptions options, Model200Callback mod200Callback) {
		...
		errorPage.addSelectionHandler( new  SelectionHandler<ValidationMessage2019>() {
			
			@Override
			public void onSelection(SelectionEvent<ValidationMessage2019> event) {
				ValidationMessage2019 msg = event.getSelectedItem();
				if ( msg.getPage() >= 0 ) {
	...
	protected void paintHeaderTable(final Mod2002019 mod200) {
	...
	private class PopupAsyncCallback implements AsyncCallback<Mod2002019> {
		PopupPanel popup;
	...
	public void startModel(final Mod2002019Object modObject ) {
	...
	void onInitializeClick(ClickEvent event) {
		...
		mod200Object.initializeMod200(new AsyncCallback<Mod2002019>() {
			@Override
			public void onSuccess(Mod2002019 result) {
				popup.hide();
				dump(false);
	...
	private void refreshButtonsVisibility() {
		...
		validateButton.setVisible(false); // A partir del 2019 no se utiliza "VALIDAR" de AON ya que está desactualizado y "Validar/Imprimir" de la Agencia Tributaria hace todas las validaciones posibles
	...
	void onAeatAccountingFileButtonClick(ClickEvent event) {
		...
		cd.confirm("Se va a proceder a la generaci\u00F3n de un fichero\n"
			...
			new ConfirmDialogCallback() {
				...
				@Override
				public void onAccept() {
					submitForm("/aon_gwt_fiscal/Model2002019AccountingFile");
				}
			}
		);
	}
	...
	void onAeatFileButtonClick(ClickEvent event) {
		...
		cd.confirm("Se va a proceder a la generaci\u00F3n de un fichero\n"
			...
			new ConfirmDialogCallback() {
				...
				@Override
				public void onAccept() {
					submitForm("/aon_gwt_fiscal/Model2002019File");
				}
			}
		);
	}
	...
	void onAeatPrintButtonClick(ClickEvent event) {
		...
		cd.confirm("La impresi\u00F3n se genera en los servidores de la Agencia Tributaria "
			...
			,new ConfirmDialogCallback() {
				...
				@Override
				public void onAccept() {
					submitForm("/aon_gwt_fiscal/Model2002019Print");
				}
			}
		);
	}
	...
	void onImportAccountingButtonClick(ClickEvent event) {
		UploadDialog ud = new UploadDialog(AON.MSG.importAccounting(),GWT.getModuleBaseURL() +"Mod2002019AccountingUpload") {
			...
			protected void onAccept() {
				mod200Object.fillMod2002019AccountingData(new AsyncCallback<Mod2002019>() {
					@Override
					public void onSuccess(Mod2002019 result) {
						hide();
					}
