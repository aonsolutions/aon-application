...
public abstract class Model123Base extends DockLayoutPanel {
	...
	private void showFinalizePopup(Mod123 model) {
		FinishDeclarationPopup<Mod123,Model123ModuleOptions> finalizeDialog = new FinishDeclarationPopup<>(
				model,
				getCallback(), 
	...
	private void paintLiquidationTab(TabLayoutPanel tabPanel) {
		...
		container.add(table);
		defineTable(table);
		for (IModelScript<Mod123Key> ms : Model123ScriptProvider.obtainScript(getModel())) {
	...
	private void paintInfoCol(FlexTable table, int row, int col, final Model123Callback callback, final IModelScript<Mod123Key> script) {
		...
		for (final FiscalModelKeyInfo infoKey : script.getInfoKeys()) {
			...
			if 	(infoKey != FiscalModelKeyInfo.NONE) {
				...
				button.addClickHandler(event -> Model123.SERVICE.getInfo(
	...
	private void paintIdentificationTab(TabLayoutPanel tabPanel) {
		AonFiscalModelIdentificationPanel<Mod123> identificationData = new AonFiscalModelIdentificationPanel<>( getModel() ) ;
