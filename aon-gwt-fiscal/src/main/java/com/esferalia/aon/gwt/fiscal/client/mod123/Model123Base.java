...
public abstract class Model123Base extends DockLayoutPanel {
	protected FiscalModelAdmonPanel<Mod123, Model123ModuleOptions> admonPanel;
	private AonFiscalModelIdentificationPanel<Mod123> identificationData; 
	...
	private EnumMap<Mod123Key,AonDoubleBox> fieldsMap;
	...
	protected Model123Base(Mod123 mod123, Model123Callback callback) {
		...
		AonFiscalModelHeader modelHeader = new AonFiscalModelHeader( mod123 );
	...
	public Model123Callback getCallback() {
		return callback;
	}
	protected Mod123 getModel() {
		return model;
	}
	public void setModel(Mod123 mod123) {
		this.model = mod123;
	}
	
	private void select( Mod123 mod123) {
		setModel(mod123);
		refreshToolbarState();
		styleStatusLabel(mod123);
	}
	
	void selectAndPopulate( Mod123 mod123) {
		select(mod123);
		populate(mod123);
		showPaymentInfo(mod123);
	...
	private void populate(Mod123 mod123) {
		identificationData.populate(mod123);
	...
	private AonToolbar getToolbarPanel() {
		...
		commentsButton.addClickHandler( event -> {
			...
			comment.addValueChangeHandler(event1 -> {
				...
				styleCommentsButton();
				Model123.SERVICE.saveComments( getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod123>() {
					@Override
	...
	private void styleStatusLabel(Mod123 mod123) {
		statusLabel.setText(mod123.getStatus().getName());
		statusLabel.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB( mod123.getStatus() ));
		statusLabel.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB( mod123.getStatus() ));
	...
	private void save(AsyncCallback<Mod123> cbk) {
	...
	private void delete() {
		...
		cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
			...
			@Override
			public void onAccept() {
				Model123.SERVICE.delete(getCallback().getOptions().getOccam(),getModel(), new AsyncCallback<Void>() {
					@Override
	...
	private void markAsFinished() {
		...
		Model123.SERVICE.initializeForFinish(getCallback().getOptions().getOccam(),getModel(),
	...
	private void showFinalizePopup(Mod123 model) {
		AonFinishDeclarationPopup<Mod123,Model123ModuleOptions> finalizeDialog = new AonFinishDeclarationPopup<>(
			model,
			getCallback(), 
			new IFinishDeclarationPopupCallback<Mod123>() {

				@Override
				public void onAccept(Mod123 t) {
					Model123Base.this.doMarkAsFinished();
				}

				@Override
				public void onCancel(Mod123 t) {
					// Nothing
					
				}

				@Override
				public void onCustomerCheck(Mod123 t) {
					Model123Base.this.markAsCustomerCheck();
				}
		});
...
//			Label label = new Label( buff.toString() );
	...
	private void paintLiquidationTab(TabLayoutPanel tabPanel) {
		...
		container.add(table);
		defineTable(table);
		IModelScript<Mod123Key>[] script = null;
		try {
			script = Model123ScriptProvider.obtainScript(getModel());
			for (IModelScript<Mod123Key> ms : script) {
	...
	private void paintHeader(FlexTable table) {
		...
		table.setWidget(row, 1, new Label((getModel().isAEAT() || getModel().isGipuzkoa()) && getModel().getYear() >= 2024 ? "N\u00FAmero de rentas" : AON.MSG.receivers()));
...
//		table.setWidget(row, 3, new Label(AON.MSG.retentionAccountShort()));
	...
	private void paintRow(FlexTable table,final Model123Callback callback, IModelScript<Mod123Key> script) {
	...
	protected void paintLabel( FlexTable table, int row, IModelScript<Mod123Key> script) {
	...
	private int paintBox(FlexTable table, int row, int col, Mod123Key key) {
		table.setWidget(row, col, new AonBoxLabel(key.getBox()));
		return ++col;
	}

	private int paintField(FlexTable table, int row, int col, final Model123Callback callback, IModelScript<Mod123Key> script, final Mod123Key key) {
	...
	private void paintInfoCol(FlexTable table, int row, int col, final Model123Callback callback, final IModelScript<Mod123Key> script) {
		...
		for (final FiscalModelKeyInfo infoKey : script.getInfoKeys()) {
			...
			infoKey.visit(new IFiscalModelKeyInfoVisitor<Void>() {
				...
				private void showComputeKeyInfo(AonTableButton button) {
					...
					Model123.SERVICE.getInfo(callback.getOptions().getOccam(),getModel(),script, infoKey, new AsyncCallback<String>() {
						@Override
						public void onFailure(Throwable caught) {
	...
	public void calculateAndRefresh(final Model123Callback callback) {
		Model123.SERVICE.calculate(callback.getOptions().getOccam(),getModel(),
				new AsyncCallback<Mod123>() {
	...
	private void showInvoice(JsIRPFBreakdown br) {
		...
		Model123.SERVICE.getInvoice(getCallback().getOptions().getOccam(), invoiceId,new AsyncCallback<Invoice>() {
			@Override
