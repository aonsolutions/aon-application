...
@WebServlet(name = "Mod123 Validate Print AEAT", urlPatterns = { "/aon_gwt_fiscal/ms/Mod123ValidatePrintAEAT" })
public class Mod123ValidatePrintAEAT extends HttpServlet {
	
	private static final Logger LOGGER = Logger.getLogger(Mod123ValidatePrintAEAT.class.getName()); 
	...
	private enum AeatUrl {
		URL_2021_2_SEMESTER {
			...
			@Override
			protected boolean accept(Mod123 mod123) {
				return mod123.getYear() <= 2024;
			}
			...
			@Override
			protected String getUrlParameters(Mod123 mod123) throws IOException {
				...
				return MessageFormat.format("MOD=123&EJF={0}&FIC={1}&IDI=ES"
						,AonNumberUtils.toString( mod123.getYear())
						,ModelAdmonUtils.getEncodedFile(output.toByteArray(),StandardCharsets.ISO_8859_1));
			}
		};
		private static AeatUrl getAeatUrl(Mod123 mod123) {
			for (AeatUrl aeatUrl : AeatUrl.values()) {
				if (aeatUrl.accept(mod123)) {
					return aeatUrl;
				}
			}
		...
		protected abstract boolean accept( Mod123 mod123);
		protected abstract String getUrl();
		protected abstract String getUrlParameters(Mod123 mod123) throws IOException;
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (mod123.getYear() >= 2025) {
				ModelAdmonUtils.serValiDos(resp, aeatParams, mod123);
			} else {
				AeatUrl aeatURL = AeatUrl.getAeatUrl(mod123);
				HttpRequest request = HttpRequest.newBuilder()
					.uri(URI.create( aeatURL.getUrl() ))
					.POST(HttpRequest.BodyPublishers.ofString(aeatURL.getUrlParameters(mod123)))
