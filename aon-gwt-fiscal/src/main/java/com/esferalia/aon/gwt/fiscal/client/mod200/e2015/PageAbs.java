...
import com.esferalia.aon.gwt.fiscal.client.mod200.e2015.Mod2002015Object.IMod200ChangeListener;
...
public abstract class PageAbs extends ResizeComposite {
	...
	private HashMap<Mod2002015Key, DoubleBox> inputs = new HashMap<Mod2002015Key, DoubleBox>();
	private HashMap<Mod2002015Key, BoxLabel> labels = new HashMap<Mod2002015Key, BoxLabel>();
			...
			public void mod200Changed(Mod2002015 mod200) {
				for (Mod2002015Key key : mod200.getDraftMap().keySet()) {
					if (inputs.containsKey(key)) {
						...
						DoubleVariable2015 var = mod200.getDraftMap().get(key);
	...
	private void refreshDraftMap(Mod2002015 mod200) {
		for (Mod2002015Key key : mod200.getDraftMap().keySet()) {
			if (inputs.containsKey(key)) {
				DoubleBox input = inputs.get(key);
				DoubleVariable2015 var = mod200.getDraftMap().get(key);
	...
	public Map<Mod2002015Key, DoubleBox> getInputs() {
		return inputs;
	}
	public Map<Mod2002015Key, BoxLabel> getLabels() {
		return labels;
	}
	
	protected int paintKey(final Mod2002015Key key,int row) {
		return paintKey(table,key,row);
	}
	
	protected int paintKey(FlexTable tab,final Mod2002015Key key,int row) {
	...
	protected void paintKeyDescription(FlexTable tab, Mod2002015Key key, int row,int col) {
	...
	protected void paintKeyField(FlexTable tab,final Mod2002015Key key,int row, int col) {
		paintKeyField(tab, key, row, col, DoubleBox.VISIBLE_LENGTH);
	}
	protected void paintKeyField(FlexTable tab,final Mod2002015Key key,int row, int col, int fieldLength) {
	...
	protected boolean isTitle(Mod2002015Key key) {
		Boolean[] behaviour = BEHAVIOUR_KEYS_MAP.get(key.toString());
