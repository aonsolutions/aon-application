...
public abstract class ModPrintAEAT extends HttpServlet{
	...
	protected void send(HttpServletRequest req, HttpServletResponse resp, String request, String urlParameters, Boolean isI) throws NoSuchAlgorithmException, KeyManagementException, UnrecoverableKeyException, KeyStoreException, CertificateException, IOException, JSONException, ScriptException {
		if(isI && !isPrint()) {
			System.out.println("4.---> MODELO 390 ERROR E00 - La declaración del modelo es de tipo ingreso (I). Realice la operación desde la pantalla del modelo.");
			...
			System.out.println("4.---> MODELO 390 CONECTANDO CON LA AGENCIA TRIBUTARIA ...");
			...
			if(isPrint()) {
				if(isCert()) {
					System.out.println("5.---> MODELO 390 RECIBIENDO INFORMACIÓN DE LA AGENCIA TRIBUTARIA ...");

					String html = readFullyAsString(connection.getInputStream(), "ISO-8859-1");
					System.out.println((html != null && !"".equals(html)) ? "6.---> MODELO 390 LA RESPUESTA PARECE CORRECTA" :  "6.---> MODELO 390 RESPUESTA NULA O VACÍA");
					JSONObject json = parseHTML(html);
					System.out.println("7.---> MODELO 390 JSON DE RESPUESTA - " + json.toString());
					json.put("nrc", getNrc());
					saveHistory(json);
					ByteArrayInputStream input = new ByteArrayInputStream(html.getBytes());
					System.out.println("8.---> MODELO 390 DEVOLVIENDO RESPUESTA A GWT!!!");
