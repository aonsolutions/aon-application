...
public class OperationReportPanel extends ScrollPanel{
	...
	private static final String HEADER_IVA = AonStringUtils.rightPad("ID", 5)
			...
			+ AonStringUtils.rightPad("CONCEPTO",60)
			+ AonStringUtils.rightPad("N\u00BA DOCUMENTO",15)
			+ AonStringUtils.rightPad("TITULAR",41)
	...
	public OperationReportPanel(String domainName,String user, int domain, OperationParams params, boolean expenses, boolean irpf, String title, String subtitle) {
					...
					try {
						...
						for (JsOperationBreakdown op = read(text); text != null; op = read(text)) {
							if (html.getWidgetCount() == 0) {
								addHeaderWidget(html, title, subtitle, irpf);
							}
					...
					if (html.getWidgetCount() == 0) {
						...
						if (irpf == true) { // IRPF
							...
							Label summaryIrpfHeader = new Label(AonStringUtils.rightPad("CUENTA", 15)
								...
								+ AonStringUtils.rightPad("TOTAL", 15));
							...
							for (Entry<String, Object[]> entry : mapConceptSummary.entrySet()) {
								html.add(new Label(AonStringUtils.rightPad(entry.getValue()[0].toString(), 15)
									...
									+ AonStringUtils.rightPad(AON.CURRENCY_FORMAT.format((double) entry.getValue()[1]), 15)));
							}
							html.add(new Label(AonStringUtils.repeat(AonStringUtils.SPACE, HEADER_IVA.length())));
							html.add(new Label(AonStringUtils.repeat(AonStringUtils.SPACE, HEADER_IVA.length())));
								
						} else { // IVA
							Label total = new Label(AonStringUtils.leftPad("TOTAL: ",149)				
								+ AonStringUtils.leftPad(AON.CURRENCY_FORMAT.format(sumBase),17)
							...
							html.add(bold(new Label(AonStringUtils.repeat(AonStringUtils.HYPHEN, 45))));
							Label summaryIvaHeader = new Label(AonStringUtils.rightPad("BASE", 15)
								+ AonStringUtils.rightPad("TIPO IVA", 15)
								+ AonStringUtils.rightPad("CUOTA", 15));
							...
							for (Entry<Double, Double[]> entry : mapIvaSummary.entrySet()) {
								html.add(new Label(AonStringUtils.rightPad(AON.CURRENCY_FORMAT.format(entry.getValue()[0]), 15)
									...
									+ AonStringUtils.rightPad(AON.CURRENCY_FORMAT.format(entry.getValue()[1]), 15)));
							}
							html.add(new Label(AonStringUtils.repeat(AonStringUtils.SPACE, HEADER_IVA.length())));
							html.add(new Label(AonStringUtils.repeat(AonStringUtils.SPACE, HEADER_IVA.length())));
	...
	private void addHeaderWidget(FlowPanel html, String title, String subtitle, boolean irpf) {
		if (irpf == true) {
			if (AonStringUtils.isNotBlank(title)) {
				html.add(bold(new Label(AonStringUtils.center(title, HEADER_IRPF.length()))));
			}	
			if (AonStringUtils.isNotBlank(subtitle)) {
				html.add(bold(new Label(AonStringUtils.center(subtitle, HEADER_IRPF.length()))));
			}
			html.add(bold(new Label(AonStringUtils.repeat(AonStringUtils.HYPHEN, HEADER_IRPF.length()))));
			...
			if (AonStringUtils.isNotBlank(title)) {
				html.add(bold(new Label(AonStringUtils.center(title, HEADER_IVA.length()))));
			}	
			if (AonStringUtils.isNotBlank(subtitle)) {
				html.add(bold(new Label(AonStringUtils.center(subtitle, HEADER_IVA.length()))));
			}
			html.add(bold(new Label(AonStringUtils.repeat(AonStringUtils.HYPHEN, HEADER_IVA.length()))));
	...
	private Widget getIvaWidget(JsOperationBreakdown op, int i) {
		...
		builder.appendEscaped( AonStringUtils.rightPad(op.getConcept(), 60));
