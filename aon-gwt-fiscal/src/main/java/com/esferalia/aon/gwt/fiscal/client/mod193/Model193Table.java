...
public class Model193Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod193>{
	...
	protected Model193Table(Model193Callback cbk) {
	...
	private Widget getToolbarPanel(Model193Callback cbk) {
		AonToolbar toolbar = new AonToolbar( "Modelo 193");
	...
	public void refresh(Model193Callback cbk) {
		...
		Model193.SERVICE.getMod193s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod193> result) {
		for ( Mod193 mod193 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod193.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod193.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod193.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod193.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model193Table.this, mod193));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod193)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod193.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod193.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod193.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod193.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod193.getStatus()) );
			row.add( statusCell );
			
			row.addCell( comp , AON.CSS.aonTextCenter())
				.addCell( sust , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod193.getDocument()))
				.addCell( new InlineLabel(mod193.getFullName()))
				;
		}
