...
public class Model349Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod349>{

	private static class Model349CellTable extends CellTable<Mod349> {
		private static final CellTable.Resources TABLE_STYLE = GWT.create(AonCellTable.class);
		
		public static final ProvidesKey<Mod349> MOD349_PROVIDES_KEY = new ProvidesKey<Mod349>() {
			@Override
			public Object getKey(Mod349 mod349) {
				return mod349 == null ? null : mod349.getId();
			}
		};
	
		private NoSelectionModel<Mod349> model;
		
		public Model349CellTable(ProvidesKey<Mod349> providesKey) {
			...
			model = new NoSelectionModel<Mod349>(MOD349_PROVIDES_KEY);
			this.setSelectionModel(model);
			this.setEmptyTableWidget(new HTML(AON.MSG.noData()));
		}
	
		private void addSelectorColumn() {
			final Column<Mod349, ImageResource> selectorColumn = new Column<Mod349, ImageResource>(
					new ImageResourceCell()) {
				@Override
				public ImageResource getValue(Mod349 mod349) {
		...
		private void addAdministrationColumn() {
			final Column<Mod349, ImageResource> iconColumn = new Column<Mod349, ImageResource>(
					new ImageResourceCell()) {
				@Override
				public ImageResource getValue(Mod349 mod349) {
					return FiscalModelUtils.getAdministrationIconResource(mod349.getAdministration());
				}
			};
		...
		private void addYearColumn() {
			final TextColumn<Mod349> yearColumn = new TextColumn<Mod349>() {
				@Override
				public String getValue(Mod349 mod349) {
					return Integer.toString(mod349.getYear());
				}
			};
		...
		private void addPeriodColumn() {
			final TextColumn<Mod349> periodColumn = new TextColumn<Mod349>() {
				@Override
				public String getValue(Mod349 mod349) {
					return mod349.getPeriod().getDescription();
				}
			};
			this.addColumn(periodColumn, AON.MSG.period());
			periodColumn.setCellStyleNames(AON.AON_CSS.aonTextCenter());
			this.setColumnWidth(periodColumn, 70, Unit.PX);
		}
		...
		private void addStatusColumn() {
			final Column<Mod349, ImageResource> iconColumn = new Column<Mod349, ImageResource>(
					new ImageResourceCell()) {
				@Override
				public ImageResource getValue(Mod349 model) {
		...
		private void addStatusLabelColumn() {
			final TextColumn<Mod349> statusLabelColumn = new TextColumn<Mod349>() {
				@Override
				public String getValue(Mod349 mod349) {
					return mod349.getStatus().getName();
				}
			};
		...
		private void addReplacementColumn() {
			Column<Mod349, ImageResource> replacementColumn = new Column<Mod349, ImageResource>(
					new ImageResourceCell()) {
				@Override
				public ImageResource getValue(Mod349 mod349) {
					return (mod349.isComplementary() || mod349.isReplacement())  
		...
		private void addDocumentColumn() {
			final TextColumn<Mod349> documentColumn = new TextColumn<Mod349>() {
				@Override
				public String getValue(Mod349 mod349) {
					return mod349.getDocument();
				}
			};
		...
		private void addNameColumn() {
			final TextColumn<Mod349> nameColumn = new TextColumn<Mod349>() {
				@Override
				public String getValue(Mod349 mod349) {
					return mod349.getName();
				}
			};
	...
	public Model349Table(Model349Callback cbk) {
		...
		if (table.getSelectionModel() != null) {
			table.getSelectionModel().addSelectionChangeHandler(new com.google.gwt.view.client.SelectionChangeEvent.Handler() {
				...
				public void onSelectionChange(SelectionChangeEvent event) {
					SelectionEvent.fire(Model349Table.this, table.getSelected());
				}
			});
		}
		table.addRangeChangeHandler( new com.google.gwt.view.client.RangeChangeEvent.Handler() {
			...
			public void onRangeChange(RangeChangeEvent event) {
				Model349.SERVICE.getMod349s(Model349.getCurrentDomainName(), Model349.getCurrentDomain(),
	...
	@Override
	public HandlerRegistration addSelectionHandler(SelectionHandler<Mod349> handler) {
