...
class Model303AEATActivity2018Panel extends AonCustomDialog implements HasSelectionHandlers<Epigraph>{

	protected Model303AEATActivity2018Panel() {
		setVisible(false);
		...
		for (Epigraph epi : Epigraph.values()) {
			if (epi.hasVATModules() ) {
				...
				row.addClickHandler(event -> {
					hide();
					SelectionEvent.fire(Model303AEATActivity2018Panel.this, epi);
				});	
			}
		}
