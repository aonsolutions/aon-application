...
public class Model180DetailTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod180Detail> {
	...
	private Mod180 model;
	private ScrollPanel tablePanel;
	private Integer selectionIndex;
	private AonTextBox filterBox;
	
	public Model180DetailTable(Model180Callback cbk, Mod180 model, Integer selectedIndex) {
		this.model = model;
	...
	private Mod180Detail getSelected() {
	...
	public void refresh() {
		...
		int firstMatch = -1;
		for (Mod180Detail detail : this.model.getDetails()) {
			...
			if (i == Model180DetailTable.this.selectionIndex) {
				row.addStyleName(AON.CSS.aonBackgroundLigthBlue());
				resetSelection = !visible; 
			}
			final int idx = i;
			row.addClickHandler( event ->  {
				Model180DetailTable.this.selectionIndex = idx;
				styleTable( tab );				
				SelectionEvent.fire(Model180DetailTable.this, detail);	
			});
		...
		if (resetSelection) {
			...
			SelectionEvent.fire(Model180DetailTable.this, getSelected() );
		}
	}
