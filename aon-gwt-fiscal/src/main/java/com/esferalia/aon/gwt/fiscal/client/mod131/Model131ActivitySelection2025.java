...
class Model131ActivitySelection2025 extends ScrollPanel implements HasSelectionHandlers<Epigraph>{

	protected Model131ActivitySelection2025() {
		...
		Arrays.stream(Epigraph.values())
			...
			.forEach(epi -> {
				...
				row.addClickHandler(event -> {
					SelectionEvent.fire(Model131ActivitySelection2025.this, epi);
				});	
			});
