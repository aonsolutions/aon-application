...
import com.esferalia.aon.gwt.fiscal.client.accounting.wizard.tedi.OLDEditableInvoicePanel.IEditableInvoicePanelCallback;
...
public class OLDInvoiceVATPanel extends ScrollPanel implements HasValueChangeHandlers<InvoiceVAT>, HasSelectionHandlers<Account>, Focusable {
	...
	private FlowPanel directTaxPercentLabelCell;
	private FlowPanel directTaxQuotaLabelCell;
	private FlowPanel adjDirectTaxAccountLabelCell;
	...
	public OLDInvoiceVATPanel(IEditableInvoicePanelCallback callback) {
	...
	private void paintHeader() {
		...
		directTaxPercentLabelCell = getHeaderCell(new Label(AON.MSG.directTaxPercent()));
		headerRow.add(directTaxPercentLabelCell);
		directTaxQuotaLabelCell = getHeaderCell(new Label(AON.MSG.directTaxQuota()));
		headerRow.add(directTaxQuotaLabelCell);
		adjDirectTaxAccountLabelCell = getHeaderCell(new Label(AON.MSG.adjDirectTaxAccount()));
		headerRow.add(adjDirectTaxAccountLabelCell);
	...
	private void paintRows() {
		rows = new LinkedList<>();
		AonCollectionUtils.stream(callback.getInvoice().getVats())
			.forEach(vat -> addRow(vat, false));
...
//		if (vat.getExpAccountId() == null && suggestedAccounts != null && !suggestedAccounts.isEmpty() && suggestedAccounts.size() > rows.size()) {
	...
	private class InvoicePanelRow extends FlowPanel implements Focusable {
		...
		private final FlowPanel directTaxPercentCell = new FlowPanel();
		private final FlowPanel directTaxQuotaCell = new FlowPanel();
		private final FlowPanel directTaxAccountCell = new FlowPanel();
		
		private final AonAccountBox expAccount = new AonAccountBox(callback.getOccam(), false);
		...
		private final AonAccountBox inputVatAccount = new AonAccountBox(callback.getOccam(), false);
		private final AonAccountBox outputVatAccount = new AonAccountBox(callback.getOccam(), false);
		...
		private final AonAccountBox adjAccount = new AonAccountBox(callback.getOccam(), false);
		private final AonDoubleBox directTaxPercent = new AonDoubleBox(6);
		private final AonDoubleBox directTaxQuota = new AonDoubleBox(8);
		private final AonAccountBox directTaxAccount = new AonAccountBox(callback.getOccam(), false);
			...
			investAsset.addChangeHandler(new ChangeHandler() {
				...
				public void onChange(ChangeEvent event) {
					...
					if (investAsset.getValue() == null) {
						...
						vat.setDirectTaxPercent(100.0);
					} else {
						vat.setInvestAsset( investAsset.getValue().getId() );
						vat.setDeductiblePercent(investAsset.getValue().getVatPercent());
						vat.setDirectTaxPercent(investAsset.getValue().getRetentionPercent());
					}
					...
					directTaxPercentCell.setVisible(investAsset.getValue() != null);
					directTaxQuotaCell.setVisible(investAsset.getValue() != null);
					directTaxAccountCell.setVisible(investAsset.getValue() != null);

					directTaxPercentLabelCell.setVisible(investAsset.getValue() != null || otherLineWithInvestAssests);
					directTaxQuotaLabelCell.setVisible(investAsset.getValue() != null || otherLineWithInvestAssests);
					adjDirectTaxAccountLabelCell.setVisible(investAsset.getValue() != null || otherLineWithInvestAssests);
...
//			});
		...
		private void enableInvestAsset(InvoicePanelRow row) {
			...
			final boolean investAssetVisible = !isUndeductible() && callback.isInvestAssetsAvailable();
			final boolean investAssetDataVisible = investAssetVisible && !prepayment.getValue();
			final boolean investAssetAdditionalDataVisible = investAssetDataVisible && investAsset.getValue() != null;
			
			investAssetLabelCell.setVisible(investAssetVisible);
			...
			directTaxPercentLabelCell.setVisible(!isUndeductible() && otherLineWithInvestAssests);
			directTaxQuotaLabelCell.setVisible(!isUndeductible() && otherLineWithInvestAssests);
			adjDirectTaxAccountLabelCell.setVisible(!isUndeductible() && otherLineWithInvestAssests);

			investAssetCell.setVisible(investAssetVisible);
			investAsset.setVisible(investAssetDataVisible);
			dedPercentCell.setVisible		(investAssetAdditionalDataVisible);
			dedQuotaCell.setVisible			(investAssetAdditionalDataVisible);
			adjAccountCell.setVisible		(investAssetAdditionalDataVisible);
			directTaxPercentCell.setVisible	(investAssetAdditionalDataVisible);
			directTaxQuotaCell.setVisible	(investAssetAdditionalDataVisible);
			directTaxAccountCell.setVisible	(investAssetAdditionalDataVisible);
		}
