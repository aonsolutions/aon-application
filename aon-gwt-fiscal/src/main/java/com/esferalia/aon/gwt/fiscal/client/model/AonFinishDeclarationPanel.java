...
public class AonFinishDeclarationPanel<T extends FiscalModel,O extends FiscalModelModuleOptions<T>> extends DockLayoutPanel {
...
//	protected IFinishDeclarationPopupCallback<T> finishPopupCallback;
	...
	public AonFinishDeclarationPanel(T model
		,final IFiscalModelCallback<T,O> callback
		,final IFinishDeclarationPopupCallback<T> finishPopupCallback) {
		super( Unit.PX );
...
//		this.finishPopupCallback = finishPopupCallback;
		...
		cancelButton.addClickHandler(event ->  finishPopupCallback.onCancel(model) );
		toolbar.add(cancelButton);
		addNorth(toolbar, AonToolbar.HEIGTH);

		paint(model,callback,finishPopupCallback);	
	}
	
	
	private void paint(T model, final IFiscalModelCallback<T,O> callback
			,final IFinishDeclarationPopupCallback<T> finishPopupCallback) {
	...
	private void paintResult(T model) {
	...
	private void paintDeclarationType(T model, IFiscalModelCallback<T, O> callback) {
		tab.getFlexCellFormatter().addStyleName(row, 0, AON.CSS.aonTableLabel());
	...
	private void paintButtons(T model, final IFiscalModelCallback<T,O> callback
			,final IFinishDeclarationPopupCallback<T> finishPopupCallback) {
