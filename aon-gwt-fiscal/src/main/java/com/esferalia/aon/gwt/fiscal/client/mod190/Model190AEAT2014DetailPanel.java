...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190AEATDetail2014.IModel190DetailCallback;
...
public class Model190AEAT2014DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190AEAT2014DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902014Key k : Mod1902014Key.values()) {
			key.addItem(k.getValue());
		}
		
		Model190AEAT2014DetailPanel.setValue(key, subkey, detail);
		...
		key.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				Model190AEAT2014DetailPanel.enableOrDisableAdditionalDataPanel(key, subkey, additionalDataPanel);
				callback.onValueChanged(detail);
			}
		});
		...
		workActivityExtension.setStyleName(AON.AON_CSS.aonFontMedium());
		...
		tab5.setWidget(1, 1, new MediumLabel(AON.MSG.applicableReduction()));
		tab5.setWidget(1, 2, new MediumLabel(AON.MSG.deducibleExpense()));
		tab5.setWidget(1, 3, new MediumLabel(AON.MSG.compensatoryPension()));
		tab5.setWidget(1, 4, new MediumLabel(AON.MSG.foodAnnuality()));
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902014Key.A.toString());
		}
		Mod1902014Key keyEnum = Mod1902014Key.valueOf(detail.getKey());
