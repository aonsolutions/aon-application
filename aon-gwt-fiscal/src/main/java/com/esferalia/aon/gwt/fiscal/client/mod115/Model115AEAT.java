...
public class Model115AEAT extends Model115Base {
	...
	final Mod115ServiceAsync MOD115Service = GWT.create(Mod115Service.class);
	
	public Model115AEAT(IFiscalModelCallback<Mod115> callback, AonData aonData) {
	...
	public FlowPanel getAdministrationPanel() {
				...
				CertificationPopup certPopup = new CertificationPopup(getAPI(), getModel().getName(), getModel().getDocument(), showNRC) {
					...
					protected void onAccept() {
						if(getModel().isSent()) {
							...
							submitAEAT(MODEL115_PRINT_AEAT, getCert(), getPass(), getDocument(), getName(), showNRC ? getNRC() : null);
							getCallback().showVisorAEAT();
						} else {
							getCallback().showInfoPanel("Para generar el fichero debe finalizar la confecci\u00F3n del modelo.");
						}	
		...
		MOD115Service.presentationFile(getCallback().getDomainName(), getCallback().getDomain(), getCallback().getUser(), getModel().getId(), new AsyncCallback<Integer>() {
