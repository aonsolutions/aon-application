...
public abstract class AssignScopeAuthorizationList extends AonCustomDockLayout {
	...
	public AssignScopeAuthorizationList(RegistryModuleOptions options) {
		...
		getSupportSellers("", sellers -> {
			...
			oracle.setDefaultSuggestionsFromText(sellerLabels);
			onSearch();	
		});
	}
	...
	private void paintRow(UserScopeFull userScope) {
		...
		infoButon.addClickHandler(e -> {
		    ...
		    HashMap<Integer, Domain> map = userScope.getScope().getScopeDomains();
	...
	private void getSupportSellers(String description, Consumer<List<Seller>> success) {
		commonService.getTaskHolderUsers(options.getDomainName(), options.getDomain(), options.getUser(),
				new AsyncCallback<List<Seller>>() {

					@Override
					public void onSuccess(List<Seller> result) {
						List<Seller> rsellers = result.stream().filter(s -> null != s.getTaskHolder().getUserId() && AonStringUtils.containsIgnoreCase(s.get().getName(), description)).collect(Collectors.toList());
						success.accept(rsellers);
					}

					@Override
					public void onFailure(Throwable caught) {
						showError("Error agentes de soporte: " + caught.getMessage());
					}
				});
	}
