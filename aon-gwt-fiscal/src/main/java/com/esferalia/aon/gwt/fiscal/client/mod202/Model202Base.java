...
public abstract class Model202Base extends SimplePanel implements IMod202Declaration {
	...
	private ExpressionResolver resolver = new ExpressionResolver() {
		...
		public void resolve(String expression, AsyncCallback<Double> callback) {
			Model202.fiscalService.mathExpression(expression,callback);
		}
	}; 
	...
	protected EnumMap<Mod202Key, DoubleBox> getFieldsMap() {
		return fieldsMap;
	}
	...
	private int paintBox(int row, int col, Mod202Key key) {
	...
	private void paintInfoCol(int row, int col, final IFiscalModelCallback<Mod202> callback, final IModelScript<Mod202Key> script) {
		...
		for (final FiscalModelKeyInfo infoKey : script.getInfoKeys()) {
			...
			if 	(infoKey != FiscalModelKeyInfo.NONE) {
				...
				if 	(infoKey == FiscalModelKeyInfo.CORPORATE) button.addStyleName(AON.AON_CSS.aonIconInvoice());
				...
				button.addClickHandler(new ClickHandler() {
					...
					public void onClick(ClickEvent event) {
						Model202.fiscalService.getInfo(Model202.getCurrentDomainName(),Model202.getCurrentDomain(),
	...
	public void calculateAndRefresh(final IFiscalModelCallback<Mod202> callback) {
		Model202.fiscalService.calculateMod202(Model202.getCurrentDomainName(),callback.getFiscalModel(),
				new AsyncCallback<Mod202>() {
					...
					public void onSuccess(Mod202 result) {
						for (Mod202Key key : fieldsMap.keySet()) {
	...
	protected FlowPanel getAnchorPanel(Mod202 mod202, String label, String href) {
