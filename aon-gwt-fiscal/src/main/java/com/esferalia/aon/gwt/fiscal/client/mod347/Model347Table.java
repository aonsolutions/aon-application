...
public class Model347Table extends SimpleLayoutPanel implements HasSelectionHandlers<Mod347>{
	...
	protected Model347Table(Model347Callback cbk) {
	...
	public void refresh(Model347Callback cbk) {
		...
		Model347.SERVICE.getMod347s(cbk.getOptions().getOccam(),
	...
	private void paint(LinkedList<Mod347> result) {
		for ( Mod347 mod347 : result) {
			
			InlineLabel admon = new InlineLabel();
			admon.setTitle( mod347.getAdministration().getDescription() );
			admon.setStyleName(AON.CSS.aonIconLabel());
			admon.addStyleName(FiscalModelUtils.getAdministrationIconStyle(mod347.getAdministration()) );
			
			InlineLabel comp = new InlineLabel();
			comp.setTitle( AON.MSG.complementary());
			comp.setStyleName(AON.CSS.aonIconLabel());
			comp.addStyleName( mod347.isComplementary()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			InlineLabel sust = new InlineLabel();
			sust.setTitle( AON.MSG.replacement());
			sust.setStyleName(AON.CSS.aonIconLabel());
			sust.addStyleName( mod347.isReplacement()?AON.CSS.aonIconChecked():AON.CSS.aonIconCheck() );
			
			AonDisplayGridRow row = tab.addRow();
			row.addStyleName(AON.CSS.aonClickable());
			row.addClickHandler( event ->  SelectionEvent.fire(Model347Table.this, mod347));					
			
			row.addCell( new InlineLabel(FiscalModelUtils.getModelName(mod347)), AON.CSS.aonTextCenter())
				.addCell( admon , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(AonNumberUtils.toString( mod347.getYear())), AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod347.getPeriod().getDescription()), AON.CSS.aonTextCenter());
			
			AonDisplayGridCell statusCell = new AonDisplayGridCell();
			statusCell.add(new InlineLabel(mod347.getStatus().getName()));
			statusCell.addStyleName(AON.CSS.aonTextCenter());
			statusCell.getElement().getStyle().setBackgroundColor(FiscalModelUtils.getStatusBckColorRGB(mod347.getStatus()) );
			statusCell.getElement().getStyle().setColor(FiscalModelUtils.getStatusFrgColorRGB(mod347.getStatus()) );
			row.add( statusCell );
			
			row.addCell( comp , AON.CSS.aonTextCenter())
				.addCell( sust , AON.CSS.aonTextCenter())
				.addCell( new InlineLabel(mod347.getDocument()))
				.addCell( new InlineLabel(mod347.getFullName()))
				;
		}
