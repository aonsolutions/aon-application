...
public class Model349DetailTable extends SimpleLayoutPanel implements HasSelectionHandlers<Mod349Detail> {
	...
	private Mod349 model;
	private ScrollPanel tablePanel;
	private Integer selectionIndex;
	private AonTextBox filterBox;
	
	public Model349DetailTable(Model349Callback cbk, Mod349 model, Integer selectedIndex) {
		this.model = model;
	...
	private Mod349Detail getSelected() {
	...
	public void refresh() {
		...
		int firstMatch = -1;
		for (Mod349Detail detail : this.model.getDetails()) {
			...
			if (i == Model349DetailTable.this.selectionIndex) {
				row.addStyleName(AON.CSS.aonBackgroundLigthBlue());
				resetSelection = !visible; 
			}
			final int idx = i;
			row.addClickHandler( event ->  {
				Model349DetailTable.this.selectionIndex = idx;
				styleTable( tab );				
				SelectionEvent.fire(Model349DetailTable.this, detail);	
			});
		...
		if (resetSelection) {
			...
			SelectionEvent.fire(Model349DetailTable.this, getSelected() );
		}
	}
