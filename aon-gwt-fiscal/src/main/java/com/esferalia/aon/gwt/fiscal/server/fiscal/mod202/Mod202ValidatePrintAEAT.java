...
@WebServlet(name = "Mod202 Validate Print AEAT", urlPatterns = { "/aon_gwt_fiscal/ms/Mod202ValidatePrintAEAT" })
public class Mod202ValidatePrintAEAT extends HttpServlet {
	
	private static final Logger LOGGER = Logger.getLogger(Mod202ValidatePrintAEAT.class.getName()); 
	...
	private enum AeatUrl {
		URL_2021_2_SEMESTER {
			...
			@Override
			protected boolean accept(Mod202 mod202) {
				return mod202.getYear() <= 2024;
			}
			...
			@Override
			protected String getUrlParameters(Mod202 mod202) throws IOException {
				...
				return MessageFormat.format("MOD=202&EJF={0}&FIC={1}&IDI=ES"
						,AonNumberUtils.toString( mod202.getYear())
						,ModelAdmonUtils.getEncodedFile(output.toByteArray(),StandardCharsets.ISO_8859_1));
			}
		};
		private static AeatUrl getAeatUrl(Mod202 mod202) {
			for (AeatUrl aeatUrl : AeatUrl.values()) {
				if (aeatUrl.accept(mod202)) {
					return aeatUrl;
				}
			}
		...
		protected abstract boolean accept( Mod202 mod202);
		protected abstract String getUrl();
		protected abstract String getUrlParameters(Mod202 mod202) throws IOException;
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			Mod202 mod202 = MODEL202.get(occam, ModelAdmonUtils.getFiscalModelId(aeatParams));
			if (mod202 == null) {
				throw new AonCoreException("[INT] Modelo no encontrado");
			}
			if (mod202.getYear() >= 2025) {
				ModelAdmonUtils.serValiDos(resp, aeatParams, mod202);
			} else {
				AeatUrl aeatURL = AeatUrl.getAeatUrl(mod202);
				HttpRequest request = HttpRequest.newBuilder()
					.uri(URI.create( aeatURL.getUrl() ))
					.POST(HttpRequest.BodyPublishers.ofString(aeatURL.getUrlParameters(mod202)))
