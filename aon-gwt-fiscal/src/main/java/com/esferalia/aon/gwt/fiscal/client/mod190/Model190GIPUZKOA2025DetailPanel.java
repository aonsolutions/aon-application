...
import com.esferalia.aon.gwt.fiscal.client.mod190.Model190GIPUZKOADetail2025.IModel190DetailCallback;
import com.esferalia.aon.occam.api.model.fiscal.Mod190Detail;
import com.esferalia.aon.occam.api.model.type.Mod1902025GipuzkoaKey;
...
public class Model190GIPUZKOA2025DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190GIPUZKOA2025DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.setWidth("40px");
		for (Mod1902025GipuzkoaKey k : Mod1902025GipuzkoaKey.values()) {
			key.addItem(k.getDescription(),k.getValue());
		}
		
		Model190GIPUZKOA2025DetailPanel.setValue(key, subkey, detail);
		
		key.addChangeHandler(event -> {
			subkey.clear();
			Mod1902025GipuzkoaKey keyEnum = Mod1902025GipuzkoaKey.values()[key.getSelectedIndex()];
		...
		tab3.getColumnFormatter().setWidth(0, "80px");
		...
		tab3.setWidget(1, 1, new Model190SmallerLabel(AON.MSG.perception()));
		tab3.setWidget(1, 2, new Model190SmallerLabel(AON.MSG.retention()));
		tab3.setWidget(1, 3, new Label());
		...
		tab3.setWidget(3, 0, new Label());
		tab3.setWidget(3, 1, new Model190SmallerLabel(AON.MSG.inKindPerception()));
		tab3.setWidget(3, 2, new Model190SmallerLabel(AON.MSG.inKindDeposit()));
		tab3.setWidget(3, 3, new Model190SmallerLabel(AON.MSG.inKindOutputDeposit()));
		tab3.setWidget(3, 4, new Label());

		tab3.setWidget(4, 0, new Model190SmallerLabel(AON.MSG.inKind()));
		
		AonDoubleBox perceptionIL1 = new AonDoubleBox();
		perceptionIL1.setValue(detail.getInKindPerceptionIL());
		perceptionIL1.addValueChangeHandler(event -> {
			detail.setInKindPerceptionIL(perceptionIL1.getValue());
			callback.onValueChanged(detail);
		});
		tab3.setWidget(4, 1, perceptionIL1);		
		
		AonDoubleBox retentionIL1 = new AonDoubleBox();
		retentionIL1.setValue(detail.getInKindDepositIL());
		retentionIL1.addValueChangeHandler(event -> {
			detail.setInKindDepositIL(retentionIL1.getValue());
			callback.onValueChanged(detail);
		});
		tab3.setWidget(4, 2, retentionIL1);
		
		AonDoubleBox outputRetentionIL = new AonDoubleBox();
		outputRetentionIL.setValue(detail.getInKindOutputDepositIL());
		outputRetentionIL.addValueChangeHandler(event -> {
			detail.setInKindOutputDepositIL(outputRetentionIL.getValue());
			callback.onValueChanged(detail);
		});
		tab3.setWidget(4, 3, outputRetentionIL);
		tab3.setWidget(4, 4, new Label());
		...
		tab31.addStyleName(AON.CSS.aonMarginTop());
	...
	private static void setValue(ListBox key, ListBox subKey, Mod190Detail detail) {
		if (AonStringUtils.isBlank( detail.getKey())) {
			detail.setKey(Mod1902025GipuzkoaKey.A.toString());
		}
		Mod1902025GipuzkoaKey keyEnum = Mod1902025GipuzkoaKey.valueOf(detail.getKey());
	...
	private void enableOrDisableAdditionalDataPanel(ListBox key, ListBox subKey, Panel  panel) {
		...
		boolean a = Mod1902025GipuzkoaKey.A == keyEnum;
		boolean abc = Mod1902025GipuzkoaKey.A == keyEnum || Mod1902025GipuzkoaKey.B == keyEnum || Mod1902025GipuzkoaKey.C == keyEnum;
		boolean e01 = Mod1902025GipuzkoaKey.E == keyEnum && "01".equals(subk);
		boolean efghi = (Mod1902025GipuzkoaKey.E == keyEnum) || 
				        (Mod1902025GipuzkoaKey.F == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk) || "04".equals(subk) || "05".equals(subk) || "06".equals(subk))) ||
				        (Mod1902025GipuzkoaKey.G == keyEnum && ("01".equals(subk) || "02".equals(subk) || "03".equals(subk) || "04".equals(subk) || "05".equals(subk) || "06".equals(subk) || "08".equals(subk))) ||
				        (Mod1902025GipuzkoaKey.H == keyEnum) ||
				        (Mod1902025GipuzkoaKey.I == keyEnum);
		boolean l051027 = Mod1902025GipuzkoaKey.L == keyEnum && ("05".equals(subk) || "10".equals(subk) || "27".equals(subk));
		boolean l29 = Mod1902025GipuzkoaKey.L == keyEnum && "29".equals(subk);
		
		panel.setVisible(abc || efghi || l051027 || l29); 
		
		disability.setEnabled(a);                            // Discapacidad: Clave A.
		contract.setEnabled(a);                              // Tipo de relación: Clave A.
		applicableReduction.setEnabled(abc || efghi);        // Importe no integrado: Claves A, B,	C, E, F (subclaves 01 a 06), G (subclaves 01 a 06 y 08), H e I.
		deducibleExpense.setEnabled(abc || e01 || l051027);  // Gastos deducibles: Claves A, B, C, E01, L05, L10 y L27.
		compensatoryPension.setEnabled(abc);                 // Pensiones compensatorias: Claves A, B y C.
		otherDescendent.setEnabled(abc);                     // Descendientes: Claves A, B y C.
		titConvivivencia.setEnabled(l29);                    // Titular de la unidad de convivencia: Clave L29.
		spouseDocument.setEnabled(l29);                      // NIF titular unidad de convivencia: Clave L29.
		compInfancia.setEnabled(l29);                        // Complemento ayuda a la infancia: Clave L29.
	...
	private void enableOrDisableIlPanel(ListBox key, ListBox subKey, Panel  panel) {
		Mod1902025GipuzkoaKey keyEnum = Mod1902025GipuzkoaKey.values()[key.getSelectedIndex()];
		String subk = ((subKey.getSelectedIndex() == -1) ? null : subKey.getValue(subKey.getSelectedIndex()));
		panel.setVisible( Mod1902025GipuzkoaKey.A == keyEnum || (Mod1902025GipuzkoaKey.B == keyEnum && "01".equals(subk)) );
	}
