...
public abstract class CreditorPanel extends ScrollPanel {

	private static RegistryServiceAsync REGISTRY_SERVICE;
	
	private static final Logger LOGGER = Logger.getLogger(CreditorPanel.class.getName());
	...
	private Map<Integer, Creditor> rowCreditors = new HashMap<>();
	...
	public CreditorPanel(RegistryParams params, SimplePanel centerPanel) {
	...
	private void searchData() {
		...
		getList(creditors -> {
			...
			if (creditors.size() < limit) {
				disableMoreData();
			} else {
				offset.setValue(offset.intValue() + creditors.size() - 1);
				enableMoreData();
			}
	...
	private void paintRow(Creditor creditor) {
		HTMLPanel row = tab.createRow();
		row.addDomHandler(e -> onCreditorOpen(creditor), ClickEvent.getType());
		
		Label document = new Label(creditor.getDocument());
		document.setTitle(creditor.getDocument());
		tab.addInlineStyle(document, COLS.DOC.getStyles());
		tab.addRow(row, document, COLS.DOC.getColWidth());
		
		Label name = new Label(creditor.getName());
		name.setTitle(creditor.getName());
		tab.addInlineStyle(name, COLS.DES.getStyles());
		tab.addRow(row, name, COLS.DES.getColWidth());
		
		Label alias = new Label(creditor.getAlias());
		alias.setTitle(creditor.getAlias());
