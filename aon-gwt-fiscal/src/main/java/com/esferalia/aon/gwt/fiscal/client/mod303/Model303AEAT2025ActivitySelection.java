...
class Model303AEAT2025ActivitySelection extends ScrollPanel implements HasSelectionHandlers<Epigraph>{

	protected Model303AEAT2025ActivitySelection() {
		...
		Arrays.stream(Epigraph.values())
			...
			.forEach(epi -> {
				...
				row.addClickHandler(event -> {
					SelectionEvent.fire(Model303AEAT2025ActivitySelection.this, epi);
				});	
			});
