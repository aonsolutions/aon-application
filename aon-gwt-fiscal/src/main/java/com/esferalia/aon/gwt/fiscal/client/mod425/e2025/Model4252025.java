...
public class Model4252025 extends DockLayoutPanel {
	
	private static final Integer DEFAULT_YEAR = 2025;
	private static final String MOD425_2025_DRAFT = "/aon_gwt_fiscal/Model4252025Draft";
			
	protected static final Mod4252025ServiceAsync MOD4252025_SERVICE;
	static {
		Mod4252025ServiceAsync mod4252025ServiceRaw = GWT.create(Mod4252025Service.class);
		MOD4252025_SERVICE = new Mod4252025ServiceAsyncDecorator(mod4252025ServiceRaw);
	}
	
	class Model4252025Callback {
		void markAsDirty() {
			Model4252025.this.markAsDirty();
		}
		Mod4252025 getModel() {
			return Model4252025.this.getModel(); 
		}
		void setModel(Mod4252025 model) {
			Model4252025.this.setModel(model);
		}
		void refreshDeclarationToolbarPanel() {
			Model4252025.this.refreshDeclarationToolbarPanel();			
		}
	}
	
	private SimpleLayoutPanel contentContainer = new SimpleLayoutPanel();
	private final Model390Callback callback; 
	private Mod4252025 model;
	...
	public Model4252025(final Model390Callback mod390Callback, Mod390 mod390) {
		super(Unit.PX);
		...
		popup.center();
		try {
			MOD4252025_SERVICE.get(getCallback().getOptions().getOccam(), mod390, new AsyncCallback<Mod4252025>() {
				@Override
	...
	public Mod4252025 getModel() {
		return model;
	}
	public void setModel(Mod4252025 model) {
		this.model = model;
	}
	...
	private Widget getToolbar() {
		...
		saveButton.addClickHandler(event -> {
			...
			try {
				...
				MOD4252025_SERVICE.save(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Mod4252025>() {
					@Override
					public void onSuccess(Mod4252025 result) {
						setDirty(false);
						select(result);
						popup.hide();
		...
		deleteButton.addClickHandler(event -> {
			...
			cd.confirm(AON.MSG.confirmDeclarationDeleteAction(), new AonConfirmDialogCallback() {
				...
				public void onAccept() {
					MOD4252025_SERVICE.delete(getCallback().getOptions().getOccam(), getModel(), new AsyncCallback<Void>() {
						@Override
		...
		draftButton.addClickHandler(event -> submitForm(MOD425_2025_DRAFT));
	...
	public void select(Mod4252025 m425) {
		if ("INVALID".equals(m425.getXmlFormat()) ) {
			AonConfirmDialog cd = new AonConfirmDialog();
			cd.confirm( "La declaraci\u00F3n est\u00E1 creada con un formato incorrecto y es inv\u00E1lida. \u00BFDesea borrarla?"
				...
				@Override
				public void onAccept() {
					MOD4252025_SERVICE.delete(getCallback().getOptions().getOccam(),
							m425, new AsyncCallback<Void>() {
						@Override
						public void onSuccess(Void result) {
							getCallback().cleanErrorPanel();
							getCallback().onRemove(m425);
						}
	...
	private AonToolbar getDeclarationToolbarPanel() {
		...
		markAsFinishedButton.addClickHandler(event -> {
			...
			MOD4252025_SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.FINISHED, new AsyncCallback<Mod4252025>() {
				@Override
				public void onSuccess(Mod4252025 result) {
		...
		markAsSentButton.addClickHandler(event -> {
			...
			MOD4252025_SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.SENT, new AsyncCallback<Mod4252025>() {
				@Override
				public void onSuccess(Mod4252025 result) {
					getCallback().reload(result.getId());
				}
		...
		markAsPendingButton.addClickHandler(event -> {
			...
			MOD4252025_SERVICE.changeStatus(getCallback().getOptions().getOccam(), getModel(), FiscalStatus.PENDING, new AsyncCallback<Mod4252025>() {
				@Override
				public void onSuccess(Mod4252025 result) {
					getCallback().reload(result.getId());
				}
