...
public class Model190AEAT2016DetailPanel extends SimpleLayoutPanel implements Focusable {
	...
	public Model190AEAT2016DetailPanel(Mod190Detail detail, IModel190DetailCallback callback) {
		...
		key.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				Model190AEAT2016DetailPanel.enableOrDisableIlPanel(key, subkey, ilPanel);
				Model190AEAT2016DetailPanel.enableOrDisableAdditionalDataPanel(key, subkey, additionalDataPanel);
				callback.onValueChanged(detail);
			}
		});
