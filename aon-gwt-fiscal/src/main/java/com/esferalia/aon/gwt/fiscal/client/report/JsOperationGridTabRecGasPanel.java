...
public class JsOperationGridTabRecGasPanelNew extends JsOperationGridPanel {
	...
	private void paintHeader() {
		...
		row.addCell(new Label("Fecha Exp."), AON.CSS.aonWidth80())
			...
			.addCell(new Label("Cuota IVA. Deducible"), AON.CSS.aonWidth80())
	...
	public void addRow(JsOperationBreakdownNew br) {
		...
		addCell(row, br.getDeductibleQuota()); 
		...
		setSumDeductibleQuota(getSumDeductibleQuota() + br.getDeductibleQuota());
		...
		if (!isSomething()) {
			...
			AonDisplayGridFooterRow row = getGrid().addFooterRow()
				...
				row.addCell(new Label())
					...
					.addCell(new Label(AON.CURRENCY_FORMAT.format(getSumDeductibleQuota())),AON.CSS.aonTextRight(),AON.CSS.aonBold())
					.addCell(new Label())
