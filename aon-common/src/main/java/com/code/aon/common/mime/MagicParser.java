...
public class MagicParser extends DefaultHandler implements ContentHandler, ErrorHandler
{
    ...
    private static Logger log = Logger.getLogger(MagicParser.class.getName());
        ...
        throws MagicParseException
    {
        ...
        if (!initialized) {
            ...
            try {
                ...
                try {
                    log.fine("falling back to default parser: " + DEFAULT_PARSER_NAME);
            ...
            try {
                ...
                log.fine("initialize(): warning: Parser does not support feature (" +
            ...
            try {
                ...
                if (magicURL == null) {
                    log.severe("initialize(): couldn't load '" + magicURL + "'");
        ...
        throws SAXException
    {
        ...
        log.fine("characters(): value is '" + value + "'");
        ...
        throws SAXException
    {
        ...
        log.fine("startElement(): localName is '" + localName + "'");

        // create a new matcher
        if (localName.equals("match")) {
            log.fine("startElement(): creating new matcher");
            // match to hold data
            ...
            if (localName.equals("mimetype")) {
                ...
                if ((name != null) && (value != null)) {
                    ...
                    if (!properties.containsKey(name)) {
                        ...
                        log.fine("startElement():   setting property '" + name + "'='" + value +
                            "'");
                    } else {
                        log.fine("startElement():   not setting property '" + name +
                            "', duplicate key");
                    }
                }
            } else if (localName.equals("match-list")) {
                log.fine("startElement(): found submatcher list");

                // this means we are processing a child match, so we need to push
                // the existing match on the stack
                log.fine("startElement(): pushing current matcher to stack");
                stack.add(matcher);
            } else {
    ...
    {
        log.fine("endElement()");
        log.fine("endElement(): localName is '" + localName + "'");

        // determine which tag these chars are for and save them
        if (isMimeType) {
            isMimeType = false;
            match.setMimeType(finalValue);
            log.fine("characters(): setting mimetype to '" + finalValue + "'");
        } else if (isExtension) {
            isExtension = false;
            match.setExtension(finalValue);
            log.fine("characters(): setting extension to '" + finalValue + "'");
        } else if (isDescription) {
            isDescription = false;
            match.setDescription(finalValue);
            log.fine("characters(): setting description to '" + finalValue + "'");
        } else if (isTest) {
            isTest = false;
            match.setTest(convertOctals(finalValue));
            log.fine("characters(): setting test to '" + convertOctals(finalValue) + "'");
        } else {
            // do nothing
        }
        ...
        if (localName.equals("match")) {
            ...
            if (matcher.isValid()) {
                ...
                // add root match
                if (stack.size() == 0) {
                    log.fine("endElement(): adding root matcher");
                    matchers.add(matcher);
                } else {
                    // we need to add the match to it's parent which is on the
                    // stack
                    log.fine("endElement(): adding sub matcher");
            ...
            if (stack.size() > 0) {
                log.fine("endElement(): popping from the stack");
    ...
    private ByteBuffer convertOctals(String s)
    {
            ...
            if (s.charAt(end + 1) != '\\') {
                ...
                if ((end + 4) <= s.length()) {
                    try {
                        ...
                        //log.fine("converted octal '"+s.substring(end+1,end+4)+"' to '"+chr);
                        //log.fine("converted octal back to '"+Integer.toOctalString(chr));

                        //log.fine("converted '"+s.substring(end+1,end+4)+"' to "+chr+"/"+((char)chr));
                        buf.write(chr);
                        beg = end + 4;
                        end = beg;
                    ...
                    //log.fine("not an octal, not enough chars left in string");
                    buf.write((int) '\\');
                    beg = end + 1;
                    end = beg;
                }
        ...
        try {
            log.fine("convertOctals(): returning buffer size '" + buf.size() + "'");

            ByteBuffer b = ByteBuffer.allocate(buf.size());

            return b.put(buf.toByteArray());
        } catch (Exception e) {
            log.severe("convertOctals(): error parsing string: " + e);
