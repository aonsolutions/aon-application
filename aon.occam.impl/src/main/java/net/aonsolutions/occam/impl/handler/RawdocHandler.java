...
class RawdocHandler {
	...
	static void delete(AONContext ctx, Integer domain, Integer id) {
		ctx.checkWrite();
	...
	private static class RawdocFiller extends Filler<Rawdoc> {
		@Override
		public Rawdoc apply(Record r) {
			return build(r);
		}
		...
		public static Rawdoc build(Record r) {
			return new Rawdoc()
				...
				.setNature(RawdocNature.value(getValue(r, RAWDOC.NATURE)).orElse(null))
				.setType(RawdocType.value(getValue(r, RAWDOC.TYPE)).orElse(null))
				.setStatus(RawdocStatus.value(getValue(r, RAWDOC.STATUS)).orElse(null))
				.setJson(getValue(r, RAWDOC.JSON))
				.setLog(getValue(r, RAWDOC.LOG))
				.setMimeType(MimeType.value(getValue(r, RAWDOC.MIME_TYPE)).orElse(null))
