...
import net.aonsolutions.occam.impl.handler.InvestAssetHandler.InvestAssetFiller;
import net.aonsolutions.occam.impl.handler.SellerHandler.SellerFiller;

class InvoiceDetailHandler {

   
    private InvoiceDetailHandler() {
	...
	private static SelectOnConditionStep<Record> select(AONContext ctx){  
	  return ctx.getDslContext()
          ...
          .leftOuterJoin(SellerHandler.REGISTRY_SELLER).on(SellerHandler.REGISTRY_SELLER.ID.eq(INVOICE_DETAIL.SELLER))
	...
	private static class InvoiceDetailFiller extends Filler<InvoiceDetail> {
		...
		public static InvoiceDetail build(Record r) {
	    	if (isNull(r, INVOICE_DETAIL.ID)) return null;
			return new InvoiceDetail()
				...
				.setSeller( SellerFiller.build(r, SellerHandler.REGISTRY_SELLER))
				.setItem( getValue(r, ITEM.ID) )
				...
				.setWorkplace(getValue(r, INVOICE_DETAIL.WORKPLACE))
				.setWarehouse(getValue(r, INVOICE_DETAIL.WAREHOUSE))
				.setSource(InvoiceSource.value(getValue(r, INVOICE_DETAIL.SOURCE)).orElse(null))
...
//	}
	...
	static void deleteDetail(AONContext ctx, InvoiceDetail detail) {
		...
		InvoiceTaxHandler.delete(ctx,detail);
