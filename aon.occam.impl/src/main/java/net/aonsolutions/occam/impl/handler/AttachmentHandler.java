...
class AttachmentHandler {
	...
	static Integer insertInvoiceAttach(AONContext ctx, Attach attach){
		ctx.checkWrite();
		return ctx.getDslContext()
			.insertInto(INVOICE_ATTACH)
			.set(INVOICE_ATTACH.ATTACH_DATE, AonDateUtils.toSql(attach.getDate()))
			.set(INVOICE_ATTACH.DATA, attach.getData())
			.set(INVOICE_ATTACH.DESCRIPTION, attach.getDescription())
			.set(INVOICE_ATTACH.DOMAIN, attach.getId())
			.set(INVOICE_ATTACH.DRIVEID, attach.getDriveId())
			.set(INVOICE_ATTACH.INVOICE, attach.getAttachModule())
			.set(INVOICE_ATTACH.MIMETYPE, MimeType.value(attach.getMimeType()))
			.set(INVOICE_ATTACH.TYPE, InvoiceAttachmentType.INVOICE.value()  ) 
		.returning(INVOICE_ATTACH.ID)
		.fetchOne()
		.getId();
	}
