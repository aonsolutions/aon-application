...
import net.aonsolutions.occam.api.model.UserScope;
...
class SecurityHandler {
	...
	static Optional<User> getUser(AONContext ctx, Configuration config) {
		return getUser(ctx, config.getDomainId(), ctx.getUser());
	}
	
	static Optional<Integer[]> getUserScopes(AONContext ctx, Configuration config) {
		User user = ctx.getCurrentUser(config.getDomainId());
		Integer[] scopes = getUserScopeStream(ctx, user.getId() )
			.map( Scope::getId )
	...
	static Optional<Scope> getDefaultScope(AONContext ctx, Configuration config) {
		User user = ctx.getCurrentUser(config.getDomainId());
		Optional<Scope> defaultScope = SecurityHandler.getUserScopeStream(ctx, user.getId()).findFirst();
		if (defaultScope.isPresent()) return defaultScope;
		
		// Si el usuario tiene un único scope, se devuelve.
		List<Scope> scopes = SecurityHandler.getUserScopeStream(ctx, user.getId())
			.collect(Collectors.toCollection(LinkedList::new));
		if (AonCollectionUtils.size( scopes ) == 1) {
			return AonCollectionUtils.stream( scopes ).findFirst();
		}
		
		// El usuario no tiene user_scope, si en el dominio sólo hay uno. se vincula y devuelve.
		List<Scope> domainScopes = SecurityHandler.getScopeStream(ctx, config.getDomainId())
			.collect(Collectors.toCollection(LinkedList::new));
		if ( AonCollectionUtils.size(domainScopes) == 1) {
			Scope generalScope = saveDefaultScope( ctx , config.getDomainId());
			insertUserScope( ctx,  user, generalScope);
			return Optional.ofNullable(generalScope);
		}
		return Optional.empty();
	}
	
	static Optional<Integer> getScopeFromContract(AONContext ctx, Integer registry) {
		Optional<Integer> optId = ctx.getDslContext()
			.select(SCOPE.ID)
			   .from(CONTRACT)
	...
	static Optional<Integer> getScopeFromRegistry(AONContext ctx, boolean payment, Integer registry) {
		if (payment) {
			Optional<Integer> optId = getScopeFromCreditor(ctx, registry);
			if (optId.isEmpty()) {
				optId = getScopeFromSupplier(ctx, registry);	
			}
			return optId;
		}
		return getScopeFromCustomer(ctx, registry);
	}
	
	static Optional<Integer> getScopeFromCustomer(AONContext ctx, Integer id) {
		return ctx.getDslContext()
			.select(CUSTOMER.SCOPE)
				...
				.stream()
				.findFirst()
				.map( r -> r.getValue( CUSTOMER.SCOPE ));
	}
	static Optional<Integer> getScopeFromSupplier(AONContext ctx, Integer id) {
		return ctx.getDslContext()
	...
	static Optional<Integer> getScopeFromCreditor(AONContext ctx, Integer id) {
		return ctx.getDslContext()
			.select(CREDITOR.SCOPE)
				.from(CREDITOR)
	...
	private static Optional<User> getUser(AONContext ctx, int domain, String login) {
		ctx.checkRead();
		Optional<User> optUser = ctx.getDslContext()
				.select()
				.from(USER)
				.where(USER.DOMAIN.equal(domain))
	...
	private static Stream<Scope> getScopeStream(AONContext ctx, int domain ){
		return ctx.getDslContext()
			.select()
			.from(SCOPE)
			.where(SCOPE.DOMAIN.in(ctx.getInheritanceDomainIds( domain )))
			.fetchInto(SCOPE)
			.stream()
			.map(new ScopeFiller())
		;
	...
	private static Stream<Scope> getUserScopeStream(AONContext ctx,  Integer userId){
		return ctx.getDslContext()
				.select()
				.from(SCOPE)
	...
	private static Scope saveDefaultScope(AONContext ctx, Integer domainId) {
		return saveScope( ctx,new Scope().setDomain(domainId).setDescription("GENERAL"));
	}
	
	private static Scope saveScope(AONContext ctx, Scope scope){
		ctx.checkWrite();
		if (scope.getId() == null) {
			Integer id = ctx.getDslContext().insertInto(SCOPE)
				.set(SCOPE.DOMAIN, scope.getDomain())
	...
	private static UserScope insertUserScope(AONContext ctx, User user, Scope generalScope) {
		return insertUserScope(ctx, new UserScope()
				.setDomain(generalScope.getDomain())
				.setUserId(user.getId())
				.setScope(generalScope.getId())
		);
	}
	private static UserScope insertUserScope(AONContext ctx, UserScope userScope){
		Integer id = ctx.getDslContext().insertInto(USER_SCOPE)
			.set(USER_SCOPE.DOMAIN, userScope.getDomain())
			.set(USER_SCOPE.SCOPE, userScope.getScope())
			.set(USER_SCOPE.USER_ID, userScope.getUserId())
			.returning(SCOPE.ID)
			.fetchOne()
			.getValue(SCOPE.ID);
		return userScope.setId(id);
	}
			
	// ************************************************
	// ************************************** [FILLERS]
	// ************************************************
	private static class UserFiller extends Filler<User> {
	...
	private static class ScopeFiller extends Filler<Scope> {
		@Override
		...
		public static Scope buildScope(Record r) {
			return new Scope()
				.setId(getValue(r, SCOPE.ID))
				.setDomain(getValue(r,SCOPE.DOMAIN))
				.setDescription(getValue(r,SCOPE.DESCRIPTION))
			;
		}
	}
...
/*
	...
	public static User getUser(AONContext ctx, Integer userId) {
		...
		if (record != null) {
			...
			user.setRoles( SecurityHandler.getUserRoles(ctx, user.getId()));
		}
		return user;
	}
