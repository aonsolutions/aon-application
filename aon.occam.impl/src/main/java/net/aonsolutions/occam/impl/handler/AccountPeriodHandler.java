...
class AccountPeriodHandler {
	...
	static Stream<AccountPeriod> stream(AONContext ctx, int domain) {
		return stream( ctx, domain, null);
	}
	...
	static AccountPeriod ensurePeriod(AONContext ctx, int domain, Date entryDate) {
		return get(ctx, domain, entryDate)
			.orElseGet(() -> save(ctx, 
				new AccountPeriod()
					.setDomain(domain)
	...
	static AccountPeriod save(AONContext ctx, AccountPeriod ap) {
	...
	private static AccountPeriod insert(AONContext ctx, AccountPeriod ap) {
		...
		Integer id = ctx.getDslContext().insertInto(ACCOUNT_PERIOD).set(ACCOUNT_PERIOD.DOMAIN, ap.getDomain())
				...
				.set(ACCOUNT_PERIOD.STATUS, AccountPeriodStatus.value( ap.getStatus() )  )
		...
		return get(ctx, ap.getDomain(), id)
	...
	private static AccountPeriod update(AONContext ctx, AccountPeriod ap) {
		...
		AccountPeriodAutoComplete.complete(ctx, ap);
		AccountPeriodValidation.validate(ctx, ap);
		ctx.getDslContext().update(ACCOUNT_PERIOD).set(ACCOUNT_PERIOD.DOMAIN, ap.getDomain())
			.set(ACCOUNT_PERIOD.NAME, ap.getName())
			.set(ACCOUNT_PERIOD.INITIATION_DATE, AonDateUtils.toSql(ap.getInitiationDate()))
			.set(ACCOUNT_PERIOD.DEADLINE, AonDateUtils.toSql(ap.getDeadline()))
			.set(ACCOUNT_PERIOD.STATUS, AccountPeriodStatus.value( ap.getStatus() )  )
	...
	private static void updateStatus(AONContext ctx, int domain, Integer period, AccountPeriodStatus status) { 
		ctx.checkWrite(); 
		ctx.getDslContext()
		  	.update(ACCOUNT_PERIOD)
		  		.set(ACCOUNT_PERIOD.STATUS, status.value() )
		  		.set(ACCOUNT_PERIOD.MODIFICATION_USER,ctx.getUser())
		  		.set(ACCOUNT_PERIOD.MODIFICATION_DATE, new Timestamp(System.currentTimeMillis()) ) 
		  	.where(ACCOUNT_PERIOD.ID.equal(period))
		  	.and(ACCOUNT_PERIOD.DOMAIN.equal(domain))
		  	.execute(); 
	}
	
	static void open(AONContext ctx, int domain, Integer period) {
		updateStatus(ctx, domain, period, AccountPeriodStatus.OPENING); 
	}
	static void active(AONContext ctx, int domain, Integer period) { 
		updateStatus(ctx, domain, period, AccountPeriodStatus.ACTIVE); 
	}
	static void close(AONContext ctx, int domain, Integer period) { 
		updateStatus(ctx, domain, period, AccountPeriodStatus.CLOSED);
	}
	static void operating(AONContext ctx, int domain, Integer period) { 
		updateStatus(ctx, domain, period, AccountPeriodStatus.OPERATING); 
	}
	 ...
	 * AonDateUtils.toSql(AonDateUtils.getYearLastDay(new Date()))); } 
	...
	private static class AccountPeriodValidation {
		...
		static void validate(AONContext ctx, AccountPeriod ap) throws AonCoreException {
		...
		static void validateDeletion(AONContext ctx, AccountPeriod ap) {
			HAS_ENTRIES_CHECK
				.accept(ap, ctx);
		}
