...
import net.aonsolutions.occam.impl.handler.FiscalModelHandler.FiscalModelFiller;

class AlcatrazHandler {
	
	private AlcatrazHandler() {
	}
	
	static List<FiscalModel> isInvoiceDeclared(AONContext ctx, Integer invoiceId) {
	...
	static boolean isInvoiceAlcatrazed(AONContext ctx, Integer invoiceId) {
		return ctx.getDslContext()
			.select()
			.from(ALCATRAZ)
			.where(ALCATRAZ.INVOICE.eq(invoiceId))
	...
	static List<FiscalModel> isTrackingDeclared(AONContext ctx, Integer id) {
		return ctx.getDslContext()
			.select()
			.from(ALCATRAZ)
			.leftOuterJoin(FS_MODEL).on(FS_MODEL.ID.equal(ALCATRAZ.FS_MODEL))
