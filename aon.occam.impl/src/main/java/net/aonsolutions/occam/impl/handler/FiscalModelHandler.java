	...
	private FiscalModelHandler() {
	 ...
	 static class FiscalModelFiller<T extends FiscalModel>  implements BiFunction<Record,Supplier<T>,T> {
		...
		public T apply(Record rec,Supplier<T> modelSupplier) {
			...
			model.setPeriod( Period.value(rec.getValue(FS_MODEL.PERIOD)).orElse(null));
			model.setAdministration(Administration.value(rec.getValue(FS_MODEL.ADMINISTRATION)).orElse(null));
			model.setStatus(FiscalStatus.value(rec.getValue(FS_MODEL.STATUS)).orElse(null));
			...
			model.setModel(FiscalModelType.value( rec.getValue(FS_MODEL.MODEL)).orElse(null));
			...
			model.setFinance(FinanceFiller.build(rec));
			...
			model.setDeclarationResultType(FiscalModelDeclarationType.value(rec.getValue(FS_MODEL.DECLARATION_TYPE)).orElse(null));
			model.setAccountEntry(rec.getValue(FS_MODEL.ACCOUNT_ENTRY));
			model.setResult(rec.getValue(FS_MODEL.RESULT));
