...
class AccountHandlerLoadTest extends AbstractOccamImplTest {
	...
	@Test
	void testInsert() {
		AonCollectionUtils.stream(ACCOUNTS)
			.filter(a -> AccountHandler.get(ctx, DOMAIN_ID, a.getCode()).isEmpty() )
			.map( a -> AccountHandler.save( ctx, DOMAIN_ID, a.setDomain( DOMAIN_ID ).setActive( true ) ))
			.forEach( a -> {
				assertNotNull(a);
