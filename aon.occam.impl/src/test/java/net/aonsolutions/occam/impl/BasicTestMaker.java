...
class BasicTestMaker {
	
	public static void main(String[] args) {
		BasicTestMakerImpl<Item> mm = new BasicTestMakerImpl<>(ITEM);
		mm.make( true );
	}
	
	private static class BasicTestMakerImpl<T extends Table<?>> {
		
		private final T table;
		private PrintStream out;
		
		private BasicTestMakerImpl( T table) {
			this.table = table;
			out = System.out;
		}
		
		private BasicTestMakerImpl<T> println(){
			out.println();
			return this;
		}
		private BasicTestMakerImpl<T> println( String l ){
			out.println(l);
			return this;
		}
		...
		private void make(boolean makeMetadataTest) {
			...
			println("\t\t\t.setDeleted( expected.isDeleted() )");
			println("\t\t;");
			...
			println("\t\tassertNotEquals(a1, ("+name( table.getName() )+") null);");
