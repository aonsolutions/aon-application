...
public class CheckMaster extends Check{
	...
	public static boolean parse(Master master,ArrayList<Exception> exceptions){
		...
		if (master.getOrderer() == null) {
			...
			if (master.getOrderer().getCode() == null || master.getOrderer().getCode().trim().equals("")) {
				exceptions.add(new Fd0Exception(getMessage("ERROR_MASTER_2"), master.toString()));
				status = false;
			}
			if (master.getOrderer().getName() == null || master.getOrderer().getName().trim().equals("")) {
				exceptions.add( new Fd0Exception( getMessage("ERROR_MASTER_3"), master.toString()));
				status = false;
			}
			if (master.getOrderer().getAddress() == null || master.getOrderer().getAddress().trim().equals("")) {
				exceptions.add( new Fd0Exception( getMessage("ERROR_MASTER_4"), master.toString()));
				status = false;
			}
			if (master.getOrderer().getCity() == null || master.getOrderer().getCity().trim().equals("")) {
		...
		if (master.getDetail() == null){
			...
			if (!"0".equals(master.getDetail()) && !"1".equals(master.getDetail())){
