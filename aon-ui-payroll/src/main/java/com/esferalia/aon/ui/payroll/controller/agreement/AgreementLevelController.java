...
public class AgreementLevelController extends VariablesAbstractController {
	...
	protected void initializeVariables(ActionEvent event) {
		...
		try {
			AgreementLevel agreementLevel = ((AgreementLevel)getTo());
	...
	public List<?> expressionContext(Object suggest) {
		...
		try {
			...
			AgreementLevel level = (AgreementLevel) this.getTo();
			IManagerBean bean = BeanManager.getManagerBean(AgreementLevelData.class);
			Criteria criteria = new Criteria();  
	...
	protected void resetVariable() {
		...
		((AgreementLevelData)getData()).setLevel((AgreementLevel) getTo());
	}
