...
public class SalaryDraftDeductionLinesControllerListener extends LinesControllerListener{
	...
	protected void updateDetailCriteria(IController master, boolean reset) throws ControllerListenerException {
		try {
			...
			detail.getCriteria().addLessThanOrEqualExpression(detail.getFieldName(IPayrollAlias.CONTRACT_DEDUCTION_START_DATE), startDate );
			Expression expr1 = ExpressionUtilities.getGreaterThanOrEqualExpression(detail.getFieldName(IPayrollAlias.CONTRACT_DEDUCTION_END_DATE), endDate);
			Expression expr2 = ExpressionUtilities.getNullExpression(detail.getFieldName(IPayrollAlias.CONTRACT_DEDUCTION_END_DATE));
