...
public class PaymentUpdateController {
	...
	public void onSearch(ActionEvent event) {
		try {
			...
			criteria.addEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_WORK_PLACE_ENTERPRISE_ID), getEnterprise().getId());			
			criteria.addLessThanOrEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_START_DATE), endDate);			
	...
	private ContractData getContractContext(PaymentUpdate pu, IExpression exp, SalaryCalculatorContext scc) throws SalaryException {
		try {
			IManagerBean bean = BeanManager.getManagerBean(ContractData.class);
			Criteria criteria = new Criteria();
			criteria.addEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_DATA_CONTRACT_ID), pu.getContract().getId());			
			criteria.addEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_DATA_NAME), exp.getName());
			criteria.addLessThanOrEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_DATA_START_DATE), scc.getEndDate());			
			Expression expr1 = ExpressionUtilities.getGreaterThanOrEqualExpression(bean.getFieldName(IPayrollAlias.CONTRACT_DATA_END_DATE), scc.getStartDate());
