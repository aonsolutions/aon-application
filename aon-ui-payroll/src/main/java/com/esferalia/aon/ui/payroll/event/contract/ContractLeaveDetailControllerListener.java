...
public class ContractLeaveDetailControllerListener extends ControllerAdapter{
	...
	public void afterBeanCreated(ControllerEvent event)
			throws ControllerListenerException {
		ContractLeaveDetail lastLeave = getLastConfirmLeave((ContractLeave) ((LinesController)event.getController()).getMasterController().getTo());
	...
	public ContractLeaveDetail getLastConfirmLeave(ContractLeave leave){
		try {
			IManagerBean bean = BeanManager.getManagerBean(ContractLeaveDetail.class);
			Criteria criteria = new Criteria();
			criteria.addEqualExpression(bean.getFieldName(IEntityAlias.CONTRACT_LEAVE_DETAIL_TYPE), LeaveReportType.CONFIRM);
