...
import com.esferalia.aon.sepe.api.contrata.prorrogas.*;
...
public class ContrataProrrogasWriter {
	...
	public PRORROGATIPOTYPE createFile(IProrrogaType prorrogaType, ContractExtensionParams params) throws ManagerBeanException{
		if(prorrogaType!=null){
			return createProrroga(prorrogaType, params);
		}
		return null;
	}
	
	private PRORROGATIPOTYPE createProrroga(IProrrogaType prorrogaType, ContractExtensionParams params) throws ManagerBeanException{
		PRORROGATIPOTYPE type = (PRORROGATIPOTYPE) prorrogaType;
	 ...
	 * 
	 */
	private DATOSEMPRESATYPE createDatosEmpresa(ContractExtensionParams params) throws ManagerBeanException {
		...
		datos.setCCC(completeLength(getEnterpriseCCC(params.getContract().getWorkPlace().getEnterprise()),15,"0",false));
		return datos;
	}
	 ...
	 * 
	 */
	private DATOSCONTRATOTYPE createDatosContrato(ContractExtensionParams params) {
		...
		if(datos.getCLAVECONTRATO()==null){
			...
			datos.setFECHAINICIOCTO(getFormatedDate(params.getContract().getStartDate()));
		}
		return datos;
	}
	 ...
	 * 
	 */
	private DATOSGENERALESPRORROGATYPE createDatosGeneralesProrroga(ContractExtensionParams params) {
	 ...
	 * 
	 */
	private DATOSADICIONALESPRORROGATYPE createDatosAdicionalesProrroga(ContractExtensionParams params) {
	 ...
	 * @return
	 */
	private DATOSUSOLIBREEMPRESATYPE createDatosUsoLibreEmpresa(ContractExtensionParams params) {
