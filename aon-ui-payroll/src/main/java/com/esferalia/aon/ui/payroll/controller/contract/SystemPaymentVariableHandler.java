...
public class SystemPaymentVariableHandler extends AbstractVariableHandler {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(SystemPaymentVariableHandler.class.getName());

	public SystemPaymentVariableHandler(IController controller) {
		super(controller);
	}
	...
	public void initializeVariables(ActionEvent event) {
		...
		try {
			...
			if(payment.getExpression()!=null || payment.getPaymentConcept().getExpression()!=null){
				...
				if(!vl.isEmpty()){
					for(String s: vl){
						List<ITransferObject> list = existingAgreementData(s);
						if(!list.isEmpty()){
	...
	protected List<ITransferObject> existingAgreementData(String name) throws ManagerBeanException {
		IManagerBean bean = BeanManager.getManagerBean(SystemData.class);
		Criteria criteria = new Criteria();
//		criteria.addEqualExpression(bean.getFieldName(IEntityAlias.AGREEMENT_DATA_AGREEMENT_ID), agreement.getId());
		criteria.addEqualExpression(bean.getFieldName(IEntityAlias.SYSTEM_DATA_NAME), name);
	...
	protected void resetVariable() {
		...
		getData().setVariableData(new SystemData());
//		((SystemData)getData().getVariableData()).setAgreement(((AgreementPayment) getController().getTo()).getAgreement());
	}
