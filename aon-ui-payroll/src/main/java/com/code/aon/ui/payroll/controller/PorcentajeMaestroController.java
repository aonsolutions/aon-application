	...
	public void onSearch(ActionEvent event) {
		...
		try {
			if(searchFecini != null){
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.PORCENTAJE_MAESTRO_PORCENTAJES_ID_FECINI), searchFecini);
			}
			if(searchFecfin != null){
				//getCriteria().addEqualExpression(getFieldName(event.getComponent().getId()), searchFecfin);
				getCriteria().addEqualExpression(getFieldName(IPayrollAlias.PORCENTAJE_MAESTRO_PORCENTAJES_FECFIN), searchFecfin);
			}
