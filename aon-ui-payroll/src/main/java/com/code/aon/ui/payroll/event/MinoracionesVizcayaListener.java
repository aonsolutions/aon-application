...
public class MinoracionesVizcayaListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		MinoracionesVizcaya n = (MinoracionesVizcaya) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(MinoracionesVizcaya.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.MINORACIONES_VIZCAYA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				MinoracionesVizcaya cra = (MinoracionesVizcaya) iter.next();
