...
public class CuotaRetencionAlavaListener extends ControllerAdapter {
	...
	private void overLap(ControllerEvent event) throws ControllerListenerException{
		...
		CuotaRetencionAlava n = (CuotaRetencionAlava) c.getTo();
		...
		try {
			...
			IManagerBean bean = BeanManager
					.getManagerBean(CuotaRetencionAlava.class);

			Criteria criteria = new Criteria();
			String alias = bean
					.getFieldName(IPayrollAlias.CUOTA_RETENCION_ALAVA_ID_NUM_TRAMO);
			criteria.addEqualExpression(alias, n.getId().getnumTramo());

			List<ITransferObject> lista = bean.getList(criteria);
			Iterator iter = lista.iterator();
			while (iter.hasNext()) {
				CuotaRetencionAlava cra = (CuotaRetencionAlava) iter.next();
