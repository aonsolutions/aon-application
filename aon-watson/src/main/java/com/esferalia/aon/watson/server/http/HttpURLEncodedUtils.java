...
class HttpURLEncodedUtils {
     ...
     * @since 4.5
     */
    static List <NameValuePair> parse(final URI uri, final Charset charset) {
        HttpArgs.notNull(uri, "URI");
    ...
    static List <NameValuePair> parse(
        ...
        HttpArgs.notNull(entity, "HTTP entity");
        final HttpContentType contentType = HttpContentType.get(entity);
        if (contentType == null || !contentType.getMimeType().equalsIgnoreCase(CONTENT_TYPE)) {
            return createEmptyList();
        }
        final long len = entity.getContentLength();
        HttpArgs.check(len <= Integer.MAX_VALUE, "HTTP entity is too large");
        final Charset charset = contentType.getCharset() != null ? contentType.getCharset() : HTTP.DEF_CONTENT_CHARSET;
        final InputStream inStream = entity.getContent();
        if (inStream == null) {
            return createEmptyList();
        }
        final HttpCharArrayBuffer buf;
        try {
            buf = new HttpCharArrayBuffer(len > 0 ? (int) len : 1024);
     ...
     * @since 4.2
     */
    static List<NameValuePair> parse(final String s, final Charset charset) {
        if (s == null) {
            return createEmptyList();
        }
        final HttpCharArrayBuffer buffer = new HttpCharArrayBuffer(s.length());
        buffer.append(s);
     ...
     * @since 4.3
     */
    static List<NameValuePair> parse(final String s, final Charset charset, final char... separators) {
        if (s == null) {
            return createEmptyList();
        }
        final HttpCharArrayBuffer buffer = new HttpCharArrayBuffer(s.length());
     ...
     */
    static List<NameValuePair> parse(
            final HttpCharArrayBuffer buf, final Charset charset, final char... separators) {
        HttpArgs.notNull(buf, "Char array buffer");
        final HttpTokenParser tokenParser = HttpTokenParser.INSTANCE;
        final BitSet delimSet = new BitSet();
        for (final char separator: separators) {
            delimSet.set(separator);
        }
        final HttpParserCursor cursor = new HttpParserCursor(0, buf.length());
        ...
        while (!cursor.atEnd()) {
            ...
            if (!name.isEmpty()) {
                list.add(new HttpBasicNameValuePair(
     ...
     * @since 4.5
     */
    static List<String> parsePathSegments(final CharSequence s, final Charset charset) {
        HttpArgs.notNull(s, "Char sequence");
        final List<String> list = splitPathSegments(s);
        for (int i = 0; i < list.size(); i++) {
            list.set(i, urlDecode(list.get(i), charset != null ? charset : HTTP.UTF_8, false));
        }
        return list;
    }
     ...
     * @since 4.5
     */
    static List<String> parsePathSegments(final CharSequence s) {
        return parsePathSegments(s, HTTP.UTF_8);
    }
    ...
    static String formatSegments(final Iterable<String> segments, final Charset charset) {
        HttpArgs.notNull(segments, "Segments");
     ...
     * @since 4.5
     */
    static String formatSegments(final String... segments) {
        return formatSegments(Arrays.asList(segments), HTTP.UTF_8);
    }
    ...
    static String format(
            ...
            final char parameterSeparator,
            final Charset charset) {
        HttpArgs.notNull(parameters, "Parameters");
        final StringBuilder result = new StringBuilder();
        for (final NameValuePair parameter : parameters) {
            final String encodedName = encodeFormFields(parameter.getName(), charset);
            final String encodedValue = encodeFormFields(parameter.getValue(), charset);
    ...
    private static String decodeFormFields (final String content, final Charset charset) {
        ...
        return urlDecode(content, charset != null ? charset : HTTP.UTF_8, true);
    }
    ...
    private static String encodeFormFields(final String content, final String charset) {
        ...
        return urlEncode(content, charset != null ? Charset.forName(charset) : HTTP.UTF_8, URLENCODER, true);
    }
    ...
    private static String encodeFormFields (final String content, final Charset charset) {
        ...
        return urlEncode(content, charset != null ? charset : HTTP.UTF_8, URLENCODER, true);
    }
