 ...
 * @since 4.0
 */
final class HttpCharArrayBuffer implements CharSequence, Serializable {
    ...
    /**
     * Creates an instance of {@link HttpCharArrayBuffer} with the given initial
     * capacity.
     *
     * @param capacity the capacity
     */
    HttpCharArrayBuffer(final int capacity) {
        super();
        HttpArgs.notNegative(capacity, "Buffer capacity");
     ...
     * {@code off} + {@code len} is out of range.
     */
    void append(final HttpCharArrayBuffer b, final int off, final int len) {
        if (b == null) {
            return;
        }
     ...
     * @param   b        the source buffer to be appended.
     */
    void append(final HttpCharArrayBuffer b) {
        if (b == null) {
            return;
        }
     ...
     * {@code off} + {@code len} is out of range.
     */
    void append(final HttpByteArrayBuffer b, final int off, final int len) {
        if (b == null) {
            return;
        }
        append(b.buffer(), off, len);
     ...
     *   not occur.
     */
    int indexOf(final int ch, final int from, final int to) {
        int beginIndex = from;
        if (beginIndex < 0) {
            beginIndex = 0;
        }
     ...
     *             {@code endIndex}.
     */
    String substringTrimmed(final int beginIndex, final int endIndex) {
        if (beginIndex < 0) {
            throw new IndexOutOfBoundsException("Negative beginIndex: " + beginIndex);
        }
        if (endIndex > this.len) {
