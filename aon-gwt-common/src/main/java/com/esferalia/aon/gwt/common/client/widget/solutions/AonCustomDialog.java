...
public class AonCustomDialog extends PopupPanel implements AonCustomDialogListener {
    ...
    public AonCustomDialog() {
        ...
        focusAll.getElement().getStyle().setOutlineStyle(OutlineStyle.NONE);
        ...
        // -------------------------------
        focusAll.addKeyDownHandler(event -> {
            if (event.getNativeKeyCode() == KeyCodes.KEY_ESCAPE) {
                hide();
            }
        });
        focusAll.setFocus(true);
    }
    ...
    public void handleMove(int absX, int absY) {
        ...
        int viewportLeft = Window.getScrollLeft();
        int viewportTop = Window.getScrollTop();
        int viewportWidth = Window.getClientWidth();
        int viewportHeight = Window.getClientHeight();

        int currentLeft = getPopupLeft();
        int currentTop = getPopupTop();

        int newAbsLeft = currentLeft + absX;
        int newAbsTop = currentTop + absY;

        int dialogWidth = getOffsetWidth();
        int dialogHeight = getOffsetHeight();

        if (newAbsLeft < viewportLeft) newAbsLeft = viewportLeft;
        if (newAbsTop < viewportTop) newAbsTop = viewportTop;

        if (newAbsLeft + dialogWidth > viewportLeft + viewportWidth)
            newAbsLeft = viewportLeft + viewportWidth - dialogWidth;

        if (newAbsTop + dialogHeight > viewportTop + viewportHeight)
            newAbsTop = viewportTop + viewportHeight - dialogHeight;

        setPopupPosition(newAbsLeft, newAbsTop);
    }
    ...
    public void incrementPixelSize(int width, int height) {
        ...
        super.setPixelSize(offsetWidth + width, offsetHeight + height);
