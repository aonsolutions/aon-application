...
public class AonCustomComboBox<T> extends ListBox implements HasData<T> {
	...
	public static class FormatSafeHtmlCell<E> extends AbstractSafeHtmlCell<E> {
		...
		private AonCustomComboBox<E> comboBox;
		...
		public void setComboBox(AonCustomComboBox<E> comboBox) {
	...
	public AonCustomComboBox() {
		this(new DefaultFormat<T>());
	}

	public AonCustomComboBox(Format<T> format) {
		this(new FormatSafeHtmlCell<T>(format));
	}

	public AonCustomComboBox(AbstractFormatSafeHtmlRenderer<T> renderer) {
		this(new FormatSafeHtmlCell<T>(renderer));
	}

	protected AonCustomComboBox(FormatSafeHtmlCell<T> cell) {
		dropDowncell = cell;
		...
		super.setStyleName(AON.CSS.aonCustomTextBoxInput());
	...
	protected void onResizeDropDownList(int dropDownListWidth) {
		setWidth(this.getOffsetWidth() + "px");
	}
	...
	private void addHandlers2ComboBox() {
		class ComboBoxHandlers implements MouseDownHandler {
			...
			public void onMouseDown(MouseDownEvent event) {
				AonCustomComboBox.this.showDropDownList();
	...
	private void addHandlers2DropDownCellList() {
		class DropDownCellListHandler implements RangeChangeEvent.Handler {
			...
			public void onRangeChange(RangeChangeEvent event) {
				AonCustomComboBox.this.onResizeDropDownPopup();
			}
		}
	...
	private void addHandlers2DropDownSelectionModel() {
		class SelectionModelHandlers implements Handler {
			...
			public void onSelectionChange(SelectionChangeEvent event) {
				T selected = AonCustomComboBox.this.dropDownselectionModel
						.getSelectedObject();
				Format<T> format = AonCustomComboBox.this.dropDowncell
						.getFormatSafeHtmlRenderer().getFormat();
				String text = format.format(selected);
				AonCustomComboBox.this.setItemText(0, text);
	...
	private void addHandlers2DropDownScrollPanel() {
		...
		class ScrollPanelHandlers implements ScrollHandler, ScheduledCommand {
			...
			public void onScroll(ScrollEvent event) {
				...
				int lastScrollPos = AonCustomComboBox.this.dropDownScrollPanel
						.getVerticalScrollPosition();

				int maxScrollPos = AonCustomComboBox.this.dropDownScrollPanel
						.getMaximumVerticalScrollPosition();
				int minScrollPos = AonCustomComboBox.this.dropDownScrollPanel
						.getMinimumVerticalScrollPosition();

				Range range = AonCustomComboBox.this.getVisibleRange();
				...
				if (lastScrollPos <= minScrollPos) {
					...
					AonCustomComboBox.this.setVisibleRange(newRangeStart, rangeLength
							+ incrementSize);
					...
					AonCustomComboBox.this.setVisibleRange(rangeStart, rangeLength
