...
public class Base64 {
	...
	public static String decode(String data) {
		...
		for (int i = 0; i < decoded.length();) {
			byte e0 = dtab[decoded.charAt(i++) & 0x7f];
			byte e1 = dtab[decoded.charAt(i++) & 0x7f];
			byte e2 = dtab[decoded.charAt(i++) & 0x7f];
			byte e3 = dtab[decoded.charAt(i++) & 0x7f];
