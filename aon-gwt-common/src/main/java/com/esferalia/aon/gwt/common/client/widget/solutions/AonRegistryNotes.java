...
public class AonRegistryNotes extends ScrollPanel {

	private static final Logger LOGGER = Logger.getLogger(AonRegistryNotes.class.getName());
	static {
	...
	public AonRegistryNotes(String domainName, int domain, String user, Integer registryId, String noteTypeSource) {
		initializeService();
		...
		this.notesSource = noteTypeSource;
	...
	private void getRegistryNotes(Consumer<List<RegistryNote>> success) {
		COMMON_SERVICE.getRegistryNotes(domainName, domain, user, registryId,
	...
	private void moveToNotes(RegistryNote observation) {
		...
		COMMON_SERVICE.saveNote(domainName, domain, user, observation,
			new AsyncCallback<RegistryNote>() {

				@Override
				public void onSuccess(RegistryNote result) {
					COMMON_SERVICE.saveNote(domainName, domain, user, newNote,
							new AsyncCallback<RegistryNote>() {
	...
	private void saveNote(RegistryNote note) {
		...
		COMMON_SERVICE.saveNote(domainName, domain, user, note,
			new AsyncCallback<RegistryNote>() {
	...
	private void deleteNote(RegistryNote note) {
		COMMON_SERVICE.deleteNote(domainName, domain, user, note.getId(),
