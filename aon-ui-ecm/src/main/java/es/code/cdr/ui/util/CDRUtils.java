...
public class CDRUtils {
	...
	public static void addMessage(String[] message, FacesMessage.Severity severity) {
		...
		FacesMessage msg = getMessage(ctx, CDRUtils.AON_ERROR, message);
		msg.setSeverity(severity);
		ctx.addMessage(CDRUtils.AON_ERROR, msg);
	}
	 ...
	 * @throws IOException
	 */
	public static void download(FacesContext context, Document selected) throws RepositoryException, IOException {
...
//Streams we will use to read, write the file bytes to our response, first we load the file in our InputStream
		InputStream is = selected.getContent();
