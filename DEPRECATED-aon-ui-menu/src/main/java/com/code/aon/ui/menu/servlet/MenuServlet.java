...
/**
 * Servlet that returns in an XML document the content of an {@link IMenu}.
...
public class MenuServlet extends HttpServlet {
    ...
    /**
     * Se procesa el parámetro enviado por parámetro (
     * <code>IMenuConstants.MENU_PARAM</code>), el cual debe ser un
     * identificador del menu. Este menú deberá estar almacenado en el contexto
     * de aplicación bajo la clave <code>IMenuConstants.MENU_ATTR</code>. Si
     * el parametro no se envia, o es null, o blanco se asume que se está
     * pidiendo el nodo root. Si la estrutura del menú no se encuentra se
     * lanzará una <code>ServletException</code>. Como resultado se obtiene
     * un documento XML con el contenido del IMenu. Sólo su primer nivel.
     * 
     * @param hreq
    ...
    private void process(HttpServletRequest hreq, HttpServletResponse hres)
        ...
        try {
            ...
            String param = hreq.getParameter(IMenuConstants.MENU_ID_PARAM);
            IMenu menuItem = (param == null || "".equals(param)) ? menu
                    : (IMenu) menu.find(param);
            MenuVisitor visitor = new MenuVisitor(menuItem, bundle);
