...
public class DomainJSON {
	...
	public static Domain from(JSONObject json) {
		...
		return new Domain()
			...
			.setInheritance(AonJSONUtils.getBoolean(json, AonNames.INHERITANCE))
	...
	public static JSONObject to(Domain domain) {
		...
		return new JSONObject()
			...
			.putOpt(AonNames.PARENT, domain.getParent().map( DomainJSON::to ).orElse(null) )
			.putOpt(AonNames.TYPE, AonObjectUtils.ifNotNullGet(domain.getType(), Object::toString ))
			.putOpt(AonNames.INHERITANCE, domain.hasInheritance())
			.putOpt(AonNames.ACTIVE, domain.isActive())
			.putOpt(AonNames.SCOPE, domain.getScope().map( ScopeJSON::to).orElse(null) )
