...
public class Toolkit {
	...
	public static String[] splitDecimal(double dc, int decimals) {
		String dc_str = dc + "";
		return new String[] { dc_str.substring(0, dc_str.indexOf(".")),
				dc_str.substring(dc_str.indexOf(".") + 1, dc_str.indexOf(".") + decimals) };
	}
	...
	public static void verifyData(Object[] data) throws SepeException {
		for (Object o : data)
			if (o == null || (o instanceof String && ((String) o).trim().equals("")))
				throw new SepeException("UnfilledMandatory");
	}
	...
	public static String getIdentityType(String ipf) {
		...
		for (Entry<Pattern, Integer> entry : patterns.entrySet()) {
			if (entry.getKey().matcher(ipf).matches()) {
				identity = entry.getValue().toString();
				break;
			}
		}
		return identity;
