...
public class PaymentCardSettleSearchListener extends ControllerSearchListener {
...
//			criteria.addInExpression(getFieldName(IEntityAlias.FINANCE_INVOICE_PROJECT_ID), projectList);
//		}
		criteria.addEqualExpression(getFieldName(IEntityAlias.FINANCE_PAYMENT), false);
		criteria.addEqualExpression(getFieldName(IEntityAlias.FINANCE_REGISTRY_ID), getAgency().getId());
		criteria.addBetweenExpression(getFieldName(IEntityAlias.FINANCE_INVOICE_ISSUE_DATE), getStartDate(), getEndDate());
...
//		criteria.addEqualExpression("Finance.invoice<lines.workPlace.id", getHotel().getWorkPlace().getId());
	...
	private List<Integer> getProjectList() throws ManagerBeanException {
		IManagerBean bean = BeanManager.getManagerBean(Invoice.class);
		Criteria criteria = new Criteria();
		criteria.addEqualExpression(bean.getFieldName(IEntityAlias.INVOICE_REGISTRY_ID), getAgency().getId());
		criteria.addBetweenExpression(bean.getFieldName(IEntityAlias.INVOICE_ISSUE_DATE), getStartDate(), getEndDate());
		...
		for (ITransferObject to: bean.getList(criteria)) {
			...
			if(invoice.getProject()!=null && invoice.getProject().getId()!=null){
				list.add(invoice.getProject().getId());
			}
		}
		return list;
	}
