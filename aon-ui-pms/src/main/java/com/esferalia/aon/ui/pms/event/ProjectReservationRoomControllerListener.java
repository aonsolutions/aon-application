...
public class ProjectReservationRoomControllerListener extends ControllerAdapter {
	...
	private	int calculateNextIndex(IManagerBean bean, ProjectReservation reservation) throws ManagerBeanException {
		...
		criteria.addEqualExpression(bean.getFieldName(IPmsAlias.PROJECT_RESERVATION_ROOM_PROJECT_RESERVATION_ID), reservation.getId());
		Projection projection = Projection.max(bean.getFieldName(IPmsAlias.PROJECT_RESERVATION_ROOM_ROOM_INDEX));
