...
public class ProjectReservationGuestControllerListener extends ControllerAdapter {
	...
	private	int calculateNextIndex(IManagerBean bean, ProjectReservation reservation) throws ManagerBeanException {
		...
		criteria.addEqualExpression(bean.getFieldName(IEntityAlias.PROJECT_RESERVATION_GUEST_PROJECT_RESERVATION_ID), reservation.getId());
		Projection projection = Projection.max(bean.getFieldName(IEntityAlias.PROJECT_RESERVATION_GUEST_GUEST_INDEX));
