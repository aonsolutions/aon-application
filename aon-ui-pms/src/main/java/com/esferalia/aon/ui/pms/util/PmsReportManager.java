...
public class PmsReportManager {
	...
	public String getBoardListSQL(Hotel hotel, Product product) throws ManagerBeanException{
		...
		String select = ""+
			/* **************************************************/
	        /* servicios (directos ) sin habitacion asignada */
	        /* **************************************************/
	        " (SELECT PR.hotel," +
	        " IF ((P.code like '001%' AND PRS.extra=0 ), date_add(PRSD.effective_date,INTERVAL 1 DAY), PRSD.effective_date)," +
	        " P.name," +
	        " P.code," +
	        " GREATEST((SELECT SUM(PRR.adults+PRR.children) FROM project_reservation_room AS PRR WHERE PRR.project_reservation=PR.project)" +
	        " , sum(PRSD.quantity)) cantidad," +
	        " ' '," +
			...
			/* ****************************************************/
			/* servicios (compuestos ) sin habitacion asignada   */
			/* ****************************************************/
			" UNION" +
			" (SELECT PR.hotel," +
			" IF ( (P2.code like '001%' AND PRS.extra=0 ), date_add(PRSD.effective_date,INTERVAL 1 DAY), PRSD.effective_date)," +
