...
public class PmsReportManager {
	...
	public String getBoardBookingSQL(Hotel hotel, Product product, boolean roomGroup) throws ManagerBeanException{
		...
		String select = "(SELECT W.description,  (date(PRSD.effective_date) + INTERVAL 1 DAY) AS Fecha,"
				+ " P.name AS Servicio, P.code, A.name as Hab, PRR.adults+PRR.children AS Cantidad,"
				...
				+ " WHERE PRSD.effective_date BETWEEN (date(:start) + INTERVAL -1 DAY) AND (date(:end) + INTERVAL -1 DAY)"
