...
public class DivertReceptionSearchListener extends ControllerSearchListener {
	...
	private void addDivertHotelsToCriteria() throws ManagerBeanException {
		...
		for( ITransferObject to : hotelBean.getList(hotelCriteria) ) {
			...
			if ( to != null ) {
				if ( expToAdd == null ) {
					expToAdd = ExpressionUtilities.getEqualExpression(getFieldName(IEntityAlias.PROJECT_RESERVATION_DIVERT_DIVERT_HOTEL_ID), hotel.getId());				
				} else {
					Expression exp  = ExpressionUtilities.getEqualExpression(getFieldName(IEntityAlias.PROJECT_RESERVATION_DIVERT_DIVERT_HOTEL_ID), hotel.getId());
