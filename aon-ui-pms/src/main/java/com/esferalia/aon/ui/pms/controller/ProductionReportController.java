...
//				System.out.println(period + " - " + description +" - "+ type + " - " + amount);
	...
	private void buildRoomsReport(java.sql.Date date,
		...
		try {
			...
			if (!roomOcupationMap.containsKey("% Ocupaci�n")) {
				ReportObject ro = new ReportObject();
				ro.setDescription("% Ocupaci�n");
				roomOcupationMap.put("% Ocupaci�n", ro);
			}
...
//				System.out.println(period + " - " + roomCount +" - "+ availablesRoomCount + " - " + ocupation);
				...
				if (period.equals("ANIO")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setYearAmount(roomYear > 0.0f ? (availableRoomYear * 100 / roomYear) : 0.0);
				} else if (period.equals("ANIO_ANTERIOR")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setPreviousYearAmount(roomPreviousYear > 0.0f ? (availableRoomPreviousYear * 100 / roomPreviousYear) : 0.0);
				} else if (period.equals("MES")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setMonthAmount(roomMonth > 0.0f ? (availableRoomMonth * 100 / roomMonth) : 0.0);
				} else if (period.equals("MES_ANIO_ANTERIOR")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setPreviousMonthAmount(roomPreviousMonth > 0.0f ? (availableRoomPreviousMonth * 100 / roomPreviousMonth) : 0.0);
				} else if (period.equals("DIA")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setDayAmount(roomDay > 0.0f ? (availableRoomDay * 100 / roomDay) : 0.0);
				} else if (period.equals("DIA_ANIO_ANTERIOR")) {
					...
					roomOcupationMap.get("% Ocupaci�n").setPreviousDayAmount(roomPreviousDay > 0.0f ? (availableRoomPreviousDay * 100 / roomPreviousDay) : 0.0);
				}
			}
...
//				System.out.println(period + " - " + ocupation + " - " + days);
	...
	private Map<String, ReportObject> getProductionRatioMap() {
		...
		fillRoomPaxRatioMapObject("PAX", " PAX por Habitaci�n", paxMap, productionRatioMap);
		
		fillRoomPaxRatioMapObject("1.VENTAS", " VENTAS/Habitaci�n", pendingProductionMap, productionRatioMap);
...
//		exportData(exporter, columnMetadata, "RESUMEN", getSummaryMap(), false, true);
	...
	private ReportMetadata getContractColumnMetadata() throws ReportException {
		...
		metadata.getColumns().add(new ReportColumnMetadata("YEAR",Types.DOUBLE,"A�o",15));
		metadata.getColumns().add(new ReportColumnMetadata("PREVIOUS_YEAR",Types.DOUBLE,"A�o (" +previousYear+ ")",15));
		return metadata;
	}
