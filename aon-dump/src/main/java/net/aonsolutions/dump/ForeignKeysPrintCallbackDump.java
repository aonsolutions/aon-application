...
public class ForeignKeysPrintCallbackDump extends AbstractChaimCallbackDump {
	...
	public Field<Integer> onErrFk(DSLContext dslContext, Record r, ForeignKey<?, ?> fk, AonDump aondump, IdsMap idsMap,
		...
		try {
			fkField = dslContext.select(fk.getKey().getFields().get(0)).from(fk.getKey().getTable().getName())
	...
	public void accept(InsertSetMoreStep<?> inSet, Table<?> table, List<Table<?>> ciclica, Integer numRows) {
		...
		super.accept("LOCK TABLES " + table.getName() + " WRITE;");

		if (table.field("id") != null) {
			if (!table.getName().equals("domain"))
				super.accept("SET @" + table.getName().toUpperCase() + "=(SELECT MAX(id) FROM " + table.getName() + ");");
		}
