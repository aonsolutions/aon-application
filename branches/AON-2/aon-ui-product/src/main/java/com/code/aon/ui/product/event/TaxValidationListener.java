...
public class TaxValidationListener extends ControllerAdapter {
	...
	public Tax searchOldTax(ControllerEvent event, Tax tax) throws ControllerListenerException {
		try {
			...
			criteria.addEqualExpression(event.getController().getFieldName(IProductAlias.TAX_ID), tax.getId());
	...
	public void insertTaxDetail(Tax tax, Tax oldTax) throws ControllerListenerException {
		if (!tax.getStartDate().equals(oldTax.getStartDate())) {
			if (hasOverlap(tax)) {
				...
				throw new ControllerListenerException("Fecha Inicial incorrecta. Se solapa con algún otro periodo.");
			}
			...
			if (tax.getStartDate().after(oldTax.getStartDate())) {
				...
				try {
					...
					LinesController linesController = (LinesController)AonUtil.getController("taxDetail");
	...
	public boolean hasOverlap(Tax tax) throws ControllerListenerException {
		try {
			IManagerBean taxDetailBean = BeanManager.getManagerBean(TaxDetail.class);
			Criteria criteria = new Criteria();
			criteria.addEqualExpression(taxDetailBean.getFieldName(IProductAlias.TAX_DETAIL_TAX_ID), tax.getId());
			criteria.addGreaterThanOrEqualExpression(taxDetailBean.getFieldName(IProductAlias.TAX_DETAIL_END_DATE), tax.getStartDate());
