...
public class DatePickerCell extends AbstractEditableCell<Date, Date> {
	...
	private static DateTimeFormat formatDate = DateTimeFormat.getFormat("dd/MM/yyyy");

	public DatePickerCell() {
		this(formatDate, SimpleSafeHtmlRenderer.getInstance());
	}
	...
	public DatePickerCell(DateTimeFormat format, SafeHtmlRenderer<String> renderer) {
		...
		this.panel = createPopupPanle();
		
		panel.addCloseHandler(closeEvent -> {
			lastKey = null;
			lastValue = null;
			lastIndex = -1;
			lastColumn = -1;
			if (lastParent != null && !closeEvent.isAutoClosed())
				lastParent.focus();
	...
	private PopupPanel createPopupPanle() {
			...
			protected void onPreviewNativeEvent(NativePreviewEvent event) {
				if (Event.ONKEYUP == event.getTypeInt()) {
					if (event.getNativeEvent().getKeyCode() == ESCAPE) {
						...
						panel.hide();
					} else if (event.getNativeEvent().getKeyCode() == DELETE) {
						// Remember the values before hiding the popup.
						Element cellParent = lastParent;
						Date oldValue = lastValue;
						Object key = lastKey;
	...
	protected void onEnterKeyDown(Context context, Element parent, Date value, NativeEvent event, ValueUpdater<Date> valueUpdater) {
		...
		panel.setPopupPositionAndShow((offsetWidth, offsetHeight) -> panel.setPopupPosition(lastParent.getAbsoluteLeft() + offsetX, lastParent.getAbsoluteTop() + offsetY));
	}
