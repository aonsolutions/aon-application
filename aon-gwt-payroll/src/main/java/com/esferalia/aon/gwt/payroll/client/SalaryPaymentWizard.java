...
public abstract class SalaryPaymentWizard extends AonCustomDialog {
	...
	private class GtzdoWizardImplementation extends GtzdoWizard {
		...
		protected void onPaymentTypeChange(String paymentTypeValue) {
			if (!AonStringUtils.equalsIgnoreCase(paymentTypeValue, "MEJORA_IT")) {
				showFirstPage();
				// Fire PaymentType
				setSelectedValueLB(SalaryPaymentWizard.this.paymentType, paymentTypeValue);
				DomEvent.fireNativeEvent(Document.get().createChangeEvent(), SalaryPaymentWizard.this.paymentType);
			}
		}
	...
	interface AgreementPaymentDialogBinder extends UiBinder<Widget, SalaryPaymentWizard> {
	}
	...
	protected SalaryPaymentWizard(Set<Payment> aviablePayments, ContextProvider contextProvider) {
