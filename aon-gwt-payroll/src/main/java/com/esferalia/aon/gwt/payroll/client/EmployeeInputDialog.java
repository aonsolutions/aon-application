...
public abstract class EmployeeInputDialog extends CustomDialog {
	...
	public EmployeeInputDialog(String caption, Double valueIn, Date startDateIn, Date endDateIn, ArrayList<String> variables) {
		setCaption(caption);
		...
		variablesList.addItem("DIAS_VACACIONES_NO_DISFRUTADOS", "DIAS_VACACIONES_NO_DISFRUTADOS");
		...
		DateUtils.resetTime(startDateIn);
		DateUtils.resetTime(endDateIn);
		
		this.startDate.setValue(startDateIn);
		this.startDate.setEnabled(false);
		this.endDate.setValue(endDateIn);
			...
			public void onBlur(BlurEvent event) {
				try {
					...
					if(null != value && value != 0.0) {
						Date startDateTmp = startDate.getValue();
						Date newEndDate = DateUtils.copyDateOnly(startDateTmp);
						newEndDate = DateUtils.addDays2Date(newEndDate, value.intValue()-1);
		...
		valueTextBox.setValue(valueIn.toString());
