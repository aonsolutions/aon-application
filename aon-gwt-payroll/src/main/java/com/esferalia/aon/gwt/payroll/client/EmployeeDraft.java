...
//				Window.alert("postal_code :"+place.get("postal_code").getLongName()); //Codigo postal
		...
		this.contractType.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				modality.clear();
				modality.addItem("-");
				String contract_type = contractType.getSelectedItemText();
				Integer contractTypeId = EmployeeDraft.this.contract_type.getContractTypeId(contract_type);
...
// ----------------------------------------------------------------- UiHandlers ----------------------------------------------------------
	...
	void onContractTypeChangeValue(ChangeEvent event) {
		if (contractType.getSelectedIndex() == 0){
			...
			String contractTypeDescription = contractType.getSelectedItemText();
			Integer contractTypeId = this.contract_type.getContractTypeId(contractTypeDescription);
			employeeDraftObject.setContractType(contractTypeId.toString());
		}
	}
...
// -------------------------------------------------------------- METODOS DE LA CLASE ----------------------------------------------------
	...
	private void initializeListBox() {
		...
		for(ContractTypeRecord e : this.contract_type.getContractTypes().values()){
			this.contractType.addItem(e.getContractTypeDescription());
		}
	...
	private void showNationality(String document_type_str) {
		if(document_type_str == "CIF" || document_type_str == "Pasaporte")
