...
public abstract class PayrollEmailDialog extends AonCustomDialog {
	...
	public enum Type implements Serializable {
		ENTERPRISE, ENTERPRISE_MANAGEMENT, EMPLOYEE;
	}
	...
	public PayrollEmailDialog(Type type, HashMap<String, String> params) {
		...
		case ENTERPRISE:
			...
			passwordCheck.addValueChangeHandler(
					e -> impl.getPayrollEmailBody(type, params, passwordCheck.getValue(), new AsyncCallback<String>() {
		...
		case ENTERPRISE_MANAGEMENT:
			...
			impl.checkEnterprisesEmails(enterpriseIds, new AsyncCallback<String>() {
				...
				public void onSuccess(String message) {
					...
					passwordCheck.addValueChangeHandler(e -> impl.getPayrollEmailBody(type, params,
							passwordCheck.getValue(), new AsyncCallback<String>() {
		...
		impl.getDomainMailAccounts(new AsyncCallback<List<MailAccount>>() {
			...
			public void onSuccess(List<MailAccount> result) {
				...
				if (mailAccounts.isEmpty()) {
					AonDialog dialog = new AonDialog("ERROR",
							new HTML("No hay cuentas de correo desde las que enviar el email."));
					dialog.warning();
					return;
				}
	...
	private void initView(String emailBody) {
		...
		new Timer() {
			@Override
			public void run() {
				richTextArea.setHTML(body);
				setBodyHTML(richTextArea.getElement(), body);
			}
		}.schedule(1000); // Esperamos 1 segundo a que GWT termine
	}

	private native void setBodyHTML(Element iframeElement, String html) /*-{
		try {
			var doc = iframeElement.contentDocument
					|| iframeElement.contentWindow.document;
			// Solo escribimos en el body, sin tocar el document
			if (doc.body) {
				doc.body.innerHTML = html;
			}
		} catch (e) {
			$wnd.console.error('Error: ' + e.message);
		}
	}-*/;
