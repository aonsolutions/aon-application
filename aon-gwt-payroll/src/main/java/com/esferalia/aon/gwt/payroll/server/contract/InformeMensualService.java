...
import com.esferalia.aon.in.payroll.excel.contract.*;
...
public class InformeMensualService {
    ...
    // Método principal que genera el informe de centros de trabajo y contratos para el rango indicado
    public List<WorkplaceDaysEntryExcel> generarInformeMensualCTs(Date fechaInicio, Date fechaFin) {
        ...
        Result<Record> workplaces = dsl.select()
                .from(WORKPLACE)
                .join(CONTRACT).on(CONTRACT.WORKPLACE.eq(WORKPLACE.ID))
        ...
        for (Record workplace : workplaces) {
            ...
            informesCT.add(new WorkplaceDaysEntryExcel()
                    .setDescription(workplace.get(WORKPLACE.DESCRIPTION))
                    .setMeses(mesesCT)
                    .setContracts(informesContratos));
        }
        return informesCT;
    }

    // Obtiene los datos mensuales (CT) para un mes concreto, ajustando los límites al rango global
    private WorkplaceMonthlyDaysEntryExcel obtenerDatosMesCT(Integer workplaceId, Integer calendarId, LocalDate date, LocalDate globalStart, LocalDate globalEnd) {
        WorkplaceMonthlyDaysEntryExcel mesDTO = new WorkplaceMonthlyDaysEntryExcel().setMes(date.getMonthValue());
    ...
    // Cuenta días festivos en el intervalo usando la jerarquía de holidays del calendario
    private int contarDiasFestivos(Integer calendarId, LocalDate firstDay, LocalDate lastDay) {
        if (calendarId == null) return 0;
        Set<Integer> holidayIds = obtenerJerarquiaHolidays(calendarId);
        return dsl.selectCount()
                .from(HOLIDAY_DETAIL)
                .where(HOLIDAY_DETAIL.HOLIDAY.in(holidayIds))
                .and(HOLIDAY_DETAIL.DATE.between(Date.valueOf(firstDay), Date.valueOf(lastDay)))
                .fetchOneInto(Integer.class);
    }

    // Cuenta los días en el intervalo que se consideran fines de semana según la configuración del calendario
    ...
    private AnualDaysEntryExcel obtenerInformeMensualContrato(Integer contratoId, LocalDate globalStart, LocalDate globalEnd) {
        AnualDaysEntryExcel dto = new AnualDaysEntryExcel();

        // Obtener datos básicos del contrato
        Record registro = dsl.select(REGISTRY.NAME, REGISTRY.DOCUMENT, PERSON.SOCIAL_SECURITY_NUM, CONTRACT.START_DATE, CONTRACT.END_DATE)
                .from(CONTRACT)
                .join(PERSON).on(PERSON.REGISTRY.eq(CONTRACT.PERSON))
                .join(REGISTRY).on(REGISTRY.ID.eq(CONTRACT.PERSON))
                .where(CONTRACT.ID.eq(contratoId))
                .and(CONTRACT.DOMAIN.eq(domainId))
                .fetchOne();
        ...
        LocalDate contratoFin = registro.get(CONTRACT.END_DATE) != null ? registro.get(CONTRACT.END_DATE).toLocalDate() : LocalDate.of(9999, 12, 31);
        ...
        while (!iter.isAfter(globalEnd)) {
            ...
            if (monthStart.isBefore(globalStart)) monthStart = globalStart;
            if (monthEnd.isAfter(globalEnd)) monthEnd = globalEnd;
    ...
    private MonthlyDaysEntryExcel obtenerDatosMesContrato(Integer contratoId, int mes, LocalDate evalStart, LocalDate evalEnd) {
        ...
        int fullTimeWorkingDays = diasLaborables - diasVacaciones - diasIT;
    ...
    private int contarDiasFestivosContrato(Integer contratoId, LocalDate evalStart, LocalDate evalEnd) {
        Integer calendarHolidayId = dsl.select(CALENDAR.HOLIDAY)
                .from(CALENDAR)
                .join(PAYROLL_WORKPLACE).on(PAYROLL_WORKPLACE.CALENDAR.eq(CALENDAR.ID))
                .join(WORKPLACE).on(WORKPLACE.ID.eq(PAYROLL_WORKPLACE.WORKPLACE))
                .join(CONTRACT).on(CONTRACT.WORKPLACE.eq(WORKPLACE.ID))
                .where(CONTRACT.ID.eq(contratoId))
                .fetchOneInto(Integer.class);
        if (calendarHolidayId == null) return 0;
        Set<Integer> holidayIds = new HashSet<>();
        holidayIds.add(calendarHolidayId);
        while (calendarHolidayId != null) {
            calendarHolidayId = dsl.select(HOLIDAY.HOLIDAY_)
                    .from(HOLIDAY)
                    .where(HOLIDAY.ID.eq(calendarHolidayId))
                    .fetchOneInto(Integer.class);
    ...
    // Cuenta los fines de semana para un contrato. Se usa configuración específica si existe;
    ...
    private int contarDiasFinDeSemanaContrato(Integer contratoId, LocalDate evalStart, LocalDate evalEnd) {
        Result<ContractDataRecord> notWorkingDays = dsl.selectFrom(CONTRACT_DATA)
                ...
                .and(CONTRACT_DATA.NAME.in(
                        "LABORABLE_LUNES", "LABORABLE_MARTES", "LABORABLE_MIERCOLES",
        ...
        if (!notWorkingDays.isEmpty()) {
            ...
            for (LocalDate date = evalStart; !date.isAfter(evalEnd); date = date.plusDays(1)) {
                ...
                if (isNoLaborable) dias++;
            }
            return dias;
        } else {
            Integer calendarId = obtenerCalendarId(contratoId);
            Record7<Byte, Byte, Byte, Byte, Byte, Byte, Byte> calendario = dsl.select(
                    CALENDAR.MONDAY, CALENDAR.TUESDAY, CALENDAR.WEDNESDAY,
                    CALENDAR.THURSDAY, CALENDAR.FRIDAY, CALENDAR.SATURDAY, CALENDAR.SUNDAY)
                    .from(CALENDAR)
                    .where(CALENDAR.ID.eq(calendarId))
                    .and(CALENDAR.DOMAIN.eq(domainId))
                    .fetchOne();
            if (calendario == null) return 0;
            int dias = 0;
            for (LocalDate date = evalStart; !date.isAfter(evalEnd); date = date.plusDays(1)) {
    ...
    private int contarDiasTrabajadosPorMes(Integer contratoId, LocalDate evalStart, LocalDate evalEnd) {
        ...
        for (LocalDate date = evalStart; !date.isAfter(evalEnd); date = date.plusDays(1)) {
            final LocalDate currentDate = date; // variable final para usar en el lambda
    ...
    private ContractType getContractType(Integer contratoId) {
        Result<ContractDataRecord> contractTypeCodes = dsl.selectFrom(CONTRACT_DATA)
                .where(CONTRACT_DATA.DOMAIN.eq(domainId))
                .and(CONTRACT_DATA.CONTRACT.eq(contratoId))
                .and(CONTRACT_DATA.NAME.eq("TC2"))
                .orderBy(CONTRACT_DATA.ID.desc())
                .fetch();
        return contractTypeCodes.isEmpty() ? null : ContractType.fromStringCode(contractTypeCodes.get(0).getExpression());
    }

    // Devuelve true si la configuración indica que el contrato es de tiempo completo
    private boolean isFullTime(Integer contratoId) {
        Result<ContractDataRecord> fullTimeContractDatas = dsl.selectFrom(CONTRACT_DATA)
                .where(CONTRACT_DATA.CONTRACT.eq(contratoId))
                .and(CONTRACT_DATA.NAME.like("TIEMPO_COMPLETO"))
                .and(CONTRACT_DATA.DOMAIN.eq(domainId))
                .orderBy(CONTRACT_DATA.ID.desc())
                .fetch();
        return fullTimeContractDatas.isEmpty() || AonStringUtils.equalsIgnoreCase(fullTimeContractDatas.get(0).getExpression(), "true");
    }

    // Obtiene el calendarId para un contrato; si no se encuentra en el contrato, se busca en la relación de payroll workplace
    private Integer obtenerCalendarId(Integer contratoId) {
        Integer calendarId = dsl.select(CONTRACT.CALENDAR)
                .from(CONTRACT)
                .where(CONTRACT.ID.eq(contratoId))
                .and(CONTRACT.DOMAIN.eq(domainId))
                .fetchOneInto(Integer.class);
        if (calendarId == null) {
            calendarId = dsl.select(PAYROLL_WORKPLACE.CALENDAR)
    ...
    private Integer obtenerCalendarIdCT(Integer workplaceId) {
        return dsl.select(PAYROLL_WORKPLACE.CALENDAR)
                .from(PAYROLL_WORKPLACE)
    ...
    private Set<Integer> obtenerJerarquiaHolidays(Integer calendarId) {
        ...
        while (current != null) {
            ...
            current = dsl.select(HOLIDAY.HOLIDAY_)
                    .from(HOLIDAY)
                    .where(HOLIDAY.ID.eq(current))
                    .fetchOneInto(Integer.class);
        }
        return holidays;
    }

    // Obtiene el calendario (configuración de días laborables) a partir del calendarId
    private Record7<Byte, Byte, Byte, Byte, Byte, Byte, Byte> obtenerCalendario(Integer calendarId) {
        return dsl.select(CALENDAR.MONDAY, CALENDAR.TUESDAY, CALENDAR.WEDNESDAY,
                        CALENDAR.THURSDAY, CALENDAR.FRIDAY, CALENDAR.SATURDAY, CALENDAR.SUNDAY)
                .from(CALENDAR)
