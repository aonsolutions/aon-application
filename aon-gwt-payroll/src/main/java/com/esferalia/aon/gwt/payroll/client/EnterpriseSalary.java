	...
	private void addColumns(MultiSelectionModel<SalaryInfo> selectionModel) {
	    ...
	    Header<Boolean> selectAllHeader = new Header<Boolean>(selectAllHeaderCB)
	    {
	      ...
	      public Boolean getValue()
	      {
	    	...
	    	if(null != enterpriseSalaryObject.getEnterpriseSalaries())
	    		value = selectionModel.getSelectedSet().size() == enterpriseSalaryObject.getEnterpriseSalaries().size();
	    ...
	    salaryDataGrid.setColumnWidth(employeeNameColumn, 25, Unit.PCT);
//	    salaryDataGrid.setColumnWidth(employeeNameColumn, 350, Unit.PX);
	    ...
	    salaryDataGrid.setColumnWidth(workplaceNameColumn, 15, Unit.PCT);
	...
	public void setEnterpriseSalaryObject(EnterpriseSalaryObject enterpriseSalaryObject) {
		...
		this.enterpriseSalaryObject.getEnterpriseSalariesDB(
				s -> {
					...
					initListBox();
					initSuggestBox();
					this.allRB.setValue(true, false);
	...
	private void initSalariesTable() {		
	    ...
	    this.salaries = this.enterpriseSalaryObject.getEnterpriseSalaries();
	...
	void onBidoqPublichalary(ClickEvent event) {
		onPublish("bidoq");
	}
