...
public class StatFilter extends ScrollPanel implements HasValueChangeHandlers<StatParams> {
	...
	public void paintFilter(StatParams result) {
		...
		options.setUser( getUser() );
		InvoiceRegistryDocumentBox titular = new InvoiceRegistryDocumentBox(options);
		...
		InvoiceProductBox product = new InvoiceProductBox(getDomainName(),getDomain(),getUser() );
