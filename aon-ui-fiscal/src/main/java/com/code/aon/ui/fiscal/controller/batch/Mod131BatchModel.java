...
public class Mod131BatchModel extends AbstractFiscalBatchModel {
	...
	private static final String SELECT = "SELECT" 
			...
			+" WHERE fm.year = ?"
			+"  AND fm.period = ?"
			+"  AND fm.model = " + FiscalModelType.M131.getValue()
	...
	public byte[] getData(FiscalBatch fb) throws AonException {
		MOD131Writer writer = new MOD131Writer();
		int year = fb.getYear();
		if (year < 2010) {
		...
		try { 
			format = MOD131Format.getFormat(fb.getAdministration(), fb.getYear());
		...
		FileOutput fileOutput = writer.createMOD131(declarations, format);
