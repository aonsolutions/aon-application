...
public class Mod123BatchModel extends AbstractFiscalBatchModel {
	...
	private static final String SELECT = "SELECT" 
			...
			+" WHERE fm.year = ?"
			+"  AND fm.period = ?"
			+"  AND fm.model = " + FiscalModelType.M123.getValue()
	...
	public byte[] getData(FiscalBatch fb) throws AonException {
		MOD123Writer writer = new MOD123Writer();
		int year = fb.getYear();
		if (year < 2010) {
		...
		try { 
			format = MOD123Format.getFormat(fb.getAdministration(), fb.getYear());
		...
		FileOutput fileOutput = writer.createMOD123(declarations, format);
