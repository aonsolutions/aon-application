...
public class Mod130BatchModel extends AbstractFiscalBatchModel {
	...
	private static final String SELECT = "SELECT" 
			...
			+" WHERE fm.year = ?"
			+"  AND fm.period = ?"
			+"  AND fm.model = " + FiscalModelType.M130.getValue()
	...
	public byte[] getData(FiscalBatch fb) throws AonException {
		MOD130Writer writer = new MOD130Writer();
		int year = fb.getYear();
		if (year < 2010) {
		...
		try { 
			format = MOD130Format.getFormat(fb.getAdministration(), fb.getYear());
		...
		FileOutput fileOutput = writer.createMOD130(declarations, format);
