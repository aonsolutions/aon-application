...
public class Mod310BatchModel extends AbstractFiscalBatchModel {
	...
	private static final String SELECT = "SELECT" 
			...
			+" WHERE fm.year = ?"
			+"  AND fm.period = ?"
			+"  AND fm.model = " + FiscalModelType.M310.getValue()
	...
	public byte[] getData(FiscalBatch fb) throws AonException {
		MOD310Writer writer = new MOD310Writer();
		int year = fb.getYear();
		if (year < 2010) {
		...
		try { 
			format = MOD310Format.getFormat(fb.getAdministration(), fb.getYear());
		...
		FileOutput fileOutput = writer.createMOD310(declarations, format);
