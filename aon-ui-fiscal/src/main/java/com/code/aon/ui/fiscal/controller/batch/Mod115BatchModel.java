...
public class Mod115BatchModel extends AbstractFiscalBatchModel {
	...
	private static final String SELECT = "SELECT" 
			...
			+" WHERE fm.year = ?"
			+"  AND fm.period = ?"
			+"  AND fm.model = " + FiscalModelType.M115.getValue()
	...
	public byte[] getData(FiscalBatch fb) throws AonException {
		MOD115Writer writer = new MOD115Writer();
		int year = fb.getYear();
		if (year < 2010) {
		...
		try { 
			format = MOD115Format.getFormat(fb.getAdministration(), fb.getYear());
