...
export class AonUser extends AonElement {
	...
	sendEmail() {
		...
		d.addAcceptAction(() => {
			sendUserInfoEmail(this.user).then(() => {
				this.showToast({message: "Se ha enviado un mail a " + this.user.email + " con la contraseña nueva", type: CONSTANT.SUCCESS, delay: 3000});
			  }, err => {
				this.showError(err);
			  });
		});
