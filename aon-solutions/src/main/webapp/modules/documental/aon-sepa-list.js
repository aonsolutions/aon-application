...
import {getSepaDocuments, getDocuments, downloadDocuments, sendDocumentMail, updateFiles, deleteFile, getDomainUserRoles, downloadSepaDocument, downloadDocument} from '../../services/service.js';
  ...
  set filter(filter) {
	...
	async loadMore() {
		...
		if(aonDocumentalSepaTable && filter.page) {
			...
			if(sepaDocumentsFormat.length == 0)
				this.more = false;
			sepaDocumentsFormat.forEach((doc, i) => {
				aonDocumentalSepaTable.addRow(doc, () => this.aonDocument(doc, i), (e) => this.aonDocumentContextMenu(e, doc, i));
			});
		}
	}
	...
	async init() {
		...
		if(aonDocumentalSepaTable) {
			...
			console.log(sepaDocumentsFormat);
	...
	getAmount(sepaDoc) {
		...
		let total = sepaDoc.fbatch_details.reduce((t, detail) => t + detail.amount, 0);
		...
		return [
			{
				...
				color : "grey",
				title : "Ver nóminas",
				fn : () => console.log("Ver nóminas : " + sepaDoc.description)
			},
			{
				...
				title : "Descargar",
				fn : () => {
					console.log("Descargar fichero Sepa");
					this.downloadSepaFile(sepaDoc.rattach)
				}
			}
		];
	}
