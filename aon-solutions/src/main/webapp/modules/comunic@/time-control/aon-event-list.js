...
import { getStatus } from "../../../services/service.js";
...
export class AonEventList extends AonElement {
  ...
  async getTableDesk() {
    ...
    if (aonTable) {
      ...
      aonTable.addColumn("Estatus", "", "status", "15%");
  ...
  async getData() {
    ...
    try {
      ...
      datos.map(async ({ date, name, last_name1, status, location, time }) => {
        const letters_name = `${name.substr(0, 1)}${last_name1.substr(0, 1)}`;
        //status = out, in, pause
