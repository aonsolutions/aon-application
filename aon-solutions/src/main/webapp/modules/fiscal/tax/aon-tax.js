...
export class AonTax extends AonElement {
      ...
      if (errorJson) {
        if (errorJson.class_name == "AonApiAeatError") {
          ...
          if (this.isMobile()) {
            ...
            let text =
              '<span style="margin: 5px;color:red;font-weight: bold;font-size: medium;text-align: center; display: block">PRESENTACION DEL MODELO<br>Mensajes de Error devueltos por la Agencia Tributaria<br><hr></span>' +
            ...
            text = text + this.ERROR_TEMPLATE_AFTER + this.ERROR_TEMPLATE_END;
  ...
  search() {
    this._list = this.filterSearch(
      ["periodText", "statusText", "modelText", "newModel", "model"],
      this._list
    );
    this.getTable();
  }
