...
export const xmlToJson = (xml, tab) =>  {
    let X = {
       toObj: function(xml) {
          let o = {};
          if (xml.nodeType==1) {   // element node ..
             if (xml.attributes.length)   // element with attributes  ..
                for (const element of xml.attributes)
                   o["@"+element.nodeName] = (element.nodeValue||"").toString();
       ...
       innerXml: function(node) {
          ...
          if ("innerHTML" in node)
             ...
             let asXml = function(n) {
                ...
                if (n.nodeType == 1) {
                   s += "<" + n.nodeName;
                   for (const element of n.attributes)
                      s += " " + element.nodeName + "=\"" + (element.nodeValue||"").toString() + "\"";
                   if (n.firstChild) {
                      s += ">";
