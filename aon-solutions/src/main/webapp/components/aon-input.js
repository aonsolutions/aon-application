...
export class AonInput extends AonElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if ("value" === name) {
      ...
      if (this.isTypeList()) {
        let options = this.hasAttribute("options")
    ...
    if ("disabled" === name) {
      ...
      if (el) {
        ...
        } else el.setAttribute("disabled", this.isDisabled());
      }
    }
  ...
  build() {
    ...
    input.type =
      this.getAttribute("type") && !this.isTypeList()
    ...
    if (this.isTypeList()) {
      ...
      iconLabel.innerHTML = `<aon-icon-button id="${
        this.getAttribute("id") + "IconLabel"
      }" icon="arrow_drop_down" noHover="true"></aon-icon-button>`;
    ...
    if (this.isTypeAddress()) {
      ...
      input.setAttribute(
        "value",
        `${value.address}, ${value.zip} ${value.city}, ${value.province}, ${value.country}`
      );
  ...
  buildAddress(value) {
    ...
    let html = `
      <aon-input class='aon-width-100' id='${
        this.getAttribute("id") + "Address"
      }' description='Dirección'></aon-input>
      <aon-input class='aonWidth25' id='${
        this.getAttribute("id") + "Zip"
      }' description='C.P.'></aon-input>
      <aon-input class='aonWidth25' id='${
        this.getAttribute("id") + "City"
      }' description='Ciudad'></aon-input>
      <aon-input class='aonWidth25' id='${
        this.getAttribute("id") + "Province"
      }' description='Provincia'></aon-input>
      <aon-input type="list" class='aonWidth25' id='${
        this.getAttribute("id") + "Country"
      }' description='País' ></aon-input>
    `;
  ...
  isVisible() {
    return (
      ...
      (this.hasAttribute("visible") && "false" !== this.getAttribute("visible"))
    );
  }
  ...
  isDisabled() {
    return (
      this.hasAttribute("disabled") && "false" !== this.getAttribute("disabled")
    );
  }
  ...
  isFilled() {
    return (
      this.hasAttribute("filled") && "false" !== this.getAttribute("filled")
    );
  }
