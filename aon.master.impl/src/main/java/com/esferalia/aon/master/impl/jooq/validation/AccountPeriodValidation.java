...
public class AccountPeriodValidation {
	...
	public static final Logger LOGGER = LoggerFactory
			.getLogger(AccountPeriodValidation.class);
	
	/**
	 * La fecha inicial no puede estar vacia
	 */
	public static BiConsumer<AccountPeriod,DAOContext> EMPTY_INITIATION_DATE_VALIDATION = (ap,ctx) -> {
		if (ap.getInitiationDate() == null) 
			throw new AonDAOException(AonDAOError.ACCOUNT_PERIOD_EMPTY_INITIATION_DATE);
	};
	
	/**
	 * La fecha final debe ser posterior a la fecha inicial.
	 */
	public static BiConsumer<AccountPeriod,DAOContext> EMPTY_DEADLINE = (ap,ctx) -> {
		if (ap.getDeadline() == null)
			throw new AonDAOException(AonDAOError.ACCOUNT_PERIOD_EMPTY_DEADLINE);
	};
	
	/**
	 * La fecha final debe ser posterior a la fecha inicial.
	 */
	public static BiConsumer<AccountPeriod,DAOContext> WRONG_RANGE = (ap,ctx) -> {
		if (ap.getInitiationDate().after(ap.getDeadline()))
			throw new AonDAOException(AonDAOError.ACCOUNT_PERIOD_WRONG_RANGE);
	};
	
	/**
	 * El status del periodo no puede estar vacio.
	 */
	public static BiConsumer<AccountPeriod,DAOContext> EMPTY_STATUS = (ap,ctx) -> {
	 ...
	 */
	public static BiConsumer<AccountPeriod,DAOContext> OVERLAP = (ap,ctx) -> {
		...
		for (Record3<String, java.sql.Date, java.sql.Date> record : result) {
			...
			if (ap.getInitiationDate().compareTo(pFrom) >= 0 
				...
				throw new AonDAOException(
						AonDAOError.ACCOUNT_PERIOD_START_OVERLAP,
						record.getValue(ACCOUNT_PERIOD.NAME));
			}
			if (ap.getDeadline().compareTo(pFrom) >= 0 
				&& ap.getDeadline().compareTo(pTo) <= 0) {
				throw new AonDAOException(
						AonDAOError.ACCOUNT_PERIOD_END_OVERLAP,
						record.getValue(ACCOUNT_PERIOD.NAME));
			}
		}
	};

	public static void validatePeriod(DAOContext ctx, AccountPeriod ap)
