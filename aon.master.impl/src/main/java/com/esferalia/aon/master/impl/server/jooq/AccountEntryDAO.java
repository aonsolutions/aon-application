...
public class AccountEntryDAO {
	...
	public static Seq<AccountEntry> fetch(DAOContext ctx
				, Condition condition
				, int offset
				, int numberOfRows) {
	...
	public static String fetchCSV(DAOContext ctx
			, Condition condition
			, int offset
			, int numberOfRows) {
	...
	public static AccountEntry fetchOne(DAOContext ctx, Condition condition) {
		return populateRecord(ctx.getDslContext().fetchOne(ACCOUNT_ENTRY, condition));
	}

	public static void insert(DAOContext ctx, AccountEntry ae) {
	...
	private static AccountEntry populateRecord(AccountEntryRecord record, AccountEntry ae) {
		...
		ae.setEntryType(AonEnumUtils.getAccountEntryType(record.getEntryType()));
		ae.setJournal(record.getJournal());
		ae.setSecurityLevel(AonEnumUtils.getSecurityLevel(record.getSecurityLevel()));
		ae.setComments(record.getComments());
		return ae;
	}

	public static void update(DAOContext ctx, AccountEntry ae) {
		ctx.getDslContext().transaction( configuration -> {
			AccountEntryValidation.validateEntry(ctx, ae);
			ctx.getDslContext()
	...
	public static void delete(DAOContext ctx, AccountEntry accountEntry) {
		ctx.getDslContext().transaction( configuration -> {
	...
	public static boolean existsAnyEntry(DAOContext ctx, Integer period, AccountEntryType accountEntryType) {
	...
	private static synchronized void increaseJournal(DAOContext ctx,AccountEntry accountEntry) {
	...
	private static void afterRemove(DAOContext ctx,AccountEntry accountEntry) {
		/*
