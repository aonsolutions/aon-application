 ...
 */
public class ProjectTrackingController extends BasicController {
	...
	public void onSelectTracking( ActionEvent event ) throws ManagerBeanException {	
		...
		ct.setBackActionListener(PROJECT_TRACKING_CONTROLLER_NAME + ".onBackToProject");
		ct.select(event, to);
	}
	...
	public void onResetTracking( ActionEvent event ) throws ManagerBeanException {	
		...
		IController projectController = FormUtil.getController(PROJECT_CONTROLLER_NAME);
		Project project = (Project) projectController.getTo();
		CommercialTracking ct = (CommercialTracking) ctBean.getTo();
		ct.setProject(project);
		ctBean.setBackAction(NAVIGATION_PROJECT_FORM);
		ctBean.setBackActionListener(PROJECT_TRACKING_CONTROLLER_NAME + ".onBackToProject");
	}
