	...
	public LinkedList<RegistryImportClass> importationX(Domain domain, String login, byte[] data){
 	...
 	private void check(Domain domain , String login, String title, Cell cell, AonConfiguration aonCtx) {
		...
		if("DOMICILIO".equalsIgnoreCase(title)
				...
				|| "DIRECCI�N".equalsIgnoreCase(title)) {
		...
		if("C.P.".equalsIgnoreCase(title)
				...
				|| "C�DIGO POSTAL".equalsIgnoreCase(title)) {
		...
		if("Poblaci�n".equalsIgnoreCase(title) || "POBLACION".equalsIgnoreCase(title)
		...
		if("Pa�s".equalsIgnoreCase(title) || "PAIS".equalsIgnoreCase(title)) {
	...
	public static Error insertRegistries(Domain domain, User user,LinkedList<RegistryImportClass> rvs) {
		...
		for (RegistryImportClass r : rvs) {
			try {
				...
				verror.add("L�nea " + r.getLine() + ": " + e.getMessage());
			}
		}
