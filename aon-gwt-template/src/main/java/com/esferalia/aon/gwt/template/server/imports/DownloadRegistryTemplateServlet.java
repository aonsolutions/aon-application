...
import com.esferalia.aon.gwt.template.jooq.DBRegistry;
import com.esferalia.aon.gwt.template.server.exports.RegistryExcelExport;
...
@WebServlet(name = "DownloadRegistryExcelTemplate", urlPatterns = { "/ms/api/downloadRegistryExcel"
														,"/aon_gwt_template/downloadRegistryExcel/*"
														,"/aon_gwt_aio/downloadRegistryExcel/*"})
public class DownloadRegistryTemplateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) {        
		...
		try {
			...
			resp.addHeader("Content-Disposition","attachment; filename=\"" + type + ".xls" +"\"");
			if("customer".equalsIgnoreCase(type)) {
				List<CustomerFull> customers = DBRegistry.getCustomers(domain, user);
				RegistryExcelExport.getInstance().createCustomer(resp.getOutputStream(), customers);				
			} else if("supplier".equalsIgnoreCase(type)){
				List<SupplierFull> suppliers = DBRegistry.getSuppliers(domain, user);
				RegistryExcelExport.getInstance().createSupplier(resp.getOutputStream(), suppliers);
			} else if("creditor".equalsIgnoreCase(type)) {
				List<CreditorFull> creditors = DBRegistry.getCreditors(domain, user);
				RegistryExcelExport.getInstance().createCreditor(resp.getOutputStream(), creditors);
			}
