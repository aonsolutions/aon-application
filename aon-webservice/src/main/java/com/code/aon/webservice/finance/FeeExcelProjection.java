...
@WebServlet(name = "DownloadFeeProjectionExcel", urlPatterns = {"/aon_gwt_aio/download_fee_projection_excel/*",
																"/download_fee_projection_excel/*"})
public class FeeExcelProjection extends HttpServlet{
	...
	private Integer printTitle(HSSFWorkbook libro, HSSFSheet hoja, Date from){
    	...
    	for(Integer i = 0; i < 12; i++){
    		...
    		Integer month = AonDateUtils.getMonth(date);
    		Integer year = AonDateUtils.getYear(date);
    		createCell(row, titleStyle, AonMonth.values()[month].getName() + " " + year, i+2);
    	}
