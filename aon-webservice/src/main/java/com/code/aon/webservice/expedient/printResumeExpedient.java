...
public class printResumeExpedient extends HttpServlet {
	...
	public byte[] createExcel(Stream<Expedient> exStream, Boolean onlyOne) {
		...
		try {
			...
			boldCell(libro, fila, style, column + 4, "Concepto");
			boldCell(libro, fila, style, column + 5, "Base Imponible");
			...
			exStream.forEach(r -> {
				...
				cell(libro, row, style3, column + 4, r.getConcept());
				cell(libro, row, style3, column + 5, r.getBase());
			});
