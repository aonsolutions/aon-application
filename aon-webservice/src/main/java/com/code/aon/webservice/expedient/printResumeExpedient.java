...
public class printResumeExpedient extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Stream<Expedient> exStream = AON.getResumeExpedientStream(domainName, domainId, login, f -> projectFilter(domainId, filter, f));
	...
	public byte[] createExcel(Stream<Expedient> exStream, Boolean onlyOne) {
		...
		try {
			...
			boldCell(libro, fila, style, column + 3, "Año");
			boldCell(libro, fila, style, column + 4, "Tipo");
			boldCell(libro, fila, style, column + 5, "Concepto");
			...
			exStream.forEach(r -> {
				...
				cell(libro, row, style3, column + 3, r.getYear().toString());
				cell(libro, row, style3, column + 4, r.getType());
				cell(libro, row, style3, column + 5, r.getConcept());
