...
import com.esferalia.aon.occam.mod200.api.model.mod200_2021.Mod2002021Character;
...
public class Mod2002021DAO  {
	...
	private static enum Mod2002021RegistryType {
		...
		public static void populate(Mod2002021 mod,FsModel200RegistryRecord reg) {
			Mod2002021RegistryType type = Mod2002021RegistryType.safeValueOf(reg.getType());
			if (type != null) {
				type._populate(mod, reg);
			}
		}
		
		private static Mod2002021RegistryType safeValueOf( Byte i ) {
		...
		private static Mod2002021RegistryType safeValueOf( Integer i ) {
			...
			return Mod2002021RegistryType.values()[i];
		}
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				// Comprobar si la clave está en Mod2002021Key
				return (Mod2002021Key.valueOf((String) key) != null);	
			} catch (IllegalArgumentException e) {
				try {
					// Si no lo está, comprobar si está en Mod2002021KeyDC (Detalle correcciones)
					return (Mod2002021KeyDC.valueOf((String) key) != null);	
	...
	private static void insertRegistry(AONContext ctx,Mod2002021 mod200) {
		...
		if (mod200.getAdministrators() != null) {
			for ( Mod200CompanyAdministrator ca : mod200.getAdministrators() ) {
				...
				detail.setType( Mod2002021RegistryType.ADMINISTRATOR.byteValue());
		...
		if (mod200.getParticipationsOut() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsOut() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.PARTICPATION_OUT.byteValue());
		...
		if (mod200.getParticipationsIn() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsIn() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.PARTICPATION_IN.byteValue());
				detail.setDocument(AonStringUtils.substring(cp.getDocument(),0,9));
				detail.setName(AonStringUtils.substring(cp.getName(),0,45));
				detail.setProvince( (byte) cp.getProvince() );
				detail.setCountry( cp.getCountry() );
		...
		if (mod200.getMinorEntities() != null) {
			for ( MinorEntity me : mod200.getMinorEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.MINOR_ENTITIES.byteValue());
		...
		if (mod200.getRepresentatives() != null) {
			for ( LegalRepresentative lr : mod200.getRepresentatives() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.REPRESENTATIVE.byteValue());
		...
		if (mod200.getUteParticipations() != null) {
			for ( UteParticipation ute : mod200.getUteParticipations() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.UTE_PARTICIPATION.byteValue());
		...
		if (mod200.getUteBases() != null) {
			for ( UteBase ute : mod200.getUteBases() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.UTE_BASE.byteValue());
				detail.setNominalValue(ute.getBase());
				detail.setPercent(ute.getPercent());
				list.add(detail);
			}
		...
		if (mod200.getGroupEntities() != null) {
			for ( GroupEntitie ge : mod200.getGroupEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002021RegistryType.GROUP_ENTITIES.byteValue());
		...
		if (mod200.getEstablishments() != null) {
			for ( String es : mod200.getEstablishments() ) {
				if (AonStringUtils.isNotBlank(es)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002021RegistryType.ESTABLISHMENTS.byteValue());
	...
	private static void insertDetail(AONContext ctx, Mod2002021 mod200) {
	...
	public static Mod2002021 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
	...
	public static Mod2002021 createNewMod200(AONContext ctx, int year) {
		try {
			ctx.log().info("------ [INI] createNewMod200");
			Mod2002021 mod200 = new Mod2002021();
	...
	public static Mod2002021 getByYear(AONContext ctx, int year) {
		return getByYear(ctx, year, true);
	}
	
	public static Mod2002021 getByYear(AONContext ctx, int year, boolean initialize) {
	...
	private static Mod2002021 getMod200(FsModel200Record record) {
		
		Mod2002021 mod200 = new Mod2002021();
		mod200 = new Mod2002021();
	...
	private static Mod2002021 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002021 mod200 = null;
		if (record != null) {
			mod200 = getMod200(record);
			fillDetail(mod200,ctx);
			final Mod2002021 mod = mod200;
			ctx.getDslContext() 
				.selectFrom(FS_MODEL200_REGISTRY)
				.where(	FS_MODEL200_REGISTRY.FS_MODEL200.equal(mod200.getId()))
				.fetch()
				.stream()
				.forEach(reg -> Mod2002021RegistryType.populate(mod, reg));
		}
		return mod200;
	}
	
	private static void fillDetail(Mod2002021 mod200,AONContext ctx) {
		...
		for (FsModel200DetailRecord det : result) {
			try {
				// Comprobamos si la clave es de Mod2002021Key
				Mod2002021Key.valueOf(det.getKey());
				map.put(det.getKey(),det.getValue());
			} catch (IllegalArgumentException e) {
				try {
					// Comprobamos si la clave es de Mod2002021KeyDC (Detalle correcciones)
					Mod2002021KeyDC.valueOf(det.getKey());
					map.put(det.getKey(),det.getValue());
				} catch (IllegalArgumentException e2) {
		...
		// Cargamos las claves de Mod2002021Key
		...
		// Cargamos las claves de Mod2002021KeyDC (Detalle correcciones)
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002021Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002021Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002021Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		
		EcpnType et = EcpnType.NO_CONSTA;
		if (map.containsKey( Mod2002021Key.C0075.toString() )) {
			et = EcpnType.NORMAL;
		} else if (map.containsKey( Mod2002021Key.C0076.toString() )) {
			et = EcpnType.ABREVIADO;
		} else if (map.containsKey( Mod2002021Key.C0077.toString() )) {
			et = EcpnType.PYMES;
		}
		mod200.setEcpnType(et);
		
		bt = null;
		if (map.containsKey( Mod2002021Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002021Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002021Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setPygType( bt );
	}
	
	public static Mod2002021 initializeNewMod200(AONContext ctx, Mod2002021 mod200) {
		
		Mod2002020 old = Mod2002020DAO.getByYear(ctx, 2020, false);
		if (old != null && old.getId() != null) { 
			ctx.log().info("------ [START] INITIALIZE NEW MOD 200 FROM MOD 200 2020");
			mod200.setEnterprise(old.getEnterprise());
			Mod2002021Import2020.import2020(mod200,old);
	...
	public static Mod2002021 initializeMod200(AONContext ctx, Mod2002021 mod200) {
		try {
			...
			if (mod200.isNotChecked(Mod2002021Key.C0081)) {			
			...
			DoubleVariableEx dv = null;
			for (Mod2002021Key k : INITIALIZE_EXPRESSION_MAP.keySet()) {
	...
	private static void fillMod202(AONContext ctx,Mod2002021 mod200) {
	...
	public static Mod2002021 calculate(Mod2002021 mod200) {
...
//			ctx.setExpressionMap(Mod2002021Compute.COMPUTE_EXPRESSION_MAP);			
			...
			for (Mod2002021Key k : Mod2002021Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				...
				Object ret = ctx.evaluateExpression(k,Mod2002021Compute.COMPUTE_EXPRESSION_MAP.get(k));
				if (ret instanceof Double) {
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002021 mod200) throws AonCoreException {
	...
	private static void addCharacters(Mod2002021MVELContext ctx, Mod2002021 mod200) {
		// Caracteres de la declaración, solo están en Mod2002021Key
		for (Mod2002021Key key : Mod2002021Character.CHARACTERS_KEYS) {
			DoubleVariableEx dv = mod200.getKeysMap().get(key);
			ctx.put(key.toString(), (dv != null && dv.getBooleanValue() )); 
		}		
	}
	
	private static void initializeActiveMap(Mod2002021 mod200) {
		// Actualmente la visualizacion de determinadas casillas solo afecta a Mod2002021Key
		Mod2002021MVELContext ctx = new Mod2002021MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002021Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		addBalanceCharacters(ctx,mod200);
		for (Mod2002021Key key : Mod2002021Key.values() ) {
			if (Mod2002021Activation.ACTIVE_EXPRESSION_MAP.containsKey(key) ) {
	...
	private static void addBalanceCharacters(Mod2002021MVELContext ctx, Mod2002021 mod200) {
		ctx.put(Mod2002021Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002021Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002021Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);
		
		ctx.put(Mod2002021Key.C0075.toString(), mod200.getEcpnType() == EcpnType.NORMAL);
		ctx.put(Mod2002021Key.C0076.toString(), mod200.getEcpnType() == EcpnType.ABREVIADO);
		ctx.put(Mod2002021Key.C0077.toString(), mod200.getEcpnType() == EcpnType.PYMES);

		ctx.put(Mod2002021Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002021Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
