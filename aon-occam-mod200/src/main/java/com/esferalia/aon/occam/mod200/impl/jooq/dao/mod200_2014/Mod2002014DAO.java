...
import static com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2014.Mod2002014Initialization.INITIALIZE_EXPRESSION_MAP;
import static com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2014.Mod2002014Validation.VALIDATION_EXPRESSION_LIST;
...
import com.esferalia.aon.occam.mod200.api.model.mod200_2014.Mod2002014Character;
...
import com.esferalia.aon.occam.mod200.api.model.mod200_2014.Mod2002014.BalanceType;
import com.esferalia.aon.occam.mod200.impl.jooq.dao.Mod200DAO;
import com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2013.Mod2002013DAO;
import com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2013.jaxb.MOD2002013;
...
public class Mod2002014DAO  {
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				return (Mod2002014Key.valueOf((String) key) != null);	
	...
	private static Mod2002014 insert(AONContext ctx, Mod2002014 mod200)  {
	...
	private static void insertAdministrator(AONContext ctx,Mod2002014 mod200) {
	...
	private static void insertDetail(AONContext ctx, Mod2002014 mod200) {
		List<FsModel200DetailRecord> list = new LinkedList<FsModel200DetailRecord>();
		FsModel200DetailRecord detail = null;
		for (Mod2002014Key k : Mod2002014Key.values()) {
			DoubleVariable2014 dv = null;	
	...
	private static Mod2002014 update(AONContext ctx, Mod2002014 mod200)  {
	...
	public static Mod2002014 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
	...
	public static Mod2002014 getByYear(AONContext ctx, int year) {
		Result<FsModel200Record> result = ctx.getDslContext()
				...
				.orderBy(FS_MODEL200.COMPLEMENTARY.desc())
				.fetch();
		FsModel200Record record = null;
		Mod2002014 mod200 = null; 
	...
	private static Mod2002014 getMod200(FsModel200Record record) {
	...
	private static Mod2002014 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002014 mod200 = null;
		if (record != null) {
	...
	private static void fillDetail(Mod2002014 mod200,AONContext ctx) {
		...
		DoubleVariable2014 v = null;
		for (Mod2002014Key key : Mod2002014Key.values() ) {
			v = new DoubleVariable2014( key );
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002014Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002014Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002014Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		if (map.containsKey( Mod2002014Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002014Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002014Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setPygType( bt );
	}
	
	private static void fillRegistryLists(Mod2002014 mod200, AONContext ctx) {
		...
		for (FsModel200RegistryRecord reg : res) {
			if (reg.getType() == 0) {
				...
				cp.setValueCorrection(reg.getAValue());
				cp.setLossReversion(reg.getBValue());
				cp.setCorrectionEffect(reg.getCValue());
				cp.setCorrectionsBalance(reg.getDValue());
	...
	public static Mod2002014 initializeMod200(AONContext ctx, Mod2002014 mod200) {
		...
		for (String stringKey : INITIALIZE_EXPRESSION_MAP.keySet()) {
			Mod2002014Key k = Mod2002014Key.valueOf(stringKey.toString());
	...
	public static Mod2002014 calculate(Mod2002014 mod200) {
		return calculate(mod200,true);
	}
	
	public static Mod2002014 calculate(Mod2002014 mod200, boolean addToDraft) {
		try {
			Mod2002014MVELContext ctx = new Mod2002014MVELContext( mod200, ACCEPTER );
			ctx.setExpressionMap(Mod2002014Compute.COMPUTE_EXPRESSION_MAP);
			for (DoubleVariable2014 dv : mod200.getKeysMap().values()) {
				ctx.put(dv.getKey().toString(), dv.getValue());
			}
			addCharacters(ctx,mod200);
			addBalanceCharacters(ctx,mod200);
			
			DoubleVariable2014 d = null;
			...
			DoubleVariable2014 v = null;
			for (String stringKey : Mod2002014Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				Mod2002014Key k = Mod2002014Key.valueOf(stringKey.toString());
				String expression = Mod2002014Compute.COMPUTE_EXPRESSION_MAP.get(stringKey);
				DoubleVariable2014 existingVariable = mod200.getVariable(k);
				if ( existingVariable == null ) {
					existingVariable = new DoubleVariable2014( k );
			...
			v = mod200.getVariable(Mod2002014Key.BN621);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002014 mod200) throws AonCoreException {
...
//		}
	...
	private static void addCharacters(Mod2002014MVELContext ctx, Mod2002014 mod200) {
		for (Mod2002014Key key : Mod2002014Character.CHARACTERS_KEYS) {
	...
	private static void initializeActiveMap(Mod2002014 mod200) {
		Mod2002014MVELContext ctx = new Mod2002014MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002014Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		for (Mod2002014Key key : Mod2002014Key.values() ) {
			if (Mod2002014Activation.ACTIVE_EXPRESSION_MAP.containsKey(key.toString()) ) {
	...
	private static void addBalanceCharacters(Mod2002014MVELContext ctx, Mod2002014 mod200) {
		ctx.put(Mod2002014Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002014Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002014Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);

		ctx.put(Mod2002014Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002014Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002014Key.C0055.toString(), mod200.getPygType() == BalanceType.PYMES);
	}
	
	public static Mod2002014 validate(Mod2002014 mod200) {
		List<ValidationMessage2014> list = new LinkedList<ValidationMessage2014>();
		...
		for (ValidationMessage2014 validation : VALIDATION_EXPRESSION_LIST) {
	...
	private static void validateComplementary(List<ValidationMessage2014> list, Mod2002014 mod200) {
		if (mod200.isComplementary() ) {
			if (AonStringUtils.isEmpty(mod200.getComplementaryReceipt() )) {
				list.add(new ValidationMessage2014(PAGE00,"Si marca Decl. Complementaria, debe indicar un n. de justificante anterior."));
			} else {
				if (mod200.getComplementaryReceipt().length() != 13) {
					list.add(new ValidationMessage2014(PAGE00,"El n. de justificante anterior debe tener 13 caracteres."));
				}
				if (!mod200.getComplementaryReceipt().startsWith("200") && !mod200.getComplementaryReceipt().startsWith("206")) {
					list.add(new ValidationMessage2014(PAGE00,"El n. de justificante anterior debe empezar por 200 o 206."));
				}
			}
			
		} else if (!mod200.isComplementary() && !AonStringUtils.isEmpty(mod200.getComplementaryReceipt())) {
			list.add(new ValidationMessage2014(PAGE00,"Si no marca Decl. Complementaria, no debe indicar un n. de justificante anterior."));
		}
	}

	private static void validateDocument(List<ValidationMessage2014> list, Mod2002014 mod200) {
		if (!AonDocumentUtil.isValid(mod200.getEnterpriseDocument())) {
			list.add(new ValidationMessage2014(PAGE00,"NIF de la declaraci\u00F3n incorrecto."));
		}
	...
	private static void validateAdministrators(List<ValidationMessage2014> list,Mod2002014 mod200) {
		if (mod200.getAdministrators() == null || mod200.getAdministrators().size() == 0 ) {
			list.add(new ValidationMessage2014(PAGE01,"Debe rellenar al menos un administrador."));
		} else {
			for (int i = 0; i < mod200.getAdministrators().size(); i++ ) {
				...
				if (!AonDocumentUtil.isValid(ca.getDocument())) {
					list.add(new ValidationMessage2014(PAGE01,"NIF del administrador nº "+(i+1) +" incorrecto ["+ca.getDocument()+"]"));		
				}
				if (AonStringUtils.isEmpty(ca.getName())) {
					list.add(new ValidationMessage2014(PAGE01,"Falta nombre del administrador nº "+(i+1) +". ["+ca.getDocument()+"]"));
				}
			}
		}
	}
	
	private static void validateSecretary(List<ValidationMessage2014> list, Mod2002014 mod200) {
		if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			if (mod200.getSecretary() == null) {
				list.add(new ValidationMessage2014(PAGE01,"Para personas jur\u00EDdicas, debe rellenar los datos del secretario"));
			} else {
				if (!AonDocumentUtil.isValid(mod200.getSecretary().getDocument())) {
					list.add(new ValidationMessage2014(PAGE01,"NIF del secretario incorrecto."));
				}
				if ( AonStringUtils.isEmpty(mod200.getSecretary().getName())) {
					list.add(new ValidationMessage2014(PAGE01,"Falta nombre del secretario."));
				} else if (mod200.getSecretary().getName().length() > 25) {
					list.add(new ValidationMessage2014(PAGE01,"Longitud excedida en el nombre del secretario. Debe limitarse a 25 caracteres."));	
				}
				if (mod200.getSecretary().getIrnr() == null && (mod200.isChecked(Mod2002014Key.C0021) || mod200.isChecked(Mod2002014Key.C0046)) ) {
					list.add(new ValidationMessage2014(PAGE01,"Falta fecha IRNR."));
				}
			}
		}
	}

	private static void validateRepresentatives(List<ValidationMessage2014> list,Mod2002014 mod200) {
		if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			if (mod200.getRepresentatives() == null || mod200.getRepresentatives().size() == 0 ) {
				list.add(new ValidationMessage2014(PAGE01,"Para personas jur\u00EDdicas, debe rellenar al menos un representante."));
			} else {
				for (int i = 0; i < mod200.getRepresentatives().size(); i++ ) {
					...
					if (!AonDocumentUtil.isValid(lr.getDocument())) {
						list.add(new ValidationMessage2014(PAGE01,"NIF del representante legal nº "+(i+1) +" incorrecto ["+lr.getDocument()+"]"));		
					}
					if (AonStringUtils.isEmpty(lr.getName())) {
						list.add(new ValidationMessage2014(PAGE01,"Falta nombre del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					}
					if (AonStringUtils.isEmpty(lr.getNotary())) {
						list.add(new ValidationMessage2014(PAGE01,"Falta el dato de la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					} else if (lr.getNotary().length() > 20) {
						list.add(new ValidationMessage2014(PAGE01,"Longitud excedida en la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]. Debe limitarse a 20 caracteres."));	
					}
					if (lr.getNotaryDate() == null) {
						list.add(new ValidationMessage2014(PAGE01,"Falta el dato fecha de la notar\u00EDa del representante legal nº "+(i+1) +". ["+lr.getDocument()+"]"));
					}
				}
			}
		}
			...
			if (participations == null || participations.size() == 0) {
				...
				for (int i = 0; i < participations.size(); i++ ) {
					...
					if (!AonDocumentUtil.isValid(cp.getDocument())) {
						list.add(new ValidationMessage2014(PAGE02,"NIF de la participaci\u00F3n en la declarante nº "+(i+1) +" incorrecto ["+cp.getDocument()+"]"));		
					}
					if (AonStringUtils.isEmpty(cp.getName())) {
						list.add(new ValidationMessage2014(PAGE02,"Falta nombre de la participaci\u00F3n en la declarante nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
					if (cp.getPercent() < 0 || cp.getPercent() > 100) {
						list.add(new ValidationMessage2014(PAGE02,"Porcentaje no correcto en la participaci\u00F3n en la declarante nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
				}
			}
		}
	...
	private static void validateParticipationsOut(List<ValidationMessage2014> list,Mod2002014 mod200) {
		 if (AonDocumentUtil.isEntity(mod200.getEnterpriseDocument())) {
			...
			if (participations == null || participations.size() == 0) {
				...
				for (int i = 0; i < participations.size(); i++ ) {
					Mod200CompanyParticipation cp = participations.get(i); 
					if (!AonDocumentUtil.isValid(cp.getDocument())) {
						list.add(new ValidationMessage2014(PAGE02,"NIF de la participaci\u00F3n de la declarante en otras nº "+(i+1) +" incorrecto ["+cp.getDocument()+"]"));		
					}
					if (AonStringUtils.isEmpty(cp.getName())) {
						list.add(new ValidationMessage2014(PAGE02,"Falta nombre de la participaci\u00F3n de la declarante en otras nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
					if (cp.getPercent() < 0 || cp.getPercent() > 100) {
						list.add(new ValidationMessage2014(PAGE02,"Porcentaje no correcto en la participaci\u00F3n de la declarante en otras nº "+(i+1) +". ["+cp.getDocument()+"]"));
					}
				}
			}
		}
