...
import static com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2023.Mod2002023Initialization.INITIALIZE_EXPRESSION_MAP;
...
import com.esferalia.aon.occam.mod200.server.format.mod200_2023.Mod2002023Import2022;
...
public class Mod2002023DAO  {
	
	@FunctionalInterface
	private static interface IPopulater {
		boolean populate(Mod2002023 mod, FsModel200RegistryRecord reg);
	}

	private enum Mod2002023RegistryType {
		 ADMINISTRATOR ( 
		...
		private Mod2002023RegistryType(IPopulater populater){
		...
		private void _populate(Mod2002023 mod,FsModel200RegistryRecord reg) {
			if ( accept(reg) ) {
				this.populater.populate(mod, reg);
			}
		}
		
		public static void populate(Mod2002023 mod,FsModel200RegistryRecord reg) {
			Mod2002023RegistryType type = Mod2002023RegistryType.safeValueOf(reg.getType());
			if (type != null) {
				type._populate(mod, reg);
			}
		}
		
		private static Mod2002023RegistryType safeValueOf( Byte i ) {
			if (i == null) return null;
			return safeValueOf( i.intValue() ); 
		}
		
		private static Mod2002023RegistryType safeValueOf( Integer i ) {
			if (i == null) return null;
			if (i < 0 || i >= Mod2002023RegistryType.values().length) return null;
			return Mod2002023RegistryType.values()[i];
		}
	...
	private static final IAccMiningKeyAccept ACCEPTER = key -> {
		try {
			// Comprobar si la clave está en Mod2002023Key
			return (Mod2002023Key.valueOf((String) key) != null);
		} catch (IllegalArgumentException e) {
			try {
				// Si no lo está, comprobar si está en Mod2002023KeyDC (Detalle correcciones)
				return (Mod2002023KeyDC.valueOf((String) key) != null);
	...
	public static Mod2002023 save(AONContext ctx, Mod2002023 mod200) {
		try {
			ctx.log().info("------ [START] SAVE MOD 200");
			
			// Fechas inicio y fin de periodo para tipo de periodo = 1
			if (mod200.getPeriodType() == 1) {
				mod200.setPeriodStart(AonDateUtils.getYearFirstDay(2023));
				mod200.setPeriodEnd(AonDateUtils.getYearLastDay(2023));
			}
			
			// Casillas estados contables (Balance y PYG)
			mod200.setBooleanValue(Mod2002023Key.C0050, (mod200.getBalanceType() == BalanceType.NORMAL));
			mod200.setBooleanValue(Mod2002023Key.C0051, (mod200.getBalanceType() == BalanceType.ABREVIADO));
			mod200.setBooleanValue(Mod2002023Key.C0052, (mod200.getBalanceType() == BalanceType.PYMES));
			
			mod200.setBooleanValue(Mod2002023Key.C0053, (mod200.getPygType() == BalanceType.NORMAL));
			mod200.setBooleanValue(Mod2002023Key.C0054, (mod200.getPygType() == BalanceType.ABREVIADO));
			mod200.setBooleanValue(Mod2002023Key.C0055, (mod200.getPygType() == BalanceType.PYMES));
	...
	private static Mod2002023 insert(AONContext ctx, Mod2002023 mod200)  {
...
//			 .set(FS_MODEL200.NRC,mod200.getNrc()) // FALTA
	...
	private static void insertRegistry(AONContext ctx,Mod2002023 mod200) {
		...
		if (mod200.getAdministrators() != null) {
			for ( Mod200CompanyAdministrator ca : mod200.getAdministrators() ) {
				...
				detail.setType( Mod2002023RegistryType.ADMINISTRATOR.byteValue());
		...
		if (mod200.getParticipationsOut() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsOut() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.PARTICPATION_OUT.byteValue());
		...
		if (mod200.getParticipationsIn() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsIn() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.PARTICPATION_IN.byteValue());
				detail.setDocument(AonStringUtils.substring(cp.getDocument(),0,9));
				detail.setName(AonStringUtils.substring(cp.getName(),0,45));
				detail.setProvince( (byte) cp.getProvince() );
				detail.setCountry( cp.getCountry() );
		...
		if (mod200.getMinorEntities() != null) {
			for ( MinorEntity me : mod200.getMinorEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.MINOR_ENTITIES.byteValue());
		...
		if (mod200.getUteForeign() != null) {
			for ( UteForeign ute : mod200.getUteForeign() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.UTE_FOREIGN.byteValue());
		...
		if (mod200.getRepresentatives() != null) {
			for ( LegalRepresentative lr : mod200.getRepresentatives() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.REPRESENTATIVE.byteValue());
		...
		if (mod200.getUteParticipations() != null) {
			for ( UteParticipation ute : mod200.getUteParticipations() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.UTE_PARTICIPATION.byteValue());
		...
		if (mod200.getGroupEntities() != null) {
			for ( GroupEntitie ge : mod200.getGroupEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002023RegistryType.GROUP_ENTITIES.byteValue());
		...
		if (mod200.getEstablishments() != null) {
			for ( String es : mod200.getEstablishments() ) {
				if (AonStringUtils.isNotBlank(es)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002023RegistryType.ESTABLISHMENTS.byteValue());
		...
		if (mod200.getFilmProductions() != null) {
			for ( String fp : mod200.getFilmProductions() ) {
				if (AonStringUtils.isNotBlank(fp)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002023RegistryType.FILM_PRODUCTIONS.byteValue());
		...
		if (mod200.getSicav1() != null) {
			for ( String s1 : mod200.getSicav1() ) {
				if (AonStringUtils.isNotBlank(s1)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002023RegistryType.SICAV_1.byteValue());
		...
		if (mod200.getSicav2() != null) {
			for ( String s2 : mod200.getSicav2() ) {
				if (AonStringUtils.isNotBlank(s2)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002023RegistryType.SICAV_2.byteValue());
	...
	private static void insertDetail(AONContext ctx, Mod2002023 mod200) {
		LinkedList<FsModel200DetailRecord> list = new LinkedList<FsModel200DetailRecord>();
		FsModel200DetailRecord detail = null;
		// Insertamos los valores de Mod2002023Key
		for (Mod2002023Key k : Mod2002023Key.values()) {
		...
		// Insertamos los valores de Mod2002023KeyDC (Detalle correcciones)
		for (Mod2002023KeyDC k : Mod2002023KeyDC.values()) {
			DoubleVariableEx dv = null;	
			if (mod200.getDraftMap().containsKey(k)) {
				dv = mod200.getDraftMap().get(k);
			} else {
	...
	private static Mod2002023 update(AONContext ctx, Mod2002023 mod200)  {		
...
//		 .set(FS_MODEL200.NRC,mod200.getNrc()) // FALTA
	...
	public static void delete(AONContext ctx, Mod2002023 mod200 )  {
		try {
			int id = mod200.getId();
	...
	public static Mod2002023 getById(AONContext ctx, int id ) {
		FsModel200Record rec = ctx.getDslContext()
				.selectFrom(FS_MODEL200)
				.where(FS_MODEL200.ID.equal(id))
				.fetchOne();
		Mod2002023 mod200 = populateMod200(ctx,rec);
		initializeActiveMap(mod200);
		return mod200;
	}
	
	public static Mod2002023 createNewMod200(AONContext ctx, int year) {
		try {
			ctx.log().info("------ [INI] createNewMod200");
			Mod2002023 mod200 = new Mod2002023();
...
//	}
	...
	private static Mod2002023 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002023 mod200 = null;
		if (record != null) {
			mod200 = getMod200(record);
			fillDetail(mod200,ctx);
			final Mod2002023 mod = mod200;
			ctx.getDslContext() 
				.selectFrom(FS_MODEL200_REGISTRY)
				.where(	FS_MODEL200_REGISTRY.FS_MODEL200.equal(mod200.getId()))
				.fetch()
				.stream()
				.forEach(reg -> Mod2002023RegistryType.populate(mod, reg));
		}
		return mod200;
	}
	
	private static void fillDetail(Mod2002023 mod200,AONContext ctx) {
		...
		for (FsModel200DetailRecord det : result) {
			try {
				// Comprobamos si la clave es de Mod2002023Key
				Mod2002023Key.valueOf(det.getKey());
				map.put(det.getKey(),det.getValue());
			} catch (IllegalArgumentException e) {
				try {
					// Comprobamos si la clave es de Mod2002023KeyDC (Detalle correcciones)
					Mod2002023KeyDC.valueOf(det.getKey());
					map.put(det.getKey(),det.getValue());
				} catch (IllegalArgumentException e2) {
		...
		// Cargamos las claves de Mod2002023Key
		...
		// Cargamos las claves de Mod2002023KeyDC (Detalle correcciones)
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002023Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002023Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002023Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		
		EcpnType et = EcpnType.NO_CONSTA;
		if (map.containsKey( Mod2002023Key.C0075.toString() )) {
			et = EcpnType.NORMAL;
		} else if (map.containsKey( Mod2002023Key.C0076.toString() )) {
			et = EcpnType.ABREVIADO;
		} else if (map.containsKey( Mod2002023Key.C0077.toString() )) {
			et = EcpnType.PYMES;
		}
		mod200.setEcpnType(et);
		
		bt = null;
		if (map.containsKey( Mod2002023Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002023Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002023Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
	...
	public static Mod2002023 initializeNewMod200(AONContext ctx, Mod2002023 mod200) {
		...
		if (old != null && old.getId() != null) { 
			...
			Mod2002023Import2022.import2022(mod200,old);
	...
	public static Mod2002023 initializeMod200(AONContext ctx, Mod2002023 mod200) {
		try {
			...
			if (mod200.isNotChecked(Mod2002023Key.C0081)) {			
			...
			if (mod200.isNotChecked(Mod2002023Key.C0039)) {
			...
			if (mod200.isNotChecked(Mod2002023Key.C0021)) {
			...
			Mod2002023MVELContext mvelCtx = new Mod2002023MVELContext( mod200, ACCEPTER );		
			...
			DoubleVariableEx dv = null;
			for (Mod2002023Key k : INITIALIZE_EXPRESSION_MAP.keySet()) {
	...
	private static void fillMod202(AONContext ctx,Mod2002023 mod200) {
		Mod202DAO.getMod202s(ctx, mod200.getDomain())
				...
				.forEach(mod -> {
					...
					if (mod202.getPeriod() == Period.T1) {
						...
						key = Mod2002023Key.BN603;
					} else if (mod202.getPeriod() == Period.T3) {
						key = Mod2002023Key.BN605;
					}
					if (key != null) {
	...
	public static Mod2002023 calculate(Mod2002023 mod200) {
		return calculate(mod200,true);		
	}
	
	private static Mod2002023 calculate(Mod2002023 mod200, boolean addToDraft) {
		try {
			Mod2002023MVELContext ctx = new Mod2002023MVELContext( mod200, ACCEPTER );
...
//			ctx.setExpressionMap(Mod2002023Compute.COMPUTE_EXPRESSION_MAP);			
			...
			// Actualmente las casillas calculadas solo son de Mod2002023Key
			DoubleVariableEx v = null;
			for (Mod2002023Key k : Mod2002023Compute.COMPUTE_EXPRESSION_MAP.keySet()) {
				String stringKey = k.toString();
				...
				Object ret = ctx.evaluateExpression(k,Mod2002023Compute.COMPUTE_EXPRESSION_MAP.get(k));
				if (ret instanceof Double) {
			...
			v = mod200.getVariable(Mod2002023Key.BN621);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002023 mod200) throws AonCoreException {
	...
	private static void initializeActiveMap(Mod2002023 mod200) {
		// Actualmente la visualizacion de determinadas casillas solo afecta a Mod2002023Key
		Mod2002023MVELContext ctx = new Mod2002023MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002023Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		addBalanceCharacters(ctx,mod200);
		for (Mod2002023Key key : Mod2002023Key.values() ) {
			if (Mod2002023Activation.ACTIVE_EXPRESSION_MAP.containsKey(key) ) {
	...
	private static void addBalanceCharacters(Mod2002023MVELContext ctx, Mod2002023 mod200) {
		ctx.put(Mod2002023Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002023Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002023Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);
		
		ctx.put(Mod2002023Key.C0075.toString(), mod200.getEcpnType() == EcpnType.NORMAL);
		ctx.put(Mod2002023Key.C0076.toString(), mod200.getEcpnType() == EcpnType.ABREVIADO);
		ctx.put(Mod2002023Key.C0077.toString(), mod200.getEcpnType() == EcpnType.PYMES);

		ctx.put(Mod2002023Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002023Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
