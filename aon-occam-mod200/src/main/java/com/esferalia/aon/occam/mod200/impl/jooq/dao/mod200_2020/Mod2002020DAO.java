...
import static com.esferalia.aon.occam.mod200.impl.jooq.dao.mod200_2020.Mod2002020Initialization.INITIALIZE_EXPRESSION_MAP;
...
import com.esferalia.aon.occam.mod200.server.format.mod200_2020.Mod2002020Import2019;
...
public class Mod2002020DAO  {
	
	@FunctionalInterface
	private static interface IPopulater {
		boolean populate(Mod2002020 mod,FsModel200RegistryRecord reg);
	}

	private static enum Mod2002020RegistryType {
		 ADMINISTRATOR ( 
		...
		private Mod2002020RegistryType( IPopulater populater){
		...
		private void _populate(Mod2002020 mod,FsModel200RegistryRecord reg) {
			if ( accept(reg) ) {
				this.populater.populate(mod, reg);
			}
		}
		
		public static void populate(Mod2002020 mod,FsModel200RegistryRecord reg) {
			Mod2002020RegistryType type = Mod2002020RegistryType.safeValueOf(reg.getType());
			if (type != null) {
				type._populate(mod, reg);
			}
		}
		
		private static Mod2002020RegistryType safeValueOf( Byte i ) {
			if (i == null) return null;
			return safeValueOf( i.intValue() ); 
		}
		private static Mod2002020RegistryType safeValueOf( Integer i ) {
			if (i == null) return null;
			if (i < 0 || i >= Mod2002020RegistryType.values().length) return null;
			return Mod2002020RegistryType.values()[i];
		}
	...
	private static final IAccMiningKeyAccept ACCEPTER = new IAccMiningKeyAccept() {
		...
		public boolean acceptKey(Object key) {
			try {
				// Comprobar si la clave está en Mod2002020Key
				return (Mod2002020Key.valueOf((String) key) != null);	
			} catch (IllegalArgumentException e) {
				try {
					// Si no lo está, comprobar si está en Mod2002020KeyDC (Detalle correcciones)
					return (Mod2002020KeyDC.valueOf((String) key) != null);	
	...
	public static Mod2002020 save(AONContext ctx, Mod2002020 mod200) {
		try {
	...
	private static Mod2002020 insert(AONContext ctx, Mod2002020 mod200)  {
	...
	private static void insertRegistry(AONContext ctx,Mod2002020 mod200) {
		...
		if (mod200.getAdministrators() != null) {
			for ( Mod200CompanyAdministrator ca : mod200.getAdministrators() ) {
				...
				detail.setType( Mod2002020RegistryType.ADMINISTRATOR.byteValue());
		...
		if (mod200.getParticipationsOut() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsOut() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.PARTICPATION_OUT.byteValue());
		...
		if (mod200.getParticipationsIn() != null) {
			for ( Mod200CompanyParticipation cp : mod200.getParticipationsIn() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.PARTICPATION_IN.byteValue());
				detail.setDocument(AonStringUtils.substring(cp.getDocument(),0,9));
				detail.setName(AonStringUtils.substring(cp.getName(),0,45));
				detail.setProvince( (byte) cp.getProvince() );
				detail.setCountry( cp.getCountry() );
		...
		if (mod200.getMinorEntities() != null) {
			for ( MinorEntity me : mod200.getMinorEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.MINOR_ENTITIES.byteValue());
		...
		if (mod200.getRepresentatives() != null) {
			for ( LegalRepresentative lr : mod200.getRepresentatives() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.REPRESENTATIVE.byteValue());
		...
		if (mod200.getUteParticipations() != null) {
			for ( UteParticipation ute : mod200.getUteParticipations() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.UTE_PARTICIPATION.byteValue());
		...
		if (mod200.getUteForeign() != null) {
			for ( UteForeign ute : mod200.getUteForeign() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.UTE_FOREIGN.byteValue());
		...
		if (mod200.getUteBases() != null) {
			for ( UteBase ute : mod200.getUteBases() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.UTE_BASE.byteValue());
				detail.setNominalValue(ute.getBase());
				detail.setPercent(ute.getPercent());
				list.add(detail);
			}
		...
		if (mod200.getGroupEntities() != null) {
			for ( GroupEntitie ge : mod200.getGroupEntities() ) {
				...
				detail.setFsModel200(mod200.getId());
				detail.setDomain(mod200.getDomain());
				detail.setType(Mod2002020RegistryType.GROUP_ENTITIES.byteValue());
		...
		if (mod200.getEstablishments() != null) {
			for ( String es : mod200.getEstablishments() ) {
				if (AonStringUtils.isNotBlank(es)) {
					...
					detail.setFsModel200(mod200.getId());
					detail.setDomain(mod200.getDomain());
					detail.setType(Mod2002020RegistryType.ESTABLISHMENTS.byteValue());
	...
	private static void insertDetail(AONContext ctx, Mod2002020 mod200) {
	...
	private static Mod2002020 update(AONContext ctx, Mod2002020 mod200)  {
	...
	public static Mod2002020 getById(AONContext ctx, int id ) {
		FsModel200Record record = ctx.getDslContext()
				.selectFrom(FS_MODEL200)
				.where(FS_MODEL200.ID.equal(id))
				.fetchOne();
		Mod2002020 mod200 =  populateMod200(ctx,record);
		initializeActiveMap(mod200);
		return mod200;
	}
	
	public static Mod2002020 createNewMod200(AONContext ctx, int year) {
		try {
	...
	public static Mod2002020 getByYear(AONContext ctx, int year) {
		return getByYear(ctx, year, true);
	}
	
	public static Mod2002020 getByYear(AONContext ctx, int year, boolean initialize) {
	...
	private static Mod2002020 getMod200(FsModel200Record record) {
		
		Mod2002020 mod200 = new Mod2002020();
		mod200 = new Mod2002020();
	...
	private static Mod2002020 populateMod200(AONContext ctx, FsModel200Record record) {
		Mod2002020 mod200 = null;
		if (record != null) {
			mod200 = getMod200(record);
			fillDetail(mod200,ctx);
			final Mod2002020 mod = mod200;
			ctx.getDslContext() 
				.selectFrom(FS_MODEL200_REGISTRY)
				.where(	FS_MODEL200_REGISTRY.FS_MODEL200.equal(mod200.getId()))
				.fetch()
				.stream()
				.forEach(reg -> Mod2002020RegistryType.populate(mod, reg));
		}
		return mod200;
	}
	
	private static void fillDetail(Mod2002020 mod200,AONContext ctx) {
		...
		for (FsModel200DetailRecord det : result) {
			try {
				// Comprobamos si la clave es de Mod2002020Key
				Mod2002020Key.valueOf(det.getKey());
				map.put(det.getKey(),det.getValue());
			} catch (IllegalArgumentException e) {
				try {
					// Comprobamos si la clave es de Mod2002020KeyDC (Detalle correcciones)
					Mod2002020KeyDC.valueOf(det.getKey());
					map.put(det.getKey(),det.getValue());
				} catch (IllegalArgumentException e2) {
		...
		BalanceType bt = null;
		if (map.containsKey( Mod2002020Key.C0050.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002020Key.C0051.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002020Key.C0052.toString() )) {
			bt = BalanceType.PYMES;
		}
		mod200.setBalanceType( bt );
		
		EcpnType et = EcpnType.NO_CONSTA;
		if (map.containsKey( Mod2002020Key.C0075.toString() )) {
			et = EcpnType.NORMAL;
		} else if (map.containsKey( Mod2002020Key.C0076.toString() )) {
			et = EcpnType.ABREVIADO;
		} else if (map.containsKey( Mod2002020Key.C0077.toString() )) {
			et = EcpnType.PYMES;
		}
		mod200.setEcpnType(et);
		
		bt = null;
		if (map.containsKey( Mod2002020Key.C0053.toString() )) {
			bt = BalanceType.NORMAL;
		} else if (map.containsKey( Mod2002020Key.C0054.toString() )) {
			bt = BalanceType.ABREVIADO;
		} else if (map.containsKey( Mod2002020Key.C0055.toString() )) {
			bt = BalanceType.PYMES;
		}
	...
	public static Mod2002020 initializeMod200(AONContext ctx, Mod2002020 mod200) {
		try {
			...
			if (mod200.isNotChecked(Mod2002020Key.C0081)) {			
			...
			// Inicialización estados contables (solo afecta a claves de Mod2002020Key)
			Mod2002020MVELContext mvelCtx = new Mod2002020MVELContext( mod200, ACCEPTER );		
	...
	private static void fillMod202(AONContext ctx,Mod2002020 mod200) {
		Mod202DAO.getMod202s(ctx, mod200.getDomain())
			...
			.forEach( mod -> {
				...
				Mod2002020Key key = null;
				if (mod202.getPeriod() == Period.T1) {
					key = Mod2002020Key.BN601;
				} else if (mod202.getPeriod() == Period.T2) {
					key = Mod2002020Key.BN603;
	...
	private static Mod2002020 calculate(Mod2002020 mod200, boolean addToDraft) {
		try {
			...
			v = mod200.getVariable(Mod2002020Key.BN621);
	...
	private static AccMiningParameters getParams(AONContext ctx,Mod2002020 mod200) throws AonCoreException {
	...
	private static void initializeActiveMap(Mod2002020 mod200) {
		// Actualmente la visualizacion de determinadas casillas solo afecta a Mod2002020Key
		Mod2002020MVELContext ctx = new Mod2002020MVELContext( mod200, ACCEPTER );
		ctx.setExpressionMap(Mod2002020Activation.ACTIVE_EXPRESSION_MAP);
		addCharacters(ctx,mod200);
		addBalanceCharacters(ctx,mod200);
		for (Mod2002020Key key : Mod2002020Key.values() ) {
			if (Mod2002020Activation.ACTIVE_EXPRESSION_MAP.containsKey(key) ) {
	...
	private static void addBalanceCharacters(Mod2002020MVELContext ctx, Mod2002020 mod200) {
		ctx.put(Mod2002020Key.C0050.toString(), mod200.getBalanceType() == BalanceType.NORMAL);
		ctx.put(Mod2002020Key.C0051.toString(), mod200.getBalanceType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002020Key.C0052.toString(), mod200.getBalanceType() == BalanceType.PYMES);
		
		ctx.put(Mod2002020Key.C0075.toString(), mod200.getEcpnType() == EcpnType.NORMAL);
		ctx.put(Mod2002020Key.C0076.toString(), mod200.getEcpnType() == EcpnType.ABREVIADO);
		ctx.put(Mod2002020Key.C0077.toString(), mod200.getEcpnType() == EcpnType.PYMES);

		ctx.put(Mod2002020Key.C0053.toString(), mod200.getPygType() == BalanceType.NORMAL);
		ctx.put(Mod2002020Key.C0054.toString(), mod200.getPygType() == BalanceType.ABREVIADO);
		ctx.put(Mod2002020Key.C0055.toString(), mod200.getPygType() == BalanceType.PYMES);
	}
...
//	}
	...
	public static Mod2002020 importMod2002019(AONContext ctx, Mod2002020 mod200) {
		Mod2002019 old = Mod2002019DAO.getByYear(ctx, 2019);
		return Mod2002020Import2019.import2019(old);
	}	
}
