...
public class Warehouse extends Methods{
	...
	public void downloadUdapaTag(Integer id, Double pos, Double[] cals,  String[] dests, String var, String obv, String sup, String productor){
		String str = "domain="+ getDomainName() + "&login="+getUserName() + "&id="+id + "&position=" + pos  + "&supplier=" + (sup.isEmpty() ? "-": sup)
				+ "&productor=" + (productor.isEmpty() ? "-" : productor )+ "&observation=" + (obv.isEmpty() ? "-" : obv) + "&variety=" + (var.isEmpty() ? "-" : var);
