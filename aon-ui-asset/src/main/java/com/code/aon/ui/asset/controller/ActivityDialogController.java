	...
	public void onAccept(ActionEvent event) {
		try {
			...
			if (e.getCause() instanceof ManagerBeanVetoListenerException) {
				AonUtil.addErrorMessageFromBundle(ASSET_ERROR_HOVERLAP);
			} else {
...
//		subject & content field
		...
		content.append( AonUtil.getMessage(ASSET_ASSET) ).append( ": ");
		content.append( getAsset().getName() ).append(SystemUtils.LINE_SEPARATOR);
		content.append( AonUtil.getMessage(ASSET_START_DATE) ).append( ": ");
		...
		content.append( AonUtil.getMessage(ASSET_END_DATE) ).append( ": ");
		cal.setTime(getToDate());
		content.append( DateFormatUtils.format(cal, AonUtil.getMessage(DATE_PATTERN), locale) ).append(SystemUtils.LINE_SEPARATOR);
		content.append( AonUtil.getMessage(ASSET_FROM_TIME) ).append( ": ");
		content.append( getFromTimeHours() ).append( ":").append( getFromTimeMins() ).append(SystemUtils.LINE_SEPARATOR);
		content.append( AonUtil.getMessage(ASSET_TO_TIME) ).append( ": ");
		content.append( getToTimeHours() ).append( ":").append( getToTimeMins() ).append(SystemUtils.LINE_SEPARATOR);
		content.append( AonUtil.getMessage(ASSET_ACTIVITY_DAYS) ).append( ": ");
		...
		content.append( AonUtil.getMessage(HOLDER) ).append( ": ");
	...
	private boolean isValidDate() {
		if (fromDate.after(toDate)) {
			AonUtil.addErrorMessageFromBundle(ASSET_ERROR_DATE_RANGE);
			return false;
		}
		return true;
	}
	...
	private boolean isValidTime() {
		if (fromTime.after(toTime) || fromTime.equals(toTime)) {
			AonUtil.addErrorMessageFromBundle(ASSET_ERROR_TIME_RANGE);
			return false;
		}
		return true;
	}
