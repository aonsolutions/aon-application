...
export class AonGroupwareBeta extends AonElement {
	AON_GROUPWARE_BETA;
	ROOT_PANEL;
	dur;
	righPanel;
	...
	connectedCallback() {
		this.initialize();
		this.createApplication(this.AON_GROUPWARE_BETA, MSG.GROUPWARE, new AonApplication());
	...
	initialize() {
		this.AON_GROUPWARE_BETA = "aonGroupwareBeta";
	...
	buildToolbar(){
		...
		window.addEventListener("message", (event) => {
			if ( (event.origin === "null" || event.origin === window.origin) 
				...
				getRegistryNotes({registry: company.registry, source: 'GROUPWARE'})
					.then(notes => {
						...
						let titleToolbar = this.getElement("aonGroupwareBetaToolbarHeaderToolSectionTitle");
						titleToolbar.innerHTML = notesTitle;
						
						let existsNotes = notes.some(item => item.date);
						let notesIcon = this.getElement("aonGroupwareBetaToolbarHeaderToolSectionNotesButtonIcon");
						if(existsNotes)
	...
	buildObservations() {
		...
		if (rightSidenav.style.flexBasis === "0px" || rightSidenav.style.flexBasis.length == 0) {
			...
			localStorage.setItem("notesSource", "GROUPWARE");
	...
	getApplication() {
		return this.getElement(this.AON_GROUPWARE_BETA);
	}
}

window.customElements.define("aon-groupware-beta", AonGroupwareBeta);
