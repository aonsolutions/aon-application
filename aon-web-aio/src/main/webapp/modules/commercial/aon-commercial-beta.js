...
export class AonCommercialBeta extends AonElement {
	AON_COMMERCIAL_BETA;
	ROOT_PANEL;
	dur;
	righPanel;
	...
	connectedCallback() {
		this.initialize();
		this.createApplication(this.AON_COMMERCIAL_BETA, MSG.COMMERCIAL, new AonApplication());
	...
	initialize() {
		this.AON_COMMERCIAL_BETA = "aonCommercialBeta";
	...
	buildToolbar(){
		...
		window.addEventListener("message", (event) => {
			if ( (event.origin === "null" || event.origin === window.origin) 
				...
				getRegistryNotes({registry: company.registry, source: 'COMMERCIAL'})
					.then(notes => {
						...
						let titleToolbar = this.getElement("aonCommercialBetaToolbarHeaderToolSectionTitle");
						titleToolbar.innerHTML = notesTitle;
						
						let existsNotes = notes.some(item => item.date);
						let notesIcon = this.getElement("aonCommercialBetaToolbarHeaderToolSectionNotesButtonIcon");
						if(existsNotes)
	...
	buildObservations() {
		...
		if (rightSidenav.style.flexBasis === "0px" || rightSidenav.style.flexBasis.length == 0) {
			...
			localStorage.setItem("notesSource", "COMMERCIAL");
	...
	getApplication() {
		return this.getElement(this.AON_COMMERCIAL_BETA);
	}
}

window.customElements.define("aon-commercial-beta", AonCommercialBeta);
