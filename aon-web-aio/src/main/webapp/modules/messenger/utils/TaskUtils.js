...
const addIconToolbar = (toolbar, task) => {
  const titleSpan = setClasses(toolbar.querySelector(`.${CSS.AON_SECONDARY_TOOLBAR_TITLE}`), [
    CSS.FLEX_ROW,
...
const dialogTaskTags = (ev, task) => {
  ...
  const div = setStyles(document.createElement("div"), {
    margin: "5px",
    display: "flex",
  ...
  for (let tag of tags) {
    ...
    aonCheckbox.checked = taskTags.find((t) => t.id === tag.id || tag.name === t.name)
      ? true
      : false;
  ...
  task
    ...
    .forEach((tag) => {
      if (
        ...
        aonMessengerChat.isCau() ||
        aonMessengerChat.isMobile()
      ) {
...
const chooseIconMessage = ({ type, date, name, comment, number }) => {
  ...
  if (WORKFLOW_TYPES.OPEN.includes(type) || WORKFLOW_TYPES.REOPEN.includes(type))
    ...
    actionJson.comment = `${WORKFLOW_TYPE(type)} por <b>${
      name ? name : null
    }</b> a <b>${comment}</b> ${dateParse}`;
...
const checkFilesAddEventClick = ({ id }, parent) => {
  new Promise((r) => setTimeout(r, 1)).then(() => {
    const aonMessengerChat = document.getElementById(MESSENGER_VIEWS.AON_MESSENGER_CHAT);
    parent
      .querySelectorAll(`[${CONSTANT.TYPE}='${CONSTANT.AON_FILE}'], ${TAG.IMG}`)
      .forEach((element) => {
...
const buildForm = (task, divMain) => {
  ...
  if (
    ...
    task.registry &&
    task.registry.name &&
    task.getSource() !== TASK_SOURCE.CAU
  ) {
  ...
  const divStatic = TaskCreationUtils.createDivGrid(divCard, undefined, {
    classes: [CSS.AON_COL_XS_12],
    styles: { padding: "0" },
  });

  const dinamicDiv = TaskCreationUtils.createDivGrid(undefined, undefined, {
    classes: [CSS.AON_COL_XS_12],
    styles: { padding: "0" },
  });
  ...
  const divRequest = TaskCreationUtils.createDivGrid(divStatic, requestTypeSelect, {
    classes: [CSS.AON_COL_XS_6],
  });
  ...
  if (aonMessengerChat.isCau() || task.getSource() === TASK_SOURCE.CAU) {
  ...
  } //if(   (!task.id || task.isExternal()) && !( dataDefault.source_id && [1,3].includes(dataDefault.source_id) ))
  else if ((!task.id || task.isExternal()) && !aonMessengerChat.getDur().isEmployee()) {
    ...
    const divBtnForExternal = TaskCreationUtils.createDivGrid(divStatic, btnForExternal, {
      classes: [CSS.AON_COL_XS_6],
      styles: { top: "16px", marginLeft: "0", display: "none" },
    });
  ...
  if (workflows && workflows.length) {
    for (const workflow of workflows) {
      const message = document.querySelector(
        `#${MESSENGER_IDS.MESSENGER_CHAT} ${MESSENGER_COMPONENTS.MESSAGE}[data-id='${workflow.id}']`
      );
      if (message) {
        ...
        if (iconSendWorkflow) {
          iconSendWorkflow.title =
            "Enviado " + AonDateUtils.setDateTimestampDay(workflow.notification_date);
...
const setContentMessageChat = (task, workflowId) => {
  let content = document.querySelector(
    `${MESSENGER_COMPONENTS.MESSAGE}[data-id="${workflowId}"] > .${CSS.MESSAGE_CONTENT}`
  );
  let textArea = document.getElementById(MESSENGER_IDS.COMMENT_TASK);
...
const checkButtonsToolbar = (task, taskId) => {
  ...
  if (toolbar && task && task.id) {
    ...
    if (!aonMessengerChat.isCau() && aonMessengerChat.getDur().isEmployee() && show) {
      const myTaskHolderId =
        aonMessengerChat.MY_TASKHOLDER && aonMessengerChat.MY_TASKHOLDER.id
          ? aonMessengerChat.MY_TASKHOLDER.id
          : undefined;
  ...
  for await (const file of files) {
    ...
    if (attach) {
      ...
      const element = document.querySelector(
        `[${CONSTANT.TYPE}=${WORKFLOW_TYPES.AON_FILE}][data-id='${file.id}']`
      );
      if (element) {
...
const changeFormProcess = (task, { value, name }) => {
  ...
  if (task.id && aonMessengerChat.getDur().isDev()) {
    //BUTTON SHOW JSON
...
const formQuery = (task, dinamicDiv, forExternal = false) => {
  ...
  //-------------------------CAU--------------------------
  if (
    ((task.getSource() === TASK_SOURCE.CAU && !applicationParent.cau) || forExternal) &&
    !hideData
  ) {
    ...
    if (forExternal) {
      ...
      if (!task.isAdvisoryCompany()) {
        ...
        TaskCreationUtils.createDivGrid(dinamicDiv, advisorySelect, {
          classes: [CSS.AON_COL_XS_12],
        });
      ...
      TaskCreationUtils.createDivGrid(dinamicDiv, projectSelect, {
        classes: [CSS.AON_COL_XS_12],
        styles: { display: "none" },
      });
    }
...
const addTaskDescription = (task) => {
  ...
  if (!aonMessengerChat.isMobile()) {
    ...
    aonTextArea.addEventListener(EVENT.INPUT, ({ target }) => {
      ...
      aonTextArea.checkFileBase64().then(() => {
...
const addCauForm = (task, dinamicDiv) => {
  ...
  const selectTypeIncident = TaskCreationUtils.createSelectCau(
    "typeCau",
    MESSENGER_IDS.TYPE_REQUEST_CAU,
    MSG.TYPE_INCIDENT
  );
  const divTypeIncident = TaskCreationUtils.createDivGrid(dinamicDiv, selectTypeIncident, {
    classes: [CSS.AON_COL_XS_6],
  });
  TaskFill.fillTypeRequestCau(task);

  if (!task.getId() && aonMessengerChat.isCau()) {
    const selectApp = TaskCreationUtils.createSelectCau(
      "selectApp",
      MESSENGER_IDS.SELECT_APP,
      MSG.APPLICATION
    );
...
const addInfoDomain = (task, dinamicDiv) => {
  ...
  try {
    if (task.getSource() === TASK_SOURCE.CAU) {
      ...
      if (!isCau) {
        const domainName =
          task.id && company && company.domain && company.domain.name
            ? company.domain.name
            : undefined;
      ...
      fn(dinamicDiv, elementTwo.label, {
        classes: [CSS.AON_COL_XS_12],
        styles: { paddingBottom: "5px" },
      });
...
const isMyTask = (task, myTaskHolderId, myWorkgroups) => {
  const taskTaskholderId =
    task.task_holder && task.task_holder.id ? task.task_holder.id : undefined;
  ...
  const isMyWorkgroup =
    myWorkgroups && myWorkgroups.length
      ? myWorkgroups.some(({ id }) => id == taskWorkgroup)
      : false;

  return isMyTaskHolder || isMyWorkgroup;
};

const documentExec = (exec) =>
  document.execCommand(exec) ? document.execCommand("normal") : document.execCommand(exec);
