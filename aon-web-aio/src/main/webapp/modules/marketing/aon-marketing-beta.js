...
export class AonMarketingBeta extends AonElement {
	...
	buildToolbar(){
		...
		window.addEventListener("message", (event) => {
			if ( (event.origin === "null" || event.origin === window.origin) 
				...
				getRegistryNotes({registry: company.registry, source: 'MARKETING'})
					.then(notes => {
						...
						let titleToolbar = this.getElement("aonMarketingBetaToolbarHeaderToolSectionTitle");
						titleToolbar.innerHTML = notesTitle;
						
						let existsNotes = notes.some(item => item.date);
						let notesIcon = this.getElement("aonMarketingBetaToolbarHeaderToolSectionNotesButtonIcon");
						if(existsNotes)
	...
	buildObservations() {
		...
		if (rightSidenav.style.flexBasis === "0px" || rightSidenav.style.flexBasis.length == 0) {
			...
			localStorage.setItem("notesSource", "MARKETING");
	...
	getApplication() {
		return this.getElement(this.AON_MARKETING_BETA);
	}
}
