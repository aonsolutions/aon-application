...
import { saveTastHolder, getTaskHolderWorkGroups } from "../../../services/taskHolderService.js";
...
export class AonTaskHolder extends AonReg {
  ...
  _taskHolderListFilter;

  constructor(taskHolderListFilter) {
    super();
    if (!this._taskHolderListFilter) {
      this._taskHolderListFilter = taskHolderListFilter;
    }
  }
  ...
  taskHolderInitialize() {
    ...
    this.options = [
	  ...
	  { title: MSG.WORKGROUP, fn: () => this.buildTaskHolderWorkgroupList() },
    ];
    this.workgroups = [];
  }
  ...
  buildTaskHolderWorkgroupList(){
    ...
    if (registryId) {
      let aonTaskholderWorkgroupList = new AonTaskholderWorkgroupList(registryId);
      aonTaskholderWorkgroupList.style.width = "100%";
      aonTaskholderWorkgroupList.filter = { page: 1, perPage: 50, task_holder: registryId };
      parent.appendChild(aonTaskholderWorkgroupList);
    }
  }
  ...
  buildWorkgroup(table, workgroup, i) {
    ...
    this.getWorkgroups().then((options) => {
  ...
  back() {
    let list = new AonTaskHolderList(this._taskHolderListFilter);
