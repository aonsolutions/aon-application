...
export class AonManagementBeta extends AonElement {
	AON_MANAGEMENT_BETA;
	ROOT_PANEL;
	dur;
	righPanel;
	...
	connectedCallback() {
		this.initialize();
		this.createApplication(this.AON_MANAGEMENT_BETA, MSG.MANAGEMENT, new AonApplication());
	...
	initialize() {
		this.AON_MANAGEMENT_BETA = "aonManagementBeta";
	...
	buildToolbar(){
		...
		window.addEventListener("message", (event) => {
			if ( (event.origin === "null" || event.origin === window.origin) 
				...
				getRegistryNotes({registry: company.registry, source: 'MANAGEMENT'})
					.then(notes => {
						...
						let titleToolbar = this.getElement("aonManagementBetaToolbarHeaderToolSectionTitle");
						titleToolbar.innerHTML = notesTitle;
						
						let existsNotes = notes.some(item => item.date);
						let notesIcon = this.getElement("aonManagementBetaToolbarHeaderToolSectionNotesButtonIcon");
						if(existsNotes)
	...
	buildObservations() {
		...
		if (rightSidenav.style.flexBasis === "0px" || rightSidenav.style.flexBasis.length == 0) {
			...
			localStorage.setItem("notesSource", "MANAGEMENT");
	...
	getApplication() {
		return this.getElement(this.AON_MANAGEMENT_BETA);
	}
}

window.customElements.define("aon-management-beta", AonManagementBeta);
