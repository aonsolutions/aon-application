...
export class AonTreasuryBeta extends AonElement {
	...
	initialize() {
		this.AON_TREASURY_BETA = "aonTreasuryBeta";
	...
	buildToolbar(){
		...
		window.addEventListener("message", (event) => {
			if ( (event.origin === "null" || event.origin === window.origin) 
				...
				getRegistryNotes({registry: company.registry, source: 'TREASURY'})
					.then(notes => {
						...
						let titleToolbar = this.getElement("aonTreasuryBetaToolbarHeaderToolSectionTitle");
						titleToolbar.innerHTML = notesTitle;
						
						let existsNotes = notes.some(item => item.date);
						let notesIcon = this.getElement("aonTreasuryBetaToolbarHeaderToolSectionNotesButtonIcon");
						if(existsNotes)
	...
	buildObservations() {
		...
		if (rightSidenav.style.flexBasis === "0px" || rightSidenav.style.flexBasis.length == 0) {
			...
			localStorage.setItem("notesSource", "TREASURY");
	...
	getApplication() {
		return this.getElement(this.AON_TREASURY_BETA);
	}
}

window.customElements.define("aon-treasury-beta", AonTreasuryBeta);
