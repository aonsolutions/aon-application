...
export class AonEventList extends AonElement {
	...
	build() {
		...
		if(this._presenceFilter){
			this._filter = { ...this._filter, ...this._presenceFilter };
		}
	...
	buildToolbarSearch() {
		...
		btnSearch.addEventListener(EVENT.RESET_FILTER, ({ detail }) => {
			...
			timeOut = setTimeout(() => {
				...
				setValueName('group', this._filter.group);
	...
	addTotalMobile(aonTable, idxTotal, timeTotal) {
		const filter = !isEmptyObject(this._filter) ? this._filter : null;
	...
	async getData() {
		...
		try {
			...
			let datos = await getTaskHolderTimeControl(this._filter);
			//console.log('--------- Aon Event List ---------', this._filter, datos);
			if (datos) {
	...
	aonEvent({ }, data) {
		...
		*/
		this.applicationParentEl.showView(SIGNIN_VIEWS.AON_EVENT_DETAIL_LIST, undefined, this._filter);
	}
	...
	back() {
		let presenceFilter = this._filter;
		delete presenceFilter.taskHolderId;
		this.applicationParentEl.showView(SIGNIN_VIEWS.AON_PRESENCE_LIST, undefined, presenceFilter);
	}
}
